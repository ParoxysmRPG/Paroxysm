...
with tempfile.TemporaryDirectory(prefix="haven-memory-test-") as temporary:
    ...
    (scratch / "log").mkdir()
    gzip_name = "x" * 120 + ";$() ' player.gz"
    (scratch / "data" / gzip_name).write_bytes(gzip.compress(b"complete player data\n"))
    (scratch / "data" / "invalid-player.gz").write_bytes(b"not a gzip file")
    ...
    subprocess.run(["g++", "-g", "-Wno-deprecated", "-Wno-write-strings", *flags,
                    "-I" + str(ROOT / "src"), str(ROOT / "tools/test_memory_safety.cpp"),
                    str(ROOT / "tools/test_build_safety.cpp"),
    ...
    (build.parent / "memory-test.log").write_text(result.stdout)
    lines = result.stdout.splitlines()
    assert "CHECK_POS : item_type" not in result.stdout
    assert not re.search(r" :: Here2?\s*$", result.stdout, re.M)
