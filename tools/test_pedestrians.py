#!/usr/bin/env python3
"""Exercise pedestrians against the real engine in a disposable world."""
...
with tempfile.TemporaryDirectory(prefix='haven-pedestrians-') as temporary:
    ...
    subprocess.run(['g++', '-g', '-Wno-deprecated', '-Wno-write-strings',
                    '-I' + str(ROOT / 'src'), str(ROOT / 'tools/test_pedestrians.cpp'),
                    str(comm), *objects, '-Wl,--wrap=number_percent', '-rdynamic',
                    '-ldl', '-lcrypt', '-lcurl', '-o', str(binary)], check=True)
    result = subprocess.run([str(binary)], cwd=scratch / 'area',
                            stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                            text=True, errors='replace', timeout=120)
    (build.parent / 'pedestrians-test.log').write_text(result.stdout)
    if result.returncode:
        trace = subprocess.run(['gdb', '-batch', '-ex', 'run', '-ex', 'bt', str(binary)],
                               cwd=scratch / 'area', stdout=subprocess.PIPE,
                               stderr=subprocess.STDOUT, text=True, timeout=120)
        print('\n'.join(trace.stdout.splitlines()[-30:]))
    print('\n'.join(result.stdout.splitlines()[-55:] if result.returncode else
