...
DECLARE_DO_FUN( do_gameupdate );
DECLARE_DO_FUN( do_society );
DECLARE_DO_FUN( do_newalliance );


/* Runtime indexes: update through these helpers when list membership changes. */
void register_live_character(CHAR_DATA *ch);
void unregister_live_character(CHAR_DATA *ch);
void set_combat_state(CHAR_DATA *ch, bool fighting);
CHAR_DATA *next_combat_character(unsigned long long *cursor);
void mark_grouptexts_dirty(void);
void mark_texthistories_dirty(void);
void mark_profiles_dirty(void);
PROFILE_TYPE *edit_profile(PROFILE_TYPE *profile);
void mark_profile_string_dirty(char **field);
void invalidate_phonebook_index(void);
void invalidate_faction_index(void);
...
void	feel_feeding		args( (CHAR_DATA *ch, int type) );
...
bool                    has_symbol_society                                 args( (CHAR_DATA *ch) );
bool                    has_symbol_legacy_society                                 args( (CHAR_DATA *ch) );
...
bool 	in_society_domain		args( (CHAR_DATA *ch) );
...
char *	alliance_names		args( (int axes, int position) );
...
int operation_poi_positions args( (CHAR_DATA *ch, OperationPoiPosition *points) );
int operation_poi_type args( (const OperationPoiPosition *points, int count, int size, int mapy, int mapx) );
bool get_poi_target args( (CHAR_DATA *ch, int type, int *x, int *y) );
...
bool    save_clans              args( ( bool backup ) );
...
bool start_syndicate_auction(CHAR_DATA *victim, CHAR_DATA *seller = NULL);
...
CHAR_DATA * find_abductee	args( (CHAR_DATA *mob, bool pending = false) );
...
void	real_kill		args( (CHAR_DATA *victim, CHAR_DATA *killer, bool wound = false, bool monster = false) );
...
bool    student_dormitory        args( (ROOM_INDEX_DATA *room) );
int     student_dorm_house       args( (ROOM_INDEX_DATA *room) );
bool    student_dorm_roster      args( (CHAR_DATA *ch, ROOM_INDEX_DATA *room) );
void    dorms_update             args( (void) );
...
void sync_campus_imprint args((CHAR_DATA *ch));
...
void	maim_char		args( (CHAR_DATA *ch, char * message, CHAR_DATA *actor = NULL, bool monster = false) );
...
void	wound_char_noregen	args( (CHAR_DATA *victim, int type, CHAR_DATA *actor = NULL, bool monster = false) );
void	wound_char		args( (CHAR_DATA *victim, int type, CHAR_DATA *actor = NULL, bool monster = false) );
void 	wound_char_absolute	args( (CHAR_DATA *victim, int type, CHAR_DATA *actor = NULL, bool monster = false) );
...
void end_maintained_ritual_on_death(CHAR_DATA *ch);
bool can_see_scheme_news args( (CHAR_DATA *ch, NEWS_TYPE *news) );
...
void gain_personal_karma args( (CHAR_DATA *ch, int amount) );
...
void invalidate_room_coordinates(void);
void save_periodic_data_now(void);
...
// Full/orange protect ordinary actions; black covers only death and maim recovery.
...
bool cortex_monster_ambush(CHAR_DATA *ch, bool forced = false);
...
bool pedestrian(CHAR_DATA *ch);
bool pedestrian_helpless(CHAR_DATA *ch);
void pedestrian_knockout(CHAR_DATA *ch);
void pedestrian_bound(CHAR_DATA *ch);
bool pedestrian_drain(CHAR_DATA *actor, CHAR_DATA *victim);
bool pedestrian_hypnotise(CHAR_DATA *actor, CHAR_DATA *victim, const char *argument);
void pedestrian_mobile_update(CHAR_DATA *ch);
void pedestrian_update(void);
void pedestrian_forget(CHAR_DATA *ch);
...
int difficulty_mod(int difficulty);
...
void record_critical_injury(CHAR_DATA *, CHAR_DATA * = NULL, bool = false);
void record_death_recovery(CHAR_DATA *, CHAR_DATA *, bool, bool);
void record_maim(CHAR_DATA *, const char *, CHAR_DATA *, bool);
bool process_character_recovery(CHAR_DATA *);
void process_daily_recoveries();
void operation_recovery_wake(CHAR_DATA *);
void operation_recovery_return(CHAR_DATA *);
int operation_member_faction(OPERATION_TYPE *, CHAR_DATA *);
int operation_heal_timer(CHAR_DATA *, int);
void clear_operation_combat(CHAR_DATA *);
void write_recovery(FILE *, CHAR_DATA *);
bool read_recovery(FILE *, CHAR_DATA *, const char *);

void save_death_state(CHAR_DATA *ch);
void make_corpse(CHAR_DATA *, CHAR_DATA *);
int faction_alliance(const FACTION_TYPE *);
bool same_strategic_alliance(const FACTION_TYPE *, const FACTION_TYPE *);
bool alliance_eligible(const FACTION_TYPE *);
bool foothold_controller(const LOCATION_TYPE *, int);
