...
extern "C" {
  ...
  bool clinic_execution_allowed(CHAR_DATA *ch, CHAR_DATA *victim) {
    if (!clinic_patient(victim) || !asylum_room(victim->in_room)) return TRUE;
    if (IS_NPC(ch) || ch == victim || same_player(ch, victim)) return FALSE;
    for (INSTITUTE_TYPE *ins : InVect) {
      if (str_cmp(ins->name, victim->name) || ins->asylum_inactive != 0
          || ins->asylum_prestige <= 0 || ins->clinic_breakout > 0) continue;
      if (ins->asylum_status != ASYLUM_REMOTECOMMIT) return FALSE;
      if (!str_cmp(ch->name, ins->asylum_owner)) return TRUE;
      // Society sponsorship uses the same memberships as clinic ownership.
      for (int faction : {ch->faction, ch->factiontwo}) {
        FACTION_TYPE *fac = faction ? clan_lookup(faction) : NULL;
        if (fac && !str_cmp(fac->name, ins->asylum_owner)) return TRUE;
      }
      return FALSE;
    }
    return FALSE;
  ...
  void release_clinic_after_respawn(CHAR_DATA *ch) {
    if (!ch || IS_NPC(ch)) return;
    bool released = FALSE;
    for (INSTITUTE_TYPE *ins : InVect) {
      if (str_cmp(ins->name, ch->name) || ins->asylum_inactive != 0
          || ins->asylum_prestige <= 0) continue;
    ...
    ROOM_INDEX_DATA *discharge = get_room_index(ROOM_INDEX_CLINICDISCHARGE);
    if (asylum_room(ch->in_room) && discharge) {
      char_from_room(ch);
      char_to_room(ch, discharge);
    }
    save_institutes(FALSE);
    send_to_char("You have been discharged from the clinic after returning to life.\n\r", ch);
  }
  ...
  void commit_char(CHAR_DATA *ch) {
    if (has_territory_bonus(ch, TERRITORY_PAYROLL)) return;
    int iWear;
    OBJ_DATA *obj;
  ...
  _DOFUN(do_clinic) {
    ...
    if (!str_cmp(arg1, "arrest") || !str_cmp(arg1, "MakeArrest") || !str_cmp(arg1, "BribeArrest") || !str_cmp(arg1, "ApproveArrest")) {
      send_to_char("go capture them coward.\n\r", ch);
      return;
    }
    ...
    if (!str_cmp(arg1, "roster")) {
      ...
      if (!youins) { send_to_char("No institute record found.\n\r", ch); return; }
      ...
      it != InVect.end(); ++it) {
        ...
        if (!str_cmp(argument, ins->name)) {
          if (ins->asylum_suspended > 0 && ins->asylum_suspend_rank <= clinic_staff_rank(ch)) {
            ...
            if (!victim) { send_to_char("No character found.\n\r", ch); return; }
      ...
      if (ch->race != RACE_CIVIL_SERVANT && ch->race != RACE_PILLAR) {
        send_to_char("Arrests can only be approved by civil servants and pillars of the community.\n\r", ch);
        return;
      }
      ...
      act("Some Civic Educators escort $n into the area.", victim, NULL, NULL, TO_ROOM);
      return;
    }
      ...
      if (has_territory_bonus(victim, TERRITORY_PAYROLL)) {
        send_to_char("Their faction connections prevent commitment or arrest.\n\r", ch);
        return;
      }
      int cost = commit_cost(ch, victim->name);
      cost /= 10;
      if (clinic_staff(ch, FALSE) || ch->race == RACE_CIVIL_SERVANT) {
        cost /= 4;
      }
      ...
      if (clan_lookup(ch->faction) == NULL || !has_trust(ch, TRUST_RESOURCES, ch->faction) || !can_afford_faction_spending(clan_lookup(ch->faction), amount / 10, 5000 + amount / 10)) {
  ...
  _DOFUN(do_college) {
    ...
    if (!str_cmp(arg1, "enroll")) {
      ...
      if (college_staff(ch, TRUE)) {
        send_to_char("You're a Civic Educator.\n\r", ch);
        return;
      }
      ...
      if (!college_staff(ch, FALSE)) {
        send_to_char("You aren't an active Civic Educator.\n\r", ch);
        return;
      }
      INSTITUTE_TYPE *ins;
      for (vector<INSTITUTE_TYPE *>::iterator it = InVect.begin();
      ...
      it != InVect.end(); ++it) {
        ...
        if (!str_cmp(argument, ins->name)) {
          if (ins->college_prestige > 0 && (ins->asylum_prestige == 0 || ins->asylum_inactive != 0)) {
            ...
            if (has_territory_bonus(vic, TERRITORY_PAYROLL)) {
              send_to_char("Their faction connections prevent commitment or arrest.\n\r", ch);
              return;
            }
  ...
  void wanted_check(CHAR_DATA *ch) {
    ...
    it != InVect.end(); ++it) {
      ins = (*it);
      // Clinic warrants no longer trigger automatic capture.
      if (false && !str_cmp(ins->name, ch->name) && ins->asylum_status == ASYLUM_WANTED) {
    ...
    it != InVect.end(); ++it) {
      ...
      if (!str_cmp(ins->name, ch->name)) {
        if (ins->asylum_status == ASYLUM_REMOTECOMMIT || (safe_strlen(ins->asylum_owner) > 2 && ins->asylum_status > 0)) {
          ...
          if (fac != NULL && fac->stasis == 0 && can_afford_faction_spending(fac, manip_cost(fac, cost / 10), 8001 + cost / 10)) {
  ...
  // Stable reward ordering includes tied responses and hides unavailable intel.
  static int victim_response_at(int action, int number, bool allow_intel) {
    if (action < 1 || action > VICTIMIZE_MAX || number < 1 || number > RESPONSE_MAX)
      return -1;
    int responses[RESPONSE_MAX], count = 0;
    for (int response = 1; response <= RESPONSE_MAX; ++response) {
      if ((!allow_intel && response == RESPONSE_INTEL) || response_value(action, response) < 0)
        continue;
      int at = count++;
      while (at > 0 && response_value(action, responses[at - 1]) < response_value(action, response)) {
        responses[at] = responses[at - 1];
        --at;
      }
      responses[at] = response;
    }
    return number <= count ? responses[number - 1] : -1;
  }
  ...
  _DOFUN(do_victimtest) {
    if (safe_strlen(argument) < 2) {
      ...
      strcat(string, buf);
      sprintf(buf, "Physically Humiliate: Do something to the target to cause them some major embarassment or humiliation.\n\r");
      strcat(string, buf);
    ...
    for (int i = 0; i <= VICTIMIZE_MAX; i++) {
      if (!str_cmp(argument, victimize_actions[i])) {
        for (int x = 1; x <= RESPONSE_MAX; ++x) {
          int response = victim_response_at(i, x, true);
          if (response < 0) break;
          printf_to_char(ch, "[%02d: %s]  ", x, victimize_responses[response]);
        }
        send_to_char("\n\r", ch);
      }
    }
  ...
  void become_difficult(CHAR_DATA *ch) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (is_super(victim) && get_tier(victim) > get_tier(ch) && !institute_room(ch->in_room)) {
        give_lifeforce(victim, (amount / count) * 3 / 2, "difficult prisoner");
      }
      else {
        give_lifeforce(victim, (amount / count) * 3 / 2, "difficult prisoner");
        if (institute_room(ch->in_room))
  ...
  void handout_lifeforce_summary(CHAR_DATA *ch, int perc) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      {
        give_lifeforce(victim, (amount / count) * 3 / 2, "summary victimize");
  ...
  void handout_lifeforce(CHAR_DATA *ch) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (is_super(victim) && get_tier(victim) > get_tier(ch) && !institute_room(ch->in_room))
      give_lifeforce(victim, (amount / count) * 3 / 2, "prisoner pass out");
      else {
        give_lifeforce(victim, (amount / count) * 3 / 2, "prisoner pass out");
        if (institute_room(ch->in_room))
        institute_victimize(victim, ch, amount / count);
  ...
  // Only an actual shared scene qualifies, never matching inactive dream IDs.
  static bool victimize_together(CHAR_DATA *ch, CHAR_DATA *victim) {
    if (!ch || !victim || ch == victim || IS_NPC(ch) || IS_NPC(victim)
        || !ch->pcdata || !victim->pcdata) return false;
    if (is_dreaming(ch) || is_dreaming(victim))
      return is_dreaming(ch) && is_dreaming(victim)
        && ch->pcdata->dream_room > 0
        && ch->pcdata->dream_room == victim->pcdata->dream_room;
    return ch->in_room != NULL && ch->in_room == victim->in_room;
  }

  static bool victimize_restrained(CHAR_DATA *ch) {
    return ch && !IS_NPC(ch) && ch->pcdata
      && (is_helpless(ch) || is_pinned(ch) || dream_slave(ch));
  }

  static void clear_victim_response(CHAR_DATA *ch) {
  ...
  _DOFUN(do_victimize) {
    ...
    if (victim->pcdata->victimize_vic_timer > 0) {
      send_to_char("Wait for their current victimize response to finish.\n\r", ch);
      return;
    }
  ...
  void victimize_emote_process(CHAR_DATA *ch) {
    ...
    if (ch->pcdata->victimize_char_select > 0
        && (!victimize_together(ch, ch->pcdata->victimize_char_point)
            || !victimize_restrained(ch->pcdata->victimize_char_point)
            || ch->pcdata->victimize_char_point->pcdata->victimize_vic_timer > 0)) {
      ch->pcdata->victimize_char_select = 0;
      ch->pcdata->victimize_char_point = NULL;
      send_to_char("Your prepared victimize action is no longer available.\n\r", ch);
    }
    if (ch->pcdata->victimize_vic_timer > 0
        && (!victimize_together(ch, ch->pcdata->victimize_vic_point)
            || !victimize_restrained(ch)))
      clear_victim_response(ch);
    ...
    if (is_dreaming(ch) && !physical_dreamer(ch))
    cap = 700;
    if (ch->pcdata->victimize_char_select > 0 && ch->pcdata->victimize_char_point != NULL && victimize_together(ch, ch->pcdata->victimize_char_point)) {
      ...
      ch->pcdata->victimize_char_select = 0;
      ch->pcdata->victimize_char_point = NULL;
      victim->pcdata->victimize_vic_select = 0;
    ...
    if (ch->pcdata->victimize_vic_select > 0 && ch->pcdata->victimize_vic_timer > 0 && ch->pcdata->victimize_vic_point != NULL && victimize_together(ch, ch->pcdata->victimize_vic_point)) {
      ...
      if(ch->pcdata->victimize_vic_response_to == VICTIMIZE_BLEED && ch->pcdata->last_victim_bled <= (current_time - (3600*24*4)))
      {
  ...
  void victim_prompt(CHAR_DATA *ch) {
    for (int x = 1; x <= RESPONSE_MAX; ++x) {
      int response = victim_response_at(ch->pcdata->victimize_vic_response_to, x,
          ch->pcdata->intel >= 1000);
      if (response < 0) break;
      printf_to_char(ch, "[%02d: %s]  ", x, victimize_responses[response]);
    }
    send_to_char("\n\r", ch);
  ...
  static void cortex_victimize_standing(CHAR_DATA *actor, CHAR_DATA *victim) {
    if (!actor || !victim || actor == victim || IS_NPC(actor) || IS_NPC(victim)
        || !actor->pcdata || !victim->pcdata
        || actor->fcore != FACTION_CORTEX || victim->fcore != FACTION_CORTEX
        || is_super(actor) || !is_super(victim)) return;

    const int reward = 100;
    const int daily_cap = 1000;
    const int today = current_time / 86400; // Real calendar days, midnight UTC.
    if (actor->pcdata->cortex_victimize_day < today) {
      actor->pcdata->cortex_victimize_day = today;
      actor->pcdata->cortex_victimize_standing = 0;
    }
    int amount = UMIN(reward, daily_cap - actor->pcdata->cortex_victimize_standing);
    if (amount <= 0) return;

    // Use the unscaled respect path so charisma/beliefs cannot exceed the cap.
    const int before = actor->esteem_faction;
    give_respect_noecho(actor, amount, FACTION_CORTEX);
    amount = actor->esteem_faction - before;
    if (amount <= 0) return;
    actor->pcdata->cortex_victimize_standing += amount;
    printf_to_char(actor, "You gain %d Cortex standing for victimizing a supernatural member (%d/%d today; resets at midnight UTC).\n\r",
        amount, actor->pcdata->cortex_victimize_standing, daily_cap);
  }

  void process_victim_choice(CHAR_DATA *ch, int number) {
    if (!ch || IS_NPC(ch) || !ch->pcdata) return;
    if (!victimize_together(ch, ch->pcdata->victimize_vic_point)
        || !victimize_restrained(ch)) {
      clear_victim_response(ch);
      return;
    }
    if (ch->pcdata->victimize_vic_timer <= 0 || ch->pcdata->victimize_vic_select != 0
        || ch->pcdata->victimize_vic_response_to <= 0
        || ch->pcdata->victimize_vic_response_to > VICTIMIZE_MAX
        || number <= 0 || number > RESPONSE_MAX
        || response_value(ch->pcdata->victimize_vic_response_to, number) < 0
        || (number == RESPONSE_INTEL && ch->pcdata->intel < 1000)) return;
    int cap = 1000;
    if (is_dreaming(ch) && !physical_dreamer(ch))
    cap = 700;
    ...
    if (choice > 0 && number > 0 && pers != NULL && victimize_together(ch, pers)) {
      if (number == RESPONSE_STOIC) {
        if (is_dreaming(ch)) {
          char buf[MSL];
          sprintf(buf, "%s is stoically unresponsive.", dream_name(ch));
      ...
      cortex_victimize_standing(pers, ch);
      ...
      if (is_super(pers) && get_tier(pers) > get_tier(ch) && !institute_room(ch->in_room))
      give_lifeforce(pers, (value + pers->pcdata->victimize_pending_lf_gain) * 3 / 2, "victimize");
      else
      give_lifeforce(pers, (value + pers->pcdata->victimize_pending_lf_gain) * 3 / 2, "victimize");
  ...
  void process_victim_number(CHAR_DATA *ch, int number) {
    if (!ch || IS_NPC(ch) || !ch->pcdata || number < 1 || number > RESPONSE_MAX
        || ch->pcdata->victimize_vic_timer <= 0 || ch->pcdata->victimize_vic_select != 0)
      return;
    if (!victimize_together(ch, ch->pcdata->victimize_vic_point)
        || !victimize_restrained(ch)) {
      clear_victim_response(ch);
    ...
    int response = victim_response_at(ch->pcdata->victimize_vic_response_to, number,
        ch->pcdata->intel >= 1000);
    if (response < 0) {
      send_to_char("Choose one of the listed response numbers.\n\r", ch);
      return;
    }
    process_victim_choice(ch, response);
    if (number == 1 && ch->pcdata->victimize_vic_select > 0)
      ch->pcdata->ill_count += institute_room(ch->in_room) ? 40 : 80;
  }
  ...
  int blood_level(CHAR_DATA *ch, CHAR_DATA *victim)
  {
    ...
    if(are_allies(ch, victim) && (ch->fsociety != victim->fsociety || (clan_lookup(ch->fsociety) != NULL && clan_lookup(ch->fsociety)->college == 0)))
    base /= 10;
