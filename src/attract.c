...
extern "C" {
  ...
  static int get_jewelry_cost_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    ...
    for (iWear = 0; iWear < MAX_WEAR; iWear++) {
      if ((obj = equipment.get(iWear)) != NULL) {
        if (obj->item_type == ITEM_JEWELRY && can_see_wear_equipped(ch, iWear, equipment))
        cost += obj->cost;
  ...
  int get_jewelry_cost(CHAR_DATA *ch) {
    ...
    return get_jewelry_cost_equipped(ch, equipment);
  }
  static int get_clothes_cost_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    int iWear;
    OBJ_DATA *obj;
    int cost = 0;
    for (iWear = 0; iWear < MAX_WEAR; iWear++) {
      if ((obj = equipment.get(iWear)) != NULL) {
        if (obj->item_type == ITEM_CLOTHING && can_see_wear_equipped(ch, iWear, equipment))
        cost += obj->cost;
  ...
  int get_clothes_cost(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return get_clothes_cost_equipped(ch, equipment);
  }
  ...
  static int get_cover_amount_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    int iWear;
    int warmth = 0;
    OBJ_DATA *obj;
    for (iWear = 0; iWear < MAX_WEAR; iWear++) {
      if ((obj = equipment.get(iWear)) != NULL) {
        warmth += get_obj_cover_amount(obj, ch, iWear);
      }
    }
    return warmth;
  }
  ...
  int get_cover_amount(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return get_cover_amount_equipped(ch, equipment);
  }

  static bool has_shapewear_body_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    int iWear;
    OBJ_DATA *obj;
    for (iWear = 0; iWear < MAX_WEAR; iWear++) {
      if ((obj = equipment.get(iWear)) != NULL) {
  ...
  bool has_shapewear_body(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return has_shapewear_body_equipped(ch, equipment);
  }
  static bool has_shapewear_height_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    int iWear;
    OBJ_DATA *obj;
    for (iWear = 0; iWear < MAX_WEAR; iWear++) {
      if ((obj = equipment.get(iWear)) != NULL) {
  ...
  bool has_shapewear_height(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return has_shapewear_height_equipped(ch, equipment);
  }
  ...
  static float fashion_score_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    float cost = (sqrt((float)get_jewelry_cost_equipped(ch, equipment)) / 4) +
    (sqrt((float)get_clothes_cost_equipped(ch, equipment)) / 2);
    return cost;
  }

  float fashion_score(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return fashion_score_equipped(ch, equipment);
  }
  ...
  static float phys_score_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    int expo = 0;
    for (int j = 0; j < MAX_COVERS; j++) {
      if (!is_covered_equipped(ch, j, equipment))
      expo += 1;
    }
    float exposed = expo * expo;
    exposed -= get_cover_amount_equipped(ch, equipment);
    return exposed;
  }

  float phys_score(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return phys_score_equipped(ch, equipment);
  }

  static int fashion_boost_equipped(CHAR_DATA *ch, bool boosted, const haven::EquipmentSnapshot &equipment) {
    int boost = 0;
    ...
    if (has_shapewear_height_equipped(ch, equipment))
    boost = boost + 2;
    if (has_shapewear_body_equipped(ch, equipment))
    boost = boost + 2;
  ...
  int fashion_boost(CHAR_DATA *ch, bool boosted) {
    const haven::EquipmentSnapshot equipment(ch);
    return fashion_boost_equipped(ch, boosted, equipment);
  }
  ...
  static int phys_boost_equipped(CHAR_DATA *ch, bool boosted, const haven::EquipmentSnapshot &equipment) {
    int boost = 0;
  ...
  int phys_boost(CHAR_DATA *ch, bool boosted) {
    const haven::EquipmentSnapshot equipment(ch);
    return phys_boost_equipped(ch, boosted, equipment);
  }
  ...
  void attract_average_update(CHAR_DATA *ch) {
    ...
    ch->pcdata->attract_count_fashion += fashion_score_equipped(ch, equipment) * pcs;
    ch->pcdata->attract_count_phys += phys_score_equipped(ch, equipment) * pcs;
    ch->pcdata->attract_count_skin += skin_face_score(ch) * pcs;
  }

  static bool is_fashion_boost_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    if (fashion_score_equipped(ch, equipment) >
  ...
  bool is_fashion_boost(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return is_fashion_boost_equipped(ch, equipment);
  }
  ...
  static bool is_phys_boost_equipped(CHAR_DATA *ch, const haven::EquipmentSnapshot &equipment) {
    if (phys_score_equipped(ch, equipment) >
  ...
  bool is_phys_boost(CHAR_DATA *ch) {
    const haven::EquipmentSnapshot equipment(ch);
    return is_phys_boost_equipped(ch, equipment);
  }
  ...
  int attractive_rare_bonus(CHAR_DATA *ch) {
    ...
    const double outfit = fashion_score(ch) + sqrt((float)ch->pcdata->perfume_cost);
  ...
  int get_attract(CHAR_DATA *ch, CHAR_DATA *looker) {
    ...
    const haven::EquipmentSnapshot equipment(ch);
    int fash_boost = fashion_boost_equipped(ch, is_fashion_boost_equipped(ch, equipment), equipment);
    int skin_boost = face_skin_boost(ch, is_face_skin_boost(ch));
    int physical_boost = phys_boost_equipped(ch, is_phys_boost_equipped(ch, equipment), equipment);
  ...
  _DOFUN(do_icheckout) {
    ...
    const int clothes_attraction = get_attract_checkout(victim, ch, 1);
    const int skin_attraction = get_attract_checkout(victim, ch, 2);
    const int body_attraction = get_attract_checkout(victim, ch, 3);
    printf_to_char(ch, "%s%0.1f`x out of 10\n", attract_color(attraction), ((float)attraction)/10.0);

    if (is_fashion_boost(victim))
    printf_to_char(ch, "*Clothes: %s%0.1f`x out of 10\n", attract_color(clothes_attraction), ((float)clothes_attraction)/10.0);
    else
    printf_to_char(ch, "Clothes: %s%0.1f`x out of 10\n", attract_color(clothes_attraction), ((float)clothes_attraction)/10.0);
    if ( is_face_skin_boost(victim))
    printf_to_char(ch, "*Skin/Face/Hair: %s%0.1f`x out of 10\n", attract_color(skin_attraction), ((float)skin_attraction)/10.0);
    else
    printf_to_char(ch, "Skin/Face/Hair: %s%0.1f`x out of 10\n", attract_color(skin_attraction), ((float)skin_attraction)/10.0);
    if (is_phys_boost(victim))
    printf_to_char(ch, "*Body: %s%0.1f`x out of 10\n", attract_color(body_attraction), ((float)body_attraction)/10.0);
    else
    printf_to_char(ch, "Body: %0.1f out of 10\n", ((float)body_attraction)/10.0);
    ...
    printf_to_char(ch, "Skin bval: %f, mins: %d, minsq: %lld, reduc: %f\n\r", bval, mins, minsq, reduc);
