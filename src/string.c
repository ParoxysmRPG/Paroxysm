...
****************************************************************************/
  void string_edit(CHAR_DATA *ch, char **pString) {
    mark_profile_string_dirty(pString);
...
****************************************************************************/
  void string_append(CHAR_DATA *ch, char **pString) {
    ...
    const size_t length = safe_strlen(*pString);
    if (length == 0 || (*pString)[length - 1] != '\r')
    send_to_char("\n\r", ch);
...
****************************************************************************/
  void string_add(CHAR_DATA *ch, char *argument) {
    mark_profile_string_dirty(ch->desc->pString);
...
****************************************************************************/
  char *wrap_string(char *oldstring, int length) {
    ...
    std::vector<char> input(strlen(oldstring) + 10, '\0');
    memcpy(input.data(), oldstring, strlen(oldstring));
    rdesc = input.data();
    ...
    for (;;) {
      ...
      if (*(rdesc + i - 1) != '\r') {
        for (i = (!xbuf.empty() ? (end_of_line - 1) : (end_of_line - 4)); i; i--) {
          if (*(rdesc + i) == ' ')
          break;
        }
        if (i) {
...
*/
    ...
    while (i > 0 && *(rdesc + i) && (*(rdesc + i) == ' ' || *(rdesc + i) == '\n' || *(rdesc + i) == '\r'))
    i--;
    *(rdesc + i + 1) = 0;
    xbuf += rdesc;

    if (xbuf.size() < (MAX_STRING_LENGTH - 10) && (xbuf.size() < 2 || xbuf[xbuf.size() - 2] != '\n'))
    xbuf += "\n\r";
    xbuf2 = xbuf;
    return xbuf2.data();
  }
...
****************************************************************************/
  void spell_check(CHAR_DATA *ch, char *string) {
    ...
    if (system(buf) != 0) {
      bug("Spell checker failed", 0);
      haven::remove_optional_file("spell_check.txt");
      send_to_char("The spell checker is unavailable.\n\r", ch);
      return;
    }
