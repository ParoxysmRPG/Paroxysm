...
*/
  void show_list_to_char(OBJ_DATA *list, CHAR_DATA *ch, bool fShort, bool fShowNothing, char *arg) {
    ...
    int count;
    bool fCombine;
    const bool combine = ch != NULL && (IS_NPC(ch) || IS_FLAG(ch->comm, COMM_COMBINE));
    ...
    */
    for (obj = list; obj != NULL; obj = obj->next_content) {
      ...
      if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && !IS_SET(obj->extra_flags, ITEM_WARDROBE) && (arg == NULL || (arg != NULL && is_name(arg, obj->name)))) {
        ...
        if (combine) {
          // Keep first-occurrence output order, with case-sensitive grouping.
          std::pair<std::unordered_map<std::string, int>::iterator, bool> entry =
              shown.emplace(pstrShow, nShow);
          if (!entry.second) {
            prgnShow[entry.first->second]++;
            fCombine = TRUE;
          }
        }
  ...
  char *get_exposed_locations(CHAR_DATA *ch) {
    ...
    if (!is_covered(ch, COVERS_LOWER_FACE)) {
      ...
      if (ch->pcdata->blood[ref] > 100) {
        if (ch->pcdata->dirt[ref] > 100) {
          buf = haven::format_text(" spattered in blood and very dirty ");
          word += buf.data();
        }
        else if (ch->pcdata->dirt[ref] > 40) {
          buf = haven::format_text(" spattered in blood and dirty ");
          word += buf.data();
        }
        else {
          buf = haven::format_text(" blood spattered ");
          word += buf.data();
        }
      }
      else if (ch->pcdata->blood[ref] > 40) {
        if (ch->pcdata->dirt[ref] > 100) {
          buf = haven::format_text(" very dirty and speckled with blood ");
          word += buf.data();
        }
        else if (ch->pcdata->dirt[ref] > 40) {
          buf = haven::format_text(" speckled with blood and dirty ");
          word += buf.data();
        }
        else {
          buf = haven::format_text(" blood speckled ");
          word += buf.data();
        }
      }
      else if (ch->pcdata->dirt[ref] > 100) {
        buf = haven::format_text(" very dirty ");
        word += buf.data();
      }
      else if (ch->pcdata->dirt[ref] > 40) {
        buf = haven::format_text(" slightly dirty ");
        word += buf.data();
      }
      buf = haven::format_text("face");
      word += buf.data();
      hasplaced = TRUE;
      blast = FALSE;
    }
    else {
      last = haven::format_text(" ");
    }
    if (!is_covered(ch, COVERS_NECK)) {
      ...
      if (!hasplaced) {
        ...
        if (blast) {
          buf = haven::format_text(",%s", last.data());
          word += buf.data();
        }
        if (ch->pcdata->blood[ref] > 100) {
          if (ch->pcdata->dirt[ref] > 100) {
            buf = haven::format_text(" spattered in blood and very dirty ");
            last = haven::format_text("%s", buf.data());
          }
          else if (ch->pcdata->dirt[ref] > 40) {
            buf = haven::format_text(" spattered in blood and dirty ");
            last = haven::format_text("%s", buf.data());
    ...
    if (!is_covered(ch, COVERS_LOWER_CHEST) && !is_covered(ch, COVERS_BREASTS) && !is_covered(ch, COVERS_UPPER_CHEST)) {
      ...
      if (ch->sex == SEX_FEMALE) {
        if (!hasplaced) {
          buf = haven::format_text("`168breasts`x");
          word += buf.data();
          hasplaced = TRUE;
          blast = FALSE;
        }
      ...
      if (!is_covered(ch, COVERS_UPPER_CHEST) && ch->sex == SEX_MALE) {
        ...
        if (!hasplaced) {
          buf = haven::format_text("the top of his chest");
          word += buf.data();
          hasplaced = TRUE;
          blast = FALSE;
        }
      ...
      if (!is_covered(ch, COVERS_UPPER_CHEST) && ch->sex == SEX_FEMALE && is_covered(ch, COVERS_BREASTS)) {
        ...
        if (!hasplaced) {
          buf = haven::format_text("the top of her breasts");
          word += buf.data();
          hasplaced = TRUE;
          blast = FALSE;
        }
          ...
          last = haven::format_text(" ");
          buf = haven::format_text("top of her breasts");
          last += buf.data();
          blast = TRUE;
        }
      }
    ...
    if (!is_covered(ch, COVERS_LOWER_BACK) && !is_covered(ch, COVERS_UPPER_BACK)) {
      ...
      if (!hasplaced) {
        buf = haven::format_text("lower back");
        word += buf.data();
        hasplaced = TRUE;
        blast = FALSE;
      }
      ...
      if (!hasplaced) {
        buf = haven::format_text("upper back");
        word += buf.data();
        hasplaced = TRUE;
        blast = FALSE;
      }
    ...
    if (!is_covered(ch, COVERS_GROIN) && ch->shape != SHAPE_MERMAID) {
      ...
      if (!hasplaced) {
        buf = haven::format_text("`168groin`x");
        word += buf.data();
        hasplaced = TRUE;
        blast = FALSE;
      }
    ...
    if (!is_covered(ch, COVERS_ARSE) && ch->shape != SHAPE_MERMAID) {
      ...
      if (!hasplaced) {
        buf = haven::format_text("`168buttocks`x");
        word += buf.data();
        hasplaced = TRUE;
        blast = FALSE;
      }
    ...
    if (!is_covered(ch, COVERS_LOWER_ARMS) && !is_covered(ch, COVERS_UPPER_ARMS)) {
      ...
      if (!hasplaced) {
        buf = haven::format_text("forearms");
        word += buf.data();
        hasplaced = TRUE;
        blast = FALSE;
      }
      ...
      if (!hasplaced) {
        buf = haven::format_text("upper arms");
        word += buf.data();
        hasplaced = TRUE;
        blast = FALSE;
      }
    ...
    if (blast) {
      buf = haven::format_text(", and%s.", last.data());
      word += buf.data();
    }
    if (hasplaced)
    return str_dup(word.data());
    else
    return " clothing.";
  }
  ...
  int local_fame_level(CHAR_DATA *ch) {
    ...
    return 2;
    if (ch->race == RACE_LOCAL || ch->race == RACE_CIVIL_SERVANT)
    return 1;
    if(is_ffamily(ch))
    return 1;
    return 0;
  ...
  char *shroud_desc(CHAR_DATA *ch) {
    ...
    if (safe_strlen(ch->pcdata->caura) > 2) {
      if (get_tier(ch) > 3)
      ...
      if (get_tier(ch) == 3) {
        ...
        if (tmp[0] == '`') {
          std::string buf;
          buf = haven::format_text("%c%c%s`x", tmp[0], tmp[1], arg);
          return str_dup(buf.data());
        }
        else {
          std::string buf;
          buf = haven::format_text("`x%s`x", arg);
          return str_dup(buf.data());
        }
      }
      std::string buf;
  ...
  char *format_column(int width, char header[MSL], char field[MSL], int type) {
    ...
    sprintf(buf, "%s", field);
    limit = URANGE(1, limit, (int)sizeof(buf) - 2);
    if (safe_strlen_color(buf) > limit - 1) {
      if (limit >= 2 && buf[limit - 2] == '`') {
        limit = limit + 2;
      }
      else if (buf[limit - 1] == '`') {
        limit = limit + 1;
  ...
  void show_char_to_char_1(CHAR_DATA *victim, CHAR_DATA *ch, int type, bool photo) {
    char attractstring[MSL], dirtystring[MSL];
    std::string buf;
    OBJ_DATA *obj;
    int i;
    int pointer = 0;
    ...
    if (IS_NPC(victim)) {
      ...
      if(victim->faction != 0)
      {
        ...
        if(mfac != NULL)
        {
          buf = haven::format_text("This is a soldier of %s, wearing a %s symbol.\n\r", mfac->name, mfac->symbol);
          string += buf.data();
        }
      }
        ...
        for (int i = 0; i < 33; i++) {
          if (victim->pIndexData->vnum == monster_table[i].vnum) {
            if (get_demon_lvl(i) <= get_skill(ch, SKILL_DEMONOLOGY) * 125) {
              buf = haven::format_text("You think their %s will probably be worth something in an alchemical lab.\n\r", monster_table[i].object);
              string += buf.data();
            }
          }
        }
      ...
      if (photo == FALSE)
      page_to_char(wrap_string(string.data(), get_wordwrap(ch)), ch);
      else
      to_photo_message(ch, string.data());
    ...
    // everyone else
      ...
      if (ch->race == RACE_ANCIENTVAMPIRE || ch->race == RACE_OLDVAMPIRE) {
        if (get_skill(victim, SKILL_VIRGIN) >= 1 && victim->pcdata->virginity_lost == 0) {
          buf = haven::format_text("%s's a `Wvirgin`x.`x\n\r", (victim->sex == SEX_MALE) ? "He" : "She");
          string += buf.data();
        }
      }
      ...
      if (IS_FLAG(victim->act, PLR_GUEST) && (get_guesttrust(ch, victim) < 1 || IS_IMMORTAL(ch)) && victim->pcdata->guest_type == GUEST_PLOT) {
        buf = haven::format_text("(Guest of %s)\n\r", victim->pcdata->guest_of);
        string += buf.data();
      }
      ...
      if (can_see_details(ch, victim)) {
        ...
        if (get_bust(victim) > -1 && !guestmonster(victim)) {
          if (is_masked(victim))
          buf += haven::format_text(" with %s", cup_size[get_bust(victim)]);
          else
          buf += haven::format_text(" in %s %s with %s", (victim->sex == SEX_MALE) ? "his" : "her", age_names[pointer], cup_size[get_bust(victim)]);
        }
        else if (!is_masked(victim) && !guestmonster(victim))
        buf += haven::format_text(" in %s %s", (victim->sex == SEX_MALE) ? "his" : "her", age_names[pointer]);

        if (ch != victim) // && type == LOOK_GLANCE)
        {
          buf += " ";
          buf += relfacing[get_reldirection(victim->facing, ch->facing)];
        }
        /*
        ...
        if (!is_masked(victim) && photo == FALSE) {
          if (get_skill(victim, SKILL_MINDREADING) > 0)
            buf += " (`CMindreading Sensitive`x)";
          if (seems_suggestible(victim) && seems_suffer_sensitive(victim)) {
            buf += ". (`WDesire`D&`rSuffering`x Sensitive`x)`x\n\r";
          }
          else if (seems_suggestible(victim)) {
            buf += ". (`WDesire Sensitive`x)`x\n\r";
          }
          else if (seems_suffer_sensitive(victim)) {
            buf += ". (`rSuffering Sensitive`x)`x\n\r";
          }
          else {
            buf += ".`x\n\r";
          }
        ...
        if (in_haven(ch->in_room) && photo == FALSE && can_shroud(ch) && event_cleanse != 1 && seems_under_black(victim, ch) && !guestmonster(victim) && !higher_power(victim)) {
          ...
          if (can_shroud(ch) && event_cleanse != 1 && (!seems_super(victim) || get_skill(victim, SKILL_OBFUSCATE) > 0) && newbie_level(victim) < 10) {
            ...
            buf = haven::format_text("and `ggreen`x glow.\n\r");
            else if (newbie_level(victim) == 2)
            buf = haven::format_text("and `cblue-green`x glow.\n\r");
            else if (newbie_level(victim) == 3)
            buf = haven::format_text("and `Bblue`x glow.\n\r");
          ...
          buf = haven::format_text("%s aura has a faint `yorange`x ", (victim->sex == SEX_MALE) ? "His" : "Her");
          string += buf.data();
          ...
          if (newbie_level(victim) == 1)
          buf = haven::format_text("%s aura has a faint `ggreen`x glow.\n\r", (victim->sex == SEX_MALE) ? "His" : "Her");
          else if (newbie_level(victim) == 2)
          buf = haven::format_text("%s aura has a faint `cblue-green`x glow.\n\r", (victim->sex == SEX_MALE) ? "His" : "Her");
          else if (newbie_level(victim) == 3)
          buf = haven::format_text("%s aura has a faint `Bblue`x glow.\n\r", (victim->sex == SEX_MALE) ? "His" : "Her");

          string += buf.data();
        }
        if (photo == FALSE && (victim->shape == SHAPE_HUMAN || victim->shape == SHAPE_MERMAID)) {
          const int attraction = get_attract(victim, ch);
        ...
        buf = haven::format_text("%s%s", attractstring, dirtystring);
        else
        buf = haven::format_text("%s", dirtystring);
        ...
        if (!IS_NPC(victim) && victim->pcdata->guard_number > 0 && victim->pcdata->guard_expiration > current_time) {
          ...
          buf = haven::format_text("`DA bodyguard is rarely far away from %s.`x ", (victim->sex == SEX_MALE) ? "him" : "her");
          else if (victim->pcdata->guard_number == 2)
          buf = haven::format_text("`DA pair of bodyguards are rarely far away from %s.`x ", (victim->sex == SEX_MALE) ? "him" : "her");
          else
          buf = haven::format_text("`D%s bodyguards are rarely far away from %s.`x ", number_to_text(victim->pcdata->guard_number), (victim->sex == SEX_MALE) ? "him" : "her");
          string += buf.data();
        }
        ...
        //{
        if (is_masked(victim))
        buf = haven::format_text("%s is %d'%d\" and has ", (victim->sex == SEX_MALE) ? "He" : "She", get_height_feet(victim), get_height_inches(victim));
        else
        buf = haven::format_text("%s is %d'%d\" and has ", (victim->sex == SEX_MALE) ? "He" : "She", get_height_feet(victim), get_height_inches(victim));
        ...
        //&& type == LOOK_GLANCE)
        {
          ...
          if (IS_AFFECTED(victim, AFF_BURSTVESSEL) && !is_covered(victim, COVERS_EYES)) {
            ...
            buf = haven::format_text("%s, %s, and pale bloodshot %s. ", get_skin(ch, victim), get_hair(ch, victim), get_eyes(ch, victim));
            else
            buf = haven::format_text("%s, %s, and bloodshot %s. ", get_skin(ch, victim), get_hair(ch, victim), get_eyes(ch, victim));
          }
          else {
            ...
            buf = haven::format_text("%s, %s, and pale %s. ", get_skin(ch, victim), get_hair(ch, victim), get_eyes(ch, victim));
            else
            buf = haven::format_text("%s, %s, and %s. ", get_skin(ch, victim), get_hair(ch, victim), get_eyes(ch, victim));
          }
            ...
            if (!str_cmp(victim->pcdata->dream_origin, (*it)->name)) {
              for (int i = 0; i < 200; i++) {
                if (!str_cmp(ch->name, (*it)->participants[i]) && (*it)->participant_exp[i] >= 100) {
                  buf = haven::format_text("You recognize it from the world of %s.\n\r", (*it)->name);
                  string += buf.data();
                }
              }
            }
        ...
        if(photo == FALSE)
        {
          if (!is_masked(victim) && !guestmonster(victim)) {
            if ((local_fame_level(victim) > 0 && can_see_local_fame(ch) && photo == FALSE) || get_skill(victim, SKILL_FAMOUS) > 0) {
              if (ch->race != RACE_FACULTY) {
                buf = haven::format_text("%s ", fame_insert(victim));
                string += buf.data();
              }
            }
              ...
              if (victim->pcdata->job_type_one == JOB_FULLEMPLOY) {
                ...
                if (workplace != NULL && workplace->type == PROP_SHOP) {
                  buf = haven::format_text("%s is known for working at %s as %s%s. ", (victim->sex == SEX_MALE) ? "He" : "She", from_color(workplace->propname), smart_a_or_an(victim->pcdata->job_title_one), victim->pcdata->job_title_one);
                }
                else {
                  buf = haven::format_text("%s is known for working from home as %s%s. ", (victim->sex == SEX_MALE) ? "He" : "She", smart_a_or_an(victim->pcdata->job_title_one), victim->pcdata->job_title_one);
                }
                string += buf.data();
              }
              else if (victim->race == RACE_FACULTY && !institute_room(victim->in_room) && safe_strlen(victim->pcdata->job_title_one) > 1) {
                buf = haven::format_text("%s is known for working in the institute as %s%s. ", (victim->sex == SEX_MALE) ? "He" : "She", smart_a_or_an(victim->pcdata->job_title_one), victim->pcdata->job_title_one);
                string += buf.data();
              }
            }
            if (victim->pcdata->wilds_fame_level > 0 && safe_strlen(ch->pcdata->wilds_fame) > 2 && (in_world(ch) == WORLD_WILDS || ch->pcdata->wilds_fame_level > 0 || ch->pcdata->wexp >= 250)) {
              buf = haven::format_text("%s ", wilds_fame_insert(victim));
              string += buf.data();
            }
            if (victim->pcdata->other_fame_level > 0 && safe_strlen(ch->pcdata->other_fame) > 2 && (in_world(ch) == WORLD_OTHER || ch->pcdata->other_fame_level > 0 || ch->pcdata->oexp >= 250)) {
              buf = haven::format_text("%s ", other_fame_insert(victim));
              string += buf.data();
            }
            if (victim->pcdata->godrealm_fame_level > 0 && safe_strlen(ch->pcdata->godrealm_fame) > 2 && (in_world(ch) == WORLD_GODREALM || ch->pcdata->godrealm_fame_level > 0 || ch->pcdata->gexp >= 250)) {
              buf = haven::format_text("%s ", godrealm_fame_insert(victim));
              string += buf.data();
            }
            if (victim->pcdata->hell_fame_level > 0 && safe_strlen(ch->pcdata->hell_fame) > 2 && (in_world(ch) == WORLD_HELL || ch->pcdata->hell_fame_level > 0 || ch->pcdata->hexp >= 250)) {
              buf = haven::format_text("%s ", hell_fame_insert(victim));
              string += buf.data();
            }
            LOCATION_TYPE *hell_capital = get_loc("District 82");
            LOCATION_TYPE *other_capital = get_loc("Lauriea");
            LOCATION_TYPE *godrealm_capital = get_loc("Rhagost");
            LOCATION_TYPE *wilds_capital = get_loc("Navorost");
            if ((ch->pcdata->hexp >= 250 || ch->faction != 0) && hell_capital && hell_capital->base_faction_core != 0 && clan_lookup(hell_capital->base_faction_core) != NULL && clan_lookup(hell_capital->base_faction_core)->stasis ==
                0 && !str_cmp(victim->name, clan_lookup(hell_capital->base_faction_core)
                  ->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of `088Hell`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->hexp >= 250 || ch->faction != 0) && hell_capital && hell_capital->base_society_secondary != 0 && clan_lookup(hell_capital->base_society_secondary) != NULL && clan_lookup(hell_capital->base_society_secondary)->stasis ==
                0 && !str_cmp(victim->name, clan_lookup(hell_capital->base_society_secondary)
                  ->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of `088Hell`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->hexp >= 250 || ch->faction != 0) && hell_capital && hell_capital->base_society != 0 && clan_lookup(hell_capital->base_society) != NULL && clan_lookup(hell_capital->base_society)->stasis ==
                0 && !str_cmp(victim->name, clan_lookup(hell_capital->base_society)
                  ->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of `088Hell`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }

            if ((ch->pcdata->oexp >= 250 || ch->faction != 0) && other_capital && other_capital->base_faction_core != 0 && clan_lookup(other_capital->base_faction_core) != NULL && clan_lookup(other_capital->base_faction_core)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(other_capital->base_faction_core)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `114Fae`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->oexp >= 250 || ch->faction != 0) && other_capital && other_capital->base_society_secondary != 0 && clan_lookup(other_capital->base_society_secondary) != NULL && clan_lookup(other_capital->base_society_secondary)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(other_capital->base_society_secondary)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `114Fae`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->oexp >= 250 || ch->faction != 0) && other_capital && other_capital->base_society != 0 && clan_lookup(other_capital->base_society) != NULL && clan_lookup(other_capital->base_society)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(other_capital->base_society)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `114Fae`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }

            if ((ch->pcdata->gexp >= 250 || ch->faction != 0) && godrealm_capital && godrealm_capital->base_faction_core != 0 && clan_lookup(godrealm_capital->base_faction_core) != NULL && clan_lookup(godrealm_capital->base_faction_core)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(godrealm_capital->base_faction_core)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `152Godrealms`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->gexp >= 250 || ch->faction != 0) && godrealm_capital && godrealm_capital->base_society_secondary != 0 && clan_lookup(godrealm_capital->base_society_secondary) != NULL && clan_lookup(godrealm_capital->base_society_secondary)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(godrealm_capital->base_society_secondary)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `152Godrealms`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->gexp >= 250 || ch->faction != 0) && godrealm_capital && godrealm_capital->base_society != 0 && clan_lookup(godrealm_capital->base_society) != NULL && clan_lookup(godrealm_capital->base_society)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(godrealm_capital->base_society)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `152Godrealms`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }

            if ((ch->pcdata->wexp >= 250 || ch->faction != 0) && wilds_capital && wilds_capital->base_faction_core != 0 && clan_lookup(wilds_capital->base_faction_core) != NULL && clan_lookup(wilds_capital->base_faction_core)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(wilds_capital->base_faction_core)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `226W`227i`228l`227d`226s`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->wexp >= 250 || ch->faction != 0) && wilds_capital && wilds_capital->base_society_secondary != 0 && clan_lookup(wilds_capital->base_society_secondary) != NULL && clan_lookup(wilds_capital->base_society_secondary)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(wilds_capital->base_society_secondary)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `226W`227i`228l`227d`226s`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            if ((ch->pcdata->wexp >= 250 || ch->faction != 0) && wilds_capital && wilds_capital->base_society != 0 && clan_lookup(wilds_capital->base_society) != NULL && clan_lookup(wilds_capital->base_society)->stasis == 0 && !str_cmp(
                  victim->name, clan_lookup(wilds_capital->base_society)->leader)) {
              buf = haven::format_text("\n\r%s is a border lord of the `226W`227i`228l`227d`226s`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }

            if (IS_FLAG(victim->act, PLR_VILLAIN)) {
              buf = haven::format_text("People nearby seem to be acting cautiously, as if afraid of %s.`x\n\r", (victim->sex == SEX_MALE) ? "him" : "her");
              string += buf.data();
            }
            if (has_weakness(ch, victim) && (is_gm(ch) || ch->faction != 0)) {
              buf = haven::format_text("%s is rumored to be psychically protected.`x\n\r", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }

            if (victim->race == RACE_BROWN && ch->race == RACE_BROWN) {
              buf = haven::format_text("You think you recognize them from campus.\n\r");
              string += buf.data();
            }
          }
          ...
          //&& type == LOOK_GLANCE)
          {
            ...
            if (victim->pcdata->impregnated + (3600 * 24 * 7 * weeks_late) <
                current_time) {
              buf = haven::format_text("%s is in the late stages of pregnancy.`x ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
            else if (victim->pcdata->impregnated + (3600 * 24 * 7 * weeks_mid) <
                current_time) {
              buf = haven::format_text("%s is in the middle stages of pregnancy.`x ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
              ...
              if (covered == FALSE) {
                buf = haven::format_text("%s is in the early stages of pregnancy.`x ", (victim->sex == SEX_MALE) ? "He" : "She");
                string += buf.data();
              }
            }
          }
        ...
        for (i = 0; i < 4; i++) {
          if (!IS_NPC(victim) && victim->pcdata->mark_timer[i] > 0 && safe_strlen(victim->pcdata->mark[i]) > 2) {
            buf = haven::format_text("%s ", victim->pcdata->mark[i]);
            string += buf.data();
          }
        }
        if (IS_FLAG(victim->affected_by, AFF_STAKED)) {
          buf = haven::format_text("A stake has been lodged in %s heart. ", (victim->sex == SEX_MALE) ? "his" : "her");
          string += buf.data();
        }

        // critically wounded
        if (victim->wounds == 3) {
          buf = haven::format_text("`RThey are bleeding to death. `x ");
          string += buf.data();
        }
        if (!str_cmp(victim->name, ch->pcdata->enthralled) && (has_blackeyes(victim) || !is_abom(victim))) {
          buf = haven::format_text("`RThey are your master. `x ");
          string += buf.data();
        }
        if (!str_cmp(victim->name, ch->pcdata->enraptured) && (has_blackeyes(victim) || !is_abom(victim))) {
          buf = haven::format_text("`RThey are your adored. `x ");
          string += buf.data();
        }
        ...
        if (ch->recent_moved <= -150 && victim->recent_moved <= -150 && victim->wounds > 0 && victim->wounds < 3) {
          if (get_skill(ch, SKILL_MEDICINE) >= 4) {
            if (victim->wounds == 1)
            buf = haven::format_text("They seem mildly injured, recovery will probably take %d hours.\n\r", victim->heal_timer / 1440);
            else if (victim->wounds == 2 && !can_heal(victim))
            buf = haven::format_text("They are severely injured and need treatment from another character before they can recover.\n\r");
            else if (victim->wounds == 2)
            buf = haven::format_text("They seem severely injured, recovery will probably take %d hours.\n\r", victim->heal_timer / 1440);
            string += buf.data();
          }
          else if (get_skill(ch, SKILL_MEDICINE) >= 3) {
            if (victim->wounds == 1)
            buf = haven::format_text("They seem mildly injured.\n\r");
            else if (victim->wounds == 2)
            buf = haven::format_text("They seem severely injured.\n\r");
            string += buf.data();
          }
        }
        ...
        if (get_tier(ch) >= 3 && get_tier(victim) < 3 && photo == FALSE && victim->skills[SKILL_MENTALDISCIPLINE] <= 0 && victim->pcdata->attract[ATTRACT_PROM] < 100) {
          if (alt_count(victim) <= 1 && victim->lf_taken <= 25 && (victim->pcdata->intel >= 2500 || victim->pcdata->heroic >= 50)) {
            buf = haven::format_text("`161They look tasty.`x\n\r");
            string += buf.data();
          }
        }
        ...
        {
          buf = haven::format_text("You are fixed on them to level %d(%d charges left).\n\r", ch->pcdata->fixation_level, ch->pcdata->fixation_charges);
          string += buf.data();
        }

        if (get_skill(ch, SKILL_SENSEWEAKNESS) > 0 && photo == FALSE) {
          if (IS_AFFECTED(victim, AFF_WEAKEN) || IS_AFFECTED(victim, AFF_PENALTY)) {
            buf = haven::format_text("You sense a mystical weakness. ");
            string += buf.data();
          }
          if (victim->wounds == 1) {
            buf = haven::format_text("You sense a faint weakness caused by injury. ");
            string += buf.data();
          }
          if (victim->wounds >= 2) {
            buf = haven::format_text("You sense a potent weakness caused by injury. ");
            string += buf.data();
          }
          if (is_ill(victim)) {
            buf = haven::format_text("You sense a weakness cause by illness. ");
            string += buf.data();
          }
          if (victim->lf_taken >= 500) {
            buf = haven::format_text("You sense a weakness cause by trauma. ");
            string += buf.data();
          }
          if (is_weakness(ch, victim)) {
            buf = haven::format_text("You sense they are a psychological weakness. ");
            string += buf.data();
          }
          if (victim->pcdata->attract[ATTRACT_PROM] >= 1000) {
            buf = haven::format_text("You sense that sex is their weakness. ");
            string += buf.data();
          }
        }
        if (IS_FLAG(victim->comm, COMM_BLINDFOLD)) {
          buf = haven::format_text("`D%s`D is blindfolded. `x\n\r", PERS(victim, ch));
          string += buf.data();
        }
        if (IS_FLAG(victim->comm, COMM_GAG)) {
          buf = haven::format_text("`D%s`D is gagged. `x\n\r", PERS(victim, ch));
          string += buf.data();
        }
        if (IS_FLAG(victim->act, PLR_BOUND) && IS_FLAG(victim->act, PLR_BOUNDFEET)) {
          buf = haven::format_text("`c%s`c is bound hand and foot. `x\n\r", PERS(victim, ch));
          string += buf.data();
        }
        if (IS_FLAG(victim->act, PLR_BOUND) && !IS_FLAG(victim->act, PLR_BOUNDFEET)) {
          buf = haven::format_text("`c%s's`c hands are bound. `x\n\r", PERS(victim, ch));
          string += buf.data();
        }
        if (!IS_FLAG(victim->act, PLR_BOUND) && IS_FLAG(victim->act, PLR_BOUNDFEET)) {
          buf = haven::format_text("`c%s`c is shackled. `x\n\r", PERS(victim, ch));
          string += buf.data();
        }
        ...
        if (is_possessed(victim) && ch != victim && photo == FALSE && (get_skill(ch, SKILL_CLAIRVOYANCE) >= 2 || IS_FLAG(get_possesser(victim)->comm, COMM_PRIVATE)) && ch != victim) {
          buf = haven::format_text("You see `W%s`x inside them. ", PERS(get_possesser(victim), ch));
          string += buf.data();
        }
        ...
        if (is_smelly(victim) && photo == FALSE) {
          buf = haven::format_text("%s is giving off quite a `ystench`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
          string += buf.data();
        }

        // wetness lines
        if (victim->pcdata->wetness > 35) {
          buf = haven::format_text("%s is soaking `Bwet`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
          string += buf.data();
        }
        else if (victim->pcdata->wetness > 15) {
          buf = haven::format_text("%s is `Bwet`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
          string += buf.data();
        }
        else if (victim->pcdata->wetness > 0) {
          buf = haven::format_text("%s is damp`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
          string += buf.data();
        }
        ...
        if (!is_animal(victim) && photo == FALSE) {
          if (ch != victim && is_vampire(ch) && biteable(ch, victim, FALSE)) {
            buf = haven::format_text("`r%s is biteable.`x ", (victim->sex == SEX_MALE) ? "He" : "She");
            string += buf.data();
          }
          if (get_skill(ch, SKILL_KNOWVAMP) > 0 && seems_vampire(victim) && get_skill(victim, SKILL_OBFUSCATE) == 0) {
            buf = haven::format_text("%s is a `Dvampire`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
            string += buf.data();
          }
          if(college_staff(ch, FALSE) && college_student(victim, FALSE))
          {
            buf = haven::format_text("This is %s %s %s. ", (victim->sex == SEX_MALE) ? "Mr" : "Miss", victim->name, victim->pcdata->last_name);
            string += buf.data();
          }
          ...
          if(photo == FALSE)
          {
            ...
            {
              buf = haven::format_text("%s family has made an endowment to the institute. ", (victim->sex == SEX_MALE) ? "His" : "Her");
              string += buf.data();
            }
            if (get_skill(ch, SKILL_KNOWWERE) > 0 && seems_werewolf(victim) && get_skill(victim, SKILL_OBFUSCATE) == 0) {
              buf = haven::format_text("%s is a `ywerewolf`x. ", (victim->sex == SEX_MALE) ? "He" : "She");
              string += buf.data();
            }
        ...
        if (!is_animal(victim) && !guestmonster(victim)) {
          buf = haven::format_text("You can see %s%s ", (victim->sex == SEX_MALE) ? "his" : "her", get_exposed_locations(victim));
          string += buf.data();

          if (safe_strlen(get_detailed_locations(victim)) > 3) {
            buf = haven::format_text("More detail is visible for %s%s ", (victim->sex == SEX_MALE) ? "his" : "her", get_detailed_locations(victim));
            string += buf.data();
          }

          if (is_vampire(victim) && base_lifeforce(victim) <= 8000) {
            buf = haven::format_text("%s seems gaunt and unhealthy.", (victim->sex == SEX_MALE) ? "He" : "She");
            string += buf.data();
          }
        ...
        if (!IS_NPC(victim) && victim->race == RACE_WIGHT) {
          if (victim->pcdata->decay_stage == 1) {
            buf = haven::format_text("%s seems pale.", (victim->sex == SEX_MALE) ? "He" : "She");
            string += buf.data();
          }
            ...
            if (victim->pcdata->decay_stage >= 2 && !is_covered(victim, COVERS_HANDS)) {
              buf = haven::format_text(", `y%s hands are spotted with decaying flesh`x", (victim->sex == SEX_MALE) ? "his" : "her");
              string += buf.data();
            }
            ...
            if (victim->pcdata->decay_stage >= 3 && !is_covered(victim, COVERS_UPPER_ARMS)) {
              buf = haven::format_text(", `y%s arms are spotted with decaying flesh`x", (victim->sex == SEX_MALE) ? "his" : "her");
              string += buf.data();
            }
            if (victim->pcdata->decay_stage >= 4 && !is_covered(victim, COVERS_BREASTS)) {
              buf = haven::format_text(", `y%s torso is spotted with decaying flesh`x", (victim->sex == SEX_MALE) ? "his" : "her");
              string += buf.data();
            }
            if (victim->pcdata->decay_stage >= 5 && !is_covered(victim, COVERS_LOWER_FACE)) {
              buf = haven::format_text(", `y%s face is spotted with decaying flesh`x", (victim->sex == SEX_MALE) ? "his" : "her");
              string += buf.data();
            }
          }
        }
      ...
      if (type == LOOK_LOOK) {
        ...
        string += dream_detail(victim, ch->pcdata->identity_world, DREAM_DETAIL_DESC);
        else
        string += get_focused(ch, victim, FALSE);
      }

      // equipment and clothing
      string += equip_string(ch, victim);
  ...
  void introduce_char_to_char(CHAR_DATA *victim, CHAR_DATA *ch) {
    ...
    if (get_bust(victim) > -1 && !guestmonster(victim)) {
      if (is_masked(victim))
      snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), " with %s", cup_size[get_bust(victim)]);
      else
      snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), " in %s %s with %s", (victim->sex == SEX_MALE) ? "his" : "her", age_names[pointer], cup_size[get_bust(victim)]);
    }
    else if (!is_masked(victim) && !guestmonster(victim))
    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), " in %s %s", (victim->sex == SEX_MALE) ? "his" : "her", age_names[pointer]);

    if (get_skill(victim, SKILL_MINDREADING) > 0)
      strcat(buf, " (`CMindreading Sensitive`x)");
  ...
  bool can_see_wear_equipped(CHAR_DATA *ch, int iWear, const haven::EquipmentSnapshot &equipment) {
    switch (iWear) {
    default:
      return TRUE;
      break;
    case WEAR_BODY_24:
      if ((equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_24)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_23:
      if ((equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_23))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_23)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_22:
      if ((equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_22))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_22))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_22)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_21:
      if ((equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_21))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_21))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_21))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_21)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_20:
      if ((equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_20))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_20))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_20))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_20))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_20)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_19:
      if ((equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_19))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_19))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_19))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_19))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_19))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_19)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_18:
      if ((equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_18))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_18))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_18))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_18))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_18))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_18))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_18)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_17:
      if ((equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_17))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_17))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_17))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_17))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_17))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_17))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_17))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_17)))) {
        return TRUE;
      }
      else {
        return FALSE;
      ...
      break;
    case WEAR_BODY_16:
      if ((equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_16))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_16))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_15:
      if ((equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_15))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_15))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_14:
      if ((equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_14))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_14))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_13:
      if ((equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_13))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_13))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_12:
      if ((equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_12))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_12))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_11:
      if ((equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_11))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_11))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_10:
      if ((equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_10))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_10))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_9:
      if ((equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_9))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_9))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_8:
      if ((equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_8))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_8))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_7:
      if ((equipment.get(WEAR_BODY_8) == NULL || !does_conceal(equipment.get(WEAR_BODY_8), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_7))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_7))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_6:
      if ((equipment.get(WEAR_BODY_7) == NULL || !does_conceal(equipment.get(WEAR_BODY_7), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_8) == NULL || !does_conceal(equipment.get(WEAR_BODY_8), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_6))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_6))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_5:
      if ((equipment.get(WEAR_BODY_6) == NULL || !does_conceal(equipment.get(WEAR_BODY_6), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_7) == NULL || !does_conceal(equipment.get(WEAR_BODY_7), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_8) == NULL || !does_conceal(equipment.get(WEAR_BODY_8), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_5))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_5))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_4:
      if ((equipment.get(WEAR_BODY_5) == NULL || !does_conceal(equipment.get(WEAR_BODY_5), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_6) == NULL || !does_conceal(equipment.get(WEAR_BODY_6), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_7) == NULL || !does_conceal(equipment.get(WEAR_BODY_7), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_8) == NULL || !does_conceal(equipment.get(WEAR_BODY_8), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_4))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_4))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_3:
      if ((equipment.get(WEAR_BODY_4) == NULL || !does_conceal(equipment.get(WEAR_BODY_4), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_5) == NULL || !does_conceal(equipment.get(WEAR_BODY_5), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_6) == NULL || !does_conceal(equipment.get(WEAR_BODY_6), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_7) == NULL || !does_conceal(equipment.get(WEAR_BODY_7), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_8) == NULL || !does_conceal(equipment.get(WEAR_BODY_8), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_3))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_3))))
      return TRUE;
      else
      return FALSE;
      break;
    case WEAR_BODY_2:
      if ((equipment.get(WEAR_BODY_3) == NULL || !does_conceal(equipment.get(WEAR_BODY_3), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_4) == NULL || !does_conceal(equipment.get(WEAR_BODY_4), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_5) == NULL || !does_conceal(equipment.get(WEAR_BODY_5), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_6) == NULL || !does_conceal(equipment.get(WEAR_BODY_6), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_7) == NULL || !does_conceal(equipment.get(WEAR_BODY_7), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_8) == NULL || !does_conceal(equipment.get(WEAR_BODY_8), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_2))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_2))))
      return TRUE;
      else {
        return FALSE;
      }
      break;
    case WEAR_BODY_1:
      if ((equipment.get(WEAR_BODY_2) == NULL || !does_conceal(equipment.get(WEAR_BODY_2), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_3) == NULL || !does_conceal(equipment.get(WEAR_BODY_3), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_4) == NULL || !does_conceal(equipment.get(WEAR_BODY_4), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_5) == NULL || !does_conceal(equipment.get(WEAR_BODY_5), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_6) == NULL || !does_conceal(equipment.get(WEAR_BODY_6), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_7) == NULL || !does_conceal(equipment.get(WEAR_BODY_7), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_8) == NULL || !does_conceal(equipment.get(WEAR_BODY_8), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_9) == NULL || !does_conceal(equipment.get(WEAR_BODY_9), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_10) == NULL || !does_conceal(equipment.get(WEAR_BODY_10), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_11) == NULL || !does_conceal(equipment.get(WEAR_BODY_11), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_12) == NULL || !does_conceal(equipment.get(WEAR_BODY_12), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_13) == NULL || !does_conceal(equipment.get(WEAR_BODY_13), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_14) == NULL || !does_conceal(equipment.get(WEAR_BODY_14), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_15) == NULL || !does_conceal(equipment.get(WEAR_BODY_15), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_16) == NULL || !does_conceal(equipment.get(WEAR_BODY_16), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_17) == NULL || !does_conceal(equipment.get(WEAR_BODY_17), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_18) == NULL || !does_conceal(equipment.get(WEAR_BODY_18), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_19) == NULL || !does_conceal(equipment.get(WEAR_BODY_19), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_20) == NULL || !does_conceal(equipment.get(WEAR_BODY_20), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_21) == NULL || !does_conceal(equipment.get(WEAR_BODY_21), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_22) == NULL || !does_conceal(equipment.get(WEAR_BODY_22), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_23) == NULL || !does_conceal(equipment.get(WEAR_BODY_23), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_24) == NULL || !does_conceal(equipment.get(WEAR_BODY_24), equipment.get(WEAR_BODY_1))) && (equipment.get(WEAR_BODY_25) == NULL || !does_conceal(equipment.get(WEAR_BODY_25), equipment.get(WEAR_BODY_1))))
      return TRUE;
      else {
        return FALSE;
      }
  ...
  bool can_see_wear(CHAR_DATA *ch, int iWear) {
    // These slots have no outer clothing layers to inspect.
    if (iWear < WEAR_BODY_1 || iWear >= WEAR_BODY_25) return TRUE;
    const haven::EquipmentSnapshot equipment(ch);
    return can_see_wear_equipped(ch, iWear, equipment);
  }
  ...
  struct LookCharacter {
    CHAR_DATA *character;
    int attraction;
  };

  static std::vector<LookCharacter> sorted_look_characters(CharList *list, CHAR_DATA *ch) {
    std::vector<LookCharacter> result;
    result.reserve(list->size());
    for (CharList::iterator it = list->begin(); it != list->end(); ++it) {
      if (*it != NULL)
        result.push_back(LookCharacter{*it, get_attract(*it, ch)});
    }
    // Scores are local to this display: equipment and viewer preferences can change.
    std::sort(result.begin(), result.end(), [](const LookCharacter &a, const LookCharacter &b) {
  ...
  void show_char_to_char(CharList *list, CHAR_DATA *ch, int moving) {
    ...
    try {
      ...
      std::vector<LookCharacter> sortedList = sorted_look_characters(list, ch);

      bool have_last = false;
      int last_attraction = 0;

      for (std::vector<LookCharacter>::iterator it = sortedList.begin(); it != sortedList.end(); ++it) {
        CHAR_DATA *rch = it->character;
        ...
        if (have_last && (last_attraction - it->attraction) > 10) {
          send_to_char("\n\r", ch);
        }
        ...
        if (placefound == FALSE) {
          ...
          for (int i = 0; i < 20; i++) {
            if (safe_strlen(dynamicplaces[i]) < 2 && placeplaced == FALSE) {
              dynamicplaces[i] = person->pcdata->place;
              placeplaced = TRUE;
  ...
  void show_room_to_char(CHAR_DATA *ch, ROOM_INDEX_DATA *room) {
    ...
    if (unfinished_room_description(room->description) || room_uses_stock_description(room)) {
      page += stock_description(ch, room); // this is in world.c
      page += "`x\n\r";
    }
    ...
    */
    ...
    for (DescList::iterator it = descriptor_list.begin(); it != descriptor_list.end(); ++it) {
      ...
      if(hp->in_room == ch->in_room && higher_power(hp) && hp->possessing == NULL)
      {
        ...
        {
          buf = haven::format_text("You sense an unworldly presence.\n\r");
          page += buf.data();
        }
      }
    }
    ...
    if (!IS_SET(current_room->room_flags, ROOM_INDOORS)) {
      ...
      if (sunpos > -1) {
        if (phase == 2) {
          buf = haven::format_text("In the distance %s the `rda`yw`Yn`yin`rg `Ys`Ru`Yn`x crests the horizon. ", relspacial[get_reldirection(sunpos, ch->facing)]);
          page += buf.data();
        }
        if (phase == 3 || phase == 5) {
          buf = haven::format_text("In the distance %s the sun hangs in the sky. ", relspacial[get_reldirection(sunpos, ch->facing)]);
          page += buf.data();
        }
        if (phase == 4) {
          if (sunpos == DIR_UP) {
            buf = haven::format_text("The sun hangs in the air far overhead. ");
          }
          else {
            buf = haven::format_text("In the distance %s the sun hangs in the sky. ", relspacial[get_reldirection(sunpos, ch->facing)]);
          }
        ...
        if (phase == 6) {
          buf = haven::format_text("In the distance %s the `rsetting s`yu`rn`x, sinks below the horizon. ", relspacial[get_reldirection(sunpos, ch->facing)]);
          page += buf.data();
        }
      }
    }
    ...
    if (current_room->exit[ch->facing] != NULL && can_see_room_distance(ch, current_room->exit[ch->facing]->u1.to_room, DISTANCE_NEAR)) {
      ...
      buf = haven::format_text("Ahead of you is %s. ", nocol);
      page += buf.data();
    }
    ...
    if (!is_dreaming(ch)) {
      if (!IS_SET(current_room->room_flags, ROOM_INDOORS)) {
        ...
        if (can_hear_ocean(ch) && current_room->sector_type != SECT_WATER && current_room->sector_type != SECT_UNDERWATER && current_room->sector_type != SECT_SHALLOW) {
          buf = haven::format_text("You can hear the sound of waves %s. ", relspacial[get_reldirection(
        ...
        if (distance_from_town(current_room) > 1 && current_room->z >= 0 && !IS_SET(current_room->room_flags, ROOM_INDOORS) && !is_helpless(ch)) {
          ...
          if (miles > 0)
          buf = haven::format_text("You estimate you're about %.2f miles from the edge of town. ", ((double)(miles)) / 100);
          else
          buf = haven::format_text("You estimate you're about %d feet from the edge of town. ", dtotown);
          page += buf.data();
        }
        ...
        if (current_room->sector_type == SECT_STREET || current_room->sector_type == SECT_ALLEY || current_room->sector_type == SECT_SIDEWALK) {
          for (int dir = 0; dir < 10; dir++) {
            ...
            continue;
            ROOM_INDEX_DATA *neighbor = current_room->exit[dir] == NULL ? NULL : current_room->exit[dir]->u1.to_room;
            PROP_TYPE *property = neighbor == NULL ? NULL : room_prop(neighbor);
            if (property != NULL && neighbor->vnum == property->firstroom) {
              if (property->type == PROP_HOUSE) {
                buf = haven::format_text("%s is %s. ", property->address, relspacial[get_reldirection(dir, ch->facing)]);
                page += buf.data();
              }
              else {
                buf = haven::format_text("%s is %s. ", property->propname, relspacial[get_reldirection(dir, ch->facing)]);
                page += buf.data();
              }
            }
          }
        ...
        if (charlineofsight_landmark(ch, get_room_index(8402))) {
          if (get_dist(get_roomx(current_room), get_roomy(current_room), 48, 9) <= 50) {
            ...
            if (sunphase(current_room) == 0) {
              buf = haven::format_text("In the distance %s you see the `Willuminated face`x of the clock tower, reading %d:%02d. ", relspacial[get_reldirection(
              roomdirection(get_roomx(current_room), get_roomy(current_room), 48, 9), ch->facing)], hours, minutes);
              page += buf.data();
            }
            else {
              buf = haven::format_text("In the distance %s you see the face of the clock tower, reading %d:%02d. ", relspacial[get_reldirection(
              roomdirection(get_roomx(current_room), get_roomy(current_room), 48, 9), ch->facing)], hours, minutes);
              page += buf.data();
            }
          }
            ...
            if (sunphase(current_room) == 0) {
              buf = haven::format_text("Far off in the distance %s you see the `Willuminated face`x of the clock tower. ", relspacial[get_reldirection(
              roomdirection(get_roomx(current_room), get_roomy(current_room), 48, 9), ch->facing)]);
              page += buf.data();
            }
            else {
              buf = haven::format_text("Far off in the distance %s you see the clock tower. ", relspacial[get_reldirection(
              roomdirection(get_roomx(current_room), get_roomy(current_room), 48, 9), ch->facing)]);
              page += buf.data();
            }
          }
        ...
        if (charlineofsight_landmark(ch, get_room_index(10156))) {
          ...
          if (get_dist(get_roomx(current_room), get_roomy(current_room), 58, 25) <= (50 + get_skill(ch, SKILL_ACUTESIGHT) * 25) || (nightlight == TRUE && mist_level(get_room_index(10249)) < 3 && get_dist(get_roomx(current_room), get_roomy(current_room), 58, 25) <= (100 + get_skill(ch, SKILL_ACUTESIGHT) * 50))) {
            if (nightlight == TRUE || mist_level(get_room_index(1468)) >= 2) {
              buf = haven::format_text("In the distance %s you see the `Ysweeping beam`x of the lighthouse.", relspacial[get_reldirection(
              roomdirection(get_roomx(current_room), get_roomy(current_room), 58, 25), ch->facing)]);
              page += buf.data();
            }
            else {
              buf = haven::format_text("In the distance %s you can see the lighthouse sitting atop the bluffs.", relspacial[get_reldirection(
              roomdirection(get_roomx(current_room), get_roomy(current_room), 58, 25), ch->facing)]);
              page += buf.data();
            }
          }
  ...
  static int collect_look_places(ROOM_INDEX_DATA *room, char **places, int capacity) {
    ...
    for (CharList::iterator it = room->people->begin(); it != room->people->end(); ++it) {
      CHAR_DATA *person = *it;
      if (person == NULL || person->in_room == NULL || IS_NPC(person) ||
          person->pcdata == NULL || safe_strlen(person->pcdata->place) <= 1)
        continue;
      char *place = person->pcdata->place;
      bool found = FALSE;
      for (int i = 0; i < count; ++i) {
        if (!str_cmp(place, places[i])) {
          found = TRUE;
          break;
        }
      }
      if (found)
        continue;
      for (EXTRA_DESCR_DATA *ed = room->places; ed; ed = ed->next) {
        if (!str_cmp(place, ed->keyword)) {
          found = TRUE;
          break;
        }
      }
  ...
  static void look_room_signals(ROOM_INDEX_DATA *room, CHAR_DATA *viewer,
    ...
    if (IS_NPC(viewer) || viewer->pcdata == NULL)
      return;
    for (DescList::iterator it = descriptor_list.begin(); it != descriptor_list.end(); ++it) {
      DESCRIPTOR_DATA *d = *it;
      if (d->connected != CON_PLAYING)
        continue;
      CHAR_DATA *character = CH(d);
      if (character == NULL || character->in_room != room || IS_NPC(character) ||
          character->pcdata == NULL || character == viewer ||
          character->pcdata->account == viewer->pcdata->account ||
          IS_FLAG(character->act, PLR_SHROUD) || is_gm(character))
        continue;
      if (!desire) desire = seems_suggestible(character);
      if (!suffering) suffering = seems_suffer_sensitive(character);
      if (!fate) fate = seems_fatesensitive(character);
      if (!mindreading) mindreading = get_skill(character, SKILL_MINDREADING) > 0;
      if (desire && suffering && fate && mindreading)
        break;
    }
  }
  ...
  _DOFUN(do_look) {
    ...
    if (arg1[0] == '\0' || !str_cmp(arg1, "auto")) {
      ...
      /* 'look' or 'look auto' */
      if (!is_dreaming(ch) && in_world(ch) == WORLD_EARTH && full_moon())
        send_to_char("`Y*** `WIT IS THE FULL MOON`Y ***`x\n\r", ch);
      if (is_dreaming(ch)) {
      ...
      if (IS_FLAG(ch->act, PLR_DEEPSHROUD)) {
        ...
        char *dynamicplaces[20] = {};
        int dynamic_count = collect_look_places(current_room, dynamicplaces, 20);
        if (dynamic_count > 0) {
          char buf2[MSL];
          sprintf(buf2, "Dynamic Places: ");
          for (int i = 0; i < dynamic_count; i++) {
            if (safe_strlen(dynamicplaces[i]) > 2) {
              ...
              if (i + 1 < dynamic_count && safe_strlen(dynamicplaces[i + 1]) > 2) {
                strcat(buf2, ", ");
              }
            }
          }
  ...
  int total_disc(CHAR_DATA *ch) {
    int i, total = 0;
    for (i = 0; i < discipline_table_count; i++) {
  ...
  _DOFUN(do_score) {
    ...
    string = haven::format_text("%sRewards", fields);
    printf_to_char(ch, "%s", string.data());
    spacing = 31 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sHours%s: %s%d", fields, border, data, (target->played + (int)(current_time - target->logon)) / 3600);
    printf_to_char(ch, "%s", string.data());
    spacing = 40 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sSuper", fields);
    printf_to_char(ch, "%s", string.data());
    spacing = 6 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s|`x\n\r", border);
    // Line 3 - Credit data, Super data
    printf_to_char(ch, "%s| ", border);
    string = " Available to everyone";
    printf_to_char(ch, "%s%s", data, string.data());
    spacing = 72 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%s[%s%s%s]", border, data, (is_super(target)) ? "X" : " ", border);
    printf_to_char(ch, "%s", string.data());
    spacing = 5 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s|`x\n\r", border);
    // Line 4 - Name, Sex, Tier Level
    printf_to_char(ch, "%s| ", border);
    string = haven::format_text("%s       Name%s: %s%s %s", fields, border, data, target->name, target->pcdata->last_name);
    printf_to_char(ch, "%s", string.data());
    spacing = 33 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sSex%s: %s%s", fields, border, data, (target->sex == SEX_MALE) ? "Male" : "Female");
    printf_to_char(ch, "%s", string.data());
    spacing = 19 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sConcept Tier%s: %s%s", fields, border, data, tier_names[get_tier(target)]);
    printf_to_char(ch, "%s", string.data());
    spacing = 25 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s|`x\n\r", border);

    // Line 5 - Age, Concept Box Border
    printf_to_char(ch, "%s| ", border);
    string = haven::format_text("%s        Age%s: %s%d", fields, border, data, get_real_age(target));
    printf_to_char(ch, "%s", string.data());
    spacing = 52 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%s,----------------------,", border);
    printf_to_char(ch, "%s", string.data());
    spacing = 25 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s|`x\n\r", border);
    ...
    if (get_age(target) != get_real_age(target)) {
      string = haven::format_text("%sApparent%s: %s%d", fields, border, data, get_age(target));
      printf_to_char(ch, "%s", string.data());
      spacing = 39 - safe_strlen_color(string.data());
    }
    else {
      spacing = 39;
    }
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sArchetype %s| ", fields, border);
    printf_to_char(ch, "%s", string.data());
    spacing = 12 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%s%s", data, archetype);
    printf_to_char(ch, "%s", string.data());
    spacing = 21 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s| |`x\n\r", border);
    // Line 7 - Death, Modifier
    printf_to_char(ch, "%s| ", border);
    string = haven::format_text("%s       Born%s: %s%s %d%s, %d", fields, border, data, month_names[target->pcdata->birth_month - 1], target->pcdata->birth_day, datesuffix(target->pcdata->birth_day), target->pcdata->birth_year);
    printf_to_char(ch, "%s", string.data());
    spacing = 43 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sModifier %s| ", fields, border);
    printf_to_char(ch, "%s", string.data());
    spacing = 11 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%s%s", data, modifier_table[target->modifier].name);
    printf_to_char(ch, "%s", string.data());
    spacing = 21 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s| |`x\n\r", border);
    ...
    if (target->pcdata->doom_date > 1) {
      ...
      string = haven::format_text("%s       Dies%s:`x %s %d%s, %d", fields, border, month, day, get_day_suffix(day), year);
    ...
    if (is_vampire(target)) {
      printf_to_char(ch, "%s| ", border);
      string = haven::format_text("%sSired%s:`x %s %d%2s, %d", fields, border,
              month_names[target->pcdata->sire_month - 1], target->pcdata->sire_day,
              datesuffix(target->pcdata->sire_day), target->pcdata->sire_year);
      printf_to_char(ch, "%s", string.data());
      spacing = UMAX(0, 76 - safe_strlen_color(string.data()));
      printf_to_char(ch, "%*s%s |`x\n\r", spacing, spacer, border);
    }
    ...
    printf_to_char(ch, "%s| ", border);
    string = haven::format_text("%sAssociation%s: %s%s", fields, border, data, get_subfac_name(target->pcdata->association));
    printf_to_char(ch, "%s", string.data());
    spacing = 38 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    if (safe_strlen(ch->pcdata->home_territory) > 0) {
      string = haven::format_text("%sTerritory%s: %s%-s", fields, border, data, target->pcdata->home_territory);
    }
    else {
      string = haven::format_text("%sTerritory%s: %sNone", fields, border, data);
    }
    printf_to_char(ch, "%s", string.data());
    spacing = 38 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    ...
    spacing = 13;
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sCash", fields);
    printf_to_char(ch, "%s", string.data());
    spacing = 25 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sBanked", fields);
    printf_to_char(ch, "%s", string.data());
    spacing = 28 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%sInfluence", fields);
    printf_to_char(ch, "%s", string.data());
    spacing = 10 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s |`x\n\r", border);
    ...
    spacing = 3;
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("`G$`x%.2f", value);
    printf_to_char(ch, "%sCurrency%s:`x %s", fields, border, string.data());
    spacing = 25 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("`G$`x%.2f", amount);
    printf_to_char(ch, "%s%s", data, string.data());
    spacing = 27 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%d", target->pcdata->influence);
    printf_to_char(ch, "%s%s`x %s", fields, border, string.data());
    spacing = 10 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s |`x\n\r", border);
    ...
    spacing = 4;
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%d", UMAX(0, get_energy(target)));
    printf_to_char(ch, "%sFatigue%s:`x %s", fields, border, string.data());
    spacing = 13 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    if (get_display_lfmod(target) > 0) {
      string = haven::format_text("%d%% (+%d)", get_display_lifeforce(target), get_display_lfmod(target));
    }
    else if (get_display_lfmod(target) < 0) {
      string = haven::format_text("%d%% (%d)", get_display_lifeforce(target), get_display_lfmod(target));
    }
    else {
      string = haven::format_text("%d%%", get_display_lifeforce(target));
    }
    printf_to_char(ch, "%sLife Force%s:`x %s", fields, border, string.data());
    spacing = 18 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    string = haven::format_text("%d %s/`x %d", target->hit, border, max_hp(target));
    printf_to_char(ch, "%sDefenses%s:`x %s", fields, border, string.data());
    spacing = 10 - safe_strlen_color(string.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    printf_to_char(ch, "%s |`x\n\r", border);
    // Line 17 - Wounds
    ...
    if (is_ghost(target)) {
      ...
      spacing = 6;
      printf_to_char(ch, "%*s", spacing, spacer);
      string = haven::format_text("%d", target->pcdata->ghost_pool);
      printf_to_char(ch, "%sWillpower%s:`x %-s", fields, border, string.data());
      spacing = 61 - safe_strlen_color(string.data());
      printf_to_char(ch, "%*s", spacing, spacer);
      printf_to_char(ch, "%s|`x\n\r", border);
    }
  ...
  _DOFUN(do_oldscore) {
    ...
    buf = haven::format_text("`B#`x %-77.77s`B#`x\n\r", tempbuf);
    outbuf.strcat(buf.data());

    buf = haven::format_text("`B#`x %-64s `B#`x\n\r", " ");
    outbuf.strcat(buf.data());

    if (has_con(ch, SCON_POLICE) || ch->pcdata->police_timer > 0) {
      sprintf(tempbuf, " `RYou are wanted by the police`x");

      buf = haven::format_text("`B#`x %-69.69s`B#`x\n\r", tempbuf);
      outbuf.strcat(buf.data());
    ...
    buf = haven::format_text("`B#`x %-79.79s`B#`x\n\r", tempbuf);
    outbuf.strcat(buf.data());
    ...
    buf = haven::format_text("`B#`x %-70.70s     `B#`x\n\r", tempbuf);
    outbuf.strcat(buf.data());

    sprintf(tempbuf, "                                        `WInfluence : `x%-4d", UMAX(0, ch->pcdata->influence));

    buf = haven::format_text("`B#`x %-64.64s     `B#`x\n\r", tempbuf);
    outbuf.strcat(buf.data());
    ...
    buf = haven::format_text("`B#`x %-68.68s         `B#`x\n\r", tempbuf);
    outbuf.strcat(buf.data());

    sprintf(
    tempbuf, "`W                             `WRewards              : `x%-5s`x    ", "All unlocked");
    ...
    buf = haven::format_text("`B#`x %-58.58s           `B#`x\n\r", tempbuf);
    outbuf.strcat(buf.data());
  ...
  _DOFUN(do_help) {
    ...
    for (help = help_first; help; help = help->next) {
      ...
      if ((!str_cmp(argall, help->keyword) && number == 1) || (number > 1 && (count + 1) == number && !found)) {
        ...
        count++;
        buf = haven::format_text("`c%2d`g: `W%s`x", count, help->keyword);
        prefix.strcat(buf.data());
        prefix.strcat("\n\r");
        fPrefix = TRUE;
      }
    }
  ...
  char *get_whorank(CHAR_DATA *ch) {
    ...
    if (IS_IMMORTAL(ch) && safe_strlen(ch->pcdata->crank) > 1) {
    ...
    else if (safe_strlen(ch->pcdata->crank) > 1 && ch->pcdata->rank_type == 4) {
      return ch->pcdata->crank;
    }
      ...
      return get_fac_rank(ch);
        ...
        if (ch->race == RACE_FACULTY)
        return str_dup("`WCivic Educator`x");
        else
        {
  ...
  _DOFUN(do_rank) {
    ...
    else if (!str_cmp(argument, "custom")) {
  ...
  char *draw_horizontal_line(int span) {
    ...
    // Construct the entire rule without an overlapping formatted append.
    span = UMAX(0, span);
    memset(buf, '_', span);
    buf[span] = '\0';
  ...
  // Clip display cells, keeping complete MUD color codes in the result.
  static std::string who_clip(const char *text, int cells) {
    std::string result;
    if (!text || cells <= 0)
      return result;
    ...
    for (size_t i = 0; i < length;) {
      ...
      if (text[i] == '`') {
        ...
        if (text[i + 1] == '#')
          bytes = 8;
        else if (i + 3 < length && isdigit((unsigned char)text[i + 1]) &&
        ...
        // Formatting tags are zero-width in the existing color renderer.
        if (i + 2 < length && strchr("bBiIuU", text[i + 1]) && text[i + 2] == '>')
          bytes = 3;
        else if (i + 3 < length && text[i + 1] == '/' &&
                 strchr("bBiIuU", text[i + 2]) && text[i + 3] == '>')
          bytes = 4;
      }
      std::string token(text + i, bytes);
      int visible = bytes == 1 ? 1 : safe_strlen_color(token.c_str());
      // Reserve room in the caller's MSL buffer for padding and borders too.
      if (visible > cells || result.size() + bytes > MSL - 512)
        break;
      result += token;
      cells -= visible;
      i += bytes;
  ...
  _DOFUN(do_who) {
    ...
    int nMatch = 0;
    int vMatch = 0;
    int srOnline = 0, srWorking = 0, immOnline = 0, immVisible = 0;
    // Match the supported width setting even for an invalid saved value.
    const int width = URANGE(80, ch->linewidth, 320);
    int i;
    char spacer[] = "";
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (target->invis_level > 0 && IS_FLAG(target->act, PLR_WHOINVIS)) {
        strcpy(invis1, "`C(`w");
        strcpy(invis2, "`C)`x ");
        buf4 = haven::format_text("%sW%d%s", invis1, target->invis_level, invis2);
      }
      ...
      if (IS_IMMORTAL(ch)) {
        ...
        if (multiplayer_abom(target)) {
          buf44 = haven::format_text("(`DAbom-%d`x)%s", cheat_number(target, cheatnumber), buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }
        if (multiplayer_super(target)) {
          buf44 = haven::format_text("(`rSuper-%d`x)%s", cheat_number(target, cheatnumber), buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }
        if (multiplayer_three(target)) {
          buf44 = haven::format_text("(`yThree-%d`x)%s", cheat_number(target, cheatnumber), buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }
        if (multiplayer_account(target) && strcmp(target->pcdata->account->name, ch->pcdata->account->name)) {
          buf44 = haven::format_text("(`RAccount-%d`x)%s", cheat_number(target, cheatnumber), buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }
        if (target->pcdata->account != NULL && IS_FLAG(target->pcdata->account->flags, ACCOUNT_SPAMMER)) {
          buf44 = haven::format_text("(`rSpammer`x)%s", buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }
        if (target->pcdata->account != NULL && IS_FLAG(target->pcdata->account->flags, ACCOUNT_SHH)) {
          buf44 = haven::format_text("(`rShh`x)%s", buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }
        if (target->pcdata->account != NULL && IS_FLAG(target->pcdata->account->flags, ACCOUNT_SHADOWBAN)) {
          buf44 = haven::format_text("(`DShadowban`x)%s", buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }

        if (is_griefer(target)) {
          buf44 = haven::format_text("(`rGriefer`x)%s", buf4.data());
          buf4 = haven::format_text("%s", buf44.data());
        }
      }
      cheatnumber++;
      // Center the rank in its original twelve-cell column.
      std::string whorank = who_clip(get_whorank(target), 12);
      int rank_padding = 12 - safe_strlen_color(whorank.c_str());
      snprintf(rank, sizeof(rank), "`x%*s%s`x%*s`x", rank_padding / 2, spacer,
               whorank.c_str(), rank_padding - rank_padding / 2, spacer);

      // determining border edge after title
      limit = width - 4 - safe_strlen_color(NAME(target)) - safe_strlen_color(rank);
      ...
      limit = limit - safe_strlen_color(buf4.data()); // Accounts for whoinvis tag
      limit = URANGE(0, limit, (int)sizeof(abridged_title) - 1);

      if (same_player(ch, target) || (!spammer(target) && target->pcdata->account != NULL && IS_FLAG(target->pcdata->account->flags, ACCOUNT_SHH) == FALSE))
      snprintf(abridged_title, sizeof(abridged_title), "%s",
               who_clip(target->pcdata->whotitle, limit).c_str());
      else
      strcpy(abridged_title, "");

      std::string row = std::string(rank) + " " + buf4.data() + buf9 +
        (IS_FLAG(target->comm, COMM_AFK) ? "[`RAFK`x] " : "") +
        (IS_FLAG(target->comm, COMM_QUIET) ? "[`mQUIET`x] " : "") +
        (target->level == 0 ? "[`YCreating`x] " : "") + haven::reward_color(target->pcdata->who_name_color, "`x") +
        NAME(target) + "`x" + abridged_title;
      row = who_clip(row.c_str(), width - 3);
      snprintf(buf, sizeof(buf), "`g|`x%s", row.c_str());

      spacing = UMAX(0, width - 3 - safe_strlen_color(row.c_str()));
    ...
    spacing = width - safe_strlen_color(buf);
    mortbuf.strcat(buf);
    ...
    strcat(buf, tbuf);
    mortbuf.strcat(buf);
    sprintf(buf, " `cSRs Online`g:`x %d `g| `cSRs Working`g:`x %d", srOnline, srWorking);
    sprintf(tbuf, "`cImmortals Online`g:`x %d `g| `cImmortals Hidden`g:`x %d\n\r", immOnline, immOnline - immVisible);
    spacing = UMAX(1, width - safe_strlen_color(buf) - safe_strlen_color(tbuf));
    mortbuf.printf("%s%*s%s", buf, spacing, spacer, tbuf);
  ...
  _DOFUN(do_balance) {
    ...
    if (personal_sanctuary(ch)) {
      ...
      if (indentured_servant(ch))
        send_to_char("`YIndentured servant: sixteen-hour work days cap LF at 30 and pay twice the full-time base wage until your bank balance reaches zero.`x\n\r", ch);
    }
    return;
  }
  ...
  */
  ...
  _DOFUN(do_finger) {
    ...
    */
    ...
    // Account
    if (IS_IMMORTAL(ch)) {
      string = haven::format_text("%s| %sAccnt%s: %s%s", border, fields, border, data, (victim->pcdata->account == NULL) ? "Unavailable" : victim->pcdata->account->name);
      printf_to_char(ch, "%s", string.data());
      endcap = haven::format_text("%s|`x", border);
      spacing = 80 - safe_strlen_color(string.data()) - safe_strlen_color(endcap.data());
      printf_to_char(ch, "%*s", spacing, spacer);
      printf_to_char(ch, "%s\n\r", endcap.data());
    }
    // Name
    string = haven::format_text("%s| %s Name%s: %s%s %s", border, fields, border, data, victim->name, victim->pcdata->last_name);
    printf_to_char(ch, "%s", string.data());
    endcap = haven::format_text("%s|`x", border);
    spacing = 80 - safe_strlen_color(string.data()) - safe_strlen_color(endcap.data());
    printf_to_char(ch, "%*s", spacing, spacer);
    ...
    string = haven::format_text("%s| %s Time%s: %s%s", border, fields, border, data, when);
    // Time truncation
    ...
    string = haven::format_text("%s| %sPlace%s: %s%s", border, fields, border, data, where);
    // Place truncation
    ...
    */
    ...
    if (online && (ch->in_room == victim->in_room || can_see_char_distance(ch, victim, DISTANCE_MEDIUM))) {
      ...
      string = haven::format_text("%s| %s Idle%s: %s%d %s %d %s", border, fields, border, data, dtime / 60, (dtime / 60 == 1) ? "minute" : "minutes", dtime % 60, (dtime % 60 == 1) ? "second" : "seconds");
      printf_to_char(ch, "%s", string.data());
      endcap = haven::format_text("%s|`x", border);
      spacing = 80 - safe_strlen_color(string.data()) - safe_strlen_color(endcap.data());
      printf_to_char(ch, "%*s", spacing, spacer);
  ...
  bool is_covered_equipped(CHAR_DATA *ch, int sel, const haven::EquipmentSnapshot &equipment) {
    OBJ_DATA *obj;
    int iWear;
    bool found = FALSE;
    ...
    for (iWear = 0; iWear < MAX_WEAR && !found; iWear++) {
      if ((obj = equipment.get(iWear)) == NULL)
      continue;
  ...
  bool is_covered(CHAR_DATA *ch, int sel) {
    const haven::EquipmentSnapshot equipment(ch);
    return is_covered_equipped(ch, sel, equipment);
  }
...
#define TIME_DUSK 2
  ...
  char *displaysun_room(ROOM_INDEX_DATA *room) {
    ...
    if (!can_see_outside(room)) {
      buf = haven::format_text("It is ");
      page += buf.data();
      buf = haven::format_text("about %s%d`xF(%s%d`xC) degrees.  ", color, indoortemp, color, indoorcel);
      page += buf.data();
    }
    else {
      buf = haven::format_text("It is %s, ", sun_phases[phase]);
      page += buf.data();
      buf = haven::format_text("about %s%d`xF(%s%d`xC) degrees, ", color, temperature(room), color, cel);
      page += buf.data();
      if (cloud_density(room) == 0) {
        buf = haven::format_text("and there are clear skies.  ");
      }
      else if (cloud_density(room) < 20) {
        if (cloud_cover(room) < 30) {
          buf = haven::format_text("and there are a few wispy white clouds in the sky.  ");
          page += buf.data();
        }
        else if (cloud_cover(room) < 75) {
          buf = haven::format_text("and the sky is partly covered by wispy white clouds.  ");
          page += buf.data();
        }
        else {
          buf = haven::format_text("and the sky is covered by wispy white clouds.  ");
          page += buf.data();
        }
      }
      else if (cloud_density(room) < 40) {
        if (cloud_cover(room) < 30) {
          buf = haven::format_text("and there are a few thin white clouds in the sky.  ");
          page += buf.data();
        }
        else if (cloud_cover(room) < 75) {
          buf = haven::format_text("and the sky is partly covered by thin white clouds.  ");
          page += buf.data();
        }
        else {
          buf = haven::format_text("and the sky is covered by thin white clouds.  ");
          page += buf.data();
        }
      }
      else if (cloud_density(room) < 60) {
        if (cloud_cover(room) < 30) {
          buf = haven::format_text("and there are a few grey clouds in the sky.  ");
          page += buf.data();
        }
        else if (cloud_cover(room) < 75) {
          buf = haven::format_text("and the sky is partly covered by grey clouds.  ");
          page += buf.data();
        }
        else {
          buf = haven::format_text("and the sky is covered by grey clouds.  ");
          page += buf.data();
        }
      }
      else if (cloud_density(room) < 80) {
        if (cloud_cover(room) < 30) {
          buf = haven::format_text("and there are a few dark grey clouds in the sky.  ");
          page += buf.data();
        }
        else if (cloud_cover(room) < 75) {
          buf = haven::format_text("and the sky is partly covered by dark grey clouds.  ");
          page += buf.data();
        }
        else {
          buf = haven::format_text("and the sky is covered by dark grey clouds.  ");
          page += buf.data();
        }
      }
      else {
        if (cloud_cover(room) < 30) {
          buf = haven::format_text("and there are a few dark grey stormclouds in the sky.  ");
          page += buf.data();
        }
        else if (cloud_cover(room) < 75) {
          buf = haven::format_text("and the sky is partly covered by dark grey stormclouds.  ");
          page += buf.data();
        }
        else {
          buf = haven::format_text("and the sky is covered by dark grey stormclouds.  ");
          page += buf.data();
        }
      }
      ...
      if (IS_SET(room->room_flags, ROOM_INDOORS)) {
        if (is_raining(room)) {
          buf = haven::format_text("It's `Braining`x outside.  ");
          page += buf.data();
        }
        if (is_hailing(room)) {
          buf = haven::format_text("It's `Chailing`x outside.  ");
          page += buf.data();
        }
        if (is_snowing(room)) {
          buf = haven::format_text("It's `Wsnowing`x outside.  ");
          page += buf.data();
        }
      }
      else {
        if (is_raining(room)) {
          buf = haven::format_text("It's `Braining.`x  ");
          page += buf.data();
        }
        if (is_hailing(room)) {
          buf = haven::format_text("It's `Chailing.`x  ");
          page += buf.data();
        }
        if (is_snowing(room)) {
          buf = haven::format_text("It's `Wsnowing.`x  ");
          page += buf.data();
        }
      }

      if (mist_level(room) >= 3) {
        buf = haven::format_text("`WThe area is wreathed in mist.`x  ");
        page += buf.data();
      }
      else if (mist_level(room) == 2) {
        buf = haven::format_text("Waist high `Wmist`x flows through the area.`x  ");
        page += buf.data();
      }
      else if (mist_level(room) == 1) {
        buf = haven::format_text("Ankle high `Wmist`x flows through the area.`x  ");
        page += buf.data();
      }
    }
    ...
    if (can_see_outside(room)) {
      if ((phase == 0 || phase == 1 || phase == 7)) {
        ...
        buf = haven::format_text("There is a %s moon.  ", moon_real[moon_pointer(ptm->tm_mday, ptm->tm_mon, ptm->tm_year, NULL)]);
        page += buf.data();
      }
    }

    return str_dup(page.data());
  }
  ...
  char *displaysun(CHAR_DATA *ch) {
    ...
    if (!can_see_outside(ch->in_room)) {
      if (is_vampire(ch) || IS_AFFECTED(ch, AFF_SUNBLESS) || get_skill(ch, SKILL_DAYASPECT) > 0 || get_skill(ch, SKILL_SOLARBLESS) > 0) {
        buf = haven::format_text("It is %s, and ", sun_phases[phase]);
      }
      else {
        buf = haven::format_text("It is ");
      }
      ...
      buf = haven::format_text("It is %s, ", sun_phases[phase]);
      page += buf.data();
      buf = haven::format_text("about %s%d`xF(%s%d`xC) degrees, ", color, temperature(ch->in_room), color, cel);
      page += buf.data();
    ...
    if (can_see_outside(ch->in_room) || is_werewolf(ch) || IS_AFFECTED(ch, AFF_MOONBLESS) || get_skill(ch, SKILL_NIGHTASPECT) > 0 || get_skill(ch, SKILL_LUNARBLESS) > 0) {
      if ((phase == 0 || phase == 1 || phase == 7)) {
        ...
        buf = haven::format_text("There is a %s moon.  ", moon_real[moon_pointer(ptm->tm_mday, ptm->tm_mon, ptm->tm_year, ch)]);
        page += buf.data();
      }
    }
  ...
  _DOFUN(do_join) {
    ...
    CHAR_DATA *victim;
    CHAR_DATA *to;
    ensure_room_description(ch->in_room);
    ...
    if (is_dreaming(ch) && ch->pcdata->dream_room != 0) {
      ...
      if (dreamroom == NULL)
      return;
      ensure_room_description(dreamroom);
  ...
  void alt_remote_view(CHAR_DATA *ch) {
    ...
    for (CharList::iterator it = char_list.begin(); it != char_list.end(); ++it) {
      ...
      // These are the same coordinate limits used by charlineofsight_character.
      // Reject distant characters before world and cloak checks.
      const long long dx = (long long)ch->in_room->x - wch->in_room->x;
      const long long dy = (long long)ch->in_room->y - wch->in_room->y;
      ...
      */
      ...
      if (in_world(victim) != WORLD_EARTH && in_world(victim) != WORLD_ELSEWHERE && !is_dreaming(victim)) {
        ...
        if (IS_FLAG(victim->comm, COMM_WHEREANON)) {
          if (victim->shape == SHAPE_HUMAN || victim->shape == SHAPE_MERMAID) {
            buf = haven::format_text("%s%s`c%s `x%s%s.\n\r", header, (victim->sex == SEX_MALE) ? "A man" : "A woman", playertitle, lowercase_clause(room_name), vnum_string);
          }
          else {
            buf = haven::format_text("%sAn animal`c%s `x%s%s.\n\r", header, playertitle, lowercase_clause(room_name), vnum_string);
          }
        }
        else { // Non-anonymous strings
          if (is_dreaming(victim)) {
            intro = haven::format_text("%s%s", header, dream_introduction(victim));
          }
          else if (is_masked(victim) && victim->shape == SHAPE_HUMAN)
          intro = haven::format_text("%s%s", header, capitalize(mask_intro(victim, victim->pcdata->maskednumber)));
          else if (IS_FLAG(victim->act, PLR_DEAD))
          intro = haven::format_text("%sThe ghost of %s", header, capitalize(get_intro(victim)));
          else
          intro = haven::format_text("%s%s", header, capitalize(PERS(victim, ch)));

          buf = haven::format_text("%s`c%s `x%s%s.\n\r", intro.data(), playertitle, lowercase_clause(room_name), vnum_string);
        }
        outbuf2.strcat(buf.data());
        //}
      }
      else {
        if (population == 1) {
          ...
          if (IS_FLAG(victim->comm, COMM_WHEREANON)) {
            ...
            buf = haven::format_text("%s`c%s `x%s%s(`039%d`x).\n\r", intro.data(), playertitle, lowercase_clause(room_name), vnum_string, pc_pop(victim->in_room));
          }
          outbuf3.strcat(buf.data());
        }
      }
  ...
  bool display_hotspots(CHAR_DATA *ch, int rooms[100]) {
    ...
    char no_color[MSL], preposition[MSL], noun[MSL], population_prefix[3];
    std::string buf;
    Buffer outbuf;
  ...
  void display_dreamers(CHAR_DATA *ch, int dreamers, bool FirstHotSpot, int worlds[20], int worlds_where[20]) {
    ...
    if (dreamers > 0) {
      ...
      buf = haven::format_text("%s%d `c%s dreaming`x\n\r", population_prefix, dreamers, noun);
      outbuf.strcat(buf.data());
      for (int i = 0; i < 20; i++) {
        bool first = TRUE;
        if (worlds[i] > 0 && worlds_where[i] > 0) {
          ...
          if (fant != NULL) {
            if (first) {
              buf = haven::format_text("`c      There is activity in`c:`x %s", fant->name);
              first = FALSE;
            }
            else
            buf = haven::format_text("`c, %s", fant->name);
            outbuf.strcat(buf.data());
          }
        }
        if (first == FALSE) {
          buf = haven::format_text(".`x\n\r");
          outbuf.strcat(buf.data());
        }
      }
      page_to_char(outbuf, ch);
  ...
  bool power_possess(CHAR_DATA *ch, CHAR_DATA *victim)
  {
    ...
    if(victim->fsociety != 0)
    {
      FACTION_TYPE *fsociety = clan_lookup(victim->fsociety);
      if(!str_cmp(fsociety->eidilon, ch->name))
      canpossess = TRUE;
    }
    if(victim->legacy_society != 0)
    {
      FACTION_TYPE *legacy_society = clan_lookup(victim->legacy_society);
      if(!str_cmp(legacy_society->eidilon, ch->name))
      canpossess = TRUE;
    }
    return canpossess;
  }
  ...
  _DOFUN(do_where) {
    ...
    if(can_super_lure(ch, NULL))
    {
      ...
      it != descriptor_list.end(); ++it) {
        ...
        if(donefirst == FALSE)
        {
          buf = haven::format_text("`074Supernaturally lurable characters:`x ");
          outbuf2.strcat(buf.data());
          donefirst = TRUE;
          buf = haven::format_text("%s", victim->name);
        }
        else
        buf = haven::format_text(", %s", victim->name);
    ...
    {
      donefirst = FALSE;
      buf = haven::format_text("`DWhisper:`x ");
      outbuf2.strcat(buf.data());
      for (DescList::iterator it = descriptor_list.begin();
      it != descriptor_list.end(); ++it) {
        DESCRIPTOR_DATA *d = *it;
      ...
      outbuf2.strcat(".\n\r");
      donefirst = FALSE;
      buf = haven::format_text("`088Possess:`x ");
      outbuf2.strcat(buf.data());
      for (DescList::iterator it = descriptor_list.begin();
      it != descriptor_list.end(); ++it) {
        DESCRIPTOR_DATA *d = *it;
    ...
    buf = haven::format_text("Looking to chat with %d people, to meet with %d people.\n\r", chat_with_count(ch), rp_with_count(ch));
    outbuf2.strcat(buf.data());
    page_to_char(outbuf2, ch);
...
#define HAIR_TYPES 13
  ...
  static void edit_focused(CHAR_DATA *ch, int number, const char *replacement, bool append) {
    if (!ch || !ch->pcdata || !replacement || number < -1 || number > MAX_COVERS) return;
    char **description = number == -1 ? &ch->description : &ch->pcdata->focused_descs[number];
    std::string original = *description ? *description : "";
    size_t end = original.find_last_not_of(" \n\r");
    end = end == std::string::npos ? 0 : end + 1;
    size_t start = 0;
    while (start < end && (original[start] == ' ' || original[start] == '\n'
           || original[start] == '\r' || isdigit(static_cast<unsigned char>(original[start])))) ++start;
    std::string result = original.substr(0, start);
    if (append && end > start) result += original.substr(start, end - start) + " ";
    result += replacement;
    result += original.substr(end);
    free_string(*description);
    *description = str_dup(result.c_str());
  }

  void replace_focused(CHAR_DATA *ch, int number, char *replacement) {
    edit_focused(ch, number, replacement, false);
  }

  void append_focused(CHAR_DATA *ch, int number, char *replacement) {
    edit_focused(ch, number, replacement, true);
  }
...
#define PREP_ON 2
  ...
  char *roomtitle(ROOM_INDEX_DATA *room, bool capital) {
    if (!room) return str_dup("Somewhere");
    EXTRA_DESCR_DATA *ed;
    std::string buf;
    int firstletter = -1;
    for (ed = room->extra_descr; ed; ed = ed->next) {
    ...
    if (ed) {
      buf = haven::format_text("%s", ed->description);
    ...
    if (word == PREP_NO)
    buf = haven::format_text("`x%s", name);
    else if (name[0] == '`')
    buf = haven::format_text("%s%s %s", first_color_code(name), preposition, name);
    else
    buf = haven::format_text("`x%s %s", preposition, name);
  ...
  char *line_to_box(char *line, char *delim, int width) {
    ...
    if (safe_strlen_color(line) + safe_strlen_color(delim) * 2 <= width) {
      ...
      buf = haven::format_text("%s%s%*s%s", delim, line, space, " ", delim);

    }
    else
    buf = haven::format_text("%s%s", delim, line);
    return str_dup(buf.c_str());
  }
  char *text_block_to_box(char *text, char *delim, int width) {
    std::string fintext, otext;
    int inp;
    int txtlen = safe_strlen(text);
    for (inp = 0; inp < txtlen; inp++) {
      if (text[inp] == '\n') {
        char *line = strip_newlines(otext.data());
        char *boxed = line_to_box(line, delim, width);
        fintext += boxed;
        free_string(boxed);
        free_string(line);
        fintext += '\n';
    ...
    if (otext.size() > 2) {
      char *boxed = line_to_box(otext.data(), delim, width);
      fintext += boxed;
      free_string(boxed);
    }
    return str_dup(fintext.c_str());
  ...
  char *text_complete_box(char *text, char *delim, int width, bool cuttop) {
    ...
    char *body = text_block_to_box(text, delim, width);
    const std::string fintext = haven::format_text("%s\n%s\n%s\n\r", otext.c_str(), body, otext.c_str());
    free_string(body);
    return str_dup(fintext.c_str());
  }
  ...
  int torch_count(ROOM_INDEX_DATA *room)
  {
    int count = 0;
    int floor_lights = -1;
    bool checked_shadow = FALSE;
    bool shadowed = FALSE;
    // Evaluate the room-wide cloak scan at most once, and only for a live light.
    auto usable_light = [&](OBJ_DATA *obj) {
      if (obj == NULL || IS_SET(obj->extra_flags, ITEM_OFF) || obj->value[0] <= 1)
        return false;
      if (!checked_shadow) {
        shadowed = shadowcloaked(room);
        checked_shadow = TRUE;
      }
      return !shadowed;
    };
    ...
    for (CharList::iterator it = room->people->begin(); it != room->people->end(); ++it) {
      CHAR_DATA *victim = *it;
      if (victim == NULL || victim->in_room != room || victim->in_room == NULL)
        continue;
      if (!IS_NPC(victim) && event_cleanse == 0 && victim->pcdata->litup == 1)
        count += 3;
      if (usable_light(get_held(victim, ITEM_FLASHLIGHT)))
        count++;
      if (usable_light(get_worn(victim, ITEM_FLASHLIGHT)))
        count++;

      if (floor_lights < 0) {
        floor_lights = 0;
        for (OBJ_DATA *obj = room->contents; obj != NULL; obj = obj->next_content) {
          if (obj->item_type == ITEM_FLASHLIGHT && usable_light(obj))
            floor_lights++;
          if (obj->next_content == obj)
            break;
        }
      }
      // Preserve the existing per-occupant contribution of floor lights.
      count += floor_lights;
    }
    return count;
  }
