...
*/
    ...
    case ITEM_CORPSE_NPC:
      fprintf(fp, "%d %d %d %d %d %d\n", pObjIndex->value[0], pObjIndex->value[1], pObjIndex->value[2], pObjIndex->value[3], pObjIndex->value[4], pObjIndex->value[5]);
      break;
    ...
    case ITEM_POTION:
      fprintf(fp, "%d", pObjIndex->value[0]);
      for (int i = 1; i < MAX_OBJ_VALUES; ++i)
        fprintf(fp, " %s", fwrite_flag(pObjIndex->value[i], buf));
      fprintf(fp, "\n");
      break;
    }
    ...
    for (iHash = 0; iHash < MAX_KEY_HASH; iHash++) {
      ...
      pRoomIndex = pRoomIndex->next) {
        if (pRoomIndex->area == pArea) { // old areas don't always respond well to
          ...
          fprintf(fp, "#%d\n", pRoomIndex->vnum);
          fprintf(fp, "%s~\n", pRoomIndex->name);
          fprintf(fp, "%s~\n", fix_string(unfinished_room_description(pRoomIndex->description)
            ? default_room_description(pRoomIndex) : pRoomIndex->description));
          fprintf(fp, "0 ");
          ...
          for (door = 0; door < 10; door++) {
            if (pRoomIndex->features[door] != NULL && pRoomIndex->features[door]->type > 0)
