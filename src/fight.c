...
#define MOVE_FATIGUE 3
  ...
  static void make_vampire_ashes(CHAR_DATA *ch) {
    if (ch->in_room == NULL)
    return;

    OBJ_DATA *ashes = create_object(get_obj_index(OBJ_VNUM_CORPSE_NPC), 0);
    free_string(ashes->name);
    ashes->name = str_dup("pile ash ashes");
    free_string(ashes->short_descr);
    ashes->short_descr = str_dup("a pile of ash");
    free_string(ashes->description);
    ashes->description = str_dup("A pile of ash lies here.");
    free_string(ashes->material);
    ashes->material = str_dup("ash");
    ashes->item_type = ITEM_TRASH;
    ashes->cost = 0;
    ashes->timer = 60 * 24;
    ashes->rot_timer = 0;
    ...
    if (ch->money > 0) {
      obj_to_room(create_money(ch->money, ch), ch->in_room);
      ch->money = 0;
    }

    for (OBJ_DATA *obj = ch->carrying, *obj_next; obj != NULL; obj = obj_next) {
      obj_next = obj->next_content;
      // Wardrobe objects are stored elsewhere, not physically carried.
      if (IS_SET(obj->extra_flags, ITEM_WARDROBE))
      continue;
      obj_from_char_silent(obj);
      obj_to_room(obj, ch->in_room);
    }
    act("$n crumbles into a pile of ash.", ch, NULL, NULL, TO_ROOM);
    act("You crumble into a pile of ash.", ch, NULL, NULL, TO_CHAR);
  }
  ...
  void make_corpse(CHAR_DATA *ch, CHAR_DATA *killer) {
    ...
    if (!is_vampire(ch)) {
      ...
      for (obj = ch->carrying; obj != NULL; obj = obj_next) {
        ...
        if (IS_NPC(ch) && IS_SET(obj->extra_flags, ITEM_INVENTORY)) {
          obj_from_char_silent(obj);
          extract_obj(obj);
        }
        else {
  ...
  void real_kill(CHAR_DATA *victim, CHAR_DATA *killer, bool wound, bool monster) {
    if (IS_NPC(victim))
    return;
    ...
    record_death_recovery(victim, killer, wound, monster);
    make_corpse(victim, killer);
    ...
    end_schemes_on_death(victim);
    end_maintained_ritual_on_death(victim);
  ...
  void real_kill_headless(CHAR_DATA *victim, CHAR_DATA *killer) {
    ...
    record_death_recovery(victim, killer, false, false);
  ...
  char *defense_message(CHAR_DATA *ch, int type, bool fast, int echo) {
    ...
    if (ch->hit <= 0 && ch->wounds > 0) {
      ...
      if (ch->wounds == 1) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("mildly wounds your %s.", woundloc);
          else
          buf = haven::format_text("mildly wounds $S %s.", woundloc);
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("mildly wound your %s.", woundloc);
          else
          buf = haven::format_text("mildly wound $S %s.", woundloc);
          return str_dup(buf.data());
        }
      }
      else if (ch->wounds == 2) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("severely wounds your %s.", woundloc);
          else
          buf = haven::format_text("severely wounds $S %s.", woundloc);
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("severely wound your %s.", woundloc);
          else
          buf = haven::format_text("severely wound $S %s.", woundloc);
          return str_dup(buf.data());
        }
      }
      else if (ch->wounds == 3) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("critically wounds you.");
          else
          buf = haven::format_text("critically wounds $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("critically wound you.");
          else
          buf = haven::format_text("critically wound $M.");
          return str_dup(buf.data());
        }
      }
      else {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("kills you.");
          else
          buf = haven::format_text("kills $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("kill you.");
          else
          buf = haven::format_text("kill you $M.");
          return str_dup(buf.data());
        }
      }
    }
    ...
    if (echo == ECHO_GRAZE) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("grazes your %s.", grazeloc);
        else
        buf = haven::format_text("grazes $S %s.", grazeloc);
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("graze your %s.", grazeloc);
        else
        buf = haven::format_text("graze $S %s.", grazeloc);
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_MISS) {
      if (ch->wound_location % 2 != 0 || ch->moved < 5 || ch->in_room->sector_type == SECT_AIR) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("misses you.");
          else
          buf = haven::format_text("misses $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("miss you.");
          else
          buf = haven::format_text("miss $M.");
          return str_dup(buf.data());
        }
      }
      if (in_water(ch)) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("sends up a spray of water near your feet.");
          else
          buf = haven::format_text("sends up a spray of water near $S feet.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("send up a spray of water near your feet.");
          else
          buf = haven::format_text("send up a spray of water near $S feet.");
          return str_dup(buf.data());
        }
      }
      ...
      if (is_natural(ch->in_room)) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("sends up a spray of soil near your feet.");
          else
          buf = haven::format_text("sends up a spray of soil near $S feet.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("send up a spray of soil near your feet.");
          else
          buf = haven::format_text("send up a spray of soil near $S feet.");
          return str_dup(buf.data());
        }
      }
      else {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("sparks off the ground near your feet.");
          else
          buf = haven::format_text("sparks off the ground near $S feet.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("spark off the ground near your feet.");
          else
          buf = haven::format_text("spark off the ground near $S feet.");
          return str_dup(buf.data());
        }
      }
    }
    if (echo == ECHO_COVERSTRIKE) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("damages you.");
        else
        buf = haven::format_text("damages it.");
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("damage you.");
        else
        buf = haven::format_text("damage it.");
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_DODGE) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("is dodged by you.");
        else
        buf = haven::format_text("is dodged by $M.");
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("be dodged by you.");
        else
        buf = haven::format_text("be dodged by $M.");
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_ARMOR) {
      if (ch->disciplines[DIS_MARMOR] >= ch->disciplines[DIS_BARMOR]) {
        if (get_armor(ch) != NULL && number_percent() % 2 == 0) {
          if (fast == TRUE) {
            if (type == TO_VICT)
            buf = haven::format_text("deflects off your %s.", get_armor(ch)->short_descr);
            else
            buf = haven::format_text("deflects off $S %s.", get_armor(ch)->short_descr);
            return str_dup(buf.data());
          }
          else {
            if (type == TO_VICT)
            buf = haven::format_text("deflect off your %s.", get_armor(ch)->short_descr);
            else
            buf = haven::format_text("deflect off $S %s.", get_armor(ch)->short_descr);
            return str_dup(buf.data());
          }
        }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("deflects off your armor.");
          else
          buf = haven::format_text("deflects off $S armor.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("deflect off your armor.");
          else
          buf = haven::format_text("deflect off $S armor.");
          return str_dup(buf.data());
        }
      }
      else {
        if (get_armor(ch) != NULL && number_percent() % 2 == 0) {
          if (fast == TRUE) {
            if (type == TO_VICT)
            buf = haven::format_text("impacts against your %s.", get_armor(ch)->short_descr);
            else
            buf = haven::format_text("impacts against $S %s.", get_armor(ch)->short_descr);
            return str_dup(buf.data());
          }
          else {
            if (type == TO_VICT)
            buf = haven::format_text("impact against your %s.", get_armor(ch)->short_descr);
            else
            buf = haven::format_text("impact against $S %s.", get_armor(ch)->short_descr);
            return str_dup(buf.data());
          }
        }
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("impacts against your armor.");
          else
          buf = haven::format_text("impacts against $S armor.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("impact against your armor.");
          else
          buf = haven::format_text("impact against $S armor.");
          return str_dup(buf.data());
        }
      }
    }
    if (echo == ECHO_SHIELD) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("deflects off your shield.");
        else
        buf = haven::format_text("deflects off $S shield.");
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("deflect off your shield.");
        else
        buf = haven::format_text("deflect off $S shield.");
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_SUPERTOUGH) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("ineffectually wounds your %s.", undeadloc);
        else
        buf = haven::format_text("ineffectually wounds $S %s.", undeadloc);
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("ineffectually wound your %s.", undeadloc);
        else
        buf = haven::format_text("ineffectually wound $S %s.", undeadloc);
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_SUPERSHIELD) {
      if (get_disc(ch, DIS_FORCES, FALSE) >= get_disc(ch, DIS_FATE, FALSE) && get_disc(ch, DIS_FORCES, FALSE) >= get_disc(ch, DIS_PUSH, FALSE)) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("deflects off an almost invisible distortion in the air around you.");
          else
          buf = haven::format_text("deflects off an almost invisible distortion in the air around $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("deflect off an almost invisible distortion in the air around you.");
          else
          buf = haven::format_text("deflect off an almost invisible distortion in the air around $M.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is pushed aside at the last instant.");
          else
          buf = haven::format_text("is pushed aside at the last instant.");
          return str_dup(buf.data());
        }
          ...
          if (type == TO_VICT)
          buf = haven::format_text("be pushed aside at the last instant.");
          else
          buf = haven::format_text("be pushed aside at the last instant.");
          return str_dup(buf.data());
        }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("narrowly misses you.");
          else
          buf = haven::format_text("narrowly misses $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("narrowly miss you.");
          else
          buf = haven::format_text("narrowly miss $M.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is absorbed by your energy shield.");
          else
          buf = haven::format_text("is absorbed by $S energy shield.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be absorbed by your energy shield.");
          else
          buf = haven::format_text("be absorbed by $S energy shield.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("bounces off the firey shield that flickers to life around you.");
          else
          buf = haven::format_text("bounces off the firey shield that flickers to life around $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("bounce off the firey shield that flickers to life around you.");
          else
          buf = haven::format_text("bounce off the firey shield that flickers to life around $M.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is deflected by the shield of ice that appears the instant of impact around you.");
          else
          buf = haven::format_text("is deflected by the shield of ice that appears the instant of impact around $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be deflected by the shield of ice that appears the instant of impact around you.");
          else
          buf = haven::format_text("be deflected by the shield of ice that appears the instant of impact around $M.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is deflected by the twisting shield of wood that wraps around you.");
          else
          buf = haven::format_text("is deflected by the twisting shield of wood that wraps around $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be deflected by the twisting shield of wood that wraps around you.");
          else
          buf = haven::format_text("be deflected by the twisting shield of wood that wraps around $M.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is absorbed by the shield of lightning that flickers to life around you.");
          else
          buf = haven::format_text("is absorbed by the shield of lightning that flickers to life around $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be absorbed by the shield of lightning that flickers to life around you.");
          else
          buf = haven::format_text("be absorbed by the shield of lightning that flickers to life around $M.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("bounces off the stone barrier that appears before you.");
          else
          buf = haven::format_text("bounces off the stone barrier that appears before $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("bounce off the stone barrier that appears before you.");
          else
          buf = haven::format_text("bounce off the stone barrier that appears before $M.");
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("bounces off the shield of darkness that flickers to life around you.");
          else
          buf = haven::format_text("bounces off the shield of darkness that flickers to life around $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("bounce off the shield of darkness that flickers to life around you.");
          else
          buf = haven::format_text("bounce off the shield of darkness that flickers to life around $M.");
          return str_dup(buf.data());
        }
      }
      else {
    ...
    if (echo == ECHO_ARMORBRUISE) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("strikes your armor with bruising force.");
        else
        buf = haven::format_text("strikes $S armor with bruising force.");
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("strike your armor with bruising force.");
        else
        buf = haven::format_text("strike $S armor with bruising force.");
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_SHIELDBRUISE) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("strikes your shield with bruising force.");
        else
        buf = haven::format_text("strikes $S shield with bruising force.");
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("strike your shield with bruising force.");
        else
        buf = haven::format_text("strike $S shield with bruising force.");
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_PARRY) {
      if (get_lmelee(ch) != NULL) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is parried by your %s.", get_lmelee(ch)->short_descr);
          else
          buf = haven::format_text("is parried by $S %s.", get_lmelee(ch)->short_descr);
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be parried by your %s.", get_lmelee(ch)->short_descr);
          else
          buf = haven::format_text("be parried by $S %s.", get_lmelee(ch)->short_descr);
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is parried by your %s.", get_smelee(ch)->short_descr);
          else
          buf = haven::format_text("is parried by $S %s.", get_smelee(ch)->short_descr);
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be parried by your %s.", get_smelee(ch)->short_descr);
          else
          buf = haven::format_text("be parried by $S %s.", get_smelee(ch)->short_descr);
          return str_dup(buf.data());
        }
      }
      else {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is parried by you.");
          else
          buf = haven::format_text("is parried by $M.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be parried by you.");
          else
          buf = haven::format_text("be parried by $M.");
          return str_dup(buf.data());
        }
      }
    }
    if (echo == ECHO_SHIELDBLOCK) {
      if (get_shield_obj(ch) != NULL) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is blocked by your %s.", get_shield_obj(ch)->short_descr);
          else
          buf = haven::format_text("is blocked by $S %s.", get_shield_obj(ch)->short_descr);
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be blocked by your %s.", get_shield_obj(ch)->short_descr);
          else
          buf = haven::format_text("be blocked by $S %s.", get_shield_obj(ch)->short_descr);
          return str_dup(buf.data());
        }
      }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("is blocked by your shield.");
          else
          buf = haven::format_text("is blocked by $S shield.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("be blocked by your shield.");
          else
          buf = haven::format_text("be blocked by $S shield.");
          return str_dup(buf.data());
        }
      }
    }
    if (echo == ECHO_COVER) {
      if (get_cover(ch) != NULL) {
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("strikes the %s you're taking cover behind.", get_cover(ch)->short_descr);
          else
          buf = haven::format_text("strikes the %s $E's taking cover behind.", get_cover(ch)->short_descr);
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("strike the %s you're taking cover behind.", get_cover(ch)->short_descr);
          else
          buf = haven::format_text("strike the %s $E's taking cover behind.", get_cover(ch)->short_descr);
          return str_dup(buf.data());
        }
        ...
        if (fast == TRUE) {
          if (type == TO_VICT)
          buf = haven::format_text("strikes your cover.");
          else
          buf = haven::format_text("strikes $S cover.");
          return str_dup(buf.data());
        }
        else {
          if (type == TO_VICT)
          buf = haven::format_text("strike your cover.");
          else
          buf = haven::format_text("strike $S cover.");
          return str_dup(buf.data());
        }
      }
    }
    if (echo == ECHO_PUNCHBLOCK) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("is blocked by you.");
        else
        buf = haven::format_text("is blocked by $M.");
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("be blocked by you.");
        else
        buf = haven::format_text("be blocked by $M.");
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_GRAPPLEBLOCK) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("is fended off by you.");
        else
        buf = haven::format_text("is fended off by $M.");
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("be fended off by you.");
        else
        buf = haven::format_text("be fended off by $M.");
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_PUNCH) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("strikes you in the %s.", punchloc);
        else
        buf = haven::format_text("strikes $M in the %s.", punchloc);
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("strike you in the %s.", punchloc);
        else
        buf = haven::format_text("strike $M in the %s.", punchloc);
        return str_dup(buf.data());
      }
    }
    if (echo == ECHO_GRAPPLE) {
      if (fast == TRUE) {
        if (type == TO_VICT)
        buf = haven::format_text("strains your %s.", grappleloc);
        else
        buf = haven::format_text("strains $S %s.", grappleloc);
        return str_dup(buf.data());
      }
      else {
        if (type == TO_VICT)
        buf = haven::format_text("strain your %s.", grappleloc);
        else
        buf = haven::format_text("strain $S %s.", grappleloc);
        return str_dup(buf.data());
      }
    }
  ...
  void wound_check(CHAR_DATA *ch, CHAR_DATA *victim, int wound) {
    ...
    if (wound == 4 || wound == 3) {
      if (!state_of_emergency() && IS_NPC(victim) && victim->pIndexData->vnum == POLICE_OFFICER && !IS_NPC(ch)) {
        ch->pcdata->police_intensity++;
        ch->pcdata->police_number++;
        ch->pcdata->police_timer += 720;
      }
      if (!state_of_emergency() && IS_NPC(victim) && victim->pIndexData->vnum == POLICE_SWAT && !IS_NPC(ch)) {
        ch->pcdata->police_intensity++;
        ch->pcdata->police_number++;
        ch->pcdata->police_timer += 720;
      }
      if (!state_of_emergency() && IS_NPC(victim) && victim->pIndexData->vnum == NATIONAL_GUARD && !IS_NPC(ch)) {
        ch->pcdata->police_intensity++;
        ch->pcdata->police_number++;
        ch->pcdata->police_timer += 720;
      }
...
*/
    ...
    if (IS_NPC(victim) && (victim->pIndexData->vnum == SCUM_SOLDIER || victim->pIndexData->vnum == CORTEX_SOLDIER || victim->pIndexData->vnum == ORDER_SOLDIER || victim->pIndexData->vnum == SCUM_LT || victim->pIndexData->vnum == CORTEX_LT || victim->pIndexData->vnum == ORDER_LT || victim->pIndexData->vnum == 5)) {
      victim->fighting = FALSE;
      if (fight_over(victim->in_room) == TRUE) {
        end_fight(victim->in_room);
      }
  ...
  void damage(CHAR_DATA *victim, CHAR_DATA *ch, int amount) {
    ...
    if (dissent_crowd(victim)) {
      ...
      dissent_update();
      if (victim->wounds >= 4 || !dissent_participant(ch)) return;
    }
    amount = territory_damage_amount(victim, ch, amount);
    if (victim->wounds >= 4)
    return;
    if (ch->wounds >= 4)
    return;
    ...
    if (amount >= victim->hit) {
      ...
      if (cortex_public_enforcer(ch)) {
        cortex_enforcer_defeat(ch, victim);
        return; // Subdue the attacker without escalating wounds or killing them.
      }
      ...
      else if (victim->wounds == 0 || (victim->wounds == 1 && !lunacy_wolf(ch) && (sparring_conditions(ch, victim)))) {
        if (victim->wounds == 1)
        victim->debuff += 50;
        victim->wounds = 1;
      ...
      else if (victim->wounds == 1 || (victim->wounds == 2 && (sparring_conditions(ch, victim) || higher_power(ch))) || (victim->wounds > 1 && !shroud_monster(ch) && !guestmonster(victim) && get_skill(victim, SKILL_IMMORTALITY) > 1)) {
      ...
      else if (victim->wounds == 2 || (victim->wounds == 3 && (sparring_conditions(ch, victim)))) {
        record_critical_injury(victim, ch);
        invalidate_wound_treatment(victim);
        victim->wounds = 3;
  ...
  void next_attacker(CHAR_DATA *ch, bool initial) {
    ...
    if (!IS_NPC(part) || !IS_FLAG(part->act, ACT_COMBATOBJ)) {
      if (!is_gm(part)) {
        if (!IS_NPC(part) && !is_gm(part)) {
          pccount++;
          snprintf(pclist + strlen(pclist), sizeof(pclist) - strlen(pclist), " %s", part->name);
        }
    ...
    if (pccount >= 5 && pccount < 100) {
      snprintf(pclist + strlen(pclist), sizeof(pclist) - strlen(pclist), ", %d", pccount);
      log_string(pclist);
      convert_to_fast(ch);
      return;
    }
  ...
  // Select once per affected attack, without favoring list order or shrinking range.
  static CHAR_DATA *confused_target(CHAR_DATA *ch, CHAR_DATA *victim, bool actual) {
    if (!has_caff(ch, CAFF_CONFUSE))
      return victim;
    remove_caff(ch, CAFF_CONFUSE);
    int range = combat_distance(victim, ch, actual);
    int count = 1;
    CHAR_DATA *selected = victim;
    for (CharList::iterator it = char_list.begin(); it != char_list.end(); ++it) {
      CHAR_DATA *candidate = *it;
      if (candidate == NULL || candidate == ch || candidate == victim || is_gm(candidate))
        continue;
      if (!in_fight(candidate) || !same_fight(candidate, ch) || !is_enemy(ch, candidate))
        continue;
      if (IS_NPC(candidate) && IS_FLAG(candidate->act, ACT_COMBATOBJ))
        continue;
      if (combat_distance(candidate, ch, actual) > range)
        continue;
  ...
  _DOFUN(do_move) {
    ...
    if (!str_cmp(arg1, "flee") || !str_cmp(arg1, "sprint")) {
      ...
      if (!get_poi_target(ch, POI_EXTRACT, &target_x, &target_y)) {
        send_to_char("There is no accessible objective of that type.\n\r", ch);
        return;
      }
      if (battleground(ch->in_room)) {
      ...
      move_towards(ch, target_x, target_y, combat_move_speed(ch, MOVE_MOVE), 0, TRUE);
      move_message(ch, MOVE_MOVE, origx, origy, origroom);
    }
    else if (!str_cmp(arg1, "capture")) {
      int target_x, target_y;
      if (!get_poi_target(ch, POI_CAPTURE, &target_x, &target_y)) {
        send_to_char("There is no accessible objective of that type.\n\r", ch);
        return;
      }
      if (battleground(ch->in_room)) {
  ...
  _DOFUN(do_hostile) {
    ...
    if (victim->wounds >= 4)
    return;
    // Public hostile stances in the nightmare retain their existing ambush rule.
    if (public_target_excluded(ch, victim)) {
      send_to_char("This area's public-room protection prevents you from targeting them.\n\r", ch);
      return;
    }
    if (!cortex_breach_monster(ch) && !cortex_breach_monster(victim)
        && IS_FLAG(ch->act, PLR_SHROUD) && in_public(ch, victim) && !room_ambush(ch->in_room)) {
  ...
  void delayattacks(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    for (j = 0; j < 10; j++) {
      if (ch->delays[j][1] > 0) {
        ch->delays[j][1]--;
        if (ch->delays[j][1] == 0) {
          for (k = 0; k < discipline_table_count; k++) {
...
*/
    ...
    if (ch->fight_fast == FALSE) {
      ...
      if (point == -1) {
        ...
        if (point == -1) {
          ...
          buf = haven::format_text("%s %s", arg2, arg3);
          for (i = 0; i < discipline_table_count; i++) {
            if (!str_cmp(buf.data(), disc_name(ch, i)))
            point = i;
          }
        }
        if (point == -1) {
          argument = one_argument_nouncap(argument, arg4);
          buf += haven::format_text(" %s", arg4);
          for (i = 0; i < discipline_table_count; i++) {
            if (!str_cmp(buf.data(), disc_name(ch, i)))
            point = i;
          }
  ...
  _DOFUN(do_attack) {
    ...
    if (is_dreaming(ch) && in_fantasy(ch) != NULL) {
      buf = haven::format_text("attack %s", argument);
      do_function(ch, &do_dreamworld, buf.data());
      return;
    }
...
*/
    ...
    if (ch->fight_fast == FALSE) {
      ...
      ch->cfighting = victim;
      victim = confused_target(ch, victim, FALSE);
      ...
      if (!IS_NPC(ch)) {
        ...
        for (i = 0; i < 25; i++) {
          ...
          {
            for (j = 0; j < discipline_table_count; j++) {
      ...
      if (point == -1) {
        ...
        if (point == -1) {
          ...
          buf = haven::format_text("%s %s", arg2, arg3);
          for (i = 0; i < discipline_table_count; i++) {
            if (!str_cmp(buf.data(), disc_name(ch, i)) && safe_strlen(buf.data()) > 2)
            point = i;
          }
        }
        if (point == -1) {
      ...
      ch->attackdam = dam;
      if (dam <= victim->hit) {
        buf = haven::format_text("Your best %s attack against $N will %s", disc_name(ch, opoint), defense_message(victim, TO_CHAR, FALSE, echo));
        ...
        else if (victim->wounds == 1 && (sparring_conditions(ch, victim)))
        printf_to_char(ch, "Your best %s attack will mildly wound %s's %s.\n\r", disc_name(ch, opoint), PERS(victim, ch), wound_location(victim));
        else if (victim->wounds == 1)
        printf_to_char(
        ch, "Your best %s attack will severely wound %s's %s.\n\r", disc_name(ch, opoint), PERS(victim, ch), wound_location(victim));
...
*/
      ...
      if (!IS_NPC(ch)) {
        if (!battleground(ch->in_room) && !guestmonster(ch) && (!str_cmp(arg2, "knockout") || (IS_FLAG(ch->comm, COMM_SUBDUE) && !IS_NPC(victim) && !is_sparring(ch) && victim->hit <= 0))) {
          ...
          if (number_percent() % 2 == 0 && !guestmonster(victim)) {
            ...
            if (IS_FLAG(victim->fightflag, FIGHT_DEADSWITCH))
            dead_explode(victim);
            buf = haven::format_text("%s has knocked %s out at %s.", ch->name, victim->name, ch->in_room->name);
      ...
      if (IS_FLAG(ch->fightflag, FIGHT_SUPPRESSOR) && combat_distance(ch, victim, FALSE) >= 50) {
        buf = haven::format_text("You are shot silently, it %s", defense_message(victim, TO_VICT, TRUE, echo));
        act(buf.data(), ch, NULL, victim, TO_VICT);
        if (cpoint != -1) {
          if (dam > 0 || stag > 0) {
            buf = haven::format_text("%s %s", ch->pcdata->customstrings[cpoint][1], defense_message(victim, TO_CHAR, TRUE, echo));
            act(buf.data(), ch, NULL, victim, TO_CHAR);
          }
          else
        ...
        if (dam > 0 || stag > 0) {
          buf = haven::format_text("%s %s", ch->pcdata->customstrings[cpoint][1], defense_message(victim, TO_CHAR, TRUE, echo));
          act(buf.data(), ch, NULL, victim, TO_CHAR);
          buf = haven::format_text("%s %s", ch->pcdata->customstrings[cpoint][2], defense_message(victim, TO_VICT, TRUE, echo));
          act(buf.data(), trickcheck(ch), NULL, victim, TO_VICT);
          buf = haven::format_text("%s %s", ch->pcdata->customstrings[cpoint][3], defense_message(victim, TO_ROOM, TRUE, echo));
          act(buf.data(), trickcheck(ch), NULL, victim, TO_NOTVICT);
          dact(buf.data(), trickcheck(ch), NULL, victim, DISTANCE_NEAR);
          if (battleground(ch->in_room)) {
            op_report(logact(buf.data(), ch, victim), ch);
          }
        }
        else {
          act(ch->pcdata->customstrings[cpoint][1], ch, NULL, victim, TO_CHAR);
  ...
  // Lazily initialized and reused only during one target search. Never carry
  // attack preferences across movement, effects, equipment changes or ticks.
  ...
  static int aggression_score(CHAR_DATA *ch, CHAR_DATA *victim,
    ...
    if (IS_NPC(ch) && !IS_NPC(victim) && (ch->pIndexData->vnum == CORTEX_SOLDIER || ch->pIndexData->vnum == ALLY_TEMPLATE || ch->pIndexData->vnum == MINION_TEMPLATE) && is_helpless(victim))
    return 0;
    ...
    if (IS_NPC(ch)) {
      if (!profile.ready) {
        ...
        if (profile.ranged) {
          ...
          profile.range = disc_range(ch, point);
        }
        profile.ready = true;
      }
      if (profile.ranged) {
        ...
        dam =
        dam *
        damage_mod(combat_distance(ch, victim, TRUE), profile.range) /
        100;
  ...
  int get_agg(CHAR_DATA *ch, CHAR_DATA *victim) {
    AggressionAttackProfile profile;
    return aggression_score(ch, victim, profile);
  }
  ...
  CHAR_DATA *get_npc_target(CHAR_DATA *ch) {
    ...
    for (CharList::iterator it = char_list.begin(); it != char_list.end(); ++it) {
      ...
      if (forest_monster(ch) && !cortex_breach_monster(ch) && rch->in_room != NULL && mist_level(rch->in_room) < 3 && (rch->in_room->area->vnum == INNER_NORTH_FOREST || rch->in_room->area->vnum == INNER_SOUTH_FOREST || rch->in_room->area->vnum == INNER_WEST_FOREST || rch->in_room->area->vnum == HAVEN_TOWN_VNUM))
      continue;
      ...
      const int score = aggression_score(ch, rch, profile);
      if (score > max) {
        victim = rch;
        max = score;
    ...
    CHAR_DATA *cover = get_cover(victim);
    if (cover != NULL && combat_distance(ch, victim, TRUE) > 1)
    victim = cover;
  ...
  void npc_combat_attack(CHAR_DATA *ch) {
    ...
    victim = confused_target(ch, victim, TRUE);
    ...
    if (IS_NPC(ch) && !IS_NPC(victim) && (ch->pIndexData->vnum == CORTEX_SOLDIER || ch->pIndexData->vnum == ALLY_TEMPLATE || ch->pIndexData->vnum == MINION_TEMPLATE) && victim->hit <= 0 && !battleground(ch->in_room) && !guestmonster(victim)) {
      ...
      if (cortex_enforcer(ch)) cortex_enforcer_defeat(ch, victim);
      return;
    }
  ...
  CHAR_DATA *get_close_cover(CHAR_DATA *ch) {
    ...
    while ((victim = next_combat_character(&cursor)) != NULL) {
      if (!in_fight(victim))
      continue;
      ...
      const int distance = combat_distance(victim, ch, FALSE);
      if (distance < mindist) {
        mindist = distance;
        cover = victim;
      }
...
#define MAX_DEMON 40
  ...
  bool cortex_breach_monster(CHAR_DATA *ch) {
    return ch && IS_NPC(ch) && (IS_FLAG(ch->act, ACT_CORTEX_BREACH) || cortex_enforcer(ch));
  }

  bool cortex_enforcer(CHAR_DATA *ch) {
  ...
  bool cortex_alarm(CHAR_DATA *ch, int amount) {
    ...
    ch->pcdata->public_alarm += amount;
    if (!ch->in_room || !in_haven(ch->in_room) || is_gm(ch) || is_ghost(ch)
  ...
  bool cortex_send_enforcers(CHAR_DATA *ch) {
    ...
    for (int n = 0; n < count; ++n) {
      ...
      mob->long_descr = str_dup("An armored Cortex enforcer watches for trouble.\n\r");
  ...
  void cortex_enforcer_defeat(CHAR_DATA *mob, CHAR_DATA *victim) {
    ...
    if (public_response) {
      ...
      victim->hit = 0;
      send_to_char("The Cortex enforcers knock you out and withdraw, leaving you at the mercy of the person you attacked.\n\r", victim);
  ...
  static int cortex_breach_origin = WORLD_EARTH;

  void cortex_breach(int world) {
    cortex_breach_origin = world;
    cortex_breach_until = current_time + 3600;
  }

  bool cortex_monster_ambush(CHAR_DATA *ch, bool forced) {
    if (!forced && (current_time >= cortex_breach_until || !ch || IS_NPC(ch) || !ch->pcdata
        || !ch->in_room || !ch->in_room->area || !in_haven(ch->in_room)
        || in_world(ch) != WORLD_EARTH || ch->in_room->vnum <= 300
        || is_gm(ch) || is_ghost(ch) || higher_power(ch) || guestmonster(ch)
        || is_helpless(ch) || in_fight(ch) || battleground(ch->in_room)
        || !ch->desc || IS_FLAG(ch->comm, COMM_AFK)
        || IS_FLAG(ch->act, PLR_SHROUD) || IS_FLAG(ch->act, PLR_DEEPSHROUD)
        || ch->pcdata->spawned_monsters > 0 || number_percent() > 5)) return FALSE;
    if (!ch || IS_NPC(ch) || !ch->in_room || !ch->pcdata) return FALSE;
    const int origin = forced ? number_range(WORLD_OTHER, WORLD_HELL) : cortex_breach_origin;
    const int difficulty = forced ? number_range(1, 10) : 5;
    int chosen = -1, candidates = 0, weakest = -1, weakest_level = INT_MAX;
    for (int i = 0; i < 100 && monster_table[i].world != -1; ++i) {
      if (monster_table[i].world != origin
          || !get_mob_index(monster_table[i].vnum)) continue;
      const int level = get_demon_lvl(i);
      if (level < weakest_level) { weakest = i; weakest_level = level; }
      if (level > UMAX(10, get_tier(ch) * 10 * difficulty_mod(difficulty) / 100)) continue;
      if (number_range(1, ++candidates) == 1) chosen = i;
    }
    ...
    if (!forced) cortex_breach_until = 0; // At most one ambush per breach.
...
*/
  ...
  void decorate_warren(ROOM_INDEX_DATA *room) {
    ...
    case 0:
      ...
      set_generated_room_description(room, "Long ridges of `ysand`x divide the desert ground. Coarser grains collect between the dunes, while scattered tracks interrupt their otherwise bare surfaces. The open warren has little shelter or enclosure.`x\n\r");
      break;
    case 1:
      room->name = str_dup("A jungle terrain demonic warren");
      set_generated_room_description(room, "Massive `gtrunks`x rise through dense vines and broad leaves. Thick roots buckle the soil, and fallen wood crowds the gaps between the standing trees, giving the jungle warren a close, tangled layout.`x\n\r");
      break;
    case 2:
      room->name = str_dup("A mountain terrain demonic warren");
      set_generated_room_description(room, "Jagged `wrock`x forms a broken mountain landscape. Narrow patches of ground lie between outcrops, their hard surfaces fractured by deep mineral seams and pockets of loose stone.`x\n\r");
      break;
    case 3:
      room->name = str_dup("A swamp terrain demonic warren");
      set_generated_room_description(room, "Dark water occupies the depressions between soft banks of mud. Tangled roots and coarse wetland growth break the `gswamp`x into irregular patches, leaving the firmer ground narrow and uneven.`x\n\r");
      break;
    case 4:
      room->name = str_dup("A storm-wrecked island demonic warren");
      set_generated_room_description(room, "Scoured rock and coarse sand form the island ground. Broken vegetation and water-worn debris gather in the hollows, giving the `wisland`x the battered texture of a place repeatedly exposed to the sea.`x\n\r");
      break;
    default:
      room->name = str_dup("An industrial demonic warren");
      set_generated_room_description(room, "Dark metal structures crowd a scarred industrial floor. Their heavy frames and oily surface stains give the `wwarren`x a severe mechanical geometry, with open working space between the hulking forms.`x\n\r");
      break;
    }
  }
  ...
  _DOFUN(do_target) {
    if (ch->pcdata->ci_editing == 0) {
      ...
      ch->pcdata->ci_absorb = 1;
      if (safe_strlen(argument) > 2) {
        if (power_operation_goal(ch->pcdata->ci_discipline2)) {
          LOCATION_TYPE *loc = get_loc(ch->pcdata->ci_short);
          const char *error = power_operation_error(ch->faction,
              loc ? number_from_territory(loc) : 0, ch->pcdata->ci_discipline2, argument);
  ...
  int find_free_shadow(CHAR_DATA *ch) {
    ...
    for (int i = 0; i < 20; i++) {
      if (ch->pcdata->shadow_attacks[i][0] == 0 && ch->pcdata->shadow_attacks[i][1] == 0)
      return i;
    }
    return -1;
  }
  ...
  char *get_disc_string(int val) {
    for (int j = 0; j < discipline_table_count; j++) {
  ...
  _DOFUN(do_custom) {
    ...
    if (!str_cmp(arg1, "create")) {
      ...
      std::string tmp;
      tmp = haven::format_text("%s %s", arg1, arg2);
      std::string tmp2;
      tmp2 = haven::format_text("%s %s %s", arg1, arg2, argument);
      for (i = 0; i < 25; i++) {
        if (!str_cmp(arg2, ch->pcdata->customstrings[i][0]) || !str_cmp(tmp.data(), ch->pcdata->customstrings[i][0]) || !str_cmp(tmp2.data(), ch->pcdata->customstrings[i][0])) {
          free_string(ch->pcdata->customstrings[i][0]);
  ...
  void killplayer(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (!state_of_emergency() && in_haven(ch->in_room)) {
  ...
  void copspank(CHAR_DATA *ch) {
    ...
    if (mob != NULL) start_fight(mob, ch);
    ch->pcdata->police_timer += 720;
    ch->pcdata->police_number++;
  }
  ...
  void summon_support(CHAR_DATA *ch, int type) {
    ...
    if (type == SPECIAL_MINION) {
      ...
      for (int x = 0; x < discipline_table_count; x++) {
  ...
  void bodyguard_check(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (IS_NPC(ch) && ch->pIndexData->vnum != CORTEX_SOLDIER)
    return;
...
*/
  ...
  void weaken_character(CHAR_DATA *ch, int type, int disc) {
    ...
    if (!IS_NPC(ch) && (disc == DIS_BOWS || disc == DIS_KNIFE || disc == DIS_SPEAR)) {
      if (type == AMMO_PARALYTIC) {
        apply_caff(ch, CAFF_PARALYTICTOXIN, 1);
      ...
      if (type == AMMO_POISON) {
        apply_caff(ch, CAFF_POISONTOXIN, 1);
        act("$n is poisoned.", ch, NULL, NULL, TO_ROOM);
        act("You are poisoned.", ch, NULL, NULL, TO_CHAR);
        dact("$n is poisoned.", ch, NULL, NULL, DISTANCE_MEDIUM);
      }
  ...
  int soc_combat_mod(CHAR_DATA *ch)
  {
    ...
    int vsect = 0;
    int vfac = 0;
    FACTION_TYPE *cult = clan_lookup(ch->fsociety);
    FACTION_TYPE *sect = clan_lookup(ch->legacy_society);
...
*/
    ...
    // Combat focus affects damage against all combatants, including NPCs.
  ...
  void start_fight(CHAR_DATA *ch, CHAR_DATA *target) {
    ...
    const bool public_start = !IS_NPC(ch) && !in_fight(ch) && in_public(ch, target);
    ...
    if (forest_monster(ch) && !cortex_breach_monster(ch) && target->in_room != NULL && mist_level(target->in_room) < 3 && (target->in_room->area->vnum == INNER_NORTH_FOREST || target->in_room->area->vnum == INNER_SOUTH_FOREST || target->in_room->area->vnum == INNER_WEST_FOREST || target->in_room->area->vnum == HAVEN_TOWN_VNUM || target->in_room->sector_type != SECT_FOREST)) {
      ch->ttl = 1;
      return;
    }
    if (forest_monster(target) && !cortex_breach_monster(target) && ch->in_room != NULL && mist_level(ch->in_room) < 3 && (ch->in_room->area->vnum == INNER_NORTH_FOREST || ch->in_room->area->vnum == INNER_SOUTH_FOREST || ch->in_room->area->vnum == INNER_WEST_FOREST || ch->in_room->area->vnum == HAVEN_TOWN_VNUM || ch->in_room->sector_type != SECT_FOREST)) {
      target->ttl = 1;
      return;
    }
    ...
    if (!IS_NPC(ch)) {
      ...
      if (IS_FLAG(ch->act, PLR_SHROUD) && !IS_FLAG(target->act, PLR_SHROUD)) {
        if (is_invader(ch)) {
          if (!IS_AFFECTED(target, AFF_WAKEBOUND) && can_shroud(target))
  ...
  bool is_enemy(CHAR_DATA *ch, CHAR_DATA *victim) {
    if (public_target_excluded(ch, victim)) return FALSE;
    if (fight_problem == 1)
    return FALSE;
    if (ch == NULL || victim == NULL)
    return FALSE;
    ...
    if (forest_monster(ch) && !cortex_breach_monster(ch) && victim->in_room != NULL && mist_level(victim->in_room) < 3 && (victim->in_room->area->vnum == INNER_NORTH_FOREST || victim->in_room->area->vnum == INNER_SOUTH_FOREST || victim->in_room->area->vnum == INNER_WEST_FOREST || victim->in_room->area->vnum == HAVEN_TOWN_VNUM || victim->in_room->sector_type != SECT_FOREST))
    return FALSE;
    if (forest_monster(victim) && !cortex_breach_monster(victim) && ch->in_room != NULL && mist_level(ch->in_room) < 3 && (ch->in_room->area->vnum == INNER_NORTH_FOREST || ch->in_room->area->vnum == INNER_SOUTH_FOREST || ch->in_room->area->vnum == INNER_WEST_FOREST || ch->in_room->area->vnum == HAVEN_TOWN_VNUM || ch->in_room->sector_type != SECT_FOREST))
    return FALSE;
...
*/
  ...
  bool check_fight(CHAR_DATA *ch) {
    ...
    it != char_list.end(); ++it) {
      ...
      // Reject distant bystanders before starting another world-wide enemy
      // search. Keep visibility after has_enemy, in its original order.
      if ((ch->in_room == wch->in_room || combat_distance(ch, wch, FALSE) <= 50) && has_enemy(wch) && (ch->in_room == wch->in_room || can_see_char_distance(ch, wch, DISTANCE_MEDIUM) || can_see_char_distance(wch, ch, DISTANCE_MEDIUM))) {
  ...
  bool forest_monster(CHAR_DATA *ch) {
    if (full_moon_pack(ch)) return FALSE;
    if (!IS_NPC(ch))
    return FALSE;
  ...
  CHAR_DATA *find_prey(CHAR_DATA *mob) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (d->character != NULL && d->connected == CON_PLAYING) {
        ...
        if (!cortex_breach_monster(mob) && in_public(mob, to) && !IS_FLAG(to->act, PLR_SHROUD))
        continue;
        ...
        if (forest_monster(mob) && !cortex_breach_monster(mob) && to->in_room != NULL && mist_level(to->in_room) < 3 && (to->in_room->area->vnum == INNER_NORTH_FOREST || to->in_room->area->vnum == INNER_SOUTH_FOREST || to->in_room->area->vnum == INNER_WEST_FOREST || to->in_room->area->vnum == HAVEN_TOWN_VNUM))
        continue;

        if (forest_monster(mob) && !cortex_breach_monster(mob) && (in_prop(to) || to->in_room->sector_type != SECT_FOREST))
        continue;
  ...
  bool pvp_target(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (IS_NPC(ch) && (ch->pIndexData->vnum == MINION_TEMPLATE || ch->pIndexData->vnum == ALLY_TEMPLATE || ch->pIndexData->vnum == CORTEX_SOLDIER))
    return TRUE;
