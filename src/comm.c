...
*/
  ...
  */
  bool read_from_ident(int fd, char *buffer, size_t capacity) {
    ...
    */
    for (i = 0, k = 0; inbuf[i] != '\n' && inbuf[i] != '\r'; i++) {
      ...
      --k;
        ...
        if (static_cast<size_t>(k) + 1 >= capacity) {
          inbuf[0] = '\0';
          buffer[0] = '\0';
          return FALSE;
        }
  ...
  */
  void process_ident(DESCRIPTOR_DATA *d) {
    ...
    if (!read_from_ident(d->ifd, buffer, sizeof(buffer))) {
      return;
    }
...
void create_ident( DESCRIPTOR_DATA *d, long ip )
{
  ...
  void sig_handler(int sig) {
    ...
    if (!signalled) {
      ...
      case 0:
        signal(SIGABRT, SIG_DFL);
        abort(); // Produce the diagnostic core without undefined arithmetic.
      case (pid_t)-1:
  ...
  // Fold exactly the ASCII letters handled by str_cmp/LOWER.
  static std::string host_key(const char *host) {
    std::string key(host);
    for (char &c : key) c = LOWER(c);
    return key;
  }

  static void enforce_host_limit() {
    std::unordered_map<std::string, unsigned int> counts;
    counts.reserve(descriptor_list.size());
    for (DESCRIPTOR_DATA *d : descriptor_list)
      if (d && d->host) ++counts[host_key(d->host)];
    // Count invalid entries too, as the original check did before removal.
    for (DESCRIPTOR_DATA *d : descriptor_list)
      if (d && d->host && counts[host_key(d->host)] >= 10) d->valid = FALSE;
  }
  ...
  void game_loop_unix(int control) {
    ...
    while (!merc_down) {
      {
      haven::ScopedTimer pulse_timer("pulse_work");
      fd_set in_set;
      fd_set out_set;
      fd_set exc_set;
      int maxdesc;
      ...
      */
      ...
      if (select(maxdesc + 1, &in_set, &out_set, &exc_set, &null_time) < 0) {
        ...
        save_game_stuff();
        raise(SIGFPE); // Enter the existing fatal-error recovery handler.
        //      do_auto_shutdown();
        //      exit(1);
      }
      ...
      */
      ...
      } // Measure active work, excluding the deliberate pulse sleep.
      haven::report_performance();
  ...
  */
  ...
  const char *dest_arch_names[12] = {
    ...
    "Civic Educator", "None", "Institute Student",
    "Institute Patient"
  };
  ...
  void editing_prompt(CHAR_DATA *ch) {
    if (ch->pcdata->ci_editing == 1) {
      ...
      if (ch->pcdata->ci_discipline2 == GOAL_PSYCHIC) {
        ...
        if (ch->pcdata->ci_discipline2 < 0 || ch->pcdata->ci_discipline2 > GOAL_ATTACK_POWER)
        ...
        printf_to_char(ch, "Target (higher power, required): %s\n\r", ch->pcdata->ci_message);
        else if (ch->pcdata->ci_discipline2 == GOAL_ASSASSINATE || ch->pcdata->ci_discipline2 == GOAL_RAZE || ch->pcdata->ci_discipline2 == GOAL_KIDNAP || ch->pcdata->ci_discipline2 == GOAL_RESCUE)
        printf_to_char(ch, "Target: %s\n\r", ch->pcdata->ci_message);
    ...
    else if(ch->pcdata->ci_editing == 22)
    {
      ...
      if(profile == NULL)
      {
        ...
        edit_profile(profile)->name = str_dup(ch->name);
        edit_profile(profile)->last_active = current_time;
  ...
  void bust_a_prompt(CHAR_DATA *ch) {
    ...
    while (*str != '\0') {
      ...
      case 'p':
        ...
        snprintf(buf2 + strlen(buf2), sizeof(buf2) - strlen(buf2), " `x(`RShifted`x)");
        if (get_energy(ch) >= 100)
        snprintf(buf2 + strlen(buf2), sizeof(buf2) - strlen(buf2), " `x[`yFatigued`x]");
        if (ch->bagcarrier == 1 && ch->in_room != NULL && battleground(ch->in_room))
        snprintf(buf2 + strlen(buf2), sizeof(buf2) - strlen(buf2), " `x(`WCarrier`x)");
  ...
  char *colorout(char type[MSL], int mode, int lmode, CHAR_DATA *ch) {
    ...
    if (mode == 0) { // basic color
      if (type[0] == 'q') {
        ...
        code = haven::format_text(CLEAR);
        else
        code = haven::format_text("%s/color%s", MXP_BEG, MXP_END);
      }
      else if (type[0] == 'x') {
        code = haven::format_text(CLEAR);
      }
        ...
        code = haven::format_text(C_BLUE);
      }
      else if (type[0] == 'c') {
        code = haven::format_text(C_CYAN);
      }
      else if (type[0] == 'g') {
        code = haven::format_text(C_GREEN);
      }
      else if (type[0] == 'm') {
        code = haven::format_text(C_MAGENTA);
      }
      else if (type[0] == 'r') {
        code = haven::format_text(C_RED);
      }
      else if (type[0] == 'w') {
        code = haven::format_text(C_WHITE);
      }
      else if (type[0] == 'y') {
        code = haven::format_text(C_YELLOW);
      }
      else if (type[0] == 'd') {
        code = haven::format_text(C_D_GREY);
      }
      else if (type[0] == 'B') {
        code = haven::format_text(C_B_BLUE);
      }
      else if (type[0] == 'C') {
        code = haven::format_text(C_B_CYAN);
      }
      else if (type[0] == 'G') {
        code = haven::format_text(C_B_GREEN);
      }
      else if (type[0] == 'M') {
        code = haven::format_text(C_B_MAGENTA);
      }
      else if (type[0] == 'R') {
        code = haven::format_text(C_B_RED);
      }
      else if (type[0] == 'W') {
        code = haven::format_text(C_B_WHITE);
      }
      else if (type[0] == 'Y') {
        code = haven::format_text(C_B_YELLOW);
      }
      else if (type[0] == 'D') {
        code = haven::format_text(C_D_GREY);
      }
      else if (type[0] == '*') {
        code = haven::format_text("%c", 007);
      }
      else if (type[0] == '/') {
        code = haven::format_text("%c", 012);
      }
      // else if(type[0] == '`') {code = haven::format_text("%c", '`');}
      // else if(type[0] == '`') {code = haven::format_text("%s", "``");}
        ...
        if (type[0] == 'o') {
          if (ch->pcdata->verbal_color != NULL && safe_strlen(ch->pcdata->verbal_color) > 0) {
            if (is_number(ch->pcdata->verbal_color) && atoi(ch->pcdata->verbal_color) >= 0 && atoi(ch->pcdata->verbal_color) <= 255) {
              ...
              code = haven::format_text("\x1B[0m\x1B[38;5;%dm", color);
            }
          }
          else {
            code0 = col->say[0];
        ...
        if (code0 != -1 && code1 != -1) {
          // if(code2)
          code = haven::format_text("\033[%d;3%dm%s", code0, code1, code2 ? "\a" : "");
          // else
          //	sprintf(code.data(),"\033[%d;3%dm", code0, code1);
        }
      }
      if(lmode == 2 && ch != NULL && ch->desc != NULL && ch->desc->mxp == TRUE)
      {
        std::string tmp;
        tmp = haven::format_text("%s/color%s%s", MXP_BEG, MXP_END, code.data());
        code = haven::format_text("%s", tmp.data());
      }
      ...
      if (is_number(type) && atoi(type) >= 0 && atoi(type) <= 255) {
        int color = atoi(type);
        if(lmode == 2)
        code = haven::format_text("%s/color%s\x1B[0m\x1B[38;5;%dm", MXP_BEG, MXP_END, color);
        else
        code = haven::format_text("\x1B[0m\x1B[38;5;%dm", color);
      ...
      code = haven::format_text("%s/color%s%scolor fore=%s%s", MXP_BEG, MXP_END, MXP_BEG, type, MXP_END);
      // log_string(code.data());
    }
  ...
  { // for when regular color just isn't cutting it any more
    ...
    if (d && txt) {
      snprintf(intxt, sizeof(intxt), "%s", txt);
      printf_to_char(ch, "Intxt: %s\n\r`x", intxt); // testing
      txtlen = safe_strlen(intxt);
      printf_to_char(ch, "Intxt len: %d\n\r`x", txtlen); // testing
      for (inp = 0; inp < txtlen && outp < (int)sizeof(outtxt) - 1; inp++) {
        badcode = FALSE;

        if (intxt[inp] == '`') { // universal color codon
          inp++;
          if (inp >= txtlen) { outtxt[outp++] = '`'; break; }
          ...
          else if (inp + 2 < txtlen && isdigit(intxt[inp]) && isdigit(intxt[inp + 1]) && isdigit(intxt[inp + 2])) {
            colormode = 1;
          }
          else {
            colormode = 0;
          ...
          if (outp >= (int)sizeof(outtxt) - 1) break;
        ...
        if (badcode == FALSE) { // don't process bad codes
          ...
          if (colortag == NULL) colortag = str_dup("");
          if (!strcmp(colortag, "BADCODE")) { // perserving badcode as output
            for (i = 0; i < codelen && outp < (int)sizeof(outtxt) - 1; i++) {
              outtxt[outp] = code[i];
              outp++;
            }
          ...
          strcpy(test, outtxt);
            ...
            if ((!d->character && d->ansi) || (d->character && IS_FLAG(d->character->act, PLR_COLOR))) {
              for (i = 0; i < colorlen && outp < (int)sizeof(outtxt) - 1; i++) {
        ...
        if (colortag != NULL) free_string(colortag);
        colortag = NULL;
      }
    }
    else {
  ...
  // Helpers return values internally so escaping requires no engine allocations.
  static std::string escape_mxp(const char *input) {
    std::string output;
  ...
  char *revert_mxp_clear(char *instr) {
    std::string output;
    if (!instr) return str_dup("");
    output.reserve(strlen(instr));
    for (const char *p = instr; *p;) {
      if (*p == '&' && strncmp(p, "&lt;", 4) == 0) { output += '<'; p += 4; }
      else if (*p == '&' && strncmp(p, "&gt;", 4) == 0) { output += '>'; p += 4; }
      else if (*p == '&' && strncmp(p, "&amp;", 5) == 0) { output += '&'; p += 5; }
      else output += *p++;
    }
    return str_dup(output.c_str());
  }

  char *clear_mxp(char *instr, DESCRIPTOR_DATA *d) {
    // Preserve the legacy API: non-MXP output borrows instr.
    return d && d->mxp ? str_dup(escape_mxp(instr).c_str()) : instr;
  }

  bool been_processed(const char *argument) {
  ...
  static std::string render_text(const char *txt, DESCRIPTOR_DATA *d, int ctype) {
#define MBSL MSL*2
    char *       colortag = NULL;
    char         intxt[MBSL] = {}, code[MBSL] = {};
    std::string outtxt, tmpout;
    outtxt.reserve(MBSL);
    tmpout.reserve(MBSL);
    ...
    if (!txt) return ctype == COLOR_LENGTH ? "0" : "";
    char editedTxt[MBSL];
    const size_t input_length = strnlen(txt, MBSL - 10);
    memcpy(editedTxt, txt, input_length);
    memcpy(editedTxt + input_length, "`x", 3);
    txt = editedTxt;

    if (ctype == COLOR_COLOR) {
      if (been_processed(txt)) {
        return txt;
      }
    }
    outtxt.clear();
    ...
    if ((d || ctype == COLOR_LENGTH || ctype == COLOR_NOCOLOR) && txt) {
      const size_t copy_length = strnlen(txt, maxl);
      memcpy(intxt, txt, copy_length);
      intxt[copy_length] = '\0';
      txtlen = safe_strlen(intxt);
  ...
  char *tyr_lor(Buffer &buffer, const char *txt, DESCRIPTOR_DATA *d, int ctype) {
    const std::string rendered = render_text(txt, d, ctype);
    if (ctype == COLOR_COLOR) buffer.strcat(rendered.c_str());
    return str_dup(rendered.c_str());
  }
  ...
  _DOFUN(do_colortest) {
    ...
    DESCRIPTOR_DATA *d;
    d = ch->desc;
    char *length_text = tyr_lor(cstring, argument, d, COLOR_LENGTH);
    int slength = atoi(length_text);
    free_string(length_text);
    char buf[MSL];
  ...
  */
  ...
  void write_to_buffer(DESCRIPTOR_DATA *d, const char *txt, int length) {
    ...
    const std::string rendered = render_text(txt, d, COLOR_COLOR);

    // Convert the color.
    // colorconv_desc( buffer, txt, d );
    //    newtxt = disco_lor(txt, d);

    // Socket output needs byte length, including ANSI/MXP sequences. The
    // renderer tracks this; reparsing for visible width is wrong and costly.
    length = static_cast<int>(rendered.size());
...
*/
    ...
    for (CharList::iterator it = to_list->begin(); it != to_list->end(); ++it) {
      ...
      while (*str != '\0') {
        ...
        i = " <@@@> ";
        if (!arg2 && *str >= 'A' && *str <= 'Z') {
          diagnostic = haven::format_text("Act: missing arg2 for code %d in string %s", *str, format);
          bug(diagnostic.data(), 0);
      ...
      if (to->desc != NULL && to->desc->valid == TRUE) {
        ...
        write_to_buffer(to->desc, buf.c_str(), 0);
      }
    }
  ...
  void printf_to_char(CHAR_DATA *ch, char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    std::string buf = haven::vformat_text(fmt, args);
    va_end(args);

    if (ch != NULL)
    write_to_buffer(ch->desc, buf.c_str(), 0);
  }
  ...
  */
  ...
  char *remove_color(char *buffer, const char *txt) {
    ...
    char *plain_text = tyr_lor(inbuffer, txt, NULL, COLOR_NOCOLOR);
    snprintf(buf, sizeof(buf), "%s", plain_text);
  ...
  bool state_get_name(DESCRIPTOR_DATA *d, char *argument, CHAR_DATA *ch) {
    ...
    if (check_ban(d->host, BAN_PERMIT) && !IS_FLAG(ch->act, PLR_PERMIT)) {
      ...
      DOMAIN_TYPE *guest_domain = higher_power(ch) ? power_domain(ch->name) : NULL;
      if(ch->pcdata->guest_reward_used || (guest_domain && (guest_domain->guest_retired || guest_domain->banished_until > current_time))) {
        write_to_buffer(d, "That guest has retired or is banished from play.\n\r", 0);
        close_desc(d);
        return FALSE;
      }
    ...
    if (!str_prefix(argument, "delete") || !str_prefix("Delete", argument)) {
      ...
      for (int i = 0; i < 25; i++) {
        if (!str_cmp(argument, account->characters[i]) && account->char_stasis[i] == 1) {
          haven::remove_optional_file(std::string("haven/prp/") + account->characters[i] + ".txt");
          haven::remove_optional_file(std::string("haven/ptm/") + account->characters[i] + ".txt");
  ...
  void news_feed(CHAR_DATA *ch, int type) {
    ...
    if (ch->fsociety != 0) {
      count = 0;
      FACTION_TYPE *fac = clan_lookup(ch->fsociety);
      for (i = 0; i < 20; i++) {
	    	if (fac != NULL && fac->messages[i] != NULL && safe_strlen(fac->messages[i]) > 2 && fac->message_timer[i] > 0) {
          if (count == 0) {
            sprintf(buf, "`cIn Society news for %s;`x\n\r", fac->name);
            strcat(string, buf);
          }
          sprintf(buf, "%s\n\r", fac->messages[i]);
          strcat(string, buf);
    ...
    if (ch->legacy_society != 0) {
      count = 0;
      FACTION_TYPE *fac = clan_lookup(ch->legacy_society);
      for (i = 0; i < 20; i++) {
	    	if (fac != NULL && fac->messages[i] != NULL && safe_strlen(fac->messages[i]) > 2 && fac->message_timer[i] > 0) {
          if (count == 0) {
            sprintf(buf, "`cIn Society news for %s;`x\n\r", fac->name);
    ...
    if (ch->pcdata->account->guest_remake_reward >= 4 || IS_FLAG(ch->pcdata->account->flags, ACCOUNT_NIGHTMARE)) {
      sprintf(buf, "You have an earned T%d character remake: guest remake t%d <name>.\n\r", UMAX(4, ch->pcdata->account->guest_remake_reward), UMAX(4, ch->pcdata->account->guest_remake_reward));
      strcat(string, buf);
    }
  ...
  */
  ...
  void fread_account(ACCOUNT_TYPE *account, FILE *fp) {
    account->recolor_month = 0; // Missing field means a pre-rollout account.
    const char *word;
    bool fMatch;
    for (;;) {
      ...
      case 'C':
        ...
        KEY("ColourMonth", account->recolor_month, fread_number(fp));
      ...
      case 'E':
        if (!str_cmp(word, "End")) {
          if (IS_FLAG(account->flags, ACCOUNT_NIGHTMARE)) {
            account->guest_remake_reward = UMAX(4, account->guest_remake_reward);
            REMOVE_FLAG(account->flags, ACCOUNT_NIGHTMARE);
          }
          if (!account->name) {
      ...
      case 'M':
        ...
        KEY("MonsterRemake", account->guest_remake_reward, fread_number(fp));
        break;
      case 'N':
      ...
      case 'P':
        ...
        KEY("PKarmaRemainder", account->pkarma_remainder, fread_number(fp));
  ...
  void save_account(ACCOUNT_TYPE *account, bool backup) {
    ...
    if ((fp = fopen(strsave, "w")) == NULL) {
      ...
      fprintf(fp, "MonsterRemake %d\n", account->guest_remake_reward);
      ...
      if (account->pkarma_remainder != 0)
        fprintf(fp, "PKarmaRemainder %d\n", account->pkarma_remainder);
      if (account->pkarma != 0)
      fprintf(fp, "PKarma %d\n", account->pkarma);
      ...
      fprintf(fp, "ColourMonth %d\n", account->recolor_month);
