...
extern "C" {
  ...
  bool cortex_loyalty_brainwashed(CHAR_DATA *ch) {
    if (!ch || IS_NPC(ch) || !ch->pcdata
        || safe_strlen(ch->pcdata->brainwash_loyalty) < 2) return FALSE;
    FACTION_TYPE *fac = clan_lookup(FACTION_CORTEX);
    return !str_cmp(ch->pcdata->brainwash_loyalty, "Cortex")
        || !str_cmp(ch->pcdata->brainwash_loyalty, "The Cortex")
        || (fac && !str_cmp(ch->pcdata->brainwash_loyalty, fac->name));
  }

  int cortex_lifeforce_bonus(CHAR_DATA *ch, CHAR_DATA *view) {
    if (!ch || IS_NPC(ch) || !ch->pcdata
        || ch->fcore != FACTION_CORTEX || !is_super(ch)) return 0;
    int bonus = 0;
    if (ch->pcdata->branddate > 0 && safe_strlen(ch->pcdata->brander) > 0) {
      // Preserve the brander's last known status while they are offline.
      CHAR_DATA *brander = get_char_world_pc(ch->pcdata->brander);
      if (brander && brander != ch)
        ch->pcdata->cortex_human_brand = brander->fcore == FACTION_CORTEX && !is_super(brander);
      if (ch->pcdata->cortex_human_brand) {
        bonus += 2;
        if (view) send_to_char("  Cortex human brand: +2 LF\n\r", view);
      }
    }
    if (cortex_loyalty_brainwashed(ch)) {
      bonus += 2;
      if (view) send_to_char("  Cortex loyalty brainwashing: +2 LF\n\r", view);
    }
    return bonus;
  }

  bool is_containment_priority(CHAR_DATA *ch) {
    return ch && !IS_NPC(ch) && ch->pcdata
        && ch->pcdata->neutralized != 1
        && !cortex_loyalty_brainwashed(ch)
        && (get_tier(ch) >= 4 || skillbase_count(ch, STYPE_ABOMINATION) > 0);
  }

  bool cortex_loyalty_chip(CHAR_DATA *ch) {
    if (!ch || IS_NPC(ch) || !ch->pcdata || ch->fcore != FACTION_CORTEX) return FALSE;
    if (ch->pcdata->implant_frequency != 0) return TRUE;
    FACTION_TYPE *fac = clan_lookup(FACTION_CORTEX);
    if (!fac) return FALSE;
    // Membership chips are implicit in the same positions used by control.
    if (fac->axes[AXES_SUPERNATURAL] == AXES_FARRIGHT && seems_super(ch)) return TRUE;
    if (fac->axes[AXES_SUPERNATURAL] == AXES_FARLEFT && !seems_super(ch)) return TRUE;
    const int corruption = fac->axes[AXES_CORRUPT];
    return (corruption == AXES_FARLEFT && get_tier(ch) <= 2)
        || (corruption > 0 && corruption <= AXES_NEARLEFT && get_tier(ch) <= 1);
  }

  bool faction_allows_demonic_pacts(const FACTION_TYPE *fac) {
    if (!fac || fac->vnum == FACTION_CORTEX) return FALSE;
    return fac->axes[AXES_CORRUPT] == AXES_FARLEFT
        || (fac->type == FACTION_SOCIETY && fac->axes[AXES_CORRUPT] == AXES_DEMONIC);
  }

  bool faction_pact_eligible(CHAR_DATA *ch, FACTION_TYPE *fac, CHAR_DATA *display) {
    if (!ch || !fac) return FALSE;
    if (IS_NPC(ch) || !pact_holder(ch) || faction_allows_demonic_pacts(fac)) return TRUE;
    if (display) {
      if (fac->vnum == FACTION_CORTEX)
        send_to_char("The Cortex cannot tolerate your loyalty to a demonic master.\n\r", display);
      else
        send_to_char("Demonic pact holders may only belong to strongly corrupt or demonic societies.\n\r", display);
    }
    return FALSE;
  }

  static void show_containment_priority(CHAR_DATA *viewer, CHAR_DATA *target) {
    if (is_containment_priority(target))
      send_to_char("`RCONTAINMENT PRIORITY`x: Central authority has ordered this character to be brainwashed.\n\r", viewer);
  }

  void normalize_society_record(FACTION_TYPE *fac) {
    if (!fac) return;
    const int old_type = fac->type;
    if (old_type != 3 && old_type != 4) { fac->patron_relation = SOCIETY_PATRON_NONE; return; }
    if (fac->patron_relation < 0)
      fac->patron_relation = safe_strlen(fac->eidilon) < 1 ? SOCIETY_PATRON_NONE
          : old_type == 4 ? SOCIETY_PATRON_WORSHIP : SOCIETY_PATRON_OPPOSE;
    fac->type = FACTION_SOCIETY;
    if (fac->patron_relation < SOCIETY_PATRON_NONE || fac->patron_relation > SOCIETY_PATRON_OPPOSE)
      fac->patron_relation = SOCIETY_PATRON_NONE;
  }

  void enforce_civil_servant_membership(CHAR_DATA *ch) {
    if (!ch || IS_NPC(ch) || !ch->pcdata || ch->race != RACE_CIVIL_SERVANT || pact_holder(ch)) return;
    if (ch->fcore != FACTION_CORTEX) {
      ch->fcore = FACTION_CORTEX;
      ch->faction = FACTION_CORTEX;
      ch->factiontwo = 0;
      ch->factiontrue = 0;
      ch->fsociety = 0;
      ch->legacy_society = 0;
      ch->esteem_faction = UMAX(100, ch->esteem_faction);
    }
    FACTION_TYPE *fac = clan_lookup(FACTION_CORTEX);
    if (!fac || !ch->name || !ch->name[0]) return;
    int empty = -1;
    for (int i = 0; i < 100; ++i) {
      if (fac->member_names[i] && !str_cmp(fac->member_names[i], ch->name)) return;
      if (empty < 0 && safe_strlen(fac->member_names[i]) < 2) empty = i;
    }
    if (empty >= 0) {
      free_string(fac->member_names[empty]);
      fac->member_names[empty] = str_dup(ch->name);
      fac->member_esteem[empty] = ch->esteem_faction;
    }
  }

  void normalize_society_memberships(CHAR_DATA *ch) {
    enforce_civil_servant_membership(ch);
    if (!ch || IS_NPC(ch) || !ch->pcdata) return;
    // A sole former sect becomes the primary society. An existing second
    // membership remains available until it is left; new joins cannot fill it.
    if (ch->fsociety == 0 && ch->legacy_society != 0) {
      ch->fsociety = ch->legacy_society;
      ch->legacy_society = 0;
      ch->esteem_society = ch->esteem_legacy_society;
      ch->esteem_legacy_society = 0;
      ch->deploy_society = ch->deploy_legacy_society;
      ch->deploy_legacy_society = 0;
      std::swap(ch->pcdata->operative_society, ch->pcdata->operative_legacy_society);
    }
    if (ch->legacy_society == ch->fsociety) ch->legacy_society = 0;
    if (ch->pcdata->ftype == 4) ch->pcdata->ftype = FACTION_SOCIETY;
  }

  int selected_society(CHAR_DATA *ch) {
    if (!ch) return 0;
    normalize_society_memberships(ch);
    if (ch->faction != 0 && (ch->faction == ch->fsociety || ch->faction == ch->legacy_society)) return ch->faction;
    return ch->fsociety;
  }

  bool society_has_room(CHAR_DATA *ch) {
    return ch && ch->fcore == 0 && ch->fsociety == 0 && ch->legacy_society == 0;
  }

  int society_deployment(CHAR_DATA *ch, int faction) {
    if (!ch || !faction) return 0;
    if (ch->fsociety == faction) return ch->deploy_society;
    if (ch->legacy_society == faction) return ch->deploy_legacy_society;
    return 0;
  }

  bool player_faction_join_allowed(CHAR_DATA *ch, FACTION_TYPE *fac, bool show) {
    if (!ch || !fac || fac->vnum == 0) return FALSE;
    if (fac->vnum != FACTION_CORTEX && college_student(ch, TRUE)) {
      if (show) send_to_char("Joining a society other than the Cortex would get you expelled. You must drop out of college first.\n\r", ch);
      return FALSE;
    }
    if (!faction_pact_eligible(ch, fac, show ? ch : NULL)) return FALSE;
    if (!IS_NPC(ch) && ch->race == RACE_CIVIL_SERVANT && fac->vnum != FACTION_CORTEX) {
      if (show) send_to_char("Civil servants must belong to the Cortex and cannot leave it.\n\r", ch);
      return FALSE;
    }
    if (ch->fcore == FACTION_CORTEX && ch->modifier == MODIFIER_TEMPLE
        && fac->type == FACTION_CORE && fac->vnum != FACTION_CORTEX) {
      if (show) send_to_char("You cannot leave the Cortex while Science Augmented. They would simply turn your body off.\n\r", ch);
      return FALSE;
    }
    if (ch->fcore == FACTION_CORTEX && is_super(ch) && fac->vnum != FACTION_CORTEX) {
      if (show) send_to_char("Supernaturals cannot leave the Cortex. A human member with banishing authority must release you.\n\r", ch);
      return FALSE;
    }
    ...
    if (!IS_NPC(ch) && fac->type == FACTION_SOCIETY
        && ch->fsociety != fac->vnum && ch->legacy_society != fac->vnum && !society_has_room(ch)) {
      if (show) send_to_char("Leave your current faction or society before joining a society.\n\r", ch);
      return FALSE;
    }
    if (!IS_NPC(ch) && fac->type == FACTION_CORE && ch->fcore != fac->vnum
        && (ch->fsociety != 0 || ch->legacy_society != 0)) {
      if (show) send_to_char("Leave your society before joining a faction.\n\r", ch);
      return FALSE;
    }
    return TRUE;
  }
...
#define ADVERSARY_VNUM 115;
  ...
  static int society_service_cost(FACTION_TYPE *fac, int base) {
    return base * (fac->type == FACTION_SOCIETY ? UMAX(1, member_count(fac)) : 1);
  }
  ...
  int get_alliance args((FACTION_TYPE * fac, int issue));
  ...
  static std::unordered_map<int, FACTION_TYPE *> faction_index;
  static bool faction_index_dirty = true;
  static size_t faction_index_size = 0;
  void invalidate_faction_index(void) { faction_index_dirty = true; }
  ...
  void fread_faction(FILE *fp) {
    ...
    for (;;) {
      ...
      case 'A':
        ...
        KEY("AllianceSide", fac->strategic_alliance, fread_number(fp));
      ...
      case 'B':
        ...
        break;
      case 'C':
        KEY("CortexQuotaMonth", fac->lf_quota_month, fread_number(fp));
        KEY("CortexQuotaHarvested", fac->lf_quota_harvested, fread_number(fp));
        KEY("CortexLastDissent", fac->last_dissent, fread_number(fp));
      ...
      case 'E':
        ...
        if (!str_cmp(word, "End")) {
          normalize_society_record(fac);
            FacVect.push_back(fac);
            faction_index_dirty = true;
          return;
        }
      ...
      case 'O':
        ...
        break;
      case 'P':
        KEY("PatronRelation", fac->patron_relation, fread_number(fp));
        KEY("PowerWorships", fac->power_worships, fread_string(fp));
        KEY("PowerOpposes", fac->power_opposes, fread_string(fp));
        ...
        break;
      case 'R':
        KEY("RecoveryReceipts", fac->recovery_receipts, fread_string(fp));
        if (!str_cmp(word, "RecoveryReceipt")) {
          char *receipt = fread_string(fp);
  ...
  bool save_clans(bool backup) {
    ...
    if (backup) {
      ...
      if ((fpout = open_snapshot_file(CLAN_FILE)) == NULL) {
        bug("Cannot open clans.txt for writing", 0);
        return FALSE;
      }
    }
    ...
    it != FacVect.end(); ++it) {
      ...
      fprintf(fpout, "PatronRelation %d\n", (*it)->patron_relation);
      fprintf(fpout, "PowerWorships %s~\nPowerOpposes %s~\n", (*it)->power_worships, (*it)->power_opposes);
      ...
      fprintf(fpout, "OOC %s~\n", (*it)->ooc);
      // Each receipt is a short record; the permanent ledger can outgrow MSL.
      std::istringstream receipts((*it)->recovery_receipts);
      std::string receipt;
      while (std::getline(receipts, receipt, '|'))
        if (!receipt.empty()) fprintf(fpout, "RecoveryReceipt %s~\n", receipt.c_str());
      fprintf(fpout, "AllianceSide %d\n", (*it)->strategic_alliance);
      ...
      if ((*it)->vnum == FACTION_CORTEX) {
        fprintf(fpout, "CortexQuotaMonth %d\n", (*it)->lf_quota_month);
        fprintf(fpout, "CortexQuotaHarvested %d\n", (*it)->lf_quota_harvested);
        fprintf(fpout, "CortexLastDissent %d\n", (*it)->last_dissent);
      }
    ...
    fprintf(fpout, "#END\n");
    return close_snapshot_file(fpout) == 0;
  }

  void save_blackmarket() {
    FILE *fpout;

    if ((fpout = open_snapshot_file(MARKET_FILE)) == NULL) {
  ...
  void fread_operation(FILE *fp) {
    ...
    for (;;) {
      ...
      case 'G':
        ...
        KEY("GuestResolved", op->power_resolved, fread_number(fp));
        break;
      case 'H':
  ...
  FACTION_TYPE *clan_lookup(int vnum) {
    if (faction_index_dirty || faction_index_size != FacVect.size()) {
      faction_index.clear();
      for (FACTION_TYPE *fac : FacVect)
        faction_index.emplace(fac->vnum, fac); // Preserve first-match ordering.
      faction_index_dirty = false;
      faction_index_size = FacVect.size();
    }
    const auto found = faction_index.find(vnum);
    return found == faction_index.end() ? NULL : found->second;
  }
  ...
  FACTION_TYPE *get_cult(CHAR_DATA *ch) { return clan_lookup(ch->fsociety); }
  FACTION_TYPE *get_sect(CHAR_DATA *ch) { return clan_lookup(ch->legacy_society); }
  ...
  void give_respect(CHAR_DATA *ch, int amount, char *reason, int faction) {
    ...
    if (faction == ch->legacy_society)
    ch->esteem_legacy_society += amount;
    if (faction == ch->fsociety)
    ch->esteem_society += amount;
  ...
  void give_respect_noecho(CHAR_DATA *ch, int amount, int faction) {
    ...
    if (ch->fcore == faction)
    ch->esteem_faction += amount;
    if (ch->fsociety == faction)
    ch->esteem_society += amount;
    if (ch->legacy_society == faction)
    ch->esteem_legacy_society += amount;
  ...
  void join_to_clan(CHAR_DATA *ch, int vnum) {
    ...
    if (!player_faction_join_allowed(ch, fac, TRUE) || fac->antagonist != 0)
      return;

    if (fac && ch && fac->vnum == ch->legacy_society && fac->vnum != 0) return;
    int i;
    ...
    else if (fac->type == FACTION_SOCIETY)
    {
      if(ch->fsociety == vnum)
      return;
      ch->fsociety = vnum;
      ch->esteem_society = 100;
    ...
    if (fac->vnum == ch->legacy_society)
    fac->member_esteem[i] = ch->esteem_legacy_society;
    if (fac->vnum == ch->fsociety)
    fac->member_esteem[i] = ch->esteem_society;
  ...
  void join_to_clan_two(CHAR_DATA *ch, int vnum) {
    ...
    {
      ch->fsociety = vnum;
      if(ch->esteem_society > 100000)
      ch->esteem_society = 100000;
  ...
  void remove_from_clanroster(char *name, int vnum) {
    ...
    if (vnum == FACTION_CORTEX && victim->race == RACE_CIVIL_SERVANT && !pact_holder(victim)) {
    ...
    victim->fcore = 0;
    if(victim->fsociety == vnum)
    victim->fsociety = 0;
    if(victim->legacy_society == vnum)
    victim->legacy_society = 0;
    if(victim->factiontrue == vnum)
    victim->factiontrue = 0;
    normalize_society_memberships(victim);
  ...
  void clan_update() {
    ...
    it != FacVect.end(); ++it) {
      ...
      if (unset == FALSE && fac->alliance == 0 && fac->nopart == 0 && fac->cardinal == 0 && fac->antagonist == 0 && fac->outcast == 0 && !generic_faction_vnum(fac->vnum) && (fac->type == FACTION_SOCIETY)) {
        fac->alliance = get_alliance(fac, time_info.society_alliance_issue);
        char buf[MSL];
        sprintf(buf, "You join The %s Alliance.", alliance_names(time_info.society_alliance_issue, fac->alliance));
        send_message(fac->vnum, buf);
      }
      if (generic_faction_vnum(fac->vnum))
      fac->alliance = fac->vnum;
      if (fac->type == FACTION_CORE && alliance_eligible(fac) && !is_alliance(fac->strategic_alliance)) {
        fac->strategic_alliance = get_alliance(fac, time_info.society_alliance_issue);
        if (!is_alliance(fac->strategic_alliance))
          fac->strategic_alliance = alliance_sides[static_cast<unsigned int>(fac->vnum) % ALLIANCE_COUNT];
      }
        ...
        if (safe_strlen(fac->member_names[i]) > 1) {
          ...
          if (get_char_world_pc(fac->member_names[i]) != NULL && !IS_FLAG(get_char_world_pc(fac->member_names[i])->act, PLR_DEAD)) {
            ...
            if (fac->vnum == FACTION_ORDER && prof_focus(member) > 1 && prof_focus(member) > arcane_focus(member) && prof_focus(member) > combat_focus(member) && fac->member_esteem[i] >
              ...
              else if (member->legacy_society == fac->vnum)
              member->esteem_legacy_society = fac->member_esteem[i];
              else if (member->fsociety == fac->vnum)
              member->esteem_society = fac->member_esteem[i];
              sprintf(buf, "CLAN DEMOTION PROF ORDER %s:%s.", fac->name, member->name);
              log_string(buf);
            }

            if ((fac->axes[AXES_CORRUPT] == AXES_FARLEFT || fac->axes[AXES_CORRUPT] == AXES_DEMONIC) && get_tier(member) < 3 && fac->member_esteem[i] >
                UMIN(fac->member_esteem[i], rank_requirement(3))) {
              fac->member_esteem[i] =
              UMIN(fac->member_esteem[i], rank_requirement(3));
              if (member->fcore == fac->vnum)
...
*/
            if (!(member->race == RACE_CIVIL_SERVANT && fac->vnum == FACTION_CORTEX && !pact_holder(member))
                && ((fac->soft_restrict == 0 && !college_student(member, FALSE) && !clinic_patient(member) && !faction_elligible(member, fac, TRUE, member)) || !faction_hardelligible(member, fac, TRUE, member))) {
              ...
              if(member->fcore == fac->vnum)
              member->fcore = 0;
              if(member->legacy_society == fac->vnum)
              member->legacy_society = 0;
              if(member->fsociety == fac->vnum)
              member->fsociety = 0;
            ...
            if (removed == FALSE) {
              ...
              if (fac->axes[AXES_DEMOCRATIC] >= AXES_NEARRIGHT || str_cmp(fac->member_names[i], fac->leader) || generic_faction_vnum(fac->vnum)) {
                ...
                if (fac->vnum == member->fsociety) {
                  fac->member_esteem[i] = member->esteem_society;
                  if (!str_cmp(fac->member_names[i], fac->leader))
                  fac->leaderesteem = member->esteem_society;
                }
                if (fac->vnum == member->legacy_society) {
                  fac->member_esteem[i] = member->esteem_legacy_society;
                  if (!str_cmp(fac->member_names[i], fac->leader))
                  fac->leaderesteem = member->esteem_legacy_society;
                }
  ...
  _DOFUN(do_newalliance) {
    if (IS_NPC(ch) || get_trust(ch) < MAX_LEVEL) {
      send_to_char("You do not have permission to change alliances.\n\r", ch);
      return;
    }
    if (argument[0] != '\0') {
      send_to_char("Syntax: newalliance\n\r", ch);
      return;
    }
    new_alliance(FACTION_SOCIETY);
    send_to_char("Faction and society alliances recalculated.\n\r", ch);
  }
  ...
  _DOFUN(do_society) {
    ...
    if (!str_cmp(arg, "select")) {
      ...
      if (!fac || fac->type != FACTION_SOCIETY || (fac->vnum != ch->fsociety && fac->vnum != ch->legacy_society)) {
        send_to_char("Select a society you already belong to.\n\r", ch);
        return;
      }
      ch->faction = fac->vnum;
      printf_to_char(ch, "Selected %s.\n\r", fac->name);
      return;
    }
    do_function(ch, &do_clan, argument);
  }
  ...
  _DOFUN(do_clan) {
    ...
    if (ch->pcdata->ftype == FACTION_CORE) {
      ...
      ch->pcdata->ftype = FACTION_SOCIETY;
      ch->faction = selected_society(ch);
      fac = clan_lookup(ch->faction);
      ctype = str_dup("Society");
    }
    ...
    if (!str_cmp(arg, "join")) {
      ...
      if (!player_faction_join_allowed(ch, clan_lookup(clan_vnum_by_name(argument)), TRUE)) return;
...
*/
      ...
      FACTION_TYPE *joining = clan_lookup(clan_vnum_by_name(argument));
      if (joining->type != ch->pcdata->ftype || joining->closed || !can_join_faction(ch, joining->vnum, TRUE)) return;
      join_to_clan(ch, joining->vnum);
    ...
    if (!str_cmp(arg, "create")) {
      if (ch->pcdata->society_created) {
        send_to_char("Each character can only create one society ever.\n\r", ch);
        return;
      }
      if (ch->pcdata->ftype != FACTION_SOCIETY || !society_has_room(ch)) {
...
*/
      ...
      newfac->weekly_ops = 0;
      newfac->type = FACTION_SOCIETY;
      if (pact_holder(ch)) newfac->axes[AXES_CORRUPT] = AXES_FARLEFT;
      newfac->patron_relation = SOCIETY_PATRON_NONE;
    ...
    if (!str_cmp(arg, "setrank") && IS_IMMORTAL(ch)) {
      ...
      for (int i = 0; i < 100; i++) {
        if (!str_cmp(arg2, fac->member_names[i])) {
          ...
          if (get_char_world_pc(fac->member_names[i]) != NULL && get_char_world_pc(fac->member_names[i])->legacy_society == fac->vnum)
          get_char_world_pc(fac->member_names[i])->esteem_legacy_society =
          rank_requirement(atoi(arg3));
          if (get_char_world_pc(fac->member_names[i]) != NULL && get_char_world_pc(fac->member_names[i])->fsociety == fac->vnum)
          get_char_world_pc(fac->member_names[i])->esteem_society =
          rank_requirement(atoi(arg3));
    ...
    if (!str_cmp(arg, "newparent") && IS_IMMORTAL(ch)) {
      ...
      if (!str_cmp(arg2, "Cortex")) {
        fac->parent = FACTION_CORTEX;
      }
    }
    if (!str_cmp(arg, "newleader") && IS_IMMORTAL(ch)) {
      argument = one_argument_nouncap(argument, arg2);
    ...
    if (!str_cmp(arg, "newparent") && IS_IMMORTAL(ch)) {
      ...
      else if (!str_cmp(arg2, "Scum")) {
        fac->parent = FACTION_SCUM;
      }
    ...
    if (!str_cmp(arg, "list")) {
      ...
      send_to_char("`DThe Cortex\n\r`gThe Scum`x\n\rThe Order (not open to player membership)\n\n\r", ch);

      {
        send_to_char("`044  Factions and Societies`x\n\r", ch);
        const int issue = time_info.society_alliance_issue;
        for (int side : alliance_sides) {
          printf_to_char(ch, "%s Alliance\n\r", alliance_names(issue, side));
          for (FACTION_TYPE *listed : FacVect) {
            if (listed->stasis || listed->antagonist || safe_strlen(listed->name) < 3 || !alliance_eligible(listed) || faction_alliance(listed) != side)
              continue;
    ...
    if (!str_cmp(arg, "info")) {
      ...
      if (safe_strlen(argument) >= 2) {
        ...
        if (disp->antagonist == 0) {
          ...
          else if (disp->type == FACTION_SOCIETY) {
            send_to_char("`gType: `xSociety\n\r", ch);

          }


          show_power_relations(ch, disp);
          show_cortex_quota(ch, disp);
          printf_to_char(ch, "`gName`W:`x %s\n`gSymbol`W:`x %s\n`gLeader`W:`x %s\n\r", disp->name, disp->symbol, disp->leader);

          if (disp->college == 1)
          send_to_char("`gCollegiate`x\n\r", ch);

          if (is_alliance(faction_alliance(disp))) {
            printf_to_char(ch, "Part of the %s Alliance\n\r", alliance_names(time_info.society_alliance_issue, faction_alliance(disp)));
          }
          if (ch->faction != 0) {
...
*/
        ...
        if (disp->antagonist == 0) {
          ...
          for (int i = 1; i <= AXES_MAX; i++) {
            if (i == AXES_CORRUPT && disp->axes[i] == AXES_DEMONIC)
        ...
        if (IS_IMMORTAL(ch)) {
          ...
          it != FacVect.end(); ++it) {
            ...
            continue;
            if (same_strategic_alliance((*it), disp))
            apower += fac_power((*it));
          }
      ...
      show_power_relations(ch, fac);
      show_cortex_quota(ch, fac);
      ...
      send_to_char("`cRank`W:`x Leader \n\r", ch);
        ...
        if (fac->type == FACTION_SOCIETY && fac->vnum == ch->fsociety)
        {
          if (fac->member_esteem[i] != ch->esteem_society) {
            sprintf(buf, "FacRESPECT Mismatch: %s, %d, %d\n\r", ch->name, fac->member_esteem[i], ch->esteem_society);
            log_string(buf);
          }
          fac->member_esteem[i] = ch->esteem_society;
        }
        if (fac->type == FACTION_SOCIETY && fac->vnum == ch->legacy_society)
        {
          if (fac->member_esteem[i] != ch->esteem_legacy_society) {
            sprintf(buf, "FacRESPECT Mismatch: %s, %d, %d\n\r", ch->name, fac->member_esteem[i], ch->esteem_legacy_society);
            log_string(buf);
          }
          fac->member_esteem[i] = ch->esteem_legacy_society;
        }
        ...
        printf_to_char(ch, "`cRank`W:`x     %s\n\r", fac->ranks[fac->member_position[i]]);
        ...
        if (safe_strlen(disp->enemy_descs[0]) > 2 || disp->enemies[0][0] != 0) {
          ...
          for (int i = 0; i < 10; i++) {
            if (safe_strlen(disp->enemy_descs[i]) > 1 || disp->enemies[0][0] != 0) {
              ...
              it != FacVect.end(); ++it) {
                for (int j = 0; j < 10; j++) {
                  if ((*it)->enemies[j][0] == disp->vnum && (*it)->enemies[j][2] == i && !same_alliance(*it, disp)) {
          ...
          for (int i = 0; i < 10; i++) {
            if (disp->enemies[i][0] != 0) {
              ...
              if (opp != NULL && !same_alliance(opp, disp)) {
    ...
    if (!str_cmp(arg, "patron")) {
      if (fac->vnum == FACTION_CORTEX) {
        send_to_char("Cortex permanently opposes every higher power. This cannot be changed.\n\r", ch);
        return;
      }
      if (fac->type != FACTION_SOCIETY || !is_leader(ch, fac->vnum)) {
        send_to_char("Only a society leader can set its higher-power affiliation.\n\r", ch);
        return;
      }
      char relation[MSL];
      argument = one_argument_nouncap(argument, relation);
      int mode = !str_cmp(relation, "none") ? SOCIETY_PATRON_NONE
      ...
      if (mode != SOCIETY_PATRON_NONE && !power_domain(argument)) {
        send_to_char("No such higher power.\n\r", ch);
        return;
      }
      set_power_relation(fac, argument, mode);
      save_clans(FALSE);
      send_to_char("Society affiliation updated.\n\r", ch);
      return;
    }
    ...
    if (!str_cmp(arg, "becomespirit") || !str_cmp(arg, "becomeeidolon")) {
      send_to_char("Higher powers are account-owned guests. Log in to your guest through your account.\n\r", ch);
      return;
    }
    ...
    if (!str_cmp(arg, "banish")) {
      if (fac->vnum == FACTION_CORTEX && (ch->fcore != FACTION_CORTEX || is_super(ch))) {
        send_to_char("Only human Cortex members with banishing authority may banish Cortex members.\n\r", ch);
        return;
      }
      if (!has_trust(ch, TRUST_BANISHING, fac->vnum)) {
      ...
      if (!can_afford_faction_spending(fac, 0, 8000)) {
      ...
      if (!can_afford_faction_spending(fac, val / 10, 8000 + val / 10)) {
        send_to_char("You don't have enough resources for that.\n\r", ch);
        return;
      }
      ...
      else if (!str_cmp(arg2, "Corrupt") || !str_cmp(arg2, "Demonic")) {
        pos = AXES_CORRUPT;
        lean = 1;
      }
      ...
      if ((!str_cmp(arg2, "Demonic") && argument[0] == '\0')
          || (pos == AXES_CORRUPT && lean == 1 && !str_cmp(argument, "demonic"))) {
        if (fac->type != FACTION_SOCIETY) {
          send_to_char("Only societies can adopt the demonic stance.\n\r", ch);
          return;
        }
        fac->axes[pos] = AXES_DEMONIC;
      }
        ...
        printf_to_char(ch, "Syntax: %s position (issue) (stance). Stances: strong, moderate, mild, neutral; corrupt also allows demonic.\n\r", ctype);
        return;
      }
      fac->position_time = current_time;
      ...
      if (fac->vnum == FACTION_ORDER) {
        send_to_char("The Order is not open to player membership.\n\r", ch);
        return;
      }
      if (!has_trust(ch, TRUST_PROMOTIONS, fac->vnum)) {
        send_to_char("You're not trusted with that yet.\n\r", ch);
      ...
      if (!str_cmp(argument, "Cortex")) {
        fac->parent = FACTION_CORTEX;
      }
      else if (!str_cmp(argument, "Order")) {
        fac->parent = FACTION_ORDER;
      }
      else if (!str_cmp(argument, "Scum")) {
        fac->parent = FACTION_SCUM;
      }
        ...
        send_to_char("Syntax: Faction ally Cortex/Order/Scum/None\n\r", ch);
        return;
      }
    ...
    if (!str_cmp(arg, "vassal")) {
      ...
      fac->awards--;
      std::string award = haven::format_text("%s awards %s for %s.", ch->name, arg2, argument);
      send_message(fac->vnum, award.data());
      send_to_char("Done.\n\r", ch);
    }
      ...
      if (ch->faction != 0 && clan_lookup(ch->faction) != NULL) {
        ...
        for (i = 0; i < 20; i++) {
          if (safe_strlen(clan_lookup(ch->faction)->messages[i]) > 2 && clan_lookup(ch->faction)->message_timer[i] > 0)
      ...
      if (!can_afford_faction_spending(fac, 500, 10250)) {
        send_to_char("You don't have enough resources for that.\n\r", ch);
        return;
      }
      if (!can_spend_resources(fac)) {
      ...
      if (!can_afford_faction_spending(fac, 1000, 11000)) {
        send_to_char("You don't have enough resources for that.\n\r", ch);
        return;
      }
      if (!can_spend_resources(fac)) {
      ...
      if (!can_afford_faction_spending(fac, 2000, 12000)) {
        send_to_char("You don't have enough resources for that.\n\r", ch);
        return;
      }
      ...
      for (i = 0; i < 20; i++) {
        if (safe_strlen(fac->log[i]) > 2 && fac->log_timer[i] > 0)
        printf_to_char(ch, "%s\n\r", fac->log[i]);
      }
    }
      ...
      if (number == FACTION_CORTEX && ch->race == RACE_CIVIL_SERVANT) {
        send_to_char("Civil servants cannot leave the Cortex.\n\r", ch);
        return;
      }
      if (number == FACTION_CORTEX && ch->fcore == number && ch->modifier == MODIFIER_TEMPLE) {
        send_to_char("You cannot leave the Cortex while Science Augmented. They would simply turn your body off.\n\r", ch);
        return;
      }
      if (number == FACTION_CORTEX && ch->fcore == number && is_super(ch)) {
        send_to_char("Supernaturals cannot leave the Cortex. A human member with banishing authority must release you.\n\r", ch);
        return;
      }
      if (ch->fcore == number) {
      ...
      if (ch->fsociety == number) {
        sprintf(buf, "%s has left the society.", ch->name);
        send_log(ch->fsociety, buf);
        add_to_exroster(ch->name, number, TRUE);
        remove_from_clanroster(ch->name, number);
        ch->faction = 0;
        send_to_char("You have left your society.\n\r", ch);
      ...
      if (ch->legacy_society == number) {
        sprintf(buf, "%s has left the society.", ch->name);
        send_log(ch->legacy_society, buf);
        add_to_exroster(ch->name, number, TRUE);
        remove_from_clanroster(ch->name, number);
        ch->faction = 0;
        send_to_char("You have left your society.\n\r", ch);
      ...
      if (generic_faction_vnum(fac->vnum) && !IS_IMMORTAL(ch))
      return;
      int attribute = -1, setup = 0, reserve = 0;
      if (!str_cmp("sanctuary", argument)) {
        attribute = FACTION_UNDERSTANDING; setup = 100; reserve = 2500;
      } else if (!str_cmp("disposal", argument)) {
        attribute = FACTION_CORPSE; setup = 100; reserve = 2500;
      } else if (!str_cmp("scouts", argument)) {
        attribute = FACTION_SCOUTS; setup = 25; reserve = 25;
      } else if (!str_cmp("scanner", argument)) {
        attribute = FACTION_911; setup = 25; reserve = 25;
      } else if (!str_cmp("comms", argument) || !str_cmp("transmitter", argument)) {
        attribute = FACTION_COMMS; setup = 30; reserve = 30;
      }
      if (attribute < 0) {
        send_to_char("Develop sanctuary/disposal/scouts/scanner/comms.\n\r", ch);
        return;
      }
      if (fac->attributes[attribute] > 0) {
        fac->attributes[attribute] = 0;
        printf_to_char(ch, "You stop paying for %s.\n\r", argument);
        return;
      }
      setup = society_service_cost(fac, setup);
      reserve = society_service_cost(fac, reserve);
      if (!can_afford_faction_spending(fac, setup, reserve)) {
        printf_to_char(ch, "You need $%d in society resources to activate %s (setup costs $%d).\n\r",
                       (fac->type == FACTION_SOCIETY ? setup : reserve) * 10, argument, setup * 10);
        return;
      }
      fac->attributes[attribute] = 1;
      fac->resource -= setup;
      printf_to_char(ch, "You activate %s for $%d.\n\r", argument, setup * 10);
      return;
    }
    else
  ...
  _DOFUN(do_ssay) {
    ch->faction = selected_society(ch);
    if (!ch->faction) { send_to_char("You aren't in a society.\n\r", ch); return; }
    if (battleground(ch->in_room)) return;
    ch->pcdata->ftype = FACTION_SOCIETY;
    do_function(ch, &do_clansay, argument);
  }
  ...
  _DOFUN(do_clansay) {
    ...
    else if (sfac->type == FACTION_SOCIETY)
    send_to_char("You transmit on your society channel.\n\r", ch);
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if(!battleground(victim->in_room))
      {
        ...
        if (ch->pcdata->ftype == FACTION_SOCIETY) {
          if (victim->fsociety == ch->faction || victim->legacy_society == ch->faction)
            victim->faction = ch->faction;
          else victim->faction = selected_society(victim);
        }
      ...
      continue;
      if (!char_in_alliance_with(victim, ch->faction))
        continue;
      if (is_deaf(victim))
      continue;
      ...
      if (IS_AFFECTED(victim, AFF_MAD) && number_percent() % 2 == 0) {
        ...
        if (!IS_NPC(ch) && safe_strlen(rectalk) > 2) {
          if (sfac->type == FACTION_CORE) {
            ...
            buf = haven::format_text("Your faction comms say, %s, '%s'\n\r", rectalk, static_text(
            ch, victim, mangle_text(ch, victim, ch->pcdata->speaking, argument)));
          }
          else if (sfac->type == FACTION_SOCIETY) {
            printf_to_char(
            victim, "Your society comms say, %s, '%s'\n\r", rectalk, static_text(
            ch, victim, mangle_text(ch, victim, ch->pcdata->speaking, argument)));
            buf = haven::format_text("Your society comms say, %s, '%s'\n\r", rectalk, static_text(
            ch, victim, mangle_text(ch, victim, ch->pcdata->speaking, argument)));
          }
          prp_rplog(victim, buf.data());
        }
          ...
          if (sfac->type == FACTION_CORE) {
            ...
            ch, victim, mangle_text(ch, victim, ch->pcdata->speaking, argument)));
            buf = haven::format_text("Your faction comms say '%s'\n\r", static_text(
            ch, victim, mangle_text(ch, victim, ch->pcdata->speaking, argument)));
          }
          else if (sfac->type == FACTION_SOCIETY) {
            printf_to_char(
            victim, "Your society comms say '%s'\n\r", static_text(
            ch, victim, mangle_text(ch, victim, ch->pcdata->speaking, argument)));
            buf = haven::format_text("Your society comms say '%s'\n\r", static_text(
            ch, victim, mangle_text(ch, victim, ch->pcdata->speaking, argument)));
          }
          prp_rplog(victim, buf.data());
        }
  ...
  int fac_lt(int vnum) {
    /*
if(vnum == FACTION_CORTEX)
return CORTEX_LT;
else if(vnum == FACTION_ORDER)
return ORDER_LT;
else if(vnum == FACTION_SCUM)
return SCUM_LT;
*/
    return 5;
  }
...
*/
  ...
  void process_order(CHAR_DATA *ch, int type) {
    ...
    if ((type == ORDER_RESEARCH || type == ORDER_HRESEARCH) && is_number(ch->pcdata->order_target)) {
      // Orders have already paid their research fee; use the same contest as
      // direct investigation instead of granting details unconditionally.
      investigate_scheme(ch, atoi(ch->pcdata->order_target), TRUE);
      if (ch->pcdata->order_type != ORDER_HRESEARCH && ch->faction != 0 && clan_lookup(ch->faction) != NULL) {
        ch->pcdata->order_type = ORDER_COOLDOWN;
        ch->pcdata->order_timer = BASE_DURATION * minrank(type) / get_rank(ch, ch->faction);
      }
      return;
    }
...
*/
        ...
        show_containment_priority(ch, victim);
        ...
        else if (victim->race == RACE_CIVIL_SERVANT) {
          send_to_char("`cJob`x: Civil Servant.\n\r", ch);
        }
      ...
      if (type == ORDER_ATTACK) {
        ...
        for (int x = 0; x < ch->pcdata->order_amount; x++) {
          ...
          pMobIndex = get_mob_index(CORTEX_SOLDIER);
...
*/
  ...
  _DOFUN(do_hire) {
    ...
    if (!str_cmp(arg1, "investigate")) {
      ...
      const int research_cost = is_number(argument)
        ? territory_investigation_cost(ch, 20000) : 20000;
      if (ch->money < research_cost) {
        send_to_char("That'd cost $200.\n\r", ch);
        return;
      }
  ...
  _DOFUN(do_order) {
    ...
    const int order_cost = !str_cmp(arg1, "research") && is_number(argument)
      ? territory_investigation_cost(ch, 500) : 500;
    if (ch->pcdata->influence < order_cost) {
      send_to_char("You need 500 influence to do that.\n\r", ch);
      return;
    }
    if (ch->faction == 0 && !is_gm(ch) && ch->race != RACE_CIVIL_SERVANT && ch->race != RACE_FACULTY) {
    ...
    /*
if(ch->race == RACE_FACULTY || ch->race == RACE_CIVIL_SERVANT) {
...
*/
    ...
    if (ch->race == RACE_FACULTY) {
      if (str_cmp(arg1, "file")) {
        send_to_char("Civic Educators can only order files on students.\n\r", ch);
        return;
      }
    }

    if (!str_cmp(arg1, "investigate")) {
      if (get_rank(ch, ch->faction) < minrank(ORDER_INVESTIGATION) && ch->race != RACE_CIVIL_SERVANT) {
      ...
      if (ch->race != RACE_CIVIL_SERVANT && ch->race != RACE_FACULTY) {
...
*/
      ...
      if (get_rank(ch, ch->faction) < minrank(ORDER_TRACE) && ch->race != RACE_CIVIL_SERVANT) {
        send_to_char("You're not a high enough rank to do that.\n\r", ch);
        return;
      }
      ...
      if (get_rank(ch, ch->faction) < minrank(ORDER_TAIL) && ch->race != RACE_CIVIL_SERVANT) {
        send_to_char("You're not a high enough rank to do that.\n\r", ch);
        return;
      }
    ...
    ch->pcdata->influence -= order_cost;
  }
  ...
  _DOFUN(do_banish) {
    ...
    if (!can_afford_faction_spending(clan_lookup(ch->faction), 50, 9500)) {
  ...
  _DOFUN(do_unbanish) {
    ...
    if (!IS_IMMORTAL(ch)) {
      ...
      if (!can_afford_faction_spending(clan_lookup(ch->faction), 50, 8500)) {
        send_to_char("Your faction needs more resources first.\n\r", ch);
        return;
      }
      if (!can_spend_resources(clan_lookup(ch->faction))) {
  ...
  void make_soldier(int faction, int total, int position, int size, int bgnum) {
    ...
    if (faction == FACTION_CORTEX)
    vnum = CORTEX_SOLDIER;
    if (faction == FACTION_ORDER)
    vnum = ORDER_SOLDIER;
    if (faction == FACTION_SCUM)
    vnum = SCUM_SOLDIER;
    ...
    if (faction == FACTION_CORTEX) {
      ...
      free_string(mob->protecting);
      mob->protecting = str_dup("The Cortex");
    }
    else if (faction == FACTION_SCUM) {
      mob->faction = FACTION_SCUM;
      free_string(mob->protecting);
      mob->protecting = str_dup("The Scum");
    }
...
*/
  ...
  bool signed_up(CHAR_DATA *ch, OPERATION_TYPE *op, int antagcore, int type) {
    for (int i = 0; i < 100; i++) {
      if (!str_cmp(ch->name, op->sign_up[i]))
      {
        ...
        return TRUE;
        if(((antagcore != 0 && antagcore == FACTION_SOCIETY) || (type != 0 && type == FACTION_SOCIETY)) && ch->deploy_legacy_society > 0)
        return TRUE;
        if(((antagcore != 0 && antagcore == FACTION_SOCIETY) || (type != 0 && type == FACTION_SOCIETY)) && ch->deploy_society > 0)
        return TRUE;
  ...
  struct OperationSignupFactions {
    std::set<int> deployed;
    std::set<int> members;
  };
  ...
  void operation_assign_two(FACTION_TYPE *fac, OPERATION_TYPE *op, int antagcore) {
    ...
    if (op->faction != fac->vnum) {
      ...
      if (LOCATION_TYPE *location = territory_by_number(op->territoryvnum)) {
        int base = location->base_faction_core;
        if (base == fac->vnum && fac->type == FACTION_CORE)
        found = TRUE;
        base = location->base_society;
        if (base == fac->vnum && fac->type == FACTION_SOCIETY)
        found = TRUE;
        base = location->base_society_secondary;
        if (base == fac->vnum && fac->type == FACTION_SOCIETY)
        found = TRUE;
      }
  ...
  static void assign_operation_factions(OPERATION_TYPE *op, int antagcore,
                                         const OperationSignupFactions &signups) {
    // Keep the shuffled faction order and finish each assignment phase before
  ...
  bool targeted_operation_power(CHAR_DATA *ch, OPERATION_TYPE *op) {
    return ch && !IS_NPC(ch) && higher_power(ch) && op
        && power_operation_goal(op->goal) && !str_cmp(ch->name, op->target)
        && power_available(power_domain(ch->name));
  }
  ...
  bool can_deploy(CHAR_DATA *victim, OPERATION_TYPE *op, int antagcore)
  {
    ...
    if (battleground(victim->in_room))
    return FALSE;
    if (is_ghost(victim) && !IS_FLAG(victim->act, PLR_DEAD) && !targeted_operation_power(victim, op))
    return FALSE;
  ...
  bool deploy_operation_power(CHAR_DATA *victim, OPERATION_TYPE *op,
      STORYLINE_TYPE *storyline, int battle_total, int battlesize, int battleground_number) {
    if (!targeted_operation_power(victim, op) || !can_deploy(victim, op, 0)) return FALSE;
    int position = -1;
    for (int i = 0; i < 6; ++i) {
      if ((op->goal == GOAL_WORSHIP && battle_factions[i] == op->faction)
          || (op->goal == GOAL_ATTACK_POWER && battle_factions[i] == 0)) {
        position = i;
        break;
      }
    }
    if (position < 0) return FALSE;
    int tox = startx(position + 1, battle_total, battlesize) + number_range(-8, 8);
    int toy = starty(position + 1, battle_total, battlesize) + number_range(-8, 8);
    ROOM_INDEX_DATA *destination = battleroom_bycoord(battleground_number, tox, toy);
    ...
    victim->x = tox % 50;
    victim->y = toy % 50;
    victim->facing = roomdirection(victim->in_room->x, victim->in_room->y,
        first_battleroom(battleground_number)->x + 2, first_battleroom(battleground_number)->y + 2);
    free_string(victim->pcdata->title); victim->pcdata->title = str_dup("");
    free_string(victim->pcdata->place); victim->pcdata->place = str_dup("");
    victim->pcdata->week_tracker[TRACK_OPERATIONS_ATTENDED]++;
    victim->pcdata->life_tracker[TRACK_OPERATIONS_ATTENDED]++;
    if (storyline) join_to_storyline(victim, storyline);
    send_to_char("Reality twists around you.\n\r", victim);
    return TRUE;
  }
  ...
  void launch_operation(OPERATION_TYPE *op) {
    ...
    if (power_operation_goal(op->goal) && (op->power_resolved || op->competition != COMPETE_OPEN
      ...
      FACTION_TYPE *host = clan_lookup(op->faction);
      if (host) host->manpower += op->home_soldiers;
      for (int i = 0; i < 10; ++i) {
        FACTION_TYPE *enrolled = clan_lookup(op->enrolled[i]);
        if (enrolled) enrolled->manpower += op->soldiers[i];
      }
      op->hour = 0;
      op->valid = FALSE;
    ...
    antagcore = number_percent() % 2 == 0 ? FACTION_CORE : FACTION_SOCIETY;
    FACTION_TYPE *host = clan_lookup(op->faction);
    const OperationSignupFactions signups = operation_signup_factions(op);
    if (host->antagonist == 1) {
      int cores = 0, societies = 0;
      for (FACTION_TYPE *faction : FacVect) {
        if (signups.members.count(faction->vnum) == 0) continue;
        if (faction->type == FACTION_CORE) ++cores;
        if (faction->type == FACTION_SOCIETY) ++societies;
      }
      if (antagcore == FACTION_CORE && cores == 0) antagcore = FACTION_SOCIETY;
      else if (antagcore == FACTION_SOCIETY && societies == 0) antagcore = FACTION_CORE;
    } else antagcore = host->type;
    int mcorepower = -1;
    ...
    if (battle_territory >= 0) {
      ...
      if (loc != NULL && (loc->continent == CONTINENT_OTHER || loc->continent == CONTINENT_GODREALM || loc->continent == CONTINENT_WILDS || loc->continent == CONTINENT_HELL))
      offworld = TRUE;
    }
    ...
    // Targets join worshippers or defend themselves without society deployment settings.
    if (power_operation_goal(op->goal)) {
      for (DescList::iterator it = descriptor_list.begin(); it != descriptor_list.end(); ++it) {
        DESCRIPTOR_DATA *d = *it;
        if (d->connected == CON_PLAYING)
          deploy_operation_power(CH(d), op, storyline, battle_total, battlesize, battleground_number);
      }
    }
  ...
  void win_operation(int faction, OPERATION_TYPE *op) {
    ...
    if (active && fac && fac->antagonist == 1) {
      LOCATION_TYPE *site = territory_by_number(op->territoryvnum);
      int world = WORLD_EARTH;
      if (site) {
        if (site->continent == CONTINENT_WILDS) world = WORLD_WILDS;
        if (site->continent == CONTINENT_OTHER) world = WORLD_OTHER;
        if (site->continent == CONTINENT_GODREALM) world = WORLD_GODREALM;
        if (site->continent == CONTINENT_HELL) world = WORLD_HELL;
      }
      for (int i = 0; i < 6; ++i) {
        if (battle_factions[i] == FACTION_CORTEX) {
          if (world != WORLD_EARTH && number_percent() <= 25) cortex_breach(world);
          break;
        }
      }
    }
    ...
    if (op->goal == GOAL_LOOT && same_alliance(fac, clan_lookup(op->faction)))
    reward *= 2;
    ...
    if (generic_faction_vnum(fac->vnum)) {
      if (op->goal != GOAL_CONTROL && !same_alliance(fac, clan_lookup(op->faction)) && fac->antagonist == 0)
      goalthwarted = TRUE;
    }
    else {
    ...
    if (op->territoryvnum > 0 && op->competition != COMPETE_CLOSED) {
      ...
      if (loc != NULL) {
        ...
        if (op->goal == GOAL_CONTROL || goalthwarted == TRUE || controlgoal == TRUE) {
            ...
            for (int i = 0; i < base; i++) {
              if (!generic_faction_vnum(fac->vnum)) {
                if (fac->type == FACTION_NPC) {
                  ...
                  if (is_alliance(fac->alliance) && number_percent() % 10 == 0)
                    territory_minus(loc, opposing_alliance(fac->alliance));
                ...
                if (number_percent() % 10 && fac->vnum != FACTION_CORTEX)
                territory_minus(loc, FACTION_CORTEX);
                if (number_percent() % 10 && fac->vnum != FACTION_ORDER)
                territory_minus(loc, FACTION_ORDER);
                if (number_percent() % 10 && fac->vnum != FACTION_SCUM)
                territory_minus(loc, FACTION_SCUM);
              }
            }
          }
        }
          ...
          if (fac->antagonist == 0) {
            const int slot = faction_support_slot(fac);
            if (slot >= 0 && is_territory_support(slot))
              loc->phil_amount[slot] -= goal_influence(op->goal, fac->vnum, op_inf_cost(op->goal));
          }
          else {
            for (int i = 0; i < 5; i++) {
        ...
        if (goalthwarted != TRUE) {
          ...
          if (op->goal == GOAL_UPROOT) {
            ...
            if (fac->type == FACTION_SOCIETY && loc->base_society_secondary != 0 && loc->base_society_secondary != fac->vnum) {
              loc->base_society_secondary = 0;
              free_string(loc->base_society_secondary_desc);
              loc->base_society_secondary_desc = str_dup("");
            }
            else if (fac->type == FACTION_SOCIETY && loc->base_society != fac->vnum) {
              loc->base_society = 0;
              free_string(loc->base_society_desc);
              loc->base_society_desc = str_dup("");
            }
          }
          if (op->goal == GOAL_SUPPRESS) {
            for (int i = 0; i < 20; i++) {
              if (is_territory_support(i) && (fac->antagonist != 0 || i != faction_support_slot(fac)))
              loc->phil_amount[i] /= 3;
            }
            for (int i = 0; i < 5; i++)
          ...
          if (op->goal == GOAL_SABOUTAGE) {
            for (int i = 0; i < TERRITORY_SUPPORT_SLOTS; i++) {
              if (is_territory_support(i)) loc->phil_amount[i] /= 2;
            }
            for (int i = 0; i < 5; i++)
          ...
          if (op->goal == GOAL_FOOTHOLD) {
            ...
            if (fac->type == FACTION_SOCIETY && !has_base(fac, loc)) {
              if (loc->base_society == 0) loc->base_society = fac->vnum;
              else if (loc->base_society_secondary == 0) loc->base_society_secondary = fac->vnum;
            }
          }
        }
      }
    ...
    if (op->competition != COMPETE_CLOSED && active == TRUE) {
      ...
      it != FacVect.end(); ++it) {
        if ((*it)->vnum == 0)
        continue;
        if ((*it)->vnum == faction || same_alliance((*it), clan_lookup(faction)))
        continue;
...
*/
  ...
  static int faction_secrecy_decay(const FACTION_TYPE *fac, time_t now) {
    return 0; // Secrecy is retired; retain compatibility with legacy state.
  }
  ...
  void faction_daily(FACTION_TYPE *fac) {
    ...
    fac->secret_days = UMAX(0, fac->secret_days - 1);
      ...
      fac->secret_days += faction_secrecy_decay(fac, current_time);
    }
    if (fac->update > 0) {
      fac->update--;
      return;
    ...
    // All factions and societies pay the same upkeep per named roster member.
    const int upkeep_members = UMAX(1, member_count(fac));
    const int services[] = {FACTION_UNDERSTANDING, FACTION_SCOUTS,
                            FACTION_911, FACTION_COMMS, FACTION_CORPSE};
    const int rates[] = {200, 10, 20, 10, 100};
    for (int i = 0; i < 5; ++i) {
      const int service = services[i];
      if (service == FACTION_UNDERSTANDING && fac->college != 0)
        continue;
      if (fac->attributes[service] <= 0)
        continue;
      const int upkeep = rates[i] * upkeep_members;
      if (fac->resource >= cost + upkeep)
        cost += upkeep;
      else
        fac->attributes[service] = 0;
    }
    ...
    log_string(buf);
    // Territory resource rewards are handled once by territory_rewards_update().
  }
  ...
  static bool make_antag_op(FACTION_TYPE *fac, int hour, int days) {
    ...
    op->goal = GOAL_FOOTHOLD;
      ...
      it != descriptor_list.end(); ++it) {
        ...
        if (d != NULL && d->character != NULL && d->connected == CON_PLAYING) {
          ...
          if (to->faction == 0)
          continue;
          if (get_loc(to->pcdata->home_territory) == floc) {
            op->goal = GOAL_KIDNAP;
  ...
  void faction_antagonist_update(void) {
    // day counts occurrences of the operation's hour, not midnight boundaries.
    // Called after this hour's launches/decrements so <= means tomorrow.
    const int now = get_hour(NULL);
    static bool fill_startup_day = TRUE;
    bool changed = FALSE;
    std::vector<FACTION_TYPE *> hosts;
    for (FACTION_TYPE *fac : FacVect)
      if (fac->valid && fac->antagonist == 1 && fac->stasis == 0)
        hosts.push_back(fac);
    if (hosts.empty()) return;
    for (int date = fill_startup_day ? 0 : 1; date <= 7; ++date) {
      int count = 0;
      bool npc_hours[24] = {};
      for (OPERATION_TYPE *op : OpVect) {
        FACTION_TYPE *fac = clan_lookup(op->faction);
        if (op->valid && op->hour > 0 && op->hour < 24 && fac && fac->valid && fac->antagonist == 1
            && op->day + (op->hour <= now ? 1 : 0) == date && !npc_hours[op->hour]) {
          npc_hours[op->hour] = TRUE;
          ++count;
        }
      }
      const int hours[] = {12, 22, 10, 20, 14, 2, 16, 4, 18, 6,
                           9, 21, 11, 23, 13, 1, 15, 3, 19, 5, 8};
      for (int hour : hours) {
        if (count >= 2) break;
        if (date == 0 && hour <= now) continue;
        int days = date - (hour <= now ? 1 : 0);
        if (is_operation(hour, days)) continue;
        const int first = number_range(0, static_cast<int>(hosts.size()) - 1);
        for (size_t i = 0; i < hosts.size(); ++i) {
          if (make_antag_op(hosts[(first + i) % hosts.size()], hour, days)) {
            ++count;
            changed = TRUE;
            break;
          }
      ...
      if (date == 0 && (count >= 2 || now == 23)) fill_startup_day = FALSE;
      if (count < 2)
        log_string("NPC operations: cannot fill daily minimum (no free slots or eligible territories).");
    }
    if (changed) save_operations(FALSE);
  }
  ...
  _DOFUN(do_contribute) {
    ...
    if (!str_cmp(arg1, "influence")) {
      ...
      amount = amount * (5 + get_focus_power(ch, SKILL_SOCIALFOCUS)) / 5;
      ...
      amount = amount * (5 + get_focus_power(ch, SKILL_BUSINESSFOCUS)) / 5;
  ...
  bool generic_faction_vnum(int number) {
    if (number == FACTION_CORTEX)
    return TRUE;
    if (number == FACTION_ORDER)
    return TRUE;
    if (number == FACTION_SCUM)
    return TRUE;
    //    if(number == 11)
    //         return TRUE;
    //    if(number == 12)
  ...
  const char *visible_goal(int goal) {
    ...
    return goal >= 0 && goal <= GOAL_ATTACK_POWER ? goal_names[goal] : "Unknown";
  }
  ...
  _DOFUN(do_operation) {
    ...
    if (!str_cmp(arg, "list")) {
      ...
      it != OpVect.end(); ++it) {
        ...
        if (count == val) {
          if (power_operation_goal((*it)->goal)) {
            send_to_char("Higher-power operations must be won in play; they cannot be bribed.\n\r", ch);
            return;
          }
          ...
          if (has_trust(ch, TRUST_WAR, (*it)->faction) || IS_IMMORTAL(ch)) {
            ...
            cost = cost * 70 / 100;
                ...
                AXES_NEARRIGHT)
            cost = cost * 90 / 100;
            if (!can_afford_faction_spending(clan_lookup((*it)->faction), cost, 10000 - cost)) {
              send_to_char("You don't have enough resources for that.\n\r", ch);
              return;
            }
      ...
      it != OpVect.end(); ++it) {
        ...
        if (count == val) {
          ...
          if (higher_power(ch)) {
            if (!targeted_operation_power(ch, *it)) {
              send_to_char("You can only join worship or attack operations targeting you.\n\r", ch);
              return;
            }
            for (int i = 0; i < 100; ++i) {
              if (safe_strlen((*it)->sign_up[i]) == 0) {
                free_string((*it)->sign_up[i]);
  ...
  // A fresh snapshot per update: never retain counts across spawns or ticks.
  struct OperationPopulation {
    int players = 0;
    int defenders = 0;
    int attackers = 0;
    int cover = 0;
    int adversaries = 0;
    int battle_adversaries = 0;
  };

  static OperationPopulation operation_population(int number) {
    OperationPopulation result;
    for (CharList::iterator it = char_list.begin(); it != char_list.end(); ++it) {
      CHAR_DATA *victim = *it;
      if (victim == NULL)
      continue;
      ...
      if (npc && vnum == 115)
      ++result.adversaries;
      if (victim->in_room == NULL || !battleground(victim->in_room) ||
          bg_number(victim->in_room) != number || is_gm(victim))
      continue;
  ...
  void operations_update() {
    ...
    if (op->goal == GOAL_PSYCHIC) {
      if (population.defenders < 1) {
        win_operation(300000, NULL);
        return;
      }
      if (population.attackers < 1) {
        lose_operation();
        end_battle();
        return;
      }
    ...
    if (op->type == OPERATION_INTERCEPT || op->type == OPERATION_EXTRACT) {
      ...
      if (op->timer > 0 && op->timer % 3 == 0) {
        ...
        if (carrier != NULL) {
          for (int i = 0; i < 10; i++) {
            if (op->poibg[i] == battleground_number && op->poitype[i] == POI_EXTRACT &&
                (op->poifaction[i] == carrier->faction || op->poifaction[i] == 0) &&
                poidistance(carrier, op->poix[i], op->poiy[i]) <= 5) {
...
*/
  ...
  // Resolve each objective once for this viewer. Room lookups can traverse
  // the world, so callers drawing a map reuse this snapshot for every cell.
  static bool operation_poi_position(CHAR_DATA *ch, int bg, int px, int py,
                                     OperationPoiPosition &point) {
    ROOM_INDEX_DATA *room = battleroom_bycoord(bg, px, py);
    if (room == NULL || room->size <= 0)
    return FALSE;
    point.x = relative_x(ch, room, px % room->size);
    point.y = relative_y(ch, room, py % room->size);
    return TRUE;
  }

  int operation_poi_positions(CHAR_DATA *ch, OperationPoiPosition *points) {
    if (!isactiveoperation || activeoperation == NULL || ch == NULL || ch->in_room == NULL)
    return 0;
    const int bg = bg_number(ch->in_room);
    int count = 0;
    for (int i = 0; i < 10; ++i) {
      if (activeoperation->poibg[i] != bg)
      continue;
      OperationPoiPosition point;
      if (!operation_poi_position(ch, bg, activeoperation->poix[i],
                                  activeoperation->poiy[i], point))
      continue;
      point.type = activeoperation->poitype[i];
      points[count++] = point;
    }
    return count;
  }

  static int operation_poi_index(const OperationPoiPosition *points, int count,
                                 int size, int mapy, int mapx) {
    if (count == 0)
    return -1;
    const int offset = (size - 1) / 2;
    const int x = mapx / 2 - offset;
    const int y = mapy - offset;
    const int xmin = map_expand(x), xmax = map_expand(x + 1);
    const int ymin = map_expand(y), ymax = map_expand(y + 1);
  ...
  int operation_poi_type(const OperationPoiPosition *points, int count,
                         int size, int mapy, int mapx) {
    const int index = operation_poi_index(points, count, size, mapy, mapx);
    return index < 0 ? -1 : points[index].type;
  }
  ...
  int poidistance(CHAR_DATA *ch, int poix, int poiy) {
    ...
    return INT_MAX;
    OperationPoiPosition point;
    if (!operation_poi_position(ch, bg_number(ch->in_room), poix, poiy, point))
    return INT_MAX;
    return get_dist(0, 0, point.x, point.y);
  }

  void displaypois(CHAR_DATA *ch) {
    if (ch == NULL || !battleground(ch->in_room) || !isactiveoperation || activeoperation == NULL)
    return;
    const int bg = bg_number(ch->in_room);
    for (int i = 0; i < 10; ++i) {
      const int type = activeoperation->poitype[i];
      if (activeoperation->poibg[i] != bg ||
          (type != POI_EXTRACT && type != POI_CAPTURE) ||
          (activeoperation->poifaction[i] != 0 && activeoperation->poifaction[i] != ch->faction))
      continue;
      OperationPoiPosition point;
      if (!operation_poi_position(ch, bg, activeoperation->poix[i],
                                  activeoperation->poiy[i], point))
      continue;
      printf_to_char(ch, "(`Y%c`x) %s Point (X:%d Y:%d D:%d)\n\r",
                     type == POI_EXTRACT ? 'E' : 'C',
                     type == POI_EXTRACT ? "Extraction" : "Capture",
                     point.x, point.y, get_dist(0, 0, point.x, point.y));
    }
  }

  bool poidisplay(CHAR_DATA *ch, int size, int mapy, int mapx) {
    OperationPoiPosition points[10];
    const int count = operation_poi_positions(ch, points);
    return operation_poi_index(points, count, size, mapy, mapx) >= 0;
  }

  int poitype(CHAR_DATA *ch, int size, int mapy, int mapx) {
    if (!isactiveoperation || activeoperation == NULL)
    return FALSE;
    OperationPoiPosition points[10];
    const int count = operation_poi_positions(ch, points);
    return operation_poi_type(points, count, size, mapy, mapx);
  }

  bool get_poi_target(CHAR_DATA *ch, int type, int *x, int *y) {
    *x = 0;
    *y = 0;
    if (ch == NULL || !battleground(ch->in_room) || !isactiveoperation || activeoperation == NULL)
    return FALSE;
    ...
    bool found = FALSE;
    for (int i = 0; i < 10; ++i) {
      if (op->poibg[i] != bg || op->poitype[i] != type ||
          (op->poifaction[i] != 0 && op->poifaction[i] != ch->faction))
      continue;
      OperationPoiPosition point;
      if (!operation_poi_position(ch, bg, op->poix[i], op->poiy[i], point))
      continue;
      const int distance = get_dist(0, 0, point.x, point.y);
      if (distance < mindist) {
        mindist = distance;
        *x = point.x;
        *y = point.y;
    ...
    for (int i = 0; i < 3; i++) {
      if (poidistance(ch, op->poix[i], op->poiy[i]) <= 5 && op->poitype[i] == POI_CAPTURE && ch->in_room->sector_type != SECT_AIR) {
        if (op->upload_cooldown[i] <= 0) {
          if (clan_lookup(ch->faction) != NULL && clan_lookup(op->faction) != NULL && same_alliance(clan_lookup(ch->faction), clan_lookup(op->faction))) {
            op->home_uploads++;
            ...
            for (int j = 0; j < 10; j++) {
              if (clan_lookup(op->enrolled[j]) != NULL && clan_lookup(ch->faction) != NULL && same_alliance(clan_lookup(op->enrolled[j]), clan_lookup(ch->faction))) {
                op->uploads[j]++;
  ...
  char *operation_location(OPERATION_TYPE *op) {
    LOCATION_TYPE *location = op ? territory_by_number(op->territoryvnum) : NULL;
  ...
  static void resupply_item(CHAR_DATA *ch, FACTION_TYPE *fac, int vnum) {
    OBJ_INDEX_DATA *index = get_obj_index(vnum);
    if (!index) return;
    OBJ_DATA *obj = create_object(index, 0);
    const int cost = UMAX(1, obj->cost / 1000);
    if (!can_afford_faction_spending(fac, cost, 5000)) {
      printf_to_char(ch, "You need $%d in society resources for that.\n\r", cost * 10);
      extract_obj_silent(obj);
      return;
    }
    obj_to_char(obj, ch);
    use_resources(cost, fac->vnum, ch, "");
    send_to_char("Done.\n\r", ch);
  }
  ...
  bool join_to_operation(int facvnum, OPERATION_TYPE *op) {
    ...
    if (same_alliance(clan_lookup(facvnum), clan_lookup(op->faction)))
    return FALSE;

    if (LOCATION_TYPE *location = territory_by_number(op->territoryvnum)) {
      int base = location->base_faction_core;
      if (base > 0 && clan_lookup(base) != NULL && clan_lookup(base)->type == FACTION_CORE && same_alliance(clan_lookup(base), clan_lookup(facvnum)) && base != facvnum)
      return FALSE;
      base = location->base_society;
      if (base > 0 && clan_lookup(base) != NULL && clan_lookup(base)->type == FACTION_SOCIETY && same_alliance(clan_lookup(base), clan_lookup(facvnum)) && base != facvnum)
      return FALSE;
      base = location->base_society_secondary;
      if (base > 0 && clan_lookup(base) != NULL && clan_lookup(base)->type == FACTION_SOCIETY && same_alliance(clan_lookup(base), clan_lookup(facvnum)) && base != facvnum)
      return FALSE;
    ...
    for (int i = 0; i < 10; i++) {
      if (clan_lookup(op->enrolled[i]) != NULL && same_alliance(clan_lookup(op->enrolled[i]), clan_lookup(facvnum))) {
        if (op->soldiers[i] == 0) {
  ...
  _DOFUN(do_mindhack) {
    ...
    int faccost = 0;
    int cashcost = 0;
    if (has_trust(ch, TRUST_RESOURCES, ch->faction) && can_afford_faction_spending(clan_lookup(ch->faction), 10, 8000 + faccost) && can_spend_resources(clan_lookup(ch->faction)))
    faccost = 10;
    else
    cashcost = 10000;
    ...
    if (faccost > 0 && !can_afford_faction_spending(clan_lookup(ch->faction), faccost, 8000 + faccost)) {
  ...
  _DOFUN(do_goal) {
    if (ch->pcdata->ci_editing == 12) {
      ...
      FACTION_TYPE *fac = clan_lookup(ch->faction);
      char powergoal[MSL];
      char *target = one_argument_nouncap(argument, powergoal);
      if (!str_cmp(powergoal, "worship") || !str_cmp(powergoal, "attack")) {
        const int goal = !str_cmp(powergoal, "worship") ? GOAL_WORSHIP : GOAL_ATTACK_POWER;
        const char *name = target;
        if (*name) {
          const char *error = power_operation_error(ch->faction, number_from_territory(loc), goal, name);
          if (error) { printf_to_char(ch, "%s\n\r", error); return; }
        }
        ch->pcdata->ci_discipline2 = goal;
        std::string target_name(name);
        free_string(ch->pcdata->ci_message);
        ch->pcdata->ci_message = str_dup(target_name.c_str());
        send_to_char("Done. Higher-power operations must be open to competition.\n\r", ch);
        return;
      }
      const int phil = faction_support_slot(fac);
      if (phil < 0 || !is_territory_support(phil)) {
        send_to_char("Your society has no territory support.\n\r", ch);
        return;
      }
      ...
      if (!str_cmp(argument, "establish") || !str_cmp(argument, "foothold")) {
        ...
        if (loc->base_society != 0 && loc->base_society_secondary != 0 && fac->type == FACTION_SOCIETY) {
          send_to_char("Another group already has established a foothold here.\n\r", ch);
          return;
        }
      ...
      if (!str_cmp(argument, "loot")) {
        ...
        if (loc->base_faction_core != ch->faction && loc->base_society != ch->faction && loc->base_society_secondary != ch->faction) {
      ...
      send_to_char("Viable goals are: incite, calm, provoke, plant, setup, foothold, uproot, loot, suppress, kidnap, control, assassinate, rescue, raze, saboutage, worship <higher power>, attack <higher power>\n\r", ch);
      return;
    }
  }

  const char *goal_names[] = {"None",        "Incite",   "Calm",     "Provoke", "Plant",       "Setup",    "Foothold", "Raze", "Loot",        "Suppress", "Kidnap",   "Control", "Assassinate", "Rescue",   "Uproot",   "Saboutage", "Psychic", "Worship", "Attack Higher Power"};

  int phil_amount(FACTION_TYPE *fac, LOCATION_TYPE *loc) {
    if (!fac || !loc) return 0;
    const int slot = faction_support_slot(fac);
    if (slot >= 0 && is_territory_support(slot)) return loc->phil_amount[slot];
    if (fac->type == FACTION_NPC) {
      for (int i = 0; i < 5; i++) {
        if (!str_cmp(loc->other_name[i], fac->name)) {
  ...
  bool can_afford_faction_spending(const FACTION_TYPE *fac, int cost, int legacy_minimum) {
    if (!fac || cost < 0) return FALSE;
    // Player societies pay the actual price, without a protected cash reserve.
    if (fac->type == FACTION_SOCIETY)
      return cost == 0 || fac->resource >= cost;
    return fac->resource >= legacy_minimum;
  }
  ...
  bool alliance_eligible(const FACTION_TYPE *fac) {
    return fac && fac->vnum > 0 && fac->vnum != FACTION_ORDER
        && (fac->type == FACTION_CORE || fac->type == FACTION_SOCIETY)
        && !fac->stasis && !fac->nopart && !fac->antagonist && !fac->cardinal && !fac->outcast;
  }

  int faction_alliance(const FACTION_TYPE *fac) {
    if (!alliance_eligible(fac)) return ALLIANCE_NONE;
    int side = fac->type == FACTION_CORE ? fac->strategic_alliance : fac->alliance;
    return is_alliance(side) ? side : ALLIANCE_NONE;
  }

  bool same_strategic_alliance(const FACTION_TYPE *a, const FACTION_TYPE *b) {
    return faction_alliance(a) != ALLIANCE_NONE && faction_alliance(a) == faction_alliance(b);
  }

  int get_alliance(FACTION_TYPE *fac, int issue) {
    if (!fac || issue < 1 || issue > AXES_MAX)
      return ALLIANCE_NONE;
    const int position = fac->axes[issue];
    if (issue == AXES_CORRUPT && position == AXES_DEMONIC) return ALLIANCE_SIDELEFT;
    if (position < AXES_FARLEFT || position > AXES_FARRIGHT)
      return ALLIANCE_NONE;
    if (position < AXES_NEUTRAL) return ALLIANCE_SIDELEFT;
    if (position > AXES_NEUTRAL) return ALLIANCE_SIDERIGHT;
    // Neutral societies keep a valid allegiance; otherwise use a stable tie.
    return is_alliance(faction_alliance(fac)) ? faction_alliance(fac)
         : alliance_sides[static_cast<unsigned int>(fac->vnum) % ALLIANCE_COUNT];
  }

  bool same_alliance(const FACTION_TYPE *first, const FACTION_TYPE *second) {
    if (!first || !second || first->vnum == 0 || second->vnum == 0)
      return FALSE;
    if (first->vnum == second->vnum) return TRUE;
    if (first->type != second->type || first->alliance != second->alliance) return FALSE;
    // Core societies share a core faction affiliation, stored in this same
    // legacy field. These IDs are not society/society alliance sides.
    if (first->type == FACTION_CORE) return generic_faction_vnum(first->alliance);
    return (first->type == FACTION_SOCIETY)
        && is_alliance(first->alliance);
  }

  int faction_support_slot(const FACTION_TYPE *fac) {
    if (!fac) return -1;
    if (fac->type == FACTION_CORE && generic_faction_vnum(fac->alliance)) return fac->alliance;
    if (generic_faction_vnum(fac->vnum)) return fac->vnum;
    if (!is_alliance(fac->alliance)) return -1;
    if (fac->type == FACTION_SOCIETY) return fac->alliance;
    return -1;
  }

  void normalize_alliance_data() {
    if (time_info.society_alliance_issue < 1 || time_info.society_alliance_issue > AXES_MAX)
      time_info.society_alliance_issue = AXES_CORRUPT;
    // Only the retired ID needs migration. Existing allegiances and opt-outs
    // survive loading without a rebalance or resource/standing penalties.
    const int retired_alliance_id = 2;
    for (FACTION_TYPE *fac : FacVect) {
      if (!fac) continue;
      if (fac->type == FACTION_CORE) {
        if (!alliance_eligible(fac)) fac->strategic_alliance = ALLIANCE_NONE;
        else if (!is_alliance(fac->strategic_alliance)) {
          fac->strategic_alliance = get_alliance(fac, time_info.society_alliance_issue);
          if (!is_alliance(fac->strategic_alliance))
            fac->strategic_alliance = alliance_sides[static_cast<unsigned int>(fac->vnum) % ALLIANCE_COUNT];
        }
      ...
      if (fac->type != FACTION_SOCIETY) continue;
      if (fac->alliance == retired_alliance_id) {
        const int issue = time_info.society_alliance_issue;
        fac->alliance = fac->nopart || fac->cardinal || fac->antagonist || fac->outcast
                      ? ALLIANCE_NONE : get_alliance(fac, issue);
      }
    }
    for (LOCATION_TYPE *loc : locationVect) {
      if (!loc) continue;
      for (int offset : {0, 10}) {
        int &retired = loc->phil_amount[retired_alliance_id + offset];
        int &left = loc->phil_amount[ALLIANCE_SIDELEFT + offset];
        int &right = loc->phil_amount[ALLIANCE_SIDERIGHT + offset];
        // Split positive legacy support evenly, filling remaining capacity on
        // either side before discarding anything above the combined 200% cap.
        int remaining = UMAX(0, retired);
        int add = UMIN(UMAX(0, 100 - left), remaining / 2 + remaining % 2);
        left += add; remaining -= add;
        add = UMIN(UMAX(0, 100 - right), remaining);
        right += add; remaining -= add;
        left += UMIN(UMAX(0, 100 - left), remaining);
        retired = 0;
      }
      for (int side : alliance_sides) {
        loc->phil_amount[side] = UMIN(100, UMAX(0, loc->phil_amount[side]) + UMAX(0, loc->phil_amount[side + 10]));
        loc->phil_amount[side + 10] = 0;
      }
    }
  }
  ...
  static bool alliance_participant(const FACTION_TYPE *fac, int) {
    return alliance_eligible(fac);
  }

  static int alliance_balance(int ftype, int issue, bool current) {
    long long difference = 0;
    for (FACTION_TYPE *fac : FacVect) {
      if (!alliance_participant(fac, ftype)) continue;
      const int side = current ? faction_alliance(fac) : get_alliance(fac, issue);
      if (!current && !is_alliance(side)) return INT_MAX;
      if (side == ALLIANCE_SIDELEFT) difference += fac_power(fac);
      if (side == ALLIANCE_SIDERIGHT) difference -= fac_power(fac);
    }
    // Preserve the score scale used by monthly rebalance thresholds without
    // overflowing the old integer cube or evaluating three pairwise scores.
    const double distance = fabs(static_cast<double>(difference));
    return static_cast<int>(std::min(static_cast<double>(INT_MAX), distance * sqrt(distance)));
  }

  static int best_alliance_issue(int ftype) {
    int best = time_info.society_alliance_issue;
    if (best < 1 || best > AXES_MAX) best = AXES_CORRUPT;
    int score = alliance_balance(ftype, best, false);
    for (int issue = 1; issue <= AXES_MAX; ++issue) {
      const int candidate = alliance_balance(ftype, issue, false);
      if (candidate < score) { best = issue; score = candidate; }
    }
    return best;
  }

  int minalliancebalance(int ftype) {
    return alliance_balance(ftype, best_alliance_issue(ftype), false);
  }

  int currentalliancebalance(int ftype) {
    return alliance_balance(ftype, 0, true);
  }

  void new_alliance(int ftype) {
    if (ftype != FACTION_SOCIETY && ftype != FACTION_CORE) return;
    const int issue = best_alliance_issue(ftype);
    bool changed = false;
    for (FACTION_TYPE *fac : FacVect) {
      if (!alliance_participant(fac, ftype)) continue;
      const int side = get_alliance(fac, issue);
      if (!is_alliance(side) || side == faction_alliance(fac)) continue;
      changed = true;
      if (fac->type == FACTION_CORE) fac->strategic_alliance = side;
      else fac->alliance = side;
      char buf[MSL];
      snprintf(buf, sizeof(buf), "You join the %s Alliance.", alliance_names(issue, side));
      send_message(fac->vnum, buf);
      for (int &power : fac->member_power) power /= 10;
      fac->lifeearned /= 3;
    }
    time_info.society_alliance_issue = issue;
    if (!changed) return;
    for (LOCATION_TYPE *loc : locationVect) {
      if (!loc || !loc->valid) continue;
      for (int side : alliance_sides) loc->phil_amount[side] /= 2;
    }
  }
  ...
  char *alliance_names(int axes, int position) {
    return axes_names(axes, position);
  }

  bool faction_hardelligible(CHAR_DATA *ch, FACTION_TYPE *fac, bool show, CHAR_DATA *display) {
    if (!faction_pact_eligible(ch, fac, show ? display : NULL)) return FALSE;
    if (in_fight(ch) || battleground(ch->in_room))
    return TRUE;
    ...
    if (ch->race == RACE_FACULTY) {
      if (show == TRUE)
      send_to_char("Civic Educators cannot join societies.\n\r", display);
      return FALSE;
    }
  ...
  bool trust_elligible(CHAR_DATA *ch, FACTION_TYPE *fac, bool show, CHAR_DATA *display) {
    ...
    if (event_cleanse == 0) {
      ...
      if (fac->restrictions[RESTRICT_MURDERVAMPS] == 1) {
        ...
        if (get_tier(ch) >= 3 || (is_vampire(ch) && !animal_feeder(ch))) {
          if (show == TRUE)
          send_to_char("That society does not allow murderers.\n\r", display);
          return FALSE;
        }
  ...
  int alliance_position(FACTION_TYPE *fac) {
    if (!fac || !is_alliance(faction_alliance(fac))) return 0;
    long long own = 0, opponent = 0;
    for (FACTION_TYPE *other : FacVect) {
      if (!alliance_participant(other, fac->type)) continue;
      if (faction_alliance(other) == faction_alliance(fac)) own += other->lifeearned;
      else if (faction_alliance(other) == opposing_alliance(faction_alliance(fac))) opponent += other->lifeearned;
    }
    if (own == opponent) return 0; // Tied, no rank adjustment.
    return own > opponent ? 1 : 2;
  }

  int faction_secrecy(FACTION_TYPE *fac, CHAR_DATA *report) {
    // Legacy callers use 1000 as the neutral multiplier; secrecy is retired.
    return 1000;
  }
  ...
  void use_resources(int amount, int faction, CHAR_DATA *ch, char *message) {
    ...
    if (fac == NULL)
    return;
    if (ch != NULL) ch->pcdata->spent_resources += amount * 2;
    fac->resource -= amount;
    if (amount >= 5) {
      snprintf(buf, sizeof(buf), "Resource Use: %s for %s from %s. Amount: %d",
               fac->name, message, ch ? ch->name : "nobody", amount);
      log_string(buf);
    }
  ...
  void gain_resources(int amount, int faction, CHAR_DATA *ch, char *message) {
    ...
    if(safe_strlen(message) > 3 && vstart >= 25 && ch != NULL)
    {
      if(ch->fcore == faction)
      {
        if(ch->fsociety > 0)
        {
          FACTION_TYPE *society = clan_lookup(ch->fsociety);
          if(society != NULL)
          {
            gain_resources(amount*4/10, society->vnum, ch, "");
            if(highintel == TRUE && society->last_high_intel < current_time - (3600 * 24 * 5))
            {
              society->last_high_intel = current_time;
              sprintf(buf, "You gain a high intel contribution from %s", ch->name);
              send_log(society->vnum, buf);
      ...
      else if(ch->legacy_society == faction)
      {
        if(ch->fsociety > 0)
        {
          FACTION_TYPE *society = clan_lookup(ch->fsociety);
    ...
    if (ch != NULL) ch->pcdata->spent_resources += start;
    amount = UMIN(amount, start * 3 / 2);
    if (amount == 0) return;
    if (amount >= 5) {
      snprintf(buf, sizeof(buf), "Resource Gain: %s for %s from %s. Start: %d, End: %d",
               fac->name, message, ch ? ch->name : "nobody", start, amount);
      log_string(buf);
    }
    fac->resource += amount;
    if (ch != NULL) {
  ...
  void op_report(char *argument, CHAR_DATA *ch) {
    ...
    FACTION_TYPE *operation_faction = clan_lookup(activeoperation->faction);
    if (!operation_faction) return;
    std::string buf;
    buf = haven::format_text("%s's operation in %s", operation_faction->name, operation_location(activeoperation));
    ...
    it != FacVect.end(); ++it) {
      ...
      if (!str_cmp(fac->reportone_title, buf.data()) && fac->reportone_time > (current_time - (3600 * 2))) {
        std::string logs;
        ...
        if (safe_strlen(fac->reportone_text) > 25000) {
          ...
          if (point == 0 && safe_strlen(fac->report_overflow[0][0]) < 5) {
            append_report_text(fac->reportone_text, "\n\n`WSee society report battle 1 2 to continue.`x\n\r");
          }
          append_report_text(fac->report_overflow[0][point], argument, true);
          if (safe_strlen(fac->report_overflow[0][point]) >= 25000) {
            logs = haven::format_text("\n\n`WSee society report battle 1 %d to continue.`x\n\r", point + 3);
            append_report_text(fac->report_overflow[0][point], logs.c_str());
          }
        }
        else {
          append_report_text(fac->reportone_text, argument, true);
        }
      }
      else if (!str_cmp(fac->reporttwo_title, buf.data()) && fac->reporttwo_time > (current_time - (3600 * 2))) {
        std::string logs;
        ...
        if (safe_strlen(fac->reporttwo_text) > 25000) {
          ...
          if (point == 0 && safe_strlen(fac->report_overflow[1][0]) < 5) {
            append_report_text(fac->reporttwo_text, "\n\n`WSee society report battle 2 2 to continue.`x\n\r");
          }
          append_report_text(fac->report_overflow[1][point], argument, true);
          if (safe_strlen(fac->report_overflow[1][point]) >= 25000) {
            logs = haven::format_text("\n\n`WSee society report battle 2 %d to continue.`x\n\r", point + 3);
            append_report_text(fac->report_overflow[1][point], logs.c_str());
          }
        }
        else {
          append_report_text(fac->reporttwo_text, argument, true);
        }

      }
      else if (!str_cmp(fac->reportthree_title, buf.data()) && fac->reportthree_time > (current_time - (3600 * 2))) {
        std::string logs;
        if(ch != NULL && !IS_NPC(ch) && !is_gm(ch))
        {
          bool found = FALSE;
        ...
        if (safe_strlen(fac->reportthree_text) > 25000) {
          ...
          if (point == 0 && safe_strlen(fac->report_overflow[2][0]) < 5) {
            append_report_text(fac->reportthree_text, "\n\n`WSee society report battle 3 2 to continue.`x\n\r");
          }
          append_report_text(fac->report_overflow[2][point], argument, true);
          if (safe_strlen(fac->report_overflow[2][point]) >= 25000) {
            logs = haven::format_text("\n\n`WSee society report battle 3 %d to continue.`x\n\r", point + 3);
            append_report_text(fac->report_overflow[2][point], logs.c_str());
          }
        }
        else {
          append_report_text(fac->reportthree_text, argument, true);
        }
      }
      else if (activeoperation->speed >= 5 && operation_faction->antagonist == 0) {
        std::string logs;
        ...
        logs = haven::format_text("At %s, %s %s\n", datestr, terrain_name(activeoperation->terrain), territory_name(activeoperation->territoryvnum));
        free_string(fac->reportone_text);
        fac->reportone_text = str_dup(logs.data());
        fac->reportone_time = current_time;
        fac->reportone_log_sent = 0;
        fac->reportone_plog_sent = 0;
        free_string(fac->reportone_title);
        fac->reportone_title = str_dup(buf.data());

        if (activeoperation->goal != GOAL_PSYCHIC) {
          logs += haven::format_text("\n`cMission: %s`x\n\n%s\n\n%s\n%s\n\n\r", op_goal(activeoperation), activeoperation->description, activeoperation->room_name, displaysun_room(get_room_index(380000)));
          free_string(fac->reportone_text);
          fac->reportone_text = str_dup(logs.data());
        }
  ...
  void action_prep(CHAR_DATA *ch, int type, char *argument) {
    ...
    if (type == 1 && (IS_AFFECTED(ch, AFF_WAKEBOUND) || IS_AFFECTED(victim, AFF_WAKEBOUND))) {
      send_to_char("Wakebound prevents that nightmare pull.\n\r", ch);
      return;
    }
  ...
  void prep_process(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    return;
    if (ch->pcdata->prep_action == 1 && (IS_AFFECTED(ch, AFF_WAKEBOUND) || IS_AFFECTED(victim, AFF_WAKEBOUND))) {
      ch->pcdata->prep_action = 0;
      send_to_char("Wakebound prevents that nightmare pull.\n\r", ch);
      return;
    }
  ...
  void give_intel(CHAR_DATA *ch, int amount) {
    ...
    if(ch->fcore > 0 && ch->legacy_society > 0)
    amount *= 2;
    else if(ch->legacy_society > 0)
    amount = amount * 3/2;

    if(ch->legacy_society == 0 && ch->fcore == 0 && !has_symbol_society(ch))
    return;
  ...
  bool char_in_alliance_with(CHAR_DATA *ch, int cvnum) {
    ...
    return same_alliance(clan_lookup(ch->faction), fac)
        || same_alliance(clan_lookup(ch->factiontwo), fac);
  }

  bool clan_in_alliance(FACTION_TYPE *fac, char *astring) {
    if (!fac || !astring || !astring[0] || !is_alliance(faction_alliance(fac))) return FALSE;

    const int issue = time_info.society_alliance_issue;
    return !str_cmp(alliance_names(issue, faction_alliance(fac)), astring);
  }

  bool part_of_alliance(CHAR_DATA *ch, char *astring) {
    if (!ch) return FALSE;
    for (int id : {ch->fcore, ch->fsociety, ch->legacy_society, ch->faction, ch->factiontwo}) {
      if (id > 0 && clan_in_alliance(clan_lookup(id), astring)) return TRUE;
    }
    return FALSE;
  }
  ...
  bool has_base(FACTION_TYPE *fac, LOCATION_TYPE *loc) {
    ...
    return TRUE;
    if (fac->type == FACTION_SOCIETY && loc->base_society == fac->vnum)
    return TRUE;
    if (fac->type == FACTION_SOCIETY && loc->base_society_secondary == fac->vnum)
    return TRUE;
    return FALSE;
  }

  bool in_society_domain(CHAR_DATA *ch) {
    if (!ch || IS_NPC(ch) || ch->pcdata->in_domain <= 0) return FALSE;
    DOMAIN_TYPE *domain = vnum_domain(ch->pcdata->in_domain);
    if (!domain) return FALSE;
    for (int id : {ch->fsociety, ch->legacy_society}) {
      FACTION_TYPE *society = clan_lookup(id);
      if (society && society->patron_relation == SOCIETY_PATRON_WORSHIP
          && safe_strlen(society->eidilon) > 0 && !str_cmp(society->eidilon, domain->domain_of)) return TRUE;
    }
    return FALSE;
  }
  ...
  int max_core_power(OPERATION_TYPE *op, FACTION_TYPE *host)
  {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (signed_up(victim, op, antagcore, host->type)) {
        if(victim->fcore == FACTION_CORTEX)
        handp += (sqrt(get_lvl(victim))) * 10;
        if(victim->fcore == FACTION_SCUM)
  ...
  void create_ai_operative(CHAR_DATA *ch, int type)
  {
    ...
    if(type == FACTION_CORE)
    fac = clan_lookup(ch->fcore);
    if(type == FACTION_SOCIETY)
    fac = clan_lookup(selected_society(ch));
  ...
  void become_operative(CHAR_DATA *ch, int type)
  {
    ...
    if(type == FACTION_SOCIETY)
    {
      const char *operative = fac->vnum == ch->legacy_society
    ...
    if(load_char_obj(d, oname))
    {
      ...
      std::string buf;
      buf = haven::format_text("%s is revealed as $n.", olook);
  ...
  _DOFUN(do_operative)
  {
    ...
    char arg[MSL];
    argument = one_argument_nouncap(argument, arg);
    if(!str_cmp(arg, "society"))
    {
      if(selected_society(ch) <= 0)
      {
        send_to_char("You are not in a society.", ch);
        return;
      }
      become_operative(ch, FACTION_SOCIETY);
    }
    ...
    else if(!str_cmp(arg, "retire"))
    {
      ...
      if(!str_cmp(argument, "society"))
      {
        const int society = selected_society(ch);
        char *operative = society != 0 && society == ch->legacy_society
            ? ch->pcdata->operative_legacy_society : ch->pcdata->operative_society;
        if(safe_strlen(operative) < 2 || !character_exists(operative)) {
          send_to_char("You do not have a society operative.", ch);
          return;
        }
        sprintf(strsave, "%s%s", PLAYER_DIR, capitalize(operative));
        unlink(strsave);
        send_to_char("Done.\n\r", ch);
        return;
      }
      ...
      {
        send_to_char("Syntax: operative retire <society|faction>", ch);
        return;
      }
    }
    else
    {
      send_to_char("Syntax: operative <society|faction> OR operative retire <society|faction>", ch);
      return;
    }
