...
extern "C" {
  ...
  bool can_rpexp(CHAR_DATA *ch) {
    if (state_of_emergency() || !largeweapons(ch))
    return TRUE;
  ...
  void mobile_update(CHAR_DATA *ch) {
    ...
    if (full_moon_pack(ch)) {
      ...
      return; // Packs remain together; the event handles aggression and lifetime.
    }
    ...
    ch->ttl++;
      ...
      if (in_lair(ch))
      ch->ttl = 15;
      if (forest_monster(ch) && !cortex_breach_monster(ch) && mist_level(ch->in_room) == 0 && ch->ttl > 1)
      ch->ttl = 1;
      if (!in_fight(ch)) {
        ch->target = NULL;
        ch->target_dam = 0;
    ...
    if (!in_fight(ch) && (ch->pIndexData->vnum == MINION_TEMPLATE || ch->pIndexData->vnum == ALLY_TEMPLATE || ch->pIndexData->vnum == CORTEX_SOLDIER)) {
      ch->ttl = 0;
    }
    if (ch->ttl == 0) {
      extract_char(ch, TRUE);
...
#define MAX_AMBIANT 27
  ...
  int deputy_pop(ROOM_INDEX_DATA *room) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (victim->in_room != room)
      continue;
      if (victim->race == RACE_CIVIL_SERVANT)
      pop++;
    }
    return pop;
  }
  ...
  void seed_battle(FACTION_TYPE *fac) {
    ...
    if (fac->parent == FACTION_CORTEX)
    val = terr->hand;
    else if (fac->parent == FACTION_ORDER)
    val = terr->order;
    else if (fac->parent == FACTION_SCUM)
    val = terr->temple;
    else
    ...
    if (sun == 0) {
      ...
      for (int i = area->min_vnum; i < area->max_vnum; i++) {
        ...
        if ((room = get_room_index(i)) != NULL) {
          ...
          if (room->encroachment >= 1000 && !room->description_decorated) {
            reclaim_room(room);
  ...
  int secret_recover_points(CHAR_DATA *ch) {
    ...
    val /= 2;
    if (ch->race == RACE_LOCAL || ch->race == RACE_IMPORTANT || ch->race == RACE_CIVIL_SERVANT || ch->race == RACE_PILLAR || ch->race == RACE_CELEBRITY)
    val = val * 3 / 2;
  ...
  void once_per_day(CHAR_DATA *ch) {
    ...
    FACTION_TYPE * ocult = clan_lookup(ch->fsociety);
    if(ocult != NULL)
    {
      if(ocult->college == 1 && !str_cmp(ocult->leader, ch->name) && !college_student(ch, TRUE))
      ocult->college = 0;
    }
    FACTION_TYPE * osect = clan_lookup(ch->legacy_society);
    if(osect != NULL)
    {
    ...
    if (is_gm(ch))
    return;
    if (ch->race != RACE_LOCAL && ch->race != RACE_IMPORTANT && ch->race != RACE_CELEBRITY && ch->race != RACE_PILLAR && ch->race != RACE_FACULTY && ch->race != RACE_CIVIL_SERVANT && !college_student(ch, FALSE) && !clinic_patient(ch) && ch->pcdata->lastnotalone > current_time - (3600 * 24) && !IS_FLAG(ch->act, PLR_STASIS)) {
      ...
      if (fac != NULL) {
        ...
        if (fac->axes[AXES_CORRUPT] >= AXES_NEARRIGHT) {
          if (get_tier(ch) >= 3 || (is_vampire(ch) && !animal_feeder(ch))
              || (is_werewolf(ch) && ch->pcdata->lastshift > current_time - (3600 * 24 * 7 * 10)))
          ch->pcdata->secret_days += 3;
        }
    ...
    // Hunger is governed by feeding.c real-time tier deadlines.
    ...
    if (ch->legacy_society != 0 && ch->esteem_legacy_society > 0) {
      int minoffline = (60 * 12);
      int pen = UMIN(20000, minoffline);
      if (get_age(ch) < 20)
      pen = pen * 2;
      ...
      pen /= 2;
      if (!in_haven(ch->in_room)) {
        give_respect(ch, -1 * pen / 15, "Logon Flat2", ch->legacy_society);
        give_respect(ch, ch->esteem_legacy_society * -1 / 100, "Logon Percent2", ch->legacy_society);
      }
      else {
        give_respect(ch, -1 * pen / 25, "Logon Flat2", ch->legacy_society);
        give_respect(ch, ch->esteem_legacy_society * -1 / 200, "Logon Percent2", ch->legacy_society);
      }
    }

    if (ch->fsociety != 0 && ch->esteem_society > 0) {
      int minoffline = (60 * 12);
      int pen = UMIN(20000, minoffline);
      if (get_age(ch) < 20)
      pen = pen * 2;
      ...
      pen /= 2;
      if (!in_haven(ch->in_room)) {
        give_respect(ch, -1 * pen / 15, "Logon Flat2", ch->fsociety);
        give_respect(ch, ch->esteem_society * -1 / 100, "Logon Percent2", ch->fsociety);
      }
      else {
        give_respect(ch, -1 * pen / 25, "Logon Flat2", ch->fsociety);
        give_respect(ch, ch->esteem_society * -1 / 200, "Logon Percent2", ch->fsociety);
      }
    }
  ...
  void regenerate_lifeforce(CHAR_DATA *ch) {
    ...
    if (is_vampire(ch)) {
      ...
      if (ch->lf_used > 0) {
        ...
        cost = cost * 3 / 2;
        cost = UMAX(cost, 150);
        // Feeding surplus decays at half the normal recovery rate.
        cost /= 2;
      ...
      if (ch->lf_taken - mod > 0) {
        ...
        int cost = 25;
        if (ch->lf_taken < 0) {
          cost = (int)sqrt(-ch->lf_taken) * 4;
          if (alt_count(ch) == 1)
          cost = cost * 3 / 2;
          cost = UMAX(cost, 50) / 2;
          // Consume only the remaining bonus; hunger resumes next update.
          cost = UMIN(cost, -ch->lf_taken);
        }
        else {
          cost = UMIN(cost, mod - ch->lf_taken);
        }
        ch->lf_taken += cost;
        char buf[MSL];
...
*/
    ...
    for (CharList::iterator it = char_list.begin(); it != char_list.end();) {
      ...
      if(!is_helpless(ch) && !is_pinned(ch) && !dream_slave(ch))
      {
      ...
      if (time_info.minute == 23) {
        ...
        if (guestmonster(ch) && guest_remake_tier(ch) > 0)
          show_guest_progress(ch);
      }
...
*/
        if (ch->faction != 0 && get_day() >= 28) {
          int curbal = currentalliancebalance(FACTION_SOCIETY);
          int minbal = minalliancebalance(FACTION_SOCIETY);
          if (curbal > 2LL * minbal && curbal > 4000)
      ...
      if (!state_of_emergency() && ch->in_room != NULL && in_haven(ch->in_room) && !crazy_town() && !is_gm(ch) && !institute_room(ch->in_room)) {
        ...
        if (sunphase(ch->in_room) > 2 && sunphase(ch->in_room) < 7) {
          if (public_room(ch->in_room) && !IS_FLAG(ch->act, PLR_SHROUD) && guestmonster(ch)) {
            ...
            if (!enforcers && ch->pcdata->public_alarm >= 30 && !guestmonster(ch)) {
          ...
          else if (public_room(ch->in_room) && !IS_FLAG(ch->act, PLR_SHROUD) && !in_fight(ch) && visiblyarmed(ch) && ch->pcdata->public_alarm >= 0 && ch->race != RACE_CIVIL_SERVANT && !is_ghost(ch) && !is_animal(ch)) {
            bool enforcers = cortex_alarm(ch, 2);
            ...
            bool enforcers = cortex_alarm(ch, 20);
        ...
        else if (ch->pcdata->public_alarm == 0 && in_sheriff(ch->in_room) && ch->race != RACE_CIVIL_SERVANT && deputy_pop(ch->in_room) <= 0 && (in_cell(ch) || is_helpless(ch)) && !IS_FLAG(ch->act, PLR_DEEPSHROUD)) {
          act("The deputies let $n go.", ch, NULL, NULL, TO_ROOM);
          char_from_room(ch);
          char_to_room(ch, get_room_index(3518));
          act("The deputies let you go.", ch, NULL, NULL, TO_CHAR);
...
*/
  ...
  void fight_update(void) {
    haven::ScopedTimer timer("fight_update");
    CHAR_DATA *ch;

    unsigned long long cursor = 0;
    while ((ch = next_combat_character(&cursor)) != NULL) {
  ...
  bool payday(CHAR_DATA *ch) {
    ...
    const bool indentured = indentured_servant(ch);
    int amount = 0;

    if (ch->pcdata->emotes[EMOTE_PAY] < 5 && ch->pcdata->total_money + ch->money > 100000 && !indentured_servant(ch)) {
    ...
    if (indentured)
      amount *= 2; // Two full-time shifts, including for otherwise unemployed humans.
    else if (ch->race != RACE_CIVIL_SERVANT) {
      int mult = 20;
    ...
    if (ch->fsociety != 0) {
      amount += faction_pay(ch, ch->fsociety) / 2;
    }

    if (ch->legacy_society != 0) {
      amount += faction_pay(ch, ch->legacy_society) / 2;
    }
  ...
  void fight_check_update(void) {
    ...
    for (CharList::iterator it = char_list.begin(); it != char_list.end();) {
      ...
      if (ch->in_fight == FALSE) {
        set_combat_state(ch, check_fight(ch));
        if (ch->in_fight == TRUE) {
  ...
  void char_update(int time_value, int roleplay_value) {
    ...
    for (CharList::iterator it = char_list.begin(); it != char_list.end();) {
      ...
      if (!IS_NPC(ch)) {
        REMOVE_FLAG(ch->comm, COMM_DREAMSNARED);
...
*/
  ...
  void pc_update(CHAR_DATA *ch, int save_number) {
    ...
    if (ch->pcdata->account != NULL) {
      // Preserve spending so logging in cannot reset personal karma progression.
...
*/
    ...
    if (ch->pcdata->victimize_vic_timer > 0) {
      const int tick_seconds = PULSE_TICK / PULSE_PER_SECOND;
      int remaining = ch->pcdata->victimize_vic_timer - tick_seconds;
      if (ch->pcdata->victimize_vic_select == 0
          && ch->pcdata->victimize_vic_timer > 60 && remaining <= 60)
        send_to_char("One minute left to make your choice.\n\r", ch);
      if (remaining <= 0) {
        // Completion requires a live timer; clear it after processing the timeout.
        if (ch->pcdata->victimize_vic_select == 0)
        ...
        ch->pcdata->victimize_vic_timer = remaining;
      }
    }
...
*/
    ...
    if (ch->wounds > 0 && can_heal(ch) && get_skill(ch, SKILL_HYPERREGEN) > 0 && !in_fight(ch)) {
      if (ch->heal_timer > 50)
    ...
    if (ch->wounds > 0 && !in_fight(ch) && has_attendant(ch)) {
      ...
      else if (ch->wounds == 2 && can_heal(ch)) {
...
*/
      else if (ch->wounds > 3 && can_heal(ch))
      ch->wounds = 0;
    }
...
*/
  ...
  void resurrection(OBJ_DATA *corpse) {
    ...
    if (guestmonster(victim) || !IS_FLAG(victim->act, PLR_DEAD)) {
      if (!online)
      free_char(victim);
      return;
    }
    ...
    // Regeneration rebuilds the body, never returns its possessions.
    release_clinic_after_respawn(victim);
    free_string(corpse->material);
    corpse->material = str_dup("");
    corpse->timer = 60 * 24;
    victim->pcdata->recovery->death = RecoveryIncident();
    victim->pcdata->recovery->critical = RecoveryIncident();
    if (online)
...
*/
  ...
  static void save_periodic_houses() { save_houses(FALSE); }
  static void save_periodic_properties() { save_properties(FALSE); }
  struct PeriodicSave { const char *name; void (*save)(); };
  static const PeriodicSave periodic_saves[] = {
    {"save_weekly_characters", save_weekly_characters},
    {"save_stories", save_stories},
    {"save_plots", save_plots},
    {"save_storyideas", save_storyideas},
    {"save_petitions", save_petitions},
    {"save_locations", save_locations},
    {"save_gateways", save_gateways},
    {"save_news", save_news},
    {"save_research", save_research},
    {"save_eencounter", save_eencounter},
    {"save_epatrol", save_epatrol},
    {"save_anniversaries", save_anniversaries},
    {"save_decrees", save_decrees},
    {"save_grouptexts", save_grouptexts},
    {"save_texthistories", save_texthistories},
    {"save_houses", save_periodic_houses},
    {"save_events", save_events},
    {"save_properties", save_periodic_properties},
    {"save_rent", save_rent},
    {"save_blackmarket", save_blackmarket},
    {"save_chatrooms", save_chatrooms},
    {"save_profiles", save_profiles},
    {"save_matches", save_matches},
  };

  void save_periodic_data_now() {
    for (const auto &job : periodic_saves) {
      haven::ScopedTimer timer(job.name);
      job.save();
    }
  }

  static void periodic_save_update() {
    static unsigned int phase = 0;
    const unsigned int period = 60 * PULSE_PER_SECOND;
    const unsigned int count = sizeof(periodic_saves) / sizeof(periodic_saves[0]);
    // Each dataset is saved once per minute, at a fixed evenly spaced phase.
    const unsigned int index = phase * count / period;
    if (phase == (index * period + count - 1) / count) {
      haven::ScopedTimer timer(periodic_saves[index].name);
      periodic_saves[index].save();
    }
  ...
  void update_handler(void) {
    ...
    if (--pulse_second <= 0) {
      haven::ScopedTimer timer("second_updates");
    ...
    if (--pulse_area <= 0) {
      haven::ScopedTimer timer("area_update");
    ...
    if (--pulse_fight_check <= 0) {
      haven::ScopedTimer timer("fight_check");
      fight_check_update();
      pulse_fight_check = PULSE_FIGHT_CHECK;
    }

    if (--pulse_mud_hour <= 0) {
      haven::ScopedTimer timer("time_update");
    ...
    if (--pulse_obj <= 0) {
      haven::ScopedTimer timer("object_update");
      obj_update();
      pulse_obj = PULSE_OBJ;
    }

    if (--pulse_point <= 0) {
      haven::ScopedTimer timer("character_update");
  ...
  static bool can_get_wet_equipped(CHAR_DATA *ch, int iWear,
                                    const haven::EquipmentSnapshot &equipment) {
    if (!equipment.get(iWear)) return FALSE;
    if (!in_water(ch)) {
      OBJ_DATA *obj = equipment.get(WEAR_HOLD);

      if (obj != NULL && obj->item_type == ITEM_UMBRELLA && !IS_SET(obj->extra_flags, ITEM_OFF))
      return FALSE;

      obj = equipment.get(WEAR_HOLD_2);

      if (obj != NULL && obj->item_type == ITEM_UMBRELLA && !IS_SET(obj->extra_flags, ITEM_OFF))
      return FALSE;

      for (int i = 0; i < MAX_WEAR; i++) {
        if ((obj = equipment.get(i)) == NULL)
        continue;

        if (!is_waterproof(obj))
        continue;

        if (does_conceal(obj, equipment.get(iWear)))
        return FALSE;
      }
    }

    if (equipment.get(iWear)->item_type != ITEM_CLOTHING)
    return FALSE;

    if (is_waterproof(equipment.get(iWear)))
    return FALSE;
  ...
  bool can_get_wet(CHAR_DATA *ch, int iWear) {
    ...
    return can_get_wet_equipped(ch, iWear, equipment);
  }
  ...
  void wetify(CHAR_DATA *ch) {
    ...
    if (ch->shape == SHAPE_HUMAN) {
      for (iWear = 0; iWear < MAX_WEAR; iWear++) {
        if ((obj = equipment.get(iWear)) != NULL && (can_get_wet_equipped(ch, iWear, equipment))) {
...
*/
  ...
  void second_update() {
    haven::ScopedTimer timer("second_update");
    CHAR_DATA *ch;
    char buf[MSL];
  ...
  void hourly_update(int hour) {
    ...
    if (hour == 9) {
      ...
      if (get_day() == 1) {
        ...
        if (loc->base_society_secondary != 0 && clan_lookup(loc->base_society_secondary) != NULL && clan_lookup(loc->base_society_secondary)->stasis == 0)
        offline_potion(clan_lookup(loc->base_society_secondary)->leader, 87);
        if (loc->base_society != 0 && clan_lookup(loc->base_society) != NULL && clan_lookup(loc->base_society)->stasis == 0)
        offline_potion(clan_lookup(loc->base_society)->leader, 87);

        loc = get_loc("Lauriea");
        if (loc->base_faction_core != 0 && clan_lookup(loc->base_faction_core) != NULL && clan_lookup(loc->base_faction_core)->stasis == 0)
        offline_potion(clan_lookup(loc->base_faction_core)->leader, 85);
        if (loc->base_society_secondary != 0 && clan_lookup(loc->base_society_secondary) != NULL && clan_lookup(loc->base_society_secondary)->stasis == 0)
        offline_potion(clan_lookup(loc->base_society_secondary)->leader, 85);
        if (loc->base_society != 0 && clan_lookup(loc->base_society) != NULL && clan_lookup(loc->base_society)->stasis == 0)
        offline_potion(clan_lookup(loc->base_society)->leader, 85);

        loc = get_loc("Rhagost");
        if (loc->base_faction_core != 0 && clan_lookup(loc->base_faction_core) != NULL && clan_lookup(loc->base_faction_core)->stasis == 0)
        offline_potion(clan_lookup(loc->base_faction_core)->leader, 86);
        if (loc->base_society_secondary != 0 && clan_lookup(loc->base_society_secondary) != NULL && clan_lookup(loc->base_society_secondary)->stasis == 0)
        offline_potion(clan_lookup(loc->base_society_secondary)->leader, 86);
        if (loc->base_society != 0 && clan_lookup(loc->base_society) != NULL && clan_lookup(loc->base_society)->stasis == 0)
        offline_potion(clan_lookup(loc->base_society)->leader, 86);
      }
    }
    ...
    if (hour == 9) {
      ...
      log_string(buf);
      if (get_day() == 1 && curbal > 2LL * minbal && curbal > 4000)
  ...
  void minute_update(int minute) {
    ...
    ij != StoryVect.end(); ++ij) {
      ...
      if ((*ij)->time <= (current_time + (60 * 30)) && (*ij)->time > (current_time - (3600 * 2)) && (*ij)->advertised == 0 && safe_strlen((*ij)->type) > 2) {
        ...
        get_char_world_pc((*ij)->author)->pcdata->influence += territory_influence_gain(get_char_world_pc((*ij)->author), 2500);
      }
...
*/
  ...
  void update_operative(std::vector<std::string> tarray)
  {
    ...
    try {
      ...
      if(orig->pcdata->operative_creation_faction == 0)
      return;
      FACTION_TYPE *fac = clan_lookup(orig->pcdata->operative_creation_faction);
      if (!fac || (fac->vnum != orig->fcore && fac->vnum != orig->fsociety && fac->vnum != orig->legacy_society)
          || !player_faction_join_allowed(orig, fac, TRUE)) {
        orig->pcdata->operative_creation_faction = 0;
        return;
      }
      const int ftype = fac->type;
      char **operative = nullptr;
      if (ftype == FACTION_CORE) operative = &orig->pcdata->operative_core;
      else if (ftype == FACTION_SOCIETY)
        operative = fac->vnum == orig->legacy_society ? &orig->pcdata->operative_legacy_society : &orig->pcdata->operative_society;
      if (!operative) { orig->pcdata->operative_creation_faction = 0; return; }
      free_string(*operative);
      *operative = str_dup(tarray[2].c_str());
      char olook[MSL];
      ...
      CHAR_DATA *ch = orig;
      ch->pcdata->operative_creation_faction = 0;
      free_string(ch->name);
  ...
  void ai_operation_job()
  {
    ...
    for (OPERATION_TYPE *op : OpVect)
    {
      ...
      // Operations can outlive their faction or territory.
      FACTION_TYPE *fac = clan_lookup(op->faction);
      if (fac == NULL || fac->antagonist != 1 || fac->name == NULL)
        continue;

      LOCATION_TYPE *territory = territory_by_number(op->territoryvnum);
