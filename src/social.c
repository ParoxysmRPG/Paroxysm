...
*/
  ...
  static bool grouptexts_dirty = true;
  static time_t grouptexts_saved_at = 0;
  static time_t grouptexts_expire_at = 0;

  void mark_grouptexts_dirty(void) { grouptexts_dirty = true; }

  void save_grouptexts() {
    if (!grouptexts_dirty && current_time >= grouptexts_saved_at
        && (!grouptexts_expire_at || current_time < grouptexts_expire_at)
        && access(GROUPTEXT_FILE, F_OK) == 0) return;
    time_t next_expiry = 0;
    FILE *fpout;
    if ((fpout = open_snapshot_file(GROUPTEXT_FILE)) == NULL) {
    ...
    it != GTextVect.end(); ++it) {
      ...
      if ((*it)->last_msg > 0) {
        const time_t expiry = static_cast<time_t>((*it)->last_msg) + 3600 * 24 * 90 + 1;
        if (!next_expiry || expiry < next_expiry) next_expiry = expiry;
      }
    ...
    fprintf(fpout, "#END\n");
    if (close_snapshot_file(fpout) == 0) {
      grouptexts_dirty = false;
      grouptexts_saved_at = current_time;
      grouptexts_expire_at = next_expiry;
    }
  }
...
*/
  ...
  static bool texthistories_dirty = true;
  static time_t texthistories_saved_at = 0;
  static time_t texthistories_expire_at = 0;
  void mark_texthistories_dirty(void) { texthistories_dirty = true; }

  void save_texthistories() {
    if (!texthistories_dirty && current_time >= texthistories_saved_at
        && (!texthistories_expire_at || current_time < texthistories_expire_at)
        && access(TEXTHISTORY_FILE, F_OK) == 0) return;
    texthistories_dirty = true; // Retry even if a failed commit creates the file.
    time_t next_expiry = 0;

    FILE *fpout;
    if ((fpout = open_snapshot_file(TEXTHISTORY_FILE)) == NULL) {
    ...
    fprintf(fpout, "#END\n");
    if (close_snapshot_file(fpout) == 0) {
      texthistories_dirty = false;
      texthistories_saved_at = current_time;
      texthistories_expire_at = next_expiry;
    }
...
*/
  ...
  static time_t profiles_saved_at = 0;
  static time_t profiles_expire_at = 0;
  void mark_profiles_dirty(void) { profiles_dirty = true; }

  // Use on persisted-field writes; reads must not force a snapshot save.
  ...
  void mark_profile_string_dirty(char **field) {
    if (!field) return;
    for (PROFILE_TYPE *profile : ProfileVect) {
      if (field == &profile->name || field == &profile->handle ||
          field == &profile->display_handle || field == &profile->profile ||
          field == &profile->photo || field == &profile->quote ||
          field == &profile->joke || field == &profile->prefs) {
        mark_profiles_dirty();
        return;
      }
    }
  ...
  void save_profiles() {
    if (!profiles_dirty && current_time >= profiles_saved_at
        && (!profiles_expire_at || current_time < profiles_expire_at)
        && access(PROFILE_FILE, F_OK) == 0) return;
    profiles_dirty = true; // Retry even if a failed commit creates the file.
    time_t next_expiry = 0;
    FILE *fpout;
    if ((fpout = open_snapshot_file(PROFILE_FILE)) == NULL) {
    ...
    it != ProfileVect.end(); ++it) {
      ...
      continue;
      if ((*it)->last_active > 0) {
        const time_t expiry = static_cast<time_t>((*it)->last_active) + 3600 * 24 * 90 + 1;
        if (!next_expiry || expiry < next_expiry) next_expiry = expiry;
      }
    ...
    if (close_snapshot_file(fpout) == 0) {
      ...
      profiles_saved_at = current_time;
      profiles_expire_at = next_expiry;
    }
  }
...
*/
  ...
  void save_matches() {
    FILE *fpout;
    if ((fpout = open_snapshot_file(MATCH_FILE)) == NULL) {
...
*/
  ...
  void save_chatrooms() {
    FILE *fpout;
    if ((fpout = open_snapshot_file(CHATROOM_FILE)) == NULL) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (d->character != NULL && d->connected == CON_PLAYING) {
        ...
        if(safe_strlen(char_profile->display_handle) < 3)
        {
          ...
          edit_profile(char_profile)->display_handle = str_dup(char_profile->handle);
        }
  ...
  _DOFUN(do_handle)
  {
    if(ch->pcdata->ci_editing == 22)
    {
      ...
      edit_profile(profile)->handle = str_dup(buf);
      free_string(profile->display_handle);
      edit_profile(profile)->display_handle = str_dup(cbuf);
  ...
  _DOFUN(do_quote)
  {
    if(ch->pcdata->ci_editing == 22)
    {
      ...
      edit_profile(profile)->quote = str_dup(argument);
...
*/
  ...
  void profile_logon(CHAR_DATA *ch)
  {
    ...
    edit_profile(char_profile)->influencer = 1;
    else
    edit_profile(char_profile)->influencer = 0;

    edit_profile(char_profile)->last_active = current_time - (3600);
    edit_profile(char_profile)->wealth = get_skill(ch, SKILL_WEALTH);
    edit_profile(char_profile)->education = get_skill(ch, SKILL_EDUCATION);
    if(get_tier(ch) > 1 && (!str_cmp(ch->pcdata->last_name, "Inigo") || !str_cmp(ch->pcdata->last_name, "Arkwright")))
    edit_profile(char_profile)->premium = 1;
    ...
    if(rating_count > 1)
    {
      if(char_profile->rating_boost > 0)
      {
        ...
        edit_profile(char_profile)->rating = (initscore + rboost)/2;
        edit_profile(char_profile)->rating_count = rating_count + 1;
      }
      else
      {
        if(rating_auto_count > 0)
        {
          edit_profile(char_profile)->rating = (rating_score/rating_count + (rating_auto_score/rating_auto_count/2))/2;
        }
        else
        {
          edit_profile(char_profile)->rating = rating_score/rating_count;
          edit_profile(char_profile)->rating_count = rating_count;
        }
    ...
    edit_profile(char_profile)->host_rating = host_rating;
  ...
  _DOFUN(do_myhaven)
  {
    ...
    if(!str_cmp(arg1, "rateevent"))
    {
      ...
      if(IS_IMMORTAL(ch))
      {
        ...
        edit_profile(profile)->party_boost = (int)(rating*10);
        send_to_char("Done.\n\r", ch);
        return;
      }
      double rating = 0;
      ...
      it != MatchVect.end(); ++it) {
        if(!str_cmp(ch->name, (*it)->nameone))
        {
          ...
          if((*it)->rate_party_two == 1)
          {
            ...
            {
              edit_profile(profile)->party_total += (int)(rating*10);
              edit_profile(profile)->party_count++;
            }
            if((*it)->rate_party_type_two == CALENDAR_SOCIAL)
            {
              edit_profile(profile)->event_total += (int)(rating*10);
              edit_profile(profile)->event_count++;
            }
            if((*it)->rate_party_type_two == CALENDAR_PERFORMANCE)
            {
              edit_profile(profile)->perf_total += (int)(rating*10);
              edit_profile(profile)->perf_count++;
            }
            edit_profile(char_profile)->prating_total += (int)(rating*10);
            edit_profile(char_profile)->prating_count += 1;
    ...
    if(!str_cmp(arg1, "rate"))
    {
      ...
      if(IS_IMMORTAL(ch))
      {
        ...
        edit_profile(profile)->rating_boost = (int)(rating*10);
        send_to_char("Done.\n\r", ch);
        return;
      }
      ...
      if(!str_cmp(ch->name, match->nameone))
      {
        if(match->chat_count >= 50 && match->score_two_manual_chat == 0)
        {
          ...
          edit_profile(char_profile)->grating_total += (int)(rating*10);
          edit_profile(char_profile)->grating_count += 1;
    ...
    if(!str_cmp(arg1, "plus"))
    {
      ...
      {
        edit_profile(char_profile)->plus = 1;
        send_to_char("You are now a plus member.\n\r", ch);
        return;
      }
      else
      {
        edit_profile(char_profile)->plus = 0;
    ...
    if(!str_cmp(arg1, "browse"))
    {
      ...
      std::mt19937 g(rd());
      // Shuffle the vector
      mark_profiles_dirty(); // Vector order is persisted.
  ...
  _DOFUN(do_chatroom)
  {
    ...
    std::string buf;
    buf = haven::format_text("%s %s", arg1, argument);
    do_function(ch, &do_chat, buf.data());
  }
