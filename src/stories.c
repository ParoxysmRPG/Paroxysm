...
extern "C" {
  ...
  void save_stories() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(STORY_FILE)) == NULL) {
      bug("Cannot open stories.txt for writing", 0);
      return;
    }
  ...
  void save_storyideas_backup() {
    ...
    it != StoryIdeaVect.end(); ++it) {
      ...
      continue;
      if ((*it)->status == STATUS_INCOMPLETE && (*it)->type != PLOT_WORLDCHANGE)
      continue;

      if ((*it)->type != PLOT_WORLDCHANGE && (*it)->status == STATUS_PENDING && current_time > (*it)->time_locked)
      (*it)->status = STATUS_TAPPROVED;
      if ((*it)->type != PLOT_WORLDCHANGE && (*it)->status == STATUS_LOCKED && current_time > (*it)->time_locked)
  ...
  void save_storyideas() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(STORYIDEA_FILE)) == NULL) {
      bug("Cannot open storyideas.txt for writing", 0);
      return;
    }
    ...
    it != StoryIdeaVect.end(); ++it) {
      ...
      if ((*it)->type != PLOT_WORLDCHANGE && (*it)->time_made < (current_time - (3600 * 24 * 40)))
      continue;
  ...
  void save_plots() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(PLOT_FILE)) == NULL) {
      bug("Cannot open plots.txt for writing", 0);
      return;
    }
  ...
  _DOFUN(do_calendar) {
    ...
    if (!str_cmp(arg1, "list")) {
      ...
      if(!can_afford_faction_spending(cult, faccost, 8000 + faccost))
      {
  ...
  void save_research() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(RESEARCH_FILE)) == NULL) {
  ...
  void save_eencounter() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(ENCOUNTER_FILE)) == NULL) {
  ...
  void save_epatrol() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(PATROL_FILE)) == NULL) {
  ...
  void save_anniversaries() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(ANNIVERSARY_FILE)) == NULL) {
  ...
  void save_decrees() {
    FILE *fpout;
    if ((fpout = open_snapshot_file(DECREE_FILE)) == NULL) {
  ...
  void load_news() {
    ...
    if ((fp = fopen(NEWS_FILE, "r")) != NULL) {
      for (;;) {
        ...
        else if (!str_cmp(word, "SANCTUARY_POPULATION")) {
          restore_sanctuary_population(fread_number(fp) != 0);
          continue;
        }
        else if (!str_cmp(word, "END")) {
          break;
  ...
  void save_news_backup() {
    ...
    fprintf(fpout, "#SANCTUARY_POPULATION %d\n", sanctuary_population_blocked());
    fprintf(fpout, "#END\n");
    close_snapshot_file(fpout);
  }
  ...
  void save_news() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(NEWS_FILE)) == NULL) {
      bug("Cannot open news.txt for writing", 0);
      return;
    }
  ...
  bool can_see_news(CHAR_DATA *ch, NEWS_TYPE *news) {
    if (!can_see_scheme_news(ch, news))
    return FALSE;
  ...
  void scon_update(CHAR_DATA *ch) {
    for (unsigned i = 0; i < sizeof(ch->pcdata->storycon) / sizeof(ch->pcdata->storycon[0]); i++) {
...
*/
  ...
  void plot_reward(int type, char *argument, bool pvp) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (is_name(ch->name, argument)) {
        ...
        if (type == REWARD_INFLUENCE) {
          ch->pcdata->influence += territory_influence_gain(ch, basevalue);
        }
        if (type == REWARD_RESPECT) {
  ...
  char *plot_type(int type) {
    if (type == PLOT_WORLDCHANGE) return "WorldChange";
  ...
  _DOFUN(do_plot) {
    ...
    if (!str_cmp(arg1, "list")) {
      ...
      if (!str_cmp(arg10, "money")) {
        ...
        victim->pcdata->influence += territory_influence_gain(victim, amount * 5);
  ...
  // World changes use the existing persisted idea record. For this type only,
  // secrets stores the staff goal or denial reason; claimant records the reviewer.
  static const int WORLDCHANGE_DENIED = 8;
  static const int WORLDCHANGE_COMPLETE = 9;

  static const char *worldchange_status(int status) {
    if (status == STATUS_INCOMPLETE) return "Draft";
    if (status == STATUS_PENDING) return "Awaiting staff review";
    if (status == STATUS_APPROVED) return "Approved - goal assigned";
    if (status == WORLDCHANGE_DENIED) return "Denied";
    if (status == WORLDCHANGE_COMPLETE) return "Completed";
    return "Awaiting staff review";
  }

  static void worldchange_idea(CHAR_DATA *ch, char *argument) {
    if (IS_NPC(ch) || !ch->pcdata) return;
    char command[MSL], number[MSL];
    argument = one_argument_nouncap(argument, command);
    argument = one_argument_nouncap(argument, number);
    const bool staff = IS_IMMORTAL(ch);
    if (!str_cmp(command, "create")) {
      if (number[0] && str_cmp(number, "worldchange") && str_cmp(number, "world")) {
        send_to_char("Only WorldChange ideas are enabled.\n\r", ch);
        return;
      }
      int drafts = 0;
      for (STORYIDEA_TYPE *idea : StoryIdeaVect)
        if (idea->valid && idea->type == PLOT_WORLDCHANGE && !str_cmp(idea->author, ch->name)
            && (idea->status == STATUS_INCOMPLETE || idea->status == STATUS_PENDING)) ++drafts;
      if (drafts >= 3) {
        send_to_char("You already have three draft or pending world changes.\n\r", ch);
        return;
      }
      STORYIDEA_TYPE *idea = new_storyidea();
      free_string(idea->author);
      idea->author = str_dup(ch->name);
      idea->type = PLOT_WORLDCHANGE;
      idea->time_made = current_time;
      StoryIdeaVect.push_back(idea);
      save_storyideas();
      send_to_char("WorldChange draft created. Use storyidea list to find its number.\n\r", ch);
      return;
    }
    if (!str_cmp(command, "list")) {
      ...
      for (STORYIDEA_TYPE *idea : StoryIdeaVect) {
        ++index;
        if (!idea->valid || idea->type != PLOT_WORLDCHANGE || idea->status == STATUS_DELETE
            || (!staff && str_cmp(idea->author, ch->name))) continue;
        printf_to_char(ch, "%d) %s: %s\n\r", index, idea->author, worldchange_status(idea->status));
      }
      return;
    }
    if (!command[0] || !is_number(number)) {
      send_to_char("WorldChange ideas are reviewed by staff and never auto-approved.\n\r"
                   "Syntax: storyidea create [WorldChange] | list | info <number>\n\r"
                   "        storyidea description <number> <proposal> | submit <number>\n\r", ch);
      if (staff) send_to_char("Staff: storyidea approve <number> <goal> | deny <number> <reason>\n\r"
                              "       storyidea complete <number> (after implementing the approved change)\n\r", ch);
      return;
    }
    const int index = atoi(number) - 1;
    if (index < 0 || index >= (int)StoryIdeaVect.size()) {
      send_to_char("No such world change.\n\r", ch);
      return;
    }
    STORYIDEA_TYPE *idea = StoryIdeaVect[index];
    if (!idea->valid || idea->type != PLOT_WORLDCHANGE || idea->status == STATUS_DELETE
        || (!staff && str_cmp(idea->author, ch->name))) {
      send_to_char("No such world change.\n\r", ch);
      return;
    }
    if (!str_cmp(command, "info")) {
      printf_to_char(ch, "WorldChange by %s: %s\n\rProposal: %s\n\rStaff goal / decision: %s\n\rReviewer: %s\n\r",
                    idea->author, worldchange_status(idea->status), idea->description, idea->secrets, idea->claimant);
      return;
    }
    if (!str_cmp(command, "type")) {
      send_to_char("Only the WorldChange type is enabled.\n\r", ch);
      return;
    }
    if (!str_cmp(command, "description")) {
      if (idea->status != STATUS_INCOMPLETE || str_cmp(idea->author, ch->name)) {
        send_to_char("Only the author can edit a draft.\n\r", ch);
        return;
      }
      if (!argument[0]) {
        string_append(ch, &idea->description);
        return;
      }
      smash_tilde(argument);
      free_string(idea->description);
      idea->description = str_dup(argument);
    } else if (!str_cmp(command, "submit")) {
      if (idea->status != STATUS_INCOMPLETE || str_cmp(idea->author, ch->name)
          || safe_strlen(idea->description) < 10) {
        send_to_char("Submit your own draft after describing the change you want.\n\r", ch);
        return;
      }
      idea->status = STATUS_PENDING;
    } else if (!str_cmp(command, "approve") || !str_cmp(command, "deny")) {
      if (!staff) {
        send_to_char("Only staff can review world changes.\n\r", ch);
        return;
      }
      if (idea->status != STATUS_PENDING || safe_strlen(argument) < 10) {
        send_to_char("Review a pending idea and supply a clear goal or denial reason.\n\r", ch);
        return;
      }
      smash_tilde(argument);
      free_string(idea->secrets);
      idea->secrets = str_dup(argument);
      free_string(idea->claimant);
      idea->claimant = str_dup(ch->name);
      idea->status = !str_cmp(command, "approve") ? STATUS_APPROVED : WORLDCHANGE_DENIED;
    } else if (!str_cmp(command, "complete")) {
      if (!staff || idea->status != STATUS_APPROVED) {
        send_to_char("Only staff can complete an approved world change.\n\r", ch);
        return;
      }
      idea->status = WORLDCHANGE_COMPLETE;
    } else {
      send_to_char("Only WorldChange submissions are enabled. Type storyidea for syntax.\n\r", ch);
      return;
    }
    save_storyideas();
    printf_to_char(ch, "WorldChange saved: %s.\n\r", worldchange_status(idea->status));
  }

  _DOFUN(do_storyidea) {
    if (!STORY_IDEAS_ENABLED) {
      worldchange_idea(ch, argument);
      return;
    }
...
*/
  ...
  void send_survey(CHAR_DATA *ch, char *name) {
    ...
    if (ch->pcdata->survey_ratings[SURVEY_CREATE] > 0) {
      buf = haven::format_text("They think you do a good job as a roleplay creator.\n\r");
      string += buf.data();
    }
    if (ch->pcdata->survey_ratings[SURVEY_ANTAG] > 0) {
      buf = haven::format_text("They think you do a good job as an antagonist\n\r");
      string += buf.data();
    }
    if (ch->pcdata->survey_ratings[SURVEY_SIDE] > 0) {
      buf = haven::format_text("They think you do a good job as a Sidekick/Support character.\n\r");
      string += buf.data();
    }
    if (ch->pcdata->survey_ratings[SURVEY_BACK] > 0) {
      buf = haven::format_text("They think you do a good job as a background/environmental character.\n\r");
      string += buf.data();
    }
    if (ch->pcdata->survey_ratings[SURVEY_COMEDY] > 0) {
      buf = haven::format_text("They think you do a good job as a comedic relief character.\n\r");
      string += buf.data();
    }
    if (ch->pcdata->survey_ratings[SURVEY_VICTIM] > 0) {
      buf = haven::format_text("They think you do a good job as a graceful victim.\n\r");
      string += buf.data();
    }
    if (safe_strlen(ch->pcdata->survey_improve) > 3) {
      buf = haven::format_text("They had this recommendation to make on how you could improve: %s", ch->pcdata->survey_improve);
      string += buf.data();
    }
    if (safe_strlen(ch->pcdata->survey_comment) > 3) {
      buf = haven::format_text("They also had this to say: %s", ch->pcdata->survey_comment);
      string += buf.data();
    }
    if (get_char_world_pc(ch->pcdata->surveying) != NULL) {
      page_to_char(string.data(), get_char_world_pc(ch->pcdata->surveying));
    }
    else
    message_to_char(ch->pcdata->surveying, string.data());

    buf = haven::format_text("FEEDBACK: %s reviews %s: %s", ch->name, ch->pcdata->surveying, string.data());
    log_string(buf.data());
    wiznet(buf.data(), NULL, NULL, WIZ_LOGINS, 0, 0);
  }
  ...
  int autoallyscore(CHAR_DATA *ally, CHAR_DATA *target, bool super, bool faction, bool proximity) {
    ...
    if (faction == TRUE) {
      ...
      score = score - 600;
      if(clan_lookup(ally->fsociety) != NULL && clan_lookup(target->fsociety) != NULL && same_alliance(clan_lookup(ally->fsociety), clan_lookup(target->fsociety)))
      score = score + 500;
      else if(clan_lookup(ally->fsociety) != NULL && clan_lookup(target->fsociety) != NULL && !same_alliance(clan_lookup(ally->fsociety), clan_lookup(target->fsociety)))
      score = score - 250;
      if(clan_lookup(ally->legacy_society) != NULL && clan_lookup(target->legacy_society) != NULL && same_alliance(clan_lookup(ally->legacy_society), clan_lookup(target->legacy_society)))
      score = score + 500;
      else if(clan_lookup(ally->legacy_society) != NULL && clan_lookup(target->legacy_society) != NULL && !same_alliance(clan_lookup(ally->legacy_society), clan_lookup(target->legacy_society)))
      score = score - 250;
  ...
  _DOFUN(do_1)
  {
    ...
    char buf[MSL];
    if(!ENCOUNTERS_ENABLED || !is_gm(ch) || ch->pcdata->encounter_status != ENCOUNTER_SELECTING)
    {
  ...
  void have_encounter(CHAR_DATA *storyrunner, CHAR_DATA *target, int type) {
    ...
    for (vector<EXTRA_ENCOUNTER_TYPE *>::iterator it = EEncounterVect.begin();
    it != EEncounterVect.end(); ++it) {
      if((*it)->btype == 2 && threefound == FALSE && (*it)->encounter_id == type && (*it)->clan_id != target->fsociety && (number_percent() % 3 == 0 || (*it)->clan_id == target->legacy_society))
      {
  ...
  _DOFUN(do_storyline) {
    ...
    if (!str_cmp(arg1, "add") || !str_cmp(arg1, "create")) {
      ...
      if(!str_cmp(arg2, "eidilon") || !str_cmp(arg2, "eidolon"))
      {
        ...
        if(cult == NULL || !is_leader(ch, ch->fsociety))
        {
        ...
        it != locationVect.end(); ++it) {
          if ((*it)->valid == FALSE)
          continue;
          if ((*it)->base_faction_core == cult->vnum || (*it)->base_society == cult->vnum || (*it)->base_society_secondary == cult->vnum) {
            bases += 1;
          }
        }
        if(bases < 3)
          ...
          if (!str_cmp(arg2, (*it)->name) || atoi(arg2) == i) {
            if (IS_IMMORTAL(ch) || !str_cmp(ch->name, (*it)->owner) || is_name(ch->name, (*it)->coauthors)) {
              ...
              it != locationVect.end(); ++it) {
                ...
                if((*it)->base_society == cult->vnum)
                {
                  char nout[MSL];
                  sprintf(nout, "%s\n\n%s", (*it)->fixtures, (*it)->base_society_desc);
                  free_string((*it)->fixtures);
                  (*it)->fixtures = str_dup(nout);
                  free_string((*it)->base_society_desc);
                  (*it)->base_society_desc = str_dup("");
                  (*it)->base_society = 0;
                }
              }
              ...
              ij != FacVect.end(); ++ij) {
                if((*ij)->patron_relation == SOCIETY_PATRON_OPPOSE && !str_cmp((*ij)->eidilon, cult->eidilon))
                {
                  sect = *ij;
                  break;
                }
              ...
              if(sect != NULL)
              {
                ...
                it != locationVect.end(); ++it) {
                  ...
                  if((*it)->base_society_secondary == sect->vnum)
                  {
                    free_string((*it)->base_society_secondary_desc);
                    (*it)->base_society_secondary_desc = str_dup("");
                    (*it)->base_society_secondary = 0;
                  }
                }
      ...
      if(!str_cmp(arg2, "eidilon") || !str_cmp(arg2, "eidolon"))
      {
        ...
        if(sect == NULL || !is_leader(ch, ch->legacy_society))
        {
        ...
        it != locationVect.end(); ++it) {
          if ((*it)->valid == FALSE)
          continue;
          if ((*it)->base_faction_core == sect->vnum || (*it)->base_society == sect->vnum || (*it)->base_society_secondary == sect->vnum) {
            bases += 1;
          }
        }
        if(bases < 3)
          ...
          if (!str_cmp(arg2, (*it)->name) || atoi(arg2) == i) {
            if (IS_IMMORTAL(ch) || !str_cmp(ch->name, (*it)->owner) || is_name(ch->name, (*it)->coauthors)) {
              ...
              it != locationVect.end(); ++it) {
                ...
                {
                  char nout[MSL];
                  sprintf(nout, "%s\n\n%s", (*it)->fixtures, (*it)->base_society_secondary_desc);
                  free_string((*it)->fixtures);
                  (*it)->fixtures = str_dup(nout);
                  free_string((*it)->base_society_secondary_desc);
                  (*it)->base_society_secondary_desc = str_dup("");
              ...
              for (vector<FACTION_TYPE *>::iterator ij = FacVect.begin();
              ij != FacVect.end(); ++ij) {
                if((*ij)->patron_relation == SOCIETY_PATRON_WORSHIP && !str_cmp((*ij)->eidilon, sect->eidilon))
                {
                  cult = *ij;
                  break;
                }
...
*/
  ...
  _DOFUN(do_gift)
  {
    ...
    CHAR_DATA *victim = get_char_world_pc(argument);
    if(victim == NULL || (victim->fsociety != ch->pcdata->eidilon_of && victim->legacy_society != ch->pcdata->eidilon_of))
    {
    ...
    for (obj = ch->carrying; obj != NULL; obj = obj_next) {
      ...
      if (is_name(arg1, obj->name) && can_see_obj(ch, obj) && obj->wear_loc == WEAR_NONE && can_drop_obj(ch, obj)) {
        ...
        if(cult->patron_relation == SOCIETY_PATRON_WORSHIP && cult->axes[AXES_CORRUPT] >= AXES_NEUTRAL)
        faccost = faccost * 3/2;
        if(cult->patron_relation == SOCIETY_PATRON_OPPOSE)
        faccost = faccost * 6/5;
        ...
        if(cult->patron_relation == SOCIETY_PATRON_OPPOSE && (sline == NULL || sline->power < 5))
        {
...
*/
  ...
  void villain_mod(CHAR_DATA *ch, int amount, char *reason) {
    ...
    if (get_tier(ch) >= 3) {
      ...
      if (clan_lookup(ch->legacy_society) != NULL && (good_person(ch) && clan_lookup(ch->legacy_society)->axes[AXES_CORRUPT] >= AXES_NEARRIGHT) && (number_percent() % 2 == 0 || ch->skills[SKILL_MENTALDISCIPLINE] == 0))
      ...
      else if (clan_lookup(ch->fsociety) != NULL && (good_person(ch) && clan_lookup(ch->fsociety)->axes[AXES_CORRUPT] >= AXES_NEARRIGHT) && (number_percent() % 2 == 0 || ch->skills[SKILL_MENTALDISCIPLINE] == 0))
      ch->pcdata->heroic += amount/2;

    }

    if (clan_lookup(ch->fsociety) != NULL) {
      if (str_cmp(ch->name, clan_lookup(ch->fsociety)->leader)) {
        if (get_char_world_pc(clan_lookup(ch->fsociety)->leader) != NULL)
        villain_mod(get_char_world_pc(clan_lookup(ch->fsociety)->leader), amount / 4, "Leadership");
      }
    }
...
#define DESTINY_FILE "../data/destiny.txt"
  ...
  _DOFUN(do_roleonearchetype) {
    if (ch->pcdata->ci_editing == 16) {
      ...
      ch->pcdata->ci_zips = 7;
      else if (!str_cmp(argument, "staff") || !str_cmp(argument, "institute staff") || !str_cmp(argument, "civic educator") || !str_cmp(argument, "civic educators"))
      ch->pcdata->ci_zips = 8;
  ...
  _DOFUN(do_scene01location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[0];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      const std::string out = std::to_string(point) + " " + std::string(argtwo) + " " + std::string(argthree) + " " + std::string(argfour);
      free_string(ch->pcdata->ci_excludes[0]);
      ch->pcdata->ci_excludes[0] = str_dup(out.c_str());
  ...
  _DOFUN(do_scene01specialroleone) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      char argfour[MSL];
      argument = one_argument_nouncap(argument, argfour);
      const std::string out = std::string(arg) + " " + std::to_string(point) + " " + std::string(argthree) + " " + std::string(argfour);
      free_string(ch->pcdata->ci_excludes[0]);
      ch->pcdata->ci_excludes[0] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
  ...
  _DOFUN(do_scene01specialroletwo) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      char argfour[MSL];
      argument = one_argument_nouncap(argument, argfour);
      const std::string out = std::string(arg) + " " + std::string(argtwo) + " " + std::to_string(point) + " " + std::string(argfour);
      free_string(ch->pcdata->ci_excludes[0]);
      ch->pcdata->ci_excludes[0] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
  ...
  _DOFUN(do_scene01conclusion) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      char argfour[MSL];
      argument = one_argument_nouncap(argument, argfour);
      const std::string out = std::string(arg) + " " + std::string(argtwo) + " " + std::string(argthree) + " " + std::to_string(point);
      free_string(ch->pcdata->ci_excludes[0]);
      ch->pcdata->ci_excludes[0] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
  ...
  _DOFUN(do_scene02location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[1];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[1] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene03location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[2];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[2] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene04location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[3];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[3] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene05location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[4];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[4] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene06location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[5];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[5] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene07location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[6];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[6] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene08location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[7];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[7] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene09location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[8];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[8] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene10location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[9];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[9] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene11location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[10];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[10] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene12location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[11];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[11] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene13location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[12];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[12] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene14location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[13];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[13] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene15location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[14];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[14] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene16location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[15];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[15] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene17location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[16];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[16] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene18location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[17];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
      ...
      ch->pcdata->ci_excludes[17] = str_dup(out.c_str());
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
      ch->pcdata->ci_editing = 16;
    }
  ...
  _DOFUN(do_scene19location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[18];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
  ...
  _DOFUN(do_scene20location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      std::string stored = ch->pcdata->ci_excludes[19];
      char *argument = stored.data();
      char arg[MSL];
      argument = one_argument_nouncap(argument, arg);
      char argtwo[MSL];
  ...
  bool are_allies(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (ch->fsociety > 0) {
      if (ch->fsociety == victim->fsociety)
      return TRUE;
      FACTION_TYPE *cclan = clan_lookup(ch->fsociety);
      FACTION_TYPE *vclan = clan_lookup(victim->fsociety);
      if (cclan != NULL && vclan != NULL && same_alliance(cclan, vclan))
      return TRUE;
    }
    if (ch->legacy_society > 0) {
      if (ch->legacy_society == victim->legacy_society)
      return TRUE;
      FACTION_TYPE *cclan = clan_lookup(ch->legacy_society);
      FACTION_TYPE *vclan = clan_lookup(victim->legacy_society);
      if (cclan != NULL && vclan != NULL && same_alliance(cclan, vclan))
      return TRUE;
    }
  ...
  void launch_scene(CHAR_DATA *ch, CHAR_DATA *victim, NEWDESTINY_TYPE *dest, int inum) {
    ...
    if (!troom || !can_destiny_scene(ch, troom) || !can_destiny_scene(victim, troom)) {
...
*/
  ...
  int doom_days(CHAR_DATA *ch) {
    ...
    if (ch->race == RACE_LOCAL || ch->race == RACE_IMPORTANT || ch->race == RACE_CIVIL_SERVANT || ch->race == RACE_FACULTY)
    sdays = sdays * 3 / 2;
