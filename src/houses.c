...
#define SECURITY_PRICE 10
  ...
  // First half: residents; second half: their roommates.
...
*/
  ...
  void save_houses(bool backup) {
    ...
    if (backup) {
      ...
      if ((fpout = open_snapshot_file("../data/houses.txt")) == NULL) {
        bug("Cannot open house.txt for writing", 0);
        return;
      }
    }
  ...
  void save_domains(bool backup) {
    ...
    it != DomainVect.end(); ++it) {
      ...
      if((*it)->claimed_territory == 0 && !eidilon_fac_exists((*it)->domain_of))
      continue;
      ...
      fprintf(fpout, "GuestTerritory %d\nGuestWorships %d\nGuestBanished %ld\nGuestRetired %d\n", (*it)->claimed_territory, (*it)->successful_worships, (*it)->banished_until, (*it)->guest_retired);
  ...
  void fread_domain(FILE *fp) {
    ...
    for (;;) {
      ...
      case 'E':
        ...
        break;
      case 'G':
        KEY("GuestTerritory", domain->claimed_territory, fread_number(fp));
        KEY("GuestWorships", domain->successful_worships, fread_number(fp));
        KEY("GuestBanished", domain->banished_until, fread_number(fp));
        KEY("GuestRetired", domain->guest_retired, fread_number(fp));
        break;
      case 'M':
...
*/
  ...
  void save_properties(bool backup) {
    ...
    if (backup) {
      ...
      if ((fpout = open_snapshot_file("../data/properties.txt")) == NULL) {
        bug("Cannot open properties.txt for writing", 0);
        return;
      }
    }
...
*/
  ...
  // regardless of type - Discordance
  {
    ...
    buf = haven::format_text("`WHoldings`x\n\r");
    buf = "`g_";
    ...
    if (!str_cmp(arg1, "shops") || !str_cmp(arg1, "shop")) {
      ...
      it != PropVect.end(); ++it) {
        if (!str_cmp((*it)->owner, ch->name)) {
          ...
          buf = haven::format_text("`g[`W%3d`g]`x %-s\n\r", (*it)->vnum, (*it)->propname);
          page += buf.data();
          buf = haven::format_text("      ");
          page += buf.data();
          buf = haven::format_text("`gAddress`x: %-s", (*it)->address);
          page += buf.data();
          spacing = 48 - safe_strlen_color(buf.data());
          buf = haven::format_text("%*s", spacing, spacer);
          page += buf.data();
          buf = haven::format_text("`gProperty Type`x: Commercial`x\n\r");
          page += buf.data();

          buf = haven::format_text("     ");
          page += buf.data();
          buf = haven::format_text("`gDue Date`x: %s %d%s", month, (*it)->auction_day, datesuffix((*it)->auction_day));
          page += buf.data();
          spacing = 52 - safe_strlen_color(buf.data());
          buf = haven::format_text("%*s", spacing, spacer);
          page += buf.data();
          buf = haven::format_text("`gAmount Due`x: `G$`x%-4d\n\r\n\r", (*it)->price);
          page += buf.data();
        }
      }
    }
    // Lists only houses
      ...
      it != HouseVect.end(); ++it) {
        if (!str_cmp((*it)->owner, ch->name)) {
          ...
          buf = haven::format_text("%*s", spacing, spacer);
          page += buf.data();
          buf = haven::format_text("`gProperty Type`x: Residence\n\r");
          page += buf.data();
      ...
      it != PropVect.end(); ++it) {
        if (!str_cmp((*it)->owner, ch->name)) {
          ...
          if ((*it)->type == PROP_HOUSE) {
            ...
            buf = haven::format_text("%*s", spacing, spacer);
            page += buf.data();
            buf = haven::format_text("`gProperty Type`x: Commercial\n\r");
            page += buf.data();
          }
  ...
  _DOFUN(do_house) {
    ...
    if (!str_cmp(arg1, "list")) {
      ...
      page = haven::format_text("`WHouse List`x\n\r");
      buf = "`g_";
      frame = draw_horizontal_line(ch->linewidth - 2);
      buf += frame;
      buf += "_`x\n\r";
      ...
      if (!str_cmp(arg2, "township") || safe_strlen(arg2) < 2 || !str_cmp(arg2, "town") || !str_cmp(arg2, "gravesend")) {
        ...
        it != PropVect.end(); ++it) {
          ...
          strcpy(month, month_string((*it)->auction_month));
          if ((*it)->sale_price > 0)
          buf = haven::format_text("`g[`W%3d`g]`x %-s`x", (*it)->vnum, (*it)->address);
          else
          buf = haven::format_text("`g[`W%3d`g]`x %-s`x", (*it)->vnum, (*it)->address);
          page += buf.data();
          ...
          buf = haven::format_text("%*s", spacing, spacer);
          page += buf.data();
          buf = haven::format_text("`gStatus`x: ");
          page += buf.data();
          if ((*it)->sale_price > 0)
          buf = haven::format_text("For Sale\n\r");
          else
          buf = haven::format_text("Owned\n\r");
          page += buf.data();
        }
      }
    ...
    // Lists houses owned by the player - Discordance
      ...
      page = haven::format_text("`WHouse Owned`x\n\r");
      buf = "`g_";
      frame = draw_horizontal_line(ch->linewidth - 2);
      buf += frame;
      buf += "_`x\n\r";
      ...
      it != PropVect.end(); ++it) {
        if (!str_cmp((*it)->owner, ch->name)) {
          ...
          buf = haven::format_text("%*s", spacing, spacer);
          page += buf.data();
          buf = haven::format_text("`gPayment:`x ");
          page += buf.data();
          month_string_length = safe_strlen(month);
          if (month_string_length < longest_month_string) {
      ...
      std::string buf;
      buf = haven::format_text("%s %s", arg2, argument);
      if (in_prop(ch) != NULL) {
        PROP_TYPE *nprop = in_prop(ch);
        free_string(nprop->propname);
        nprop->propname = str_dup(buf.data());
        send_to_char("Done.\n\r", ch);
      }
    }
...
*/
  ...
  _DOFUN(do_shop) {
    ...
    if (!str_cmp(arg1, "list")) {
      ...
      page = haven::format_text("`WShop List`x\n\r");
      buf = "`g_";
      frame = draw_horizontal_line(ch->linewidth - 2);
      buf += frame;
      buf += "_`x\n\r";
      ...
      if (!str_cmp(arg2, "township") || safe_strlen(arg2) < 2 || !str_cmp(arg2, "town") || !str_cmp(arg2, "gravesend")) {
        ...
        it != PropVect.end(); ++it) {
          ...
          buf = haven::format_text("`g[`W%3d`g]`x %-s`x", (*it)->vnum, (*it)->propname);
          page += buf.data();
          buf = haven::format_text("%*s", spacing, spacer);
          page += buf.data();
          buf = haven::format_text("`gStatus`x: ");
    ...
    // Lists shops owned by the player - Discordance
      ...
      page = haven::format_text("`WShop Owned`x\n\r");
      buf = "`g_";
      frame = draw_horizontal_line(ch->linewidth - 2);
      buf += frame;
      buf += "_`x\n\r";
    ...
    // - Discordance
      ...
      if(prop->type_special != PROPERTY_INNERFOREST && prop->type_special != PROPERTY_OUTERFOREST)
      {
        if(owned_town_shop_count(ch) > get_focus_power(ch, SKILL_BUSINESSFOCUS))
        {
          printf_to_char(ch, "You can only own %d commercial properties in town.\n\r", get_focus_power(ch, SKILL_BUSINESSFOCUS)+1);
          return;
        }
      }
      house_charge(prop->owner, UMAX(prop->price / 10, prop->sale_price) * -1);
      ...
      std::string buf;
      buf = haven::format_text("%s %s", arg2, argument);
      if(strlen(from_color(buf.data())) > 48)
      {
      ...
      free_string(prop->propname);
      prop->propname = str_dup(buf.data());
      send_to_char("Done.\n\r", ch);
    }
      ...
      if (!str_cmp(arg2, "create")) {
        ...
        if (get_obj_index(vnum) == NULL) {
          ...
          buf = haven::format_text("<held>");
          temp = haven::format_text("%-21.21s", buf.data());

          free_string(pObj->wear_string);
          pObj->wear_string = str_dup(temp.data());
...
*/
    ...
    if (prop->type == PROP_SHOP) {
      ...
      int skill = 0 + focus_power_level(SKILL_BUSINESSFOCUS, offline_skill(prop->owner, SKILL_BUSINESSFOCUS));
      skill *= 1000;
      total = UMAX(total, 10);
  ...
  bool has_access(CHAR_DATA *ch, ROOM_INDEX_DATA *room) {
    ...
    PROP_TYPE *property = room_prop(room);
    if (property && property->type_special == PROPERTY_TRAILER
        && has_territory_bonus(ch, TERRITORY_KNOCK_KNOCK)) return TRUE;
  ...
  _DOFUN(do_decorate) {
    ...
    if (!str_cmp(arg1, "description")) {
      if (can_decorate(ch, room)) {
        room->description_decorated = TRUE;
...
*/
  ...
  char *create_address(ROOM_INDEX_DATA *start, int dir) {
    ...
    newbuf = haven::format_text("%d %s", num, buf);

    return str_dup(newbuf.data());
  }
  ...
  void build_property(int dir, int type, int size, ROOM_INDEX_DATA *start, char *owner, char *propname) {
    ROOM_INDEX_DATA *airmid = NULL;
    ROOM_INDEX_DATA *aireast = NULL;
    ROOM_INDEX_DATA *airwest = NULL;
    ROOM_INDEX_DATA *tunnel = NULL;
    ROOM_INDEX_DATA *room;
    ...
    if (size != PROP_SMALL && size != PROP_MEDIUM && size != PROP_LARGE
        && size != PROP_TALL && size != PROP_XLARGE) return;
    int end, begin;
...
void new_build_property(int dir, int size, ROOM_INDEX_DATA *starting_room, char * owner) { ROOM_INDEX_DATA *airmid = NULL; ROOM_INDEX_DATA *aireast = NULL;
ROOM_INDEX_DATA *airwest = NULL;
ROOM_INDEX_DATA *tunnel = NULL;
ROOM_INDEX_DATA *room;
...
*/
  ...
  void buynewland(CHAR_DATA *ch, char *argument) {
    ...
    if (dir != DIR_NORTH && dir != DIR_SOUTH && dir != DIR_EAST && dir != DIR_WEST) return;
...
*/
    ...
    for (;;) {
      ...
      case 'D':
        if (!str_cmp(word, "DormRoom")) {
          ...
          if (i < 0 || i >= 2 * MAX_DORMROOMS) {
            bug("Fread_dorms: invalid room index %d", i);
  ...
  void save_rent() {
    ...
    if ((fpout = open_snapshot_file(RENT_FILE)) == NULL) {
  ...
  _DOFUN(do_address) {
    ...
    if (found == FALSE) {
      ...
      buf = haven::format_text("It's addressed:\n%s\n%s\n", capitalize(arg2), arg3);
      free_string(parcel->description);
      parcel->description = str_dup(buf.data());
    }
  }
...
*/
  ...
  _DOFUN(do_lock) {
    ...
    if ((victim = get_char_room(ch, NULL, arg)) == NULL) {
      ...
      if (in_sheriff(ch->in_room) && ch->race != RACE_CIVIL_SERVANT) {
