...
extern "C" {
  ...
  void free_pcdata(PC_DATA *pcdata) {
    ...
    free_string(pcdata->operative_legacy_society);
    free_string(pcdata->operative_society);
  ...
  ACCOUNT_TYPE *new_account(void) {
    ...
    *account = account_zero;
    ...
    account->donated = 0;
    account->colours = 0;
    time_t recolor_now = current_time;
    struct tm *recolor_utc = gmtime(&recolor_now);
    account->recolor_month = recolor_utc == NULL ? 0
      : (recolor_utc->tm_year + 1900) * 12 + recolor_utc->tm_mon - 1;
    account->bandaids = 0;
    account->renames = 0;
  ...
  LOCATION_TYPE *new_location(void) {
    ...
    *location = location_zero;
    ...
    location->reward_controller = -1;
    for (int &owner : location->reward_footholds) owner = -1;
    ...
    location->base_society_desc = str_dup("");
    location->base_society = 0;
    location->base_society_secondary_desc = str_dup("");
    location->base_society_secondary = 0;
    VALIDATE(location);
...
*/
  ...
  GROUPTEXT_TYPE *new_grouptext(void) {
    ...
    *grouptext = grouptext_zero;
    ...
    for (unsigned i = 0; i < sizeof(grouptext->pnumber) / sizeof(grouptext->pnumber[0]); i++) {
...
*/
  ...
  EVENT_TYPE *new_event(void) {
    ...
    *event = event_zero;
    ...
    event->limited = 0;
    event->professional_focus = -1;
    event->socialite_bonus = -1;
    event->launch_cost = 0;
  ...
  DOMAIN_TYPE *new_domain(void) {
    ...
    *domain = domain_zero;
    ...
    domain->archetype = 0;
    domain->power = 0;
    domain->claimed_territory = 0;
    domain->successful_worships = 0;
    domain->banished_until = 0;
    domain->guest_retired = false;
    domain->exp = 0;
    for (i = 0; i < 250; i++) {
      domain->roomlist[i] = 0;
  ...
  CLAN_TYPE *new_clan(void) {
    ...
    *clan = clan_zero;
    ...
    for (unsigned i = 0; i < sizeof(clan->division_leaders) / sizeof(clan->division_leaders[0]); i++) {
  ...
  void free_clan(CLAN_TYPE *clan) {
    ...
    for (unsigned i = 0; i < sizeof(clan->division_leaders) / sizeof(clan->division_leaders[0]); i++)
      free_string(clan->division_leaders[i]);
  ...
  FACTION_TYPE *new_faction(void) {
    ...
    *fac = fac_zero;
    ...
    fac->power_worships = str_dup("");
    fac->power_opposes = str_dup("");
    ...
    fac->outcast = 0;
    fac->alliance = 0;
    fac->patron_relation = -1; // Old saves derive the relationship from their type.
    fac->staff = 0;
    fac->nopart = 0;
    ...
    fac->battleday = 0;
    fac->parent = 0;
    fac->strategic_alliance = ALLIANCE_NONE;
    fac->recovery_receipts = str_dup("");
    fac->resource = 0;
    fac->research = 0;
    fac->support = 0;
    fac->closed = 0;
  ...
  void free_faction(FACTION_TYPE *fac) {
    ...
    free_string(fac->power_worships);
    free_string(fac->power_opposes);
  ...
  void set_pcdata_default(PC_DATA *pcdata) {
    ...
    pcdata->recovery = new RecoveryState();
    ...
    pcdata->who_name_color = 0;
    pcdata->emote_say_color = 0;
    ...
    pcdata->operative_creation_faction = 0;
    pcdata->operative_core = str_dup("");
    pcdata->operative_society = str_dup("");
    pcdata->operative_legacy_society = str_dup("");
    ...
    pcdata->ill_count = 0;
    pcdata->ill_time = 0;
    pcdata->cauterize_until = 0;
    pcdata->bittenloss = 0;
    pcdata->rerollselftime = 0;
    pcdata->brandcool = 0;
    pcdata->branddate = 0;
    pcdata->cortex_human_brand = FALSE;
    ...
    pcdata->guest_type = 0;
    pcdata->guest_tier = 0;
    pcdata->earned_guest_tier = 0;
    pcdata->guest_reward_used = 0;
    pcdata->monster_reward_hours = 0;
    pcdata->population_last_active = 0;
