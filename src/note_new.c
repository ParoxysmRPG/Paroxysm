...
extern "C" {
  ...
  bool isNoteToGroup(CHAR_DATA *ch, Note *pNote) {
    ...
    return TRUE;
    if ((ch->race == RACE_CIVIL_SERVANT || IS_IMMORTAL(ch)) && pNote->isTo("HSD"))
    return TRUE;
    ...
    if (ch->fcore != 0 && clan_lookup(ch->fcore) != NULL && clan_lookup(ch->fcore)->parent == FACTION_CORTEX && pNote->isTo("hand"))
    return TRUE;

    if (ch->fcore != 0 && clan_lookup(ch->fcore) != NULL && clan_lookup(ch->fcore)->parent == FACTION_ORDER && pNote->isTo("order"))
    return TRUE;

    if (ch->fcore != 0 && clan_lookup(ch->fcore) != NULL && clan_lookup(ch->fcore)->parent == FACTION_SCUM && pNote->isTo("temple"))
    return TRUE;

    if (ch->fsociety != 0 && clan_lookup(ch->fsociety) != NULL && pNote->isTo(clan_lookup(ch->fsociety)->name))
    return TRUE;

    if (ch->fsociety != 0 && clan_lookup(ch->fsociety) != NULL && pNote->isTo(nospaces(clan_lookup(ch->fsociety)->name)))
    return TRUE;


    if (ch->legacy_society != 0 && clan_lookup(ch->legacy_society) != NULL && pNote->isTo(clan_lookup(ch->legacy_society)->name))
    return TRUE;

    if (ch->legacy_society != 0 && clan_lookup(ch->legacy_society) != NULL && pNote->isTo(nospaces(clan_lookup(ch->legacy_society)->name)))
    return TRUE;
