...
****************************************************************************/
  ...
  OEDIT(oedit_copy) {
    ...
    for (cnt = 0; cnt < MAX_OBJ_VALUES; cnt++)
...
*/
  ...
  MEDIT(medit_discipline) {
    ...
    argument = one_argument_nouncap(argument, arg3);
    buf = haven::format_text("%s %s %s", arg1, arg2, arg3);
    for (int i = 0; i < discipline_table_count; i++) {
      if (!str_cmp(buf.data(), discipline_table[i].name)) {
        pMob->disciplines[discipline_table[i].vnum] = atoi(argument);
        send_to_char("Disciplines set.\n\r", ch);
...
*/
  void locate_mob_reset(CHAR_DATA *ch, int vnum, bool equip_list) {
    ...
    buf = haven::format_text("You have searched for Mobile %-20s `W(`Y%5d`W) `RLevel`W: `G%3d`x\n\r", pMob->short_descr, pMob->vnum, pMob->level);
    outbuf.strcat(buf.data());

    buf = haven::format_text("%-7s          %-11s          %-12s\n\n\r", "`W[`xRoom`W]`x", "`W(`xMax`W)-(`xMin`W)`x", "Bonuses");
    outbuf.strcat(buf.data());
