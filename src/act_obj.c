  ...
  void stake(CHAR_DATA *ch, char name[MSL], OBJ_DATA *obj) {
    ...
    if (!IS_FLAG(ch->affected_by, AFF_STAKED)) {
      ...
      if (is_vampire(victim)) {
        ...
        wound_char_absolute(victim, 2, ch);
        miscarriage(victim, FALSE);
        SET_FLAG(victim->affected_by, AFF_STAKED);
      }
      else {
        wound_char_absolute(victim, 3, ch);
  ...
  */
  ...
  _DOFUN(do_buy) {
    ...
    if (is_number(arg1)) {
      for (lobj = keeper->carrying; lobj; lobj = lobj->next_content) {
        if (lobj->wear_loc == WEAR_NONE && can_see_obj(ch, lobj) && (cost = territory_shop_price(ch, get_cost(keeper, lobj, TRUE))) > 0) {
          ref++;
          if (ref == atoi(arg1)) {
            obj = lobj;
            found = TRUE;
    ...
    if (!found && obj != NULL) {
      ...
      cost = territory_shop_price(ch, get_cost(keeper, obj, TRUE));
  ...
  _DOFUN(do_list) {
    ...
    for (obj = keeper->carrying; obj; obj = obj->next_content) {
      if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && (cost = territory_shop_price(ch, get_cost(keeper, obj, TRUE))) > 0 && (arg[0] == '\0' || is_name(arg, obj->name))) {
  ...
  */
  ...
  _DOFUN(do_type) {
    if (ch->pcdata->ci_editing == 0) {
      ...
      if (!STORY_IDEAS_ENABLED) {
        send_to_char("Story ideas are currently disabled.\n\r", ch);
        return;
      }
      ch->pcdata->ci_absorb = 1;
  ...
  void custom_to_obj(CHAR_DATA *ch, OBJ_DATA *poi, bool colour) {
    if (colour) {
      ...
      if (ch->pcdata->account == NULL || ch->pcdata->account->colours <= 0) {
        send_to_char("You have no item recolors available. You receive three per month.\n\r", ch);
        return;
      }
    }
    EXTRA_DESCR_DATA *ed;
  ...
  _DOFUN(do_done) {
    ...
    if (ch->in_room->vnum == ROOM_INDEX_GENESIS) {
      if (higher_power(ch)) {
        ...
        if (!dom || !territory_by_number(dom->claimed_territory)) {
          send_to_char("You must choose a territory before finishing creation: change territory <territory>. The claim is permanent.\n\r", ch);
          return;
        }
      }
      if (!readychar(ch)) {
    ...
    if (ch->pcdata->ci_editing == 0) {
      ...
      */
      ...
      if (ch->pcdata->ci_special != SPECIAL_MINION && ch->pcdata->ci_special != SPECIAL_ALLY && ch->pcdata->ci_special != 0) {
        ...
        if (j < 0) { send_to_char("That special ability no longer exists.\n\r", ch); return; }
      ...
      if (power_operation_goal(ch->pcdata->ci_discipline2)) {
        LOCATION_TYPE *territory = get_loc(ch->pcdata->ci_short);
        const char *error = power_operation_error(ch->faction,
            territory ? number_from_territory(territory) : 0,
            ch->pcdata->ci_discipline2, ch->pcdata->ci_message);
        if (error) { printf_to_char(ch, "%s\n\r", error); return; }
        if (ch->pcdata->ci_discipline != COMPETE_OPEN || ch->pcdata->ci_vnum < 2
            || ch->pcdata->ci_zips < 5 || ch->pcdata->ci_covers < 4) {
          send_to_char("Higher-power operations must be open, scheduled at least two days ahead, speed 5 or higher, with room for at least four PCs.\n\r", ch);
          return;
        }
      }
      ...
      op->bonus += 10;
      op->bonus += get_focus_power(ch, SKILL_GENERALFOCUS) * 4;
      ...
      if(victim->fsociety != ch->pcdata->eidilon_of && victim->legacy_society != ch->pcdata->eidilon_of)
      {
      ...
      if(victim->fsociety == ch->pcdata->eidilon_of && !can_afford_faction_spending(cult, faccost, 8000 + faccost))
      {
      ...
      if(ch->pcdata->ci_disclevel == BOON_GIFT && victim->fsociety == ch->pcdata->eidilon_of)
      {
      ...
      if(victim->fsociety == ch->pcdata->eidilon_of)
      {
      ...
      return; // Preserve the disabled arrest form implementation below.
      ...
      if (ch->faction != 0 && clan_lookup(ch->faction) != NULL && has_trust(ch, TRUST_RESOURCES, ch->faction)) {
        if (!can_afford_faction_spending(clan_lookup(ch->faction), ccost / 10, 5000 + ccost / 10)) {
      ...
      if(safe_strlen(story->eidolon) > 2)
      {
        ...
        it != FacVect.end(); ++it) {
          ...
          {
            if((*it)->patron_relation == SOCIETY_PATRON_WORSHIP)
            {
              found = TRUE;
      ...
      if (clan_lookup(ch->faction) == NULL || !can_afford_faction_spending(clan_lookup(ch->faction), 0, 10000)) {
        send_to_char("You can't do that.\n\r", ch);
        return;
      }
      if (safe_strlen(ch->pcdata->ci_name) < 3) {
      ...
      if (ch->pcdata->ci_mod == MISSION_CONTROL) {
        ...
        if (clan_lookup(ch->faction)->parent == FACTION_CORTEX) {
          if (str_cmp("The Cortex", first_dom_terr(loc))) {
            if (str_cmp("The Cortex", second_dom_terr(loc)) || loc->status != STATUS_SHARED) {
              send_to_char("You're not close enough with the government of that territory for that.\n\r", ch);
              return;
            }
          }
        ...
        if (clan_lookup(ch->faction)->parent == FACTION_SCUM) {
          if (str_cmp("The Scum", first_dom_terr(loc))) {
            if (str_cmp("The Scum", second_dom_terr(loc)) || loc->status != STATUS_SHARED) {
              send_to_char("You're not close enough with the government of that territory for that.\n\r", ch);
              return;
            }
          }
  ...
  bool at_tailor(CHAR_DATA *ch) {
    ...
    for (obj = keeper->carrying; obj; obj = obj->next_content) {
      if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && (cost = territory_shop_price(ch, get_cost(keeper, obj, TRUE))) > 0) {
        if (obj->item_type == ITEM_CLOTHING)
        return TRUE;
    ...
    for (obj = ch->carrying; obj != NULL; obj = obj_next) {
      ...
      if (is_name(arg1, obj->name)) {
        ...
        if (!str_cmp(arg2, "cover")) {
          ...
          {
            send_to_char("You need to be a civil servant to do that.\n\r", ch);
            return;
          }
        }
          ...
          obj->short_descr = str_dup(colbuf);
            ...
            buf = haven::format_text("%s %s", from_color(arg3), colbuf);
          ...
          remove_color(colbuf, argument);
          buf = haven::format_text("%s", colbuf);
          // sprintf(buf, "%s", argument);
          // sprintf(buf, "%-21.21s", buf);
          free_string(obj->wear_string);
          obj->wear_string = str_dup(buf.data());
          send_to_char("Done.\n\r", ch);
          return;
        }
  ...
  _DOFUN(do_phone) {
    ...
    if (!str_cmp(arg1, "signalboost")) {
      ...
      if (!ed) {
        ...
        while (*link && *link != ed) link = &(*link)->next;
        if (*link == ed) *link = ed->next;
        free_extra_descr(ed);
...
#define GAME_SEQUENTIAL 1
        ...
        if (strcasestr(obj->name, "pregnancy") != NULL && strcasestr(obj->name, "test") != NULL) {
          ...
          if (obj->cost > 100) {
            ...
            if (ch->pcdata->impregnated != 0 && ch->pcdata->impregnated + (3600 * 60 * 24 * 7) < current_time) {
              ...
              target = 101; // Too early: no positive result is possible.
          ...
          obj->extra_descr->description = str_dup(buf.data());
          return;
        }
        // Drugs
        if (obj->item_type == ITEM_GAME) {
    ...
    for (int i = 0; i < 10; i++) {
      if (!str_cmp(victim->name, ch->pcdata->relationship[i]) && (ch->pcdata->relationship_type[i] == REL_PARENT || ch->pcdata->relationship_type[i] == REL_GUARDIAN)) {
        if (is_number(arg2)) {
          ...
          buf = haven::format_text("%s %s %s", arg2, argument, PERS(ch, victim));
          do_function(victim, &do_give, buf.data());
          return;
        }
        else {
          for (obj = victim->carrying; obj != NULL; obj = obj->next_content) {
            ...
            if (is_name(arg2, obj->name)) {
              ...
              if (IS_SET(obj->extra_flags, ITEM_WARDROBE))
              REMOVE_BIT(obj->extra_flags, ITEM_WARDROBE);
              buf = haven::format_text("%s %s", arg2, PERS(ch, victim));
              do_function(victim, &do_give, buf.data());
            }
          }
        }
  ...
  static std::unordered_map<int, PHONEBOOK_TYPE *> phonebook_index;
  static size_t phonebook_index_size = static_cast<size_t>(-1);

  void invalidate_phonebook_index(void) {
    phonebook_index_size = static_cast<size_t>(-1);
  }

  void update_phonebook(int number, char *owner) {
    if (phonebook_index_size != PhoneVect.size()) {
      phonebook_index.clear();
      for (PHONEBOOK_TYPE *phone : PhoneVect)
        phonebook_index.emplace(phone->number, phone); // First duplicate wins.
      phonebook_index_size = PhoneVect.size();
    }
    auto found = phonebook_index.find(number);
    if (found != phonebook_index.end()) {
      PHONEBOOK_TYPE *phone = found->second;
      phone->inactivity = 0;
      // Exact comparison preserves case-only ownership changes.
      if (!phone->owner || strcmp(phone->owner, owner ? owner : "")) {
        char *replacement = str_dup(owner);
        free_string(phone->owner);
        phone->owner = replacement;
      }
      return;
    }
    ...
    PhoneVect.push_back(phone);
    phonebook_index.emplace(number, phone);
    phonebook_index_size = PhoneVect.size();
  }
