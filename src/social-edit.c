...
#define SOCIAL_FILE "../area/socials.txt" /* or whatever fits you */
  ...
  void load_social_table() {
    ...
    if (fscanf(fp, "%d\n", &maxSocial) != 1 || maxSocial < 0 || maxSocial == INT_MAX) {
      bug("Invalid social count in " SOCIAL_FILE, 0);
      fclose(fp);
      exit(1);
    }
    ...
    social_table = (struct social_type *)malloc(sizeof(struct social_type) *
    (maxSocial + 1));
  ...
  void save_social(const struct social_type *s, FILE *fp) {
    /* get rid of (null) */
    fprintf(fp, "%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n\n", s->name ? s->name : "", s->char_no_arg ? s->char_no_arg : "", s->others_no_arg ? s->others_no_arg : "", s->char_found ? s->char_found : "", s->others_found ? s->others_found : "", s->vict_found ? s->vict_found : "", s->char_auto ? s->char_auto : "", s->others_auto ? s->others_auto : "");
  }
  ...
  void see_random_social(CHAR_DATA *ch) {
    CHAR_DATA *victim = NULL;
    int i = number_range(1, 100000) % maxSocial;
    ...
    if (victim != NULL) {
      switch (number_percent() % 2) {
      case 0:
        act(social_table[i].others_auto, victim, NULL, ch, TO_VICT);
        break;
      case 1:
        act(social_table[i].others_found, victim, NULL, ch, TO_VICT);
        break;
      case 2:
        act(social_table[i].others_no_arg, victim, NULL, ch, TO_VICT);
        break;
      }
    }
  }
...
*/
  ...
  _DOFUN(do_sedit) {
    ...
    if (!str_cmp(cmd, "delete")) /* Remove a social */
    {
      ...
      (struct social_type *)malloc(sizeof(struct social_type) * maxSocial);
...
*/
    ...
    else if (!str_cmp(cmd, "new")) /* Create a new social */
    {
      ...
      maxSocial++;
      new_table = (struct social_type *)realloc(
      social_table, sizeof(struct social_type) * maxSocial + 1);
    ...
    else if (!str_cmp(cmd, "show")) /* Show a certain social */
    {
      ...
      social_table[iSocial].name, social_table[iSocial].char_no_arg, social_table[iSocial].others_no_arg, social_table[iSocial].char_found, social_table[iSocial].others_found, social_table[iSocial].vict_found, social_table[iSocial].char_auto, social_table[iSocial].others_auto);
