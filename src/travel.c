...
*/
  ...
  _DOFUN(do_travel) {
    ...
    if (!str_cmp(arg1, "car")) {
      ...
      send_to_char("Type roomlist for a list of rooms you can travel to.\n\r", ch);
      return;
    }
    if (desti < 0) {
...
*/
  ...
  _DOFUN(do_newdrive) {
    ...
    set_generated_room_description(travelroom, buf);
...
*/
  ...
  _DOFUN(do_makeroom) {
    ...
    if (!str_cmp(arg1, "mission")) {
      ...
      if (!can_afford_faction_spending(clan_lookup(ch->faction), 0, 10000)) {
  ...
  void subwaytowalk(CHAR_DATA *ch) {
    if (ch->in_room == NULL || ch->in_room->description_decorated) return;
    if (IS_SET(ch->in_room->room_flags, ROOM_INDOORS))
    ...
    set_generated_room_description(ch->in_room, NULL);
  }
  ...
  void cartowalk(CHAR_DATA *ch) {
    ...
    set_generated_room_description(ch->in_room, vehicle_desc(ch));
  }
...
*/
  ...
  const int trolly_stop_count = sizeof(trolly_stops) / sizeof(trolly_stops[0]);
...
*/
  ...
  int carcost(CHAR_DATA *ch, int point) {
    if (ch->pcdata->garage_typeone[point] != CAR_HORSE
        && has_territory_bonus(ch, TERRITORY_FREE_LEASE)) return 0;
