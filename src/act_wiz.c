...
extern "C" {
  ...
  **************************************************************/
  ...
  _DOFUN(do_pstat) {
    ...
    sprintf(tempbuf, "`W                           `WRewards              : `x%-5s`x   ", "All unlocked");
  ...
  _DOFUN(do_mstat) {
    ...
    if (!IS_NPC(victim)) {
      ...
      if (victim->pcdata->last_sex > 0) {
        ...
        buf = haven::format_text("LastSex: %s", (char *)ctime(&sexstat));
        send_to_char(buf.data(), ch);
      }
      else {
        buf = haven::format_text("LastSex: %d/%d\n\r", 0, 0);
        send_to_char(buf.data(), ch);
      }
      ...
      if (victim->pcdata->last_sextype > 0) {
        ...
        buf = haven::format_text("LastSexType: %s\n\r", sextype);
        send_to_char(buf.data(), ch);
      }
      else {
        buf = haven::format_text("LastSexType: %s", "NA\n\r");
        send_to_char(buf.data(), ch);
      }
      ...
      if (victim->pcdata->last_shower > 0) {
        ...
        buf = haven::format_text("LastShower: %s", (char *)ctime(&sexstat));

        send_to_char(buf.data(), ch);
      }
      else {
        buf = haven::format_text("LastShower: %d/%d\n\r", 0, 0);
        send_to_char(buf.data(), ch);
      }

      buf = haven::format_text("SexDirty: %s\n\r", (victim->pcdata->sex_dirty == TRUE) ? "True" : "False");
      send_to_char(buf.data(), ch);

      if (victim->pcdata->due_date > 0) {
        sexstat = victim->pcdata->due_date;
        buf = haven::format_text("Due date: %s", (char *)ctime(&sexstat));

        send_to_char(buf.data(), ch);
      }
      else {
        buf = haven::format_text("Due date: %d/%d\n\r", 0, 0);
        send_to_char(buf.data(), ch);
      }

      if (victim->pcdata->daily_upkeep > 0) {
        sexstat = victim->pcdata->daily_upkeep;
        buf = haven::format_text("Daily Upkeep: %s", (char *)ctime(&sexstat));

        send_to_char(buf.data(), ch);
      }
      else {
        buf = haven::format_text("Sex Upkeep: %d/%d\n\r", 0, 0);
        send_to_char(buf.data(), ch);
      }

      buf = haven::format_text("Fertility: %d\n\r", victim->pcdata->natural_fertility);
      send_to_char(buf.data(), ch);

      buf = haven::format_text("Potency: %d\n\r", victim->pcdata->sex_potency);
      send_to_char(buf.data(), ch);
    }

    buf = haven::format_text("Master: %s\n\r", victim->master ? victim->master->name : "(none)");
    send_to_char(buf.data(), ch);

    // OLC
    if (!IS_NPC(victim)) {
      buf = haven::format_text("Security: %d.\n\r", victim->pcdata->security);
      send_to_char(buf.data(), ch);
    }

    buf = haven::format_text("Short description: %s\n\rLong  description: %s\n\r", victim->short_descr, victim->long_descr[0] != '\0' ? victim->long_descr : "(none)\n\r");
    send_to_char(buf.data(), ch);
  ...
  _DOFUN(do_mwhere) {
    char buf1[MIL], colorless_room[MSL];
    std::string buf;
    Buffer outbuf;
    CHAR_DATA *victim;
    bool found;
    ...
    if (argument[0] == '\0') {
      ...
      it != descriptor_list.end(); ++it) {
        ...
        if (d->character != NULL && d->connected == CON_PLAYING && d->character->in_room != NULL && can_see(ch, d->character) && can_see_room(ch, d->character->in_room) && can_see(ch, d->character)) {
          ...
          if (d->original != NULL) {
            buf = haven::format_text("`c%3d`g) `x%s `g(`cin the body of %s`g) `cis in `x%s `g[`W%d`g]`x", count, d->original->name, victim->short_descr, colorless_room, victim->in_room->vnum);
          }
          else {
            buf = haven::format_text("`c%3d`g) `x%s `cis in `x%s ", count, victim->name, colorless_room);
  ...
  *********************************************************/
  ...
  _DOFUN(do_log) {
    if (IS_IMMORTAL(ch)) {
      ...
      if (!str_cmp(arg1, "clear")) {
        ...
        if (!room) {
          send_to_char("Your logging location is unavailable.\n\r", ch);
          return;
        }
        logs = haven::format_text("`c        [`x%s`c]`x\n\n%s\n%s\n%s\n\n", datestr, roomtitle(room, TRUE), room->description, displaysun_room(room));
        free_string(ch->pcdata->rp_log[0]);
        ch->pcdata->rp_log[0] = str_dup(logs.data());
        ...
        if (system(buf) != 0) {
          bug("Could not publish RP log", 0);
  ...
  _DOFUN(do_propset) {
    ...
    if (!strcmp(arg1, "house")) {
      ...
      send_to_char("Property type must be house or shop.\n\r", ch);
      return;
    }
    ...
    if (IS_IMMORTAL(ch)) {
      ...
      if (!str_cmp(arg2, "abort") || !str_cmp(arg2, "abortion")) {
        ...
        if (wounds == 3) {
          ...
          wound_char_absolute(victim, 1, ch);
        }
...
*/
  ...
  */
  const char *name_expand(CHAR_DATA *ch) {
    ...
    {
      outbuf = ""; /* Do not return NULL, just an empty buffer */
      return outbuf.data();
    }
    ...
    outbuf = haven::format_text("%d.%s", count, name);
    return outbuf.data();
  }
          ...
          it != room->people->end(); ++it) {
            ...
            if (victim != NULL && !IS_NPC(victim) && victim->in_room != NULL && victim->in_room == room) {
              ...
              sprintf(buf, "`R%s`x, ", victim->name);
                ...
                else if (victim->faction == FACTION_CORTEX)
                sprintf(buf, "`D%s`x, ", victim->name);
                else if (victim->faction == FACTION_SCUM)
                sprintf(buf, "`G%s`x, ", victim->name);
                else if (victim->faction == FACTION_ORDER)
                sprintf(buf, "`W%s`x, ", victim->name);
                else
                sprintf(buf, "`x%s, ", victim->name);
              }
...
#define PETITION_RENOVATE 3
  ...
  void save_petitions() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(PETITION_FILE)) == NULL) {
      bug("Cannot open petitions.txt for writing", 0);
      return;
    }
