...
char *PERS(CHAR_DATA *ch, CHAR_DATA *looker) {
  ...
  if(higher_power(ch))
      return ch->name;
  if(higher_power(looker) && !IS_NPC(looker) && (looker->pcdata->eidilon_of == ch->legacy_society || looker->pcdata->eidilon_of == ch->fsociety || get_gmtrust(looker, ch) >= 1))
	return ch->name;
...
void equip_char(CHAR_DATA *ch, OBJ_DATA *obj, int iWear) {
  ...
  if (get_eqr_char(ch, iWear) != NULL) {
    buf = haven::format_text("Equip_char: [%d] in [%d] %s already equipped (%d).",
            ch->pIndexData ? ch->pIndexData->vnum : 0, ch->in_room ? ch->in_room->vnum : 0, ch->short_descr, iWear);
    bug(buf.data(), 0);
    return;
  }
  const bool preserve_hp = IS_SET(obj->extra_flags, ITEM_ARMORED) && !in_fight(ch);
  const int old_max_hp = preserve_hp ? max_hp(ch) : 0;
  const int old_hit = ch->hit;
  ...
  obj->wear_loc = newval;
  if (preserve_hp && old_max_hp > 0) {
    ch->hit = static_cast<long long>(max_hp(ch)) * old_hit / old_max_hp;
  }
...
bool can_see_room(CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex) {
  if (pRoomIndex->area->vnum == 31 &&
    ...
    if (!has_symbol_core(ch) && !has_symbol_society(ch) && !has_symbol_legacy_society(ch) && !has_other_symbol(ch) &&
        !has_goblinkey(ch)) {
...
void all_color(CHAR_DATA *ch, char *argument) {
  ...
  printf_to_char(ch, "All color settings set to %s.\n\r", buf2);
...
void logon_char(CHAR_DATA *ch) {
  ...
  if (minoffline > 240) {
   ...
   if(ch->factiontrue < 1 && ch->faction != 0)
   {
	FACTION_TYPE *tfac = clan_lookup(ch->faction);
        if(tfac != NULL)
	{
            if (player_faction_join_allowed(ch, tfac, FALSE)) {
                if (tfac->type == FACTION_SOCIETY && society_has_room(ch)) ch->fsociety = ch->faction;
                if (tfac->type == FACTION_CORE && ch->fcore == 0) ch->fcore = ch->faction;
            }
	}
    }
  ...
  if (ch->legacy_society != 0 && minoffline > 360 && ch->esteem_legacy_society > 0) {
    ...
    pen /= 2;
    if (!in_haven(ch->in_room)) {
      give_respect(ch, -1 * pen / 45, "Logon Flat", ch->legacy_society);
      give_respect(ch, ch->esteem_legacy_society * -1 / 300, "Logon Percent", ch->legacy_society);
    } else {
      give_respect(ch, -1 * pen / 75, "Logon Flat", ch->legacy_society);
      give_respect(ch, ch->esteem_legacy_society * -1 / 300, "Logon Percent", ch->legacy_society);
    }
  }
  if (ch->fsociety != 0 && minoffline > 360 && ch->esteem_society > 0) {
    ...
    pen /= 2;
    if (!in_haven(ch->in_room)) {
      give_respect(ch, -1 * pen / 45, "Logon Flat", ch->fsociety);
      give_respect(ch, ch->esteem_society * -1 / 300, "Logon Percent", ch->fsociety);
    } else {
      give_respect(ch, -1 * pen / 75, "Logon Flat", ch->fsociety);
      give_respect(ch, ch->esteem_society * -1 / 300, "Logon Percent", ch->fsociety);
    }
  }
 ...
 if (ch->esteem_legacy_society < 0) {
    give_respect(ch, ch->esteem_legacy_society * -1, "", ch->legacy_society);
  }
 if (ch->esteem_society < 0) {
    give_respect(ch, ch->esteem_society * -1, "", ch->fsociety);
  }
  ...
  if (ch->wounds == 3 && can_heal(ch) && (in_hospital(ch) || is_undead(ch))) {
  ...
  if (!in_fac(ch, ch->legacy_society) && ch->legacy_society > 0) {
    if (clan_lookup(ch->legacy_society) != NULL && clan_lookup(ch->legacy_society)->stasis == 0)
      join_to_clan(ch, ch->legacy_society);
    else
      ch->legacy_society = 0;
  }
  if (!in_fac(ch, ch->fsociety) && ch->fsociety > 0) {
    if (clan_lookup(ch->fsociety) != NULL && clan_lookup(ch->fsociety)->stasis == 0)
      join_to_clan(ch, ch->fsociety);
    else
      ch->fsociety = 0;
  }
