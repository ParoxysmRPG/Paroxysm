...
*/
    ...
    if ((fp = open_snapshot_file(strsave)) == NULL) {
    ...
    if (fp == NULL)
    return;
    close_snapshot_file(fp);
    refresh_sanctuary_character(ch);
...
*/
    ...
    if (ch->esteem_society != 0)
    fprintf(fp, "EsteemSociety %d\n", ch->esteem_society);
    if (ch->esteem_legacy_society != 0)
    fprintf(fp, "EsteemSocietyLegacy %d\n", ch->esteem_legacy_society);
    ...
    fprintf(fp, "FSocietyLegacy %d\n", ch->legacy_society);

    if (ch->fsociety != 0)
    fprintf(fp, "FSociety %d\n", ch->fsociety);

    fprintf(fp, "SocietyCreated %d\n", ch->pcdata->society_created);

    fprintf(fp, "DeployCore %d\n", ch->deploy_core);
    fprintf(fp, "DeploySocietyLegacy %d\n", ch->deploy_legacy_society);
    fprintf(fp, "DeploySociety %d\n", ch->deploy_society);
...
int summary_helpless;*/
    ...
    fprintf(fp, "OperativeCreationFaction %d\n", ch->pcdata->operative_creation_faction);
    fprintf(fp, "OperativeCore %s~\n", ch->pcdata->operative_core);
    fprintf(fp, "OperativeSocietyLegacy %s~\n", ch->pcdata->operative_legacy_society);
    fprintf(fp, "OperativeSociety %s~\n", ch->pcdata->operative_society);
    ...
    fprintf(fp, "Talk %s~\n", tch->pcdata->talk);
    fprintf(fp, "WhoNameColor %d\n", tch->pcdata->who_name_color);
    fprintf(fp, "EmoteSayColor %d\n", tch->pcdata->emote_say_color);
    ...
    if (tch->pcdata->branddate > 0 && tch->pcdata->cortex_human_brand)
    fprintf(fp, "CortexHumanBrand 1\n");

    if (tch->pcdata->cortex_victimize_day > 0)
    fprintf(fp, "CortexVictimizeDay %d\nCortexVictimizeStanding %d\n",
        tch->pcdata->cortex_victimize_day, tch->pcdata->cortex_victimize_standing);
    ...
    if (tch->pcdata->cauterize_until > current_time)
      fprintf(fp, "CauterizeUntil %ld\n", tch->pcdata->cauterize_until);
    ...
    if (ch->desc && ch->desc->connected == CON_PLAYING) tch->pcdata->population_last_active = current_time;
    fprintf(fp, "GuestActiveTime %ld\n", tch->pcdata->population_last_active > 0 ? tch->pcdata->population_last_active : tch->lastlogoff);
    fprintf(fp, "GuestTier %d\n", tch->pcdata->guest_tier);
    fprintf(fp, "GuestEarnedTier %d\nGuestRewardUsed %d\nGuestMonsterHours %d\n", tch->pcdata->earned_guest_tier, tch->pcdata->guest_reward_used, tch->pcdata->monster_reward_hours);
...
*/
  bool load_char_obj(DESCRIPTOR_DATA *d, char *name) {
    ...
    const std::string compressed = std::string(PLAYER_DIR) + capitalize(name) + ".gz";
    if ((fp = fopen(compressed.c_str(), "r")) != NULL) {
      fclose(fp);
      if (!haven::decompress_player_file(compressed)) return FALSE;
    }
#endif
...
#define KEYS(literal, field, value)                                            \
  ...
  static void read_character_data(CHAR_DATA *ch, FILE *fp, bool normalize) {
    int legacy_operative_type = 0;
    ...
    for (i = 0; i < 2000; i++) {
      ...
      fMatch = FALSE;
      if (read_recovery(fp, ch, word)) continue;
      ...
      case 'C':
        KEY("CauterizeUntil", ch->pcdata->cauterize_until, fread_number(fp));
        KEY("CortexHumanBrand", ch->pcdata->cortex_human_brand, fread_number(fp));
        KEY("CortexVictimizeDay", ch->pcdata->cortex_victimize_day, fread_number(fp));
        KEY("CortexVictimizeStanding", ch->pcdata->cortex_victimize_standing, fread_number(fp));
...
*/
      ...
      case 'D':
        ...
        KEY("DeployCult", ch->deploy_society, fread_number(fp));
        KEY("DeploySociety", ch->deploy_society, fread_number(fp));
        KEY("DeploySect", ch->deploy_legacy_society, fread_number(fp));
        KEY("DeploySocietyLegacy", ch->deploy_legacy_society, fread_number(fp));
      ...
      case 'E':
        KEY("EmoteSayColor", ch->pcdata->emote_say_color, fread_number(fp));
        ...
        KEY("EsteemCult", ch->esteem_society, fread_number(fp));
        KEY("EsteemSociety", ch->esteem_society, fread_number(fp));
        KEY("EsteemSect", ch->esteem_legacy_society, fread_number(fp));
        KEY("EsteemSocietyLegacy", ch->esteem_legacy_society, fread_number(fp));
        ...
        if (!str_cmp(word, "End")) {
          if (ch->pcdata->operative_creation_faction == 0) {
            if (legacy_operative_type == FACTION_CORE) ch->pcdata->operative_creation_faction = ch->fcore;
            else if (legacy_operative_type == 3) ch->pcdata->operative_creation_faction = ch->legacy_society;
            else if (legacy_operative_type == 4) ch->pcdata->operative_creation_faction = ch->fsociety;
          }
          if (normalize) normalize_society_memberships(ch);
          REMOVE_FLAG(ch->comm, COMM_DREAMSNARED);
          if (ch->pcdata->weakness_status == WEAKNESS_DREAMSNARED)
            ch->pcdata->weakness_status = 0;
          if (feeding_interval(ch) && ch->pcdata->last_feeding <= 0)
      ...
      case 'F':
        ...
        KEY("FCult", ch->fsociety, fread_number(fp));
        KEY("FSociety", ch->fsociety, fread_number(fp));
        KEY("FSect", ch->legacy_society, fread_number(fp));
        KEY("FSocietyLegacy", ch->legacy_society, fread_number(fp));
        KEY("FactionTrue", ch->factiontrue, fread_number(fp));
        KEY("Fatigue", ch->pcdata->fatigue, fread_number(fp));
        KEY("FeedingReminder", ch->pcdata->feeding_reminder, fread_number(fp));
      ...
      case 'G':
        ...
        KEY("GuestActiveTime", ch->pcdata->population_last_active, fread_number(fp));
        KEY("GuestTier", ch->pcdata->guest_tier, fread_number(fp));
        KEY("GuestEarnedTier", ch->pcdata->earned_guest_tier, fread_number(fp));
        KEY("GuestRewardUsed", ch->pcdata->guest_reward_used, fread_number(fp));
        KEY("GuestMonsterHours", ch->pcdata->monster_reward_hours, fread_number(fp));
      ...
      case 'O':
        ...
        KEY("OperativeCreationType", legacy_operative_type, fread_number(fp));
        KEY("OperativeCreationFaction", ch->pcdata->operative_creation_faction, fread_number(fp));
        KEYS("OperativeCore", ch->pcdata->operative_core, fread_string(fp));
        KEYS("OperativeSect", ch->pcdata->operative_legacy_society, fread_string(fp));
        KEYS("OperativeSocietyLegacy", ch->pcdata->operative_legacy_society, fread_string(fp));
        KEYS("OperativeCult", ch->pcdata->operative_society, fread_string(fp));
        KEYS("OperativeSociety", ch->pcdata->operative_society, fread_string(fp));
      ...
      case 'W':
        KEY("WhoNameColor", ch->pcdata->who_name_color, fread_number(fp));
  ...
  void fread_char(CHAR_DATA *ch, FILE *fp) { read_character_data(ch, fp, true); }

  void fread_population_char(CHAR_DATA *ch, FILE *fp) { read_character_data(ch, fp, false); }
...
*/
      ...
      ++it) {
        ...
        if (obj->pIndexData->vnum == OBJ_VNUM_CORPSE_PC && obj->contains && obj->carried_by == NULL
            && obj->in_room && safe_strlen(obj->short_descr) >= 14) {
...
******************************************************************/
  ...
  void save_death_state(CHAR_DATA *ch) {
    if (!ch || IS_NPC(ch) || IS_FLAG(ch->act, PLR_NOSAVE)) return;
    char *player = NULL, *ground = NULL;
    size_t player_size = 0, ground_size = 0;
    FILE *pf = open_memstream(&player, &player_size);
    FILE *gf = open_memstream(&ground, &ground_size);
    if (!pf || !gf) { bug("Cannot prepare death saves", 0); _Exit(EXIT_FAILURE); }
    fwrite_char(ch, pf, FALSE, FALSE, NULL);
    if (ch->carrying) fwrite_obj(ch->carrying, pf, 0);
    fprintf(pf, "#END\n");
    for (OBJ_DATA *obj : object_list) {
      if (obj && !obj->carried_by && obj->in_room && !is_obj_reset(obj, obj->in_room))
        fwrite_obj_2(obj, gf, 0);
    }
    fprintf(gf, "#END\n");
    bool ok = !ferror(pf) && !ferror(gf);
    if (fclose(pf) != 0) ok = false;
    if (fclose(gf) != 0) ok = false;
    if (ok) ok = haven::commit_death_snapshots(std::string(PLAYER_DIR) + capitalize(ch->name),
                   std::string(player, player_size), std::string(ground, ground_size));
    free(player);
    free(ground);
    // Never let a later save overwrite a partially committed transfer. A
    // prepared transaction is completed before the next boot loads players.
    if (!ok) { bug("Cannot commit death saves; restart required", 0); _Exit(EXIT_FAILURE); }
  }
  ...
  void save_ground_objects() {
    ...
    if (time_info.day % 7 == 0)
    sprintf(backup, "%s%s", BACK1_DIR, "GroundObjects");
    else if (time_info.day % 6 == 0)
    sprintf(backup, "%s%s", BACK2_DIR, "GroundObjects");
    else if (time_info.day % 5 == 0)
    sprintf(backup, "%s%s", BACK3_DIR, "GroundObjects");
    else if (time_info.day % 4 == 0)
    sprintf(backup, "%s%s", BACK4_DIR, "GroundObjects");
    else if (time_info.day % 3 == 0)
    sprintf(backup, "%s%s", BACK5_DIR, "GroundObjects");
    else if (time_info.day % 2 == 0)
    sprintf(backup, "%s%s", BACK6_DIR, "GroundObjects");
    else
    sprintf(backup, "%s%s", BACK7_DIR, "GroundObjects");
    ...
    if ((fp = open_snapshot_file(strsave)) == NULL) {
      ...
      fprintf(fp, "#END\n");
      close_snapshot_file_with_backup(fp, backup);
    }
  }
