...
extern "C" {
  ...
  */
  ...
  bool door_locked(EXIT_DATA *pexit, ROOM_INDEX_DATA *to_room, CHAR_DATA *ch) {
    ...
    if (IS_SET(pexit->exit_info, EX_LOCKED)) {
      ...
      if (in_sheriff(to_room) && ch->race != RACE_CIVIL_SERVANT && !IS_IMMORTAL(ch)) {
        return TRUE;
      }
    }
  ...
  */
  ...
  char *movement_message(CHAR_DATA *ch, bool pushing, bool springing, int door) {
    ...
    if (!IS_NPC(ch) && safe_strlen(ch->pcdata->cwalk) > 1 && ch->shape == SHAPE_HUMAN)
    ...
    if (to_room != NULL && !IS_IMMORTAL(ch)) {
      ...
      if (!IS_NPC(ch) && ch->in_room != NULL && to_room != NULL && ch->in_room->area->vnum != 12 && to_room->area->vnum != 12) {
        ...
        if (institute_room(to_room)) {
          ...
          if (to_room->vnum == ROOM_INDEX_SHERIFFCAGE && (ch->played < 100 || ch->race != RACE_CIVIL_SERVANT)) {
            send_to_char("Only civil servants with 100 hours of on the job experience are allowed access to the cage.\n\r", ch);
            return;
          }
...
*/
    ...
    */
    ...
    if (ch->dragging != NULL && !IS_NPC(ch->dragging) && is_ghost(ch->dragging) && ch->dragging->pcdata->ghost_room == in_room->vnum) {
      ch->dragging->pcdata->ghost_room = to_room->vnum;
      printf_to_char(ch, "You drag the comatose form of %s with you.\n\r", ch->dragging->name);
    }
    else if (ch->dragging != NULL && !IS_NPC(ch->dragging) && ch->dragging->pcdata->spectre > 0 && IS_FLAG(ch->dragging->act, PLR_SHROUD) && ch->dragging->pcdata->ghost_room == in_room->vnum) {
      ch->dragging->pcdata->ghost_room = to_room->vnum;
      printf_to_char(ch, "You drag the comatose form of %s with you.\n\r", ch->dragging->name);
    }
  ...
  char *smell_line(CHAR_DATA *ch) {
    ...
    if (is_animal(ch)) {
      ...
      if (safe_strlen(buf) > 1) {
        return str_dup((std::string("A human scent mixed with ") + buf).c_str());
      }
