...
extern "C" {
  ...
  // Stock terrain uses the same catalogue as the saved world. A missing
  // shroud template must never suppress the physical room description.
  const char *stock_description(CHAR_DATA *ch, ROOM_INDEX_DATA *room) {
    ensure_room_description(room);
    if (room == NULL) return default_room_description(NULL);
    if (ch != NULL && IS_FLAG(ch->act, PLR_SHROUD)) {
      if (!unfinished_room_description(room->shroud)) return room->shroud;
      int template_vnum = 0;
      switch (room->sector_type) {
      case SECT_PARK: template_vnum = 101; break;
      case SECT_WATER: template_vnum = 111; break;
      case SECT_UNDERWATER: template_vnum = 105; break;
      case SECT_ROCKY: template_vnum = 106; break;
      case SECT_BEACH: template_vnum = 107; break;
      case SECT_AIR: case SECT_ATMOSPHERE: template_vnum = 108; break;
      case SECT_STREET: template_vnum = 109; break;
      case SECT_TUNNELS: template_vnum = 110; break;
      case SECT_ALLEY: template_vnum = 112; break;
      case SECT_CEMETARY: template_vnum = 113; break;
      case SECT_PARKING: template_vnum = 114; break;
      case SECT_SIDEWALK: template_vnum = 115; break;
      }
      ROOM_INDEX_DATA *source = template_vnum ? get_room_index(template_vnum) : NULL;
      if (source != NULL && !unfinished_room_description(source->shroud))
        return source->shroud;
    }
    return unfinished_room_description(room->description) ? default_room_description(room) : room->description;
  }
...
*/
    while ((pExtra = room->extra_descr) != NULL) {
      room->extra_descr = pExtra->next;
      free_extra_descr(pExtra);
    }

    clear_room_places(room);
...
*/
  ...
  void link_room(int origin_Vnum, int coord, int direction) {
    ...
    else return; // Reject directions with no coordinate mapping.
...
*/
  ...
  void grow_forest(int i) {
    ...
    // if room exists already
      ...
      if (!room->description_decorated && (room_uses_stock_description(room) || room->size == 0)) {
        /*
if ((i>= && i<=)) {
integrate_room(i);
...
beach"); set_generated_room_description(room, NULL); room->shroud=str_dup("");
SET_BIT(room->room_flags, ROOM_UNLIT);
  ...
  void world_sync(char type[MSL]) {
    ...
    if (!str_cmp(type, "streets") || !str_cmp(type, "street")) {
      ...
      for (x = HavenWoods + 420000; x <= HavenWoods + 580000; x = x + 1000) {
        for (y = x + 420; y <= x + 580; y++) {
          if ((room = get_room_index(y)) != NULL && room->sector_type == SECT_FOREST && room_uses_stock_description(room)) {
            reset_room_index(room);
            room->name = str_dup("delete");
          }
        }
      ...
      for (z = 0; z < 10000000; z = z + 1000000) {
        ...
        x = x + 1000) {
          for (y = x + 420; y <= x + 580; y++) {
            if ((room = get_room_index(y)) != NULL && room->sector_type == SECT_AIR && room_uses_stock_description(room)) {
              reset_room_index(room);
              room->name = str_dup("delete");
            }
          }
    ...
    // Sky
      ...
      for (z = 0; z < 10000000; z = z + 1000000) {
        ...
        x = x + 1000) {
          for (y = x + 420; y <= x + 580; y++) {
            if ((room = get_room_index(y)) != NULL && room->sector_type == SECT_WATER && room_uses_stock_description(room)) {
              reset_room_index(room);
              room->name = str_dup("delete");
            }
          }
...
*/
  ...
  void fread_location(FILE *fp) {
    ...
    for (;;) {
      ...
      case '*':
        ...
        break;
      case 'B':
        KEY("Bonus", location->territory_bonus, fread_number(fp));
        KEY("BonusSecondary", location->territory_bonus_secondary, fread_number(fp));
        ...
        if (!str_cmp(word, "Basesect")) {
          location->base_society_secondary = fread_number(fp);
          free_string(location->base_society_secondary_desc);
          location->base_society_secondary_desc = fread_string(fp);
          fMatch = TRUE;
        }
        if (!str_cmp(word, "Basecult")) {
          location->base_society = fread_number(fp);
          free_string(location->base_society_desc);
          location->base_society_desc = fread_string(fp);
          fMatch = TRUE;
        }
      ...
      case 'G':
        ...
        break;
      case 'H':
        // Legacy save key for Cortex influence.
      ...
      case 'P':
        ...
        break;
      case 'R':
        KEY("RewardController", location->reward_controller, fread_number(fp));
        if (!str_cmp(word, "RewardFootholds")) {
          for (int &owner : location->reward_footholds) owner = fread_number(fp);
          fMatch = TRUE;
        }
        break;
      case 'S':
        ...
        break;
      case 'T':
        // Legacy save key for Scum influence.
  ...
  void save_locations_backup() {
    ...
    it != locationVect.end(); ++it) {
      ...
      fprintf(fpout, "BonusSecondary %d\n", (*it)->territory_bonus_secondary);
      fprintf(fpout, "RewardController %d\n", (*it)->reward_controller);
      fprintf(fpout, "RewardFootholds %d %d %d\n", (*it)->reward_footholds[0], (*it)->reward_footholds[1], (*it)->reward_footholds[2]);
      ...
      fprintf(fpout, "Phil");
      for (i = 0; i < TERRITORY_SUPPORT_SLOTS; i++)
      ...
      if ((*it)->base_society > 0 && clan_lookup((*it)->base_society) != NULL)
      fprintf(fpout, "Basecult %d %s~\n", (*it)->base_society, (*it)->base_society_desc);

      if ((*it)->base_society_secondary > 0 && clan_lookup((*it)->base_society_secondary) != NULL)
      fprintf(fpout, "Basesect %d %s~\n", (*it)->base_society_secondary, (*it)->base_society_secondary_desc);
  ...
  void save_locations() {
    FILE *fpout;
    int i = 0;
    if ((fpout = open_snapshot_file(LOCATION_FILE)) == NULL) {
      bug("Cannot open locations.txt for writing", 0);
      return;
    }
    ...
    it != locationVect.end(); ++it) {
      ...
      if ((*it)->base_society > 0)
      fprintf(fpout, "Basecult %d %s~\n", (*it)->base_society, (*it)->base_society_desc);

      if ((*it)->base_society_secondary > 0)
      fprintf(fpout, "Basesect %d %s~\n", (*it)->base_society_secondary, (*it)->base_society_secondary_desc);
  ...
  void save_gateways() {
    FILE *fpout;
    if ((fpout = open_snapshot_file(GATEWAY_FILE)) == NULL) {
      bug("Cannot open gateways.txt for writing", 0);
      return;
    }
  ...
  bool is_territory_support(int slot) {
    if (slot < 0 || slot >= TERRITORY_SUPPORT_SLOTS) return FALSE;
    return is_alliance(slot)
        || slot == FACTION_CORTEX || slot == FACTION_ORDER || slot == FACTION_SCUM;
  }

  static int territory_potion(LOCATION_TYPE *loc) {
    if (!str_cmp(loc->name, "District 82")) return 87;
    if (!str_cmp(loc->name, "Lauriea")) return 85;
    if (!str_cmp(loc->name, "Rhagost")) return 86;
    return 0;
  }

  static int territory_reward(LOCATION_TYPE *loc) {
    loc->territory_bonus_secondary = 0;
    // Named assignments are authoritative, including explicit blank rewards.
    // This also replaces bonuses in existing saves from the repeated directory.
    for (const TerritoryRewardDefault &entry : territory_reward_defaults) {
      if (str_cmp(loc->name, entry.name)) continue;
      loc->territory_bonus = territory_potion(loc) ? 0 : entry.primary;
      return loc->territory_bonus;
    }
    // Unlisted territories have no automatic reward; preserve valid custom ones.
    if (territory_potion(loc) || loc->territory_bonus < 0
        || loc->territory_bonus >= TERRITORY_BONUS_COUNT)
      loc->territory_bonus = 0;
    return loc->territory_bonus;
  }

  static const char *territory_reward_description(int bonus) {
    static const char *descriptions[] = {
      "None", "Well Fed: +10% effective LF", "Landlord: +1 Wealth",
      "Beach Bod: +1 Physical Conditioning", "On My Payroll: immunity to commitment and arrest",
      "Aristocracy: +10% influence gains", "Free Market: 10% shop discount",
      "Buy Here, Don't Pay Here: no car or bike lease payments", "Semper Fi: +1 Military Contacts",
      "Rooster's Special Ammo: +5 ranged disciplines", "Knock Knock: access to trailers",
      "Angelic Halo: +1 Lightup", "Panhandler's Friend: +1 Homeless Contacts",
      "Import-Export: $10,000 faction resources on capture", "The Sea's Kiss: +1 One with the Waves",
      "Witch of the Woods: +1 Incantation", "Feywood Veteran: deal 150% damage to monsters",
      "Darkwood Veteran: take 75% damage from monsters",
      "City Hall: +1 Government",
      "Ivy League: +1 Education",
      "House Rules: +1 Games",
      "Publicist on Retainer: +1 Media",
      "Thin Air: +1 Stamina",
      "Court Connections: +1 Politics",
      "Old Stories: +1 Occult Knowledge",
      "Haute Couture: +1 Fashion Knowledge",
      "Fancy Footwork: +1 Acrobatics",
      "Precision Engineering: +1 Engineering",
      "Beauty Secrets: +1 Beauty Rituals",
      "Ancient Rites: +1 Ritualism",
      "Know Thy Enemy: +1 Perception",
      "Hold the Line: +1 Melee Defense",
      "Midnight Society: +1 Occult",
      "Make Do and Mend: +1 Handyman",
      "Paper Trail: grants Forensics at rank 2",
      "Private Security: grants Protection Detail at rank 2",
      "Patron of the Arts: +1 Artistic Ability",
      "Horse Whisperer: +1 Animals",
      "Triple Threat: +1 Third Art",
      "Back-Alley Connections: +1 Underworld",
      "Root Access: +1 Hacking",
      "Sacred Charms: +1 Artificing Amulets",
      "Split-Second Timing: +1 Fast Reflexes",
      "Light Fingers: +1 Dexterity",
      "First Responder: +1 Medicine",
      "Another Muse: +1 Second Art",
      "Heavy Lifting: +1 Strength",
      "Long Stride: +1 Running",
      "Keep Your Head Down: +1 Ranged Defense",
      "The Alchemist's Market: +1 Alchemy",
      "Ancestral Training: +1 Melee Attack",
      "Clean Getaway: +1 Larceny",
      "Firing Range: +1 Ranged Attack",
      "Veiled Steps: +1 Stealth",
      "Extra Hands: +1 Minions",
      "Local Connections: +1 Police",
      "Wrench in the Works: +10 to thwart rolls",
      "Contingency Plans: +10 to defense against thwarts",
      "Affordable Interference: 15% less influence to attempt thwarts",
      "Open Secrets: +10 to scheme investigation rolls",
      "Closed Files: +10 to defense against scheme investigations",
      "Research Budget: free scheme investigations",
      "Seed Funding: 10% less influence to launch schemes",
      "Bought Time: failed thwarts block further attempts for seven hours"
    };
    static_assert(sizeof(descriptions) / sizeof(descriptions[0]) == TERRITORY_BONUS_COUNT,
                  "Every territory bonus needs a description");
    return descriptions[bonus >= 0 && bonus < TERRITORY_BONUS_COUNT ? bonus : 0];
  }

  // A unique majority controls a territory. Ties and sub-50% support are unowned.
  // Alliance support is shared by its societies; NPC support can deny control.
  bool foothold_controller(const LOCATION_TYPE *loc, int vnum) {
    return loc && loc->valid && vnum > 0 && (loc->base_faction_core == vnum
        || loc->base_society == vnum || loc->base_society_secondary == vnum);
  }


  bool has_territory_bonus(CHAR_DATA *ch, int bonus) {
    if (!ch || IS_NPC(ch) || !ch->pcdata || bonus <= 0 || bonus >= TERRITORY_BONUS_COUNT) return FALSE;
    const int memberships[] = {ch->fcore, ch->fsociety, ch->legacy_society};
    FACTION_TYPE *factions[3] = {};
    bool resolved = false;
    for (LOCATION_TYPE *loc : locationVect) {
      if (!loc->valid || !loc->name || !loc->name[0] || territory_reward(loc) != bonus) continue;

      if (!resolved) {
        for (int i = 0; i < 3; ++i) factions[i] = clan_lookup(memberships[i]);
        resolved = true;
      }
      for (FACTION_TYPE *fac : factions) {
        if (fac && fac->valid && fac->antagonist == 0 && foothold_controller(loc, fac->vnum))
          return TRUE;
      }
    }
    return FALSE;
  ...
  int territory_skill_bonus(CHAR_DATA *ch, int skill) {
    int bonus = 0;
    switch (skill) {
      case SKILL_WEALTH: bonus = TERRITORY_LANDLORD; break;
      case SKILL_CONDITIONING: bonus = TERRITORY_BEACH_BOD; break;
      case SKILL_CMILITARY: bonus = TERRITORY_SEMPER_FI; break;
      case SKILL_LIGHTUP: bonus = TERRITORY_HALO; break;
      case SKILL_CHOMELESS: bonus = TERRITORY_PANHANDLER; break;
      case SKILL_OCEANAFFINITY: bonus = TERRITORY_SEA_KISS; break;
      case SKILL_INCANTATION: bonus = TERRITORY_WITCH; break;
      case SKILL_CGOVERNMENT: bonus = TERRITORY_CITY_HALL; break;
      case SKILL_EDUCATION: bonus = TERRITORY_IVY_LEAGUE; break;
      case SKILL_GAMES: bonus = TERRITORY_HOUSE_RULES; break;
      case SKILL_CMEDIA: bonus = TERRITORY_PUBLICIST; break;
      case SKILL_STAMINA: bonus = TERRITORY_THIN_AIR; break;
      case SKILL_CHARISMA: bonus = TERRITORY_COURT_CONNECTIONS; break;
      case SKILL_DEMONOLOGY: bonus = TERRITORY_OLD_STORIES; break;
      case SKILL_FASHION: bonus = TERRITORY_HAUTE_COUTURE; break;
      case SKILL_ACROBATICS: bonus = TERRITORY_FANCY_FOOTWORK; break;
      case SKILL_ENGINEERING: bonus = TERRITORY_PRECISION_ENGINEERING; break;
      case SKILL_BEAUTY: bonus = TERRITORY_BEAUTY_SECRETS; break;
      case SKILL_RITUALISM: bonus = TERRITORY_ANCIENT_RITES; break;
      case SKILL_PERCEPTION: bonus = TERRITORY_KNOW_ENEMY; break;
      case SKILL_DODGING: bonus = TERRITORY_HOLD_LINE; break;
      case SKILL_COCCULT: bonus = TERRITORY_MIDNIGHT_SOCIETY; break;
      case SKILL_HANDYMAN: bonus = TERRITORY_MAKE_DO; break;
      case SKILL_FORENSICS: bonus = TERRITORY_PAPER_TRAIL; break;
      case SKILL_PROTECTIONDETAIL: bonus = TERRITORY_PRIVATE_SECURITY; break;
      case SKILL_ART: bonus = TERRITORY_ART_PATRON; break;
      case SKILL_ANIMALS: bonus = TERRITORY_HORSE_WHISPERER; break;
      case SKILL_ART3: bonus = TERRITORY_TRIPLE_THREAT; break;
      case SKILL_CCRIMINAL: bonus = TERRITORY_BACK_ALLEY; break;
      case SKILL_HACKING: bonus = TERRITORY_ROOT_ACCESS; break;
      case SKILL_ARTAMULET: bonus = TERRITORY_SACRED_CHARMS; break;
      case SKILL_FASTREFLEXES: bonus = TERRITORY_SPLIT_SECOND; break;
      case SKILL_DEXTERITY: bonus = TERRITORY_LIGHT_FINGERS; break;
      case SKILL_MEDICINE: bonus = TERRITORY_FIRST_RESPONDER; break;
      case SKILL_ART2: bonus = TERRITORY_ANOTHER_MUSE; break;
      case SKILL_STRENGTH: bonus = TERRITORY_HEAVY_LIFTING; break;
      case SKILL_RUNNING: bonus = TERRITORY_LONG_STRIDE; break;
      case SKILL_EVADING: bonus = TERRITORY_HEAD_DOWN; break;
      case SKILL_ALCHEMY: bonus = TERRITORY_ALCHEMIST_MARKET; break;
      case SKILL_MARTIALART: bonus = TERRITORY_ANCESTRAL_TRAINING; break;
      case SKILL_LARCENY: bonus = TERRITORY_CLEAN_GETAWAY; break;
      case SKILL_COMBATTRAINING: bonus = TERRITORY_FIRING_RANGE; break;
      case SKILL_STEALTH: bonus = TERRITORY_VEILED_STEPS; break;
      case SKILL_MINIONS: bonus = TERRITORY_EXTRA_HANDS; break;
      case SKILL_CPOLICE: bonus = TERRITORY_LOCAL_CONNECTIONS; break;
      default: return 0;
    }
    return has_territory_bonus(ch, bonus) ? 1 : 0;
  }

  int territory_investigation_cost(CHAR_DATA *ch, int amount) {
    return has_territory_bonus(ch, TERRITORY_FREE_INVESTIGATION) ? 0 : amount;
  }

  int territory_influence_gain(CHAR_DATA *ch, int amount) {
    return amount > 0 && has_territory_bonus(ch, TERRITORY_ARISTOCRACY)
      ? amount + amount / 10 : amount;
  }

  int territory_shop_price(CHAR_DATA *ch, int amount) {
    return amount > 0 && has_territory_bonus(ch, TERRITORY_FREE_MARKET)
      ? UMAX(1, amount - amount / 10) : amount;
  }

  int territory_damage_amount(CHAR_DATA *victim, CHAR_DATA *ch, int amount) {
    if (ch != victim && amount > 0) {
      if (forest_monster(victim) && has_territory_bonus(ch, TERRITORY_FEYWOOD))
        amount = amount * 150 / 100;
      if (forest_monster(ch) && has_territory_bonus(victim, TERRITORY_DARKWOOD))
        amount = amount * 75 / 100;
    }
    return amount;
  }

  void territory_rewards_update(void) {
    for (LOCATION_TYPE *loc : locationVect) {
      if (!loc->valid || !loc->name || !loc->name[0]) continue;
      territory_reward(loc);
      const int owners[] = {loc->base_faction_core, loc->base_society, loc->base_society_secondary};
      std::vector<FACTION_TYPE *> recipients;
      for (int i = 0; i < 3; ++i) {
        const int previous = loc->reward_footholds[i];
        loc->reward_footholds[i] = owners[i];
        if (previous == -1 || previous == owners[i] || owners[i] <= 0) continue;
        FACTION_TYPE *fac = clan_lookup(owners[i]);
        if (fac && fac->valid && fac->stasis == 0
            && std::find(recipients.begin(), recipients.end(), fac) == recipients.end())
          recipients.push_back(fac);
      }
      if (territory_reward(loc) != TERRITORY_IMPORT_EXPORT) continue;
      if (recipients.empty()) continue;
      // One resource unit is $10. This is a fixed capture award, exempt from
      // the secrecy, intel and diminishing-return modifiers of gain_resources.
      int share = 1000 / recipients.size(), remainder = 1000 % recipients.size();
      for (FACTION_TYPE *fac : recipients) {
        int amount = share + (remainder-- > 0 ? 1 : 0);
        fac->resource += amount;
        char message[MSL];
        sprintf(message, "You gain $%d resources from Import-Export in %s.", amount * 10, loc->name);
        send_log(fac->vnum, message);
      }
    }
  }

  void territory_plus(LOCATION_TYPE *loc, int type) {
    if (!loc || !is_territory_support(type)) return;
  ...
  static void format_territory_support(char *buf, size_t size, const LOCATION_TYPE *loc) {
    snprintf(buf, size, "Society Support: `r%s`x: %d%%, `g%s`x: %d%%\n `DThe Cortex`x: %d%%, `WThe Order`x: %d%%, `gThe Scum`x: %d%%\n", alliance_names(time_info.society_alliance_issue, ALLIANCE_SIDELEFT), loc->phil_amount[ALLIANCE_SIDELEFT], alliance_names(time_info.society_alliance_issue, ALLIANCE_SIDERIGHT), loc->phil_amount[ALLIANCE_SIDERIGHT], loc->phil_amount[FACTION_CORTEX], loc->phil_amount[FACTION_ORDER], loc->phil_amount[FACTION_SCUM]);
  }
  ...
  void show_territory_to_char(CHAR_DATA *ch, LOCATION_TYPE *loc) {
    ...
    if (loc->base_society != 0 && clan_lookup(loc->base_society) != NULL) {
      sprintf(buf, "`WControlling Cult:`x %s\n%s\n\r", clan_lookup(loc->base_society)->name, loc->base_society_desc);
      strcat(string, buf);
    }
    if (loc->base_society_secondary != 0 && clan_lookup(loc->base_society_secondary) != NULL) {
      sprintf(buf, "`WControlling Sect:`x %s\n%s\n\r", clan_lookup(loc->base_society_secondary)->name, loc->base_society_secondary_desc);
      strcat(string, buf);
    }
    ...
    for (DOMAIN_TYPE *dom : DomainVect) {
      if (dom && dom->valid && !dom->guest_retired && dom->claimed_territory == number_from_territory(loc))
        printf_to_char(ch, "Higher power claiming this territory: %s (%s).\n\r", dom->domain_of,
            dom->banished_until > current_time ? "banished" : "in play");
    }
  ...
  _DOFUN(do_territory) {
    ...
    if (ch->pcdata->territory_editing[TEDIT_NUMBER] > 0 && ch->pcdata->territory_editing[TEDIT_TIMER] > 0) {
      ...
      if ((!str_cmp(arg, "base") || !str_cmp(arg, "establish")) && ch->pcdata->territory_editing[TEDIT_ESTABLISH] > 0) {
        ...
        else if (fac->type == FACTION_SOCIETY && (floc->base_society == 0 || floc->base_society == fac->vnum)) {
          string_append(ch, &floc->base_society_desc);
          floc->base_society = ch->pcdata->factionwin;
          floc->base_type = FACTION_SOCIETY;
          send_to_char("Enter the description of the new foothold base.\n\r", ch);
          ch->pcdata->territory_editing[TEDIT_ESTABLISH] = 0;
          return;
        }
        else if (fac->type == FACTION_SOCIETY && (floc->base_society_secondary == 0 || floc->base_society_secondary == fac->vnum)) {
          string_append(ch, &floc->base_society_secondary_desc);
          floc->base_society_secondary = ch->pcdata->factionwin;
          floc->base_type = FACTION_SOCIETY;
          send_to_char("Enter the description of the new foothold base.\n\r", ch);
          ch->pcdata->territory_editing[TEDIT_ESTABLISH] = 0;
          return;
    ...
    if (!str_cmp(arg, "create") && IS_IMMORTAL(ch)) {
      ...
      boost_territory(loc, 3, atoi(argument), TRUE);
        ...
        send_to_char("Territory boost (territory) cortex/order/scum (amount)\n\r", ch);
        return;
      }
      if (!IS_IMMORTAL(ch))
      ...
      hit_territory(loc, 3, atoi(argument), TRUE);
        ...
        send_to_char("Territory hit (territory) cortex/order/scum (amount)\n\r", ch);
        return;
      }
      if (!IS_IMMORTAL(ch))
      ch->pcdata->tboosts -= atoi(argument);
    ...
    if (!str_cmp(arg, "set")) {
      ...
      cost = cost * 90 / 100;
      if (!can_afford_faction_spending(fac, cost, 10000 + cost)) {
...
*/
  ...
  typedef std::tuple<int, int, int> RoomCoordinates;
  static std::map<RoomCoordinates, std::vector<ROOM_INDEX_DATA *> > coordinate_rooms;
  static bool coordinate_index_dirty = true;

  void invalidate_room_coordinates() {
    coordinate_index_dirty = true;
  }

  static void rebuild_coordinate_index() {
    coordinate_rooms.clear();
    for (int bucket = 0; bucket < MAX_KEY_HASH; ++bucket) {
      for (ROOM_INDEX_DATA *room = room_index_hash[bucket]; room; room = room->next) {
        // Match get_room_index if a malformed world contains duplicate vnums.
        if (get_room_index(room->vnum) == room)
          coordinate_rooms[RoomCoordinates(room->x, room->y, room->z)].push_back(room);
      }
    }
    for (auto &entry : coordinate_rooms)
      std::sort(entry.second.begin(), entry.second.end(),
        [](ROOM_INDEX_DATA *a, ROOM_INDEX_DATA *b) { return a->vnum < b->vnum; });
    coordinate_index_dirty = false;
  }

  ROOM_INDEX_DATA *room_by_coordinates(int x, int y, int z) {
    if (coordinate_index_dirty) rebuild_coordinate_index();
    const auto entry = coordinate_rooms.find(RoomCoordinates(x, y, z));
    if (entry == coordinate_rooms.end()) return NULL;
    const auto &rooms = entry->second;
    for (AREA_DATA *area : area_list) {
      if (area->minx == -1 && area->miny == -1 && area->maxx == -1 && area->maxy == 1)
        continue;
      const bool unbounded = area->minx == 0 && area->miny == 0 && area->maxx == 0 && area->maxy == 0;
      if (!unbounded && !(area->minx <= x && area->maxx >= x && area->miny <= y && area->maxy >= y))
        continue;
      const auto room = std::lower_bound(rooms.begin(), rooms.end(), area->min_vnum,
        [](ROOM_INDEX_DATA *candidate, int vnum) { return candidate->vnum < vnum; });
      if (room != rooms.end() && (*room)->vnum <= area->max_vnum) return *room;
    }
    return NULL;
  }

  bool coordinates_exist(int x, int y, int z) {
    return room_by_coordinates(x, y, z) != NULL;
  }
  ...
  void makesky(int x, int y, int z, int area, bool overwrite) {
    ...
    if ((toremove = room_by_coordinates(x, y, z)) != NULL) {
      if (toremove->description_decorated) return;
      if (overwrite == TRUE)
      kill_room(toremove);
      else
      return;
    ...
    set_generated_room_description(newroom, NULL);
    newroom->shroud = str_dup("");
    SET_BIT(newroom->room_flags, ROOM_UNLIT);
    newroom->size = 50;
    ensure_room_description(newroom);
...
*/
  ...
  const char *territory_name(int number) {
    LOCATION_TYPE *location = territory_by_number(number);
    return location && location->name ? location->name : "Somewhere";
  }
  ...
  void offworld_weather_update(void)
{
  ...
  int conflict_score(LOCATION_TYPE *loc)
  {
    int bscore = 50;
    for (int i = 0; i < TERRITORY_SUPPORT_SLOTS; ++i)
      if (is_territory_support(i)) bscore += loc->phil_amount[i];
  ...
  void clean_room(ROOM_INDEX_DATA *room) {
    ...
    room->name = str_dup("");
    set_generated_room_description(room, NULL);
    while ((ed = room->extra_descr) != NULL) {
  ...
  {
    if (!ch || !loc) return FALSE;
    if(ch->fcore == loc->base_faction_core && is_leader(ch, ch->fcore))
    return TRUE;
    if(ch->fsociety == loc->base_society && is_leader(ch, ch->fsociety))
    return TRUE;
    if(ch->legacy_society == loc->base_society_secondary && is_leader(ch, ch->legacy_society))
    return TRUE;
  ...
  static const char *decree_name(int type) {
    return type >= 0 && type < 6 ? decree_names[type] : "Unknown";
  }
  ...
  _DOFUN(do_decree)
  {
    ...
    if(!str_cmp(arg1, "list"))
    {
      ...
      it != DecreeVect.end(); ++it) {
        ...
        printf_to_char(ch, "`g%02d) %s in %s`x\n\r", count, decree_name((*it)->btype), territory_name((*it)->territory_vnum));
    ...
    else if(!str_cmp(arg1, "info"))
    {
      ...
      else
      {
        ...
        it != DecreeVect.end(); ++it) {
          if((*it)->valid == FALSE)
          continue;
          LOCATION_TYPE *location = territory_by_number((*it)->territory_vnum);
          if(location && !str_cmp(argument, location->name))
          {
            decree = (*it);
            break;
          }
      ...
      printf_to_char(ch, "%s decree in %s\n\n%s\n\nVotes: %s, %s, %s\n\r", decree_name(decree->btype), territory_name(decree->territory_vnum), decree->desc, decree->vote_1, decree->vote_2, decree->vote_3);
      return;
    }
