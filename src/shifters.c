...
extern "C" {
  ...
  bool legal_size(CHAR_DATA *ch, int size) {
    ...
    if (size == ANIMAL_SMALLMED) {
      if (get_skill(ch, SKILL_FAEKNIGHTFOCUS) > 2 || get_skill(ch, SKILL_PRIMALFOCUS) > 2)
      return TRUE;
      if (get_skill(ch, SKILL_NATURALMAGICFOCUS) > 2 || get_skill(ch, SKILL_ILLUSORYMAGICFOCUS) > 2 || get_skill(ch, SKILL_DARKMAGICFOCUS) > 2)
      return TRUE;
      return FALSE;
    }
    if (size == ANIMAL_SMALL) {
      if (get_skill(ch, SKILL_FAEKNIGHTFOCUS) > 2 || get_skill(ch, SKILL_PRIMALFOCUS) > 2)
      return TRUE;
      if (get_skill(ch, SKILL_NATURALMAGICFOCUS) > 1 || get_skill(ch, SKILL_ILLUSORYMAGICFOCUS) > 2 || get_skill(ch, SKILL_DARKMAGICFOCUS) > 2)
      return TRUE;
      return FALSE;
    }
    if (size == ANIMAL_TINY) {
