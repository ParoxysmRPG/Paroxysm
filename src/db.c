...
*/
  void boot_db() {
    if (!haven::replay_death_snapshots()) {
      bug("Cannot replay death saves; refusing to load inconsistent ownership", 0);
      exit(1);
    }
    /*
...
*/
    ...
    haven::ensure_backup_directories();
...
*/
    {
      ...
      if ((weather = fopen(WEATHER_FILE, "r")) != NULL) {
        ...
        time_info.society_alliance_issue = fread_number(weather);
        (void)fread_number(weather); // Retired alliance mode; preserve file layout.
        time_info.bloodstorm = fread_number(weather);
        (void)fread_number(weather); // Retired second society alliance issue.
        (void)fread_number(weather); // Retired alliance mode; preserve file layout.
...
*/
  void load_rooms(FILE *fp) {
    ...
    for (;;) {
      ...
      pRoomIndex->places = NULL;
      pRoomIndex->area = area_last;
      pRoomIndex->vnum = vnum;
      pRoomIndex->name = fread_string(fp);
      pRoomIndex->description = fread_string(fp);
      pRoomIndex->description_decorated = FALSE;
      ...
      for (;;) {
        ...
        pRoomIndex->timezone = fread_number(fp);
          ...
          pRoomIndex->description_decorated = fread_number(fp) != 0;
        }
        else if (letter == 'P') {
          EXTRA_DESCR_DATA *ed;
...
*/
  ...
  void append_report_text(char *&text, const char *suffix, bool newline) {
    const size_t old_length = text ? strlen(text) : 0;
    const size_t added = strlen(suffix);
    if (!added && !newline) return;
    const size_t length = old_length + (newline ? 1 : 0) + added;
    // Loaded strings can be shared. Never modify the boot string pool or the
    // shared empty string. Owned strings may grow within their current bucket;
    // their new length still selects that same bucket when free_string runs.
    const bool owned = text && text != str_empty &&
        !(text >= string_space && text < top_string);
    char *result = owned && length + 1 <= rgSize_lookup(old_length + 1)
        ? text : static_cast<char *>(alloc_mem(length + 1));
    if (result != text && old_length) memcpy(result, text, old_length);
    // memmove also permits a suffix taken from the current report itself.
    memmove(result + old_length + (newline ? 1 : 0), suffix, added + 1);
    if (newline) result[old_length] = '\n';
    if (result != text) free_string(text);
    text = result;
  }
...
*/
  ...
  int safe_strlen_color(const char *argument) {
    Buffer cstring;
    char *length_text = tyr_lor(cstring, argument, NULL, COLOR_LENGTH);
    const int length = atoi(length_text);
