...
#define CREATION(fun) void fun(CHAR_DATA *ch, char *argument)
  ...
  */
  ...
  void finish_creation(CHAR_DATA *ch) {
    ...
    if (ch->faction == FACTION_ORDER) ch->faction = 0;
    if (ch->fcore == FACTION_ORDER) ch->fcore = 0;
    if (ch->factiontwo == FACTION_ORDER) ch->factiontwo = 0;
    char buf[MSL];
...
/*  {command        function        	  helpfile                  Creation Step}, */
  ...
  *****************************************************************/
    ...
    CREATION(credit_show_factions) {
      ...
      send_to_char("factions, see info Cortex or info Scum. You can also choose none. Factions\n\r", ch);
    ...
    CREATION(credit_disciplines) {
      ...
      buf = haven::format_text("%s %s", arg1, arg2);
      for (i = 0; i < discipline_table_count; i++) {
        if (!str_cmp(buf.data(), discipline_table[i].name)) {
          point = i;
          val = atoi(argument);
          if (argument[0] == '\0') {
            do_function(ch, &do_help, discipline_table[point].name);
    ...
    CREATION(credit_skills) {
      ...
      buf = haven::format_text("%s %s", arg1, arg2);
      for (i = 0; i < skill_table_count; i++) {
        if (!str_cmp(buf.data(), skill_table[i].name)) {
          point = i;
          val = atoi(argument);
          if (argument[0] == '\0') {
            do_function(ch, &do_help, skill_table[point].name);
    ...
    CREATION(credit_factions) {
      ...
      if (!str_cmp(argument, "list")) {
        ...
        it != FacVect.end(); ++it) {
          if (safe_strlen((*it)->name) > 3 && (*it)->closed == 0 && (*it)->stasis == 0
              && player_faction_join_allowed(ch, *it, FALSE)) {
            printf_to_char(ch, "%s.\n\r", (*it)->name);
          }
        }
      ...
      if (get_true_age(ch) < 18 && !(fac->type == FACTION_SOCIETY
          && fac->axes[AXES_SUPERNATURAL] >= AXES_FARLEFT
          && fac->axes[AXES_SUPERNATURAL] <= AXES_NEARLEFT)) {
    ...
    *****************************************************************/
    ...
    int charcost(CHAR_DATA *ch) {
      ...
      if (higher_power(ch) || (ch->pcdata->earned_guest_tier >= tier && ch->pcdata->earned_guest_tier >= 4)) return 0;
      int val = 0;
      if (tier == 2)
      val = 25000;
      if (tier == 3)
    ...
    static std::vector<std::string> creation_requirements(CHAR_DATA *ch) {
      std::vector<std::string> missing;
      if (ch->pcdata->guest_reward_used)
        missing.push_back("Guest remake: this character's reward has already been used.");
      if (!IS_FLAG(ch->act, PLR_GUEST) && !is_gm(ch) && get_tier(ch) >= 4 && get_tier(ch) <= 5
          && ch->pcdata->earned_guest_tier < get_tier(ch))
        missing.push_back("Archetype: choose an unlocked tier; T4/T5 requires an earned guest remake.");
      if (higher_power(ch)) {
        if (!IS_FLAG(ch->act, PLR_GUEST) || ch->pcdata->guest_type != GUEST_HIGHERPOWER)
          missing.push_back("Archetype: higher powers require a higher-power guest character.");
        if (!power_available(power_domain(ch->name)))
          missing.push_back("Territory: claim an available territory with change territory <territory>; the power must not be retired or banished.");
      } else if (ch->pcdata->guest_type == GUEST_HIGHERPOWER)
        missing.push_back("Archetype: choose a higher power for this guest character.");
      if (ch->pcdata->earned_guest_tier && (get_tier(ch) < 4 || get_tier(ch) > ch->pcdata->earned_guest_tier))
        missing.push_back("Archetype: choose T4 or an earned T5 tier for this remake.");

      if (!IS_FLAG(ch->act, PLR_GUEST) && !IS_FLAG(ch->act, PLR_GM)) {
        if (charcost(ch) == 0 || available_karma(ch) > 0) {
          if (charcost(ch) > available_karma(ch) + available_pkarma(ch)) {
            missing.push_back("Karma: choose an archetype/modifier you can afford or obtain more karma.");
          }
        }
        else {
          if (charcost(ch) > 0) {
            missing.push_back("Karma: paid creation requires available account karma.");
          }
        }
        if (!ch->pcdata->earned_guest_tier && get_tier(ch) >= 3 && get_tier(ch) <= 5 && safe_strlen(ch->pcdata->account->lasttierchar) > 1 && str_cmp(ch->name, ch->pcdata->account->lasttierchar) && ch->pcdata->account->hightiercooldown > current_time)
          missing.push_back("Archetype: choose a lower tier or wait for your high-tier creation cooldown.");
      }
      else {
        if (get_tier(ch) > ch->pcdata->guest_tier)
          missing.push_back("Archetype: choose a tier within your guest allowance.");
      }
      if (guestmonster(ch) && ch->race != RACE_LANDMONSTER && ch->race != RACE_SEAMONSTER)
        missing.push_back("Archetype: choose a land monster or sea monster for this guest.");

      if (is_name("NewCharacter", ch->name))
        missing.push_back("Name: choose a character name with change name <name>.");
      char buf[MSL];
      int min_inches = ch->pcdata->height_feet == 4 ? 8 : 0;
      int max_inches = ch->pcdata->height_feet == 6 ? 6 : 11;
      if (!guestmonster(ch) && (ch->pcdata->height_inches < min_inches || ch->pcdata->height_inches > max_inches)) {
        snprintf(buf, sizeof(buf), "Inches: choose %d to %d for your current height with change inches <number>.", min_inches, max_inches);
        missing.push_back(buf);
      }

      if (get_true_age(ch) < 18 || get_true_age(ch) < min_age(ch) || get_true_age(ch) > max_age(ch)) {
        snprintf(buf, sizeof(buf), "Birthday: set your true age between %d and %d with change birthday <yyyy mm dd>.", UMAX(18, min_age(ch)), max_age(ch));
        missing.push_back(buf);
      }
      if (get_age(ch) < 18 || get_age(ch) < min_aage(ch) || get_age(ch) > max_aage(ch)) {
        snprintf(buf, sizeof(buf), "Resulting Age: must be between %d and %d; adjust your birthday, deathday, or ageless setting as applicable.", UMAX(18, min_aage(ch)), max_aage(ch));
        missing.push_back(buf);
      }

      if (ch->pcdata->eyes_genetic == 0 && safe_strlen(ch->pcdata->eyes) < 1)
    ...
    bool readychar(CHAR_DATA *ch) {
      return creation_requirements(ch).empty();
    }
    ...
    void genesis_display(CHAR_DATA *ch) {
      ...
      if (ch->pcdata->gen_stage == 1) {
        ...
        if (!IS_FLAG(ch->act, PLR_GUEST) && get_tier(ch) >= 4 && get_tier(ch) <= 5 && !ch->pcdata->earned_guest_tier)
          send_to_char("T4/T5 creation requires a successful higher-power or monster guest remake.\n\r", ch);
        if (ch->pcdata->earned_guest_tier) printf_to_char(ch, "Earned remake: choose a T4%s archetype, with no karma cost.\n\r", ch->pcdata->earned_guest_tier == 5 ? " or T5" : "");
        if (higher_power(ch)) {
          DOMAIN_TYPE *dom = power_domain(ch->name);
          printf_to_char(ch, "Claimed territory: %s\n\r", dom && territory_by_number(dom->claimed_territory) ? territory_by_number(dom->claimed_territory)->name : "`rREQUIRED`x: change territory <territory> (permanent claim)");
        }
        if (!IS_FLAG(ch->act, PLR_GUEST)) {
          if (charcost(ch) > available_karma(ch) + available_pkarma(ch))
        ...
        const std::vector<std::string> missing = creation_requirements(ch);
        if (missing.empty())
        ...
        send_to_char("Use change eyecolor <color>, change haircolor <color>, or change skin <description>.\n\r", ch);
        send_to_char("You can change these freely during creation, including using color codes.\n\r", ch);
        if (!missing.empty()) {
          send_to_char("\n\r`rStill needs changing before you can continue:`x\n\r", ch);
          for (const std::string &requirement : missing)
            printf_to_char(ch, "`r - `x%s\n\r", requirement.c_str());
          send_to_char("\n\r", ch);
        }
        send_to_char("`RWarning: Inhuman eyes, skin, or hair will get you marked fleshformed.`x\n\r", ch);
      }
    }
