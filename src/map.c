...
#define COLOUR_CORTEX 4
#define COLOUR_ORDER 5
#define COLOUR_SCUM 6
...
#define MAP_PATRONBASE 29
  ...
  void process_territories() {
    const int map_support[] = {ALLIANCE_SIDELEFT, ALLIANCE_SIDERIGHT,
                               FACTION_CORTEX, FACTION_ORDER, FACTION_SCUM};
    int othermax, val;
...
*/
      ...
      if (read % 4 != 0) {
        ...
        if (fread(pixel, read, 1, inFile) != 1) {
          bug("Map template: incomplete row padding", 0);
          fclose(inFile);
          fclose(outFile);
          return;
        }
