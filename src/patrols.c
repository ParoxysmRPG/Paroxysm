...
extern "C" {
  ...
  ROOM_INDEX_DATA *mist_duplicate_room(ROOM_INDEX_DATA *orig) {
    ...
    while (to_room->extra_descr != NULL) {
      EXTRA_DESCR_DATA *ed = to_room->extra_descr;
      to_room->extra_descr = ed->next;
      free_extra_descr(ed);
    }
    copy_room_places(to_room, orig);
    return to_room;
  }
  ...
  bool same_faction(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    return TRUE;
    if(ch->legacy_society != 0 and ch->legacy_society == victim->legacy_society)
    return TRUE;
    if(ch->fsociety != 0 and ch->fsociety == victim->fsociety)
    return TRUE;

    if (same_alliance(clan_lookup(ch->fcore), clan_lookup(victim->fcore)))
    return TRUE;

    if (same_alliance(clan_lookup(ch->legacy_society), clan_lookup(victim->legacy_society)))
    return TRUE;

    if (same_alliance(clan_lookup(ch->fsociety), clan_lookup(victim->fsociety)))
    return TRUE;
  ...
  void launch_patrolevent(CHAR_DATA *ch, int type) {
    ...
    if (type == PATROL_GHOST) {
      ...
      if(pat != NULL && pat->clan_id != ch->fsociety)
    ...
    if (type == PATROL_ARTIFACT) {
      ...
      std::string tmp;
      tmp = haven::format_text("%s %s", a_or_an(objname), objname);
  ...
  void patrol_room_award(CHAR_DATA *ch, int type) {
    ...
    it != ch->in_room->people->end();) {
      ...
      if (type == PATROL_GHOST) {
        ...
        gain_personal_karma(fch, fch->pcdata->patrol_rp);
        char buf[MSL];
  ...
  void patrol_personal_award(CHAR_DATA *ch, int type) {
    if (type == PATROL_BRIBE) {
      if (ch->pcdata->account != NULL)
      gain_personal_karma(ch, ch->pcdata->patrol_rp * 2);
...
*/
    ...
    if (!str_cmp(arg1, "engage")) {
      if (ch->pcdata->patrol_status == PATROL_WARMOVINGATTACK && ch->pcdata->patrol_timer > 0) {
        ...
        if (IS_AFFECTED(ch, AFF_WAKEBOUND) || !can_shroud(ch)) {
  ...
  bool spirit_launch(CHAR_DATA *ch) {
    ...
    it != descriptor_list.end(); ++it) {
      ...
      if (IS_FLAG(to->comm, COMM_PRIVATE))
      continue;
      if (to->pcdata->patrol_habits[PATROL_SPIRITHABIT] > 0 || (to->legacy_society != 0)) {
        pop++;
      }
    }
    ...
    if (pop > 2 && number_percent() % 3 == 0) {
      ...
      {
        if(number_percent() % 2 == 0 || ch->legacy_society == 0)
  ...
  void patrol_launch_check(CHAR_DATA *ch) {
    ...
    if (dissent_participant(ch) && number_percent() <= 5 && dissent_launch(ch))
    return;
  ...
  void save_weekly_characters() {
    FILE *fpout;
    if ((fpout = open_snapshot_file("../data/weeklycharacters.txt")) == NULL) {
  ...
  void weekly_process(void) {
    ...
    it != WeeklyVect.end(); ++it) {
      ...
      online = TRUE;
        ...
        buf = haven::format_text("%s%s", PLAYER_DIR, capitalize((*it)->charname));
        stat(buf.data(), &sb);
        victim = d.character;
      }
      if (IS_NPC(victim)) {
      ...
      fac = clan_lookup(victim->legacy_society);
      if (fac != NULL) {
        char facbuf2[MSL];
      ...
      fac = clan_lookup(victim->fsociety);
      if (fac != NULL) {
        char facbuf3[MSL];
      ...
      if (victim->pcdata->patrol_habits[PATROL_RECKLESSHABIT] > 0 || is_weakness(NULL, victim)) {
        if (number_percent() % 2 == 0 && !clinic_patient(victim)) {
          if (number_percent() % 3 == 0 || victim->faction != 0 || get_tier(victim) >= 2) {
            snprintf(reckless + strlen(reckless), sizeof(reckless) - strlen(reckless), ", %s", victim->name);
          }
        }
      }
      ...
      if (victim->pcdata->intel >= 5000) {
        //      if (number_percent() % 2 == 0)
        snprintf(intelvalue + strlen(intelvalue), sizeof(intelvalue) - strlen(intelvalue), ", %s", victim->name);
      }
...
*/
      ...
      if ((day >= 1 && day < 7) || (day >= 14 && day < 21)) {
        ...
        if (get_tier(victim) >= 3 && victim->played / 3600 > 100 && !higher_power(victim)) {
          ...
          buf = haven::format_text("VILLAIN %s, Tier: %d, Score: %d, Week: %d, Life: %d", villains[point], villain_tier[point], villain_scores[point], victim->pcdata->week_tracker[TRACK_VILLAIN], victim->pcdata->life_tracker[TRACK_VILLAIN]);
          log_string(buf.data());
        }
    ...
    if (most_char_influence > 1500 && safe_strlen(mcinfmessage) > 2) {
      buf = haven::format_text("%s is widely thought to have been the most influential person in town this week, overheard to be saying things like, '%s'", mcinfname, mcinfmessage);
    ...
    if (most_app_influence > 1500 && safe_strlen(mainfmessage) > 2) {
      buf = haven::format_text("%s was the town's main fashion critic this week, overheard to be saying things like, '%s'\n\n\r", mainfname, mainfmessage);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s\n", buftemp);
      string += buf.data();
    ...
    if (most_rumors > 1 && safe_strlen(mrumormessage) > 2) {
      buf = haven::format_text("%s was the town gossip this week, overheard spreading stories such as, '%s'\n\n\r", mrumorname, mrumormessage);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s\n", buftemp);
      string += buf.data();
    ...
    if (most_event_pop > 2) {
      buf = haven::format_text("%s supposedly held the most successful social event this week, with %d people in attendance.\n\r", mepopname, most_event_pop);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
...
*/
    if (most_event_attend > 1) {
      buf = haven::format_text("%s is the social butterly of the week, attending %d different social events.\n\r", meattendname, most_event_attend);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      string += buf.data();
    ...
    if (number_percent() % 2 == 0 && most_influence > 3000) {
      buf = haven::format_text("Many people are trying to get close to %s, hearing that they're both influential and very selective about their company.", mostinfluencename);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      string += buf.data();
    ...
    if (number_percent() % 3 == 0 && luckiest > 65) {
      buf = haven::format_text("There are many jealous whispers about %s and plans to steal their significant other away from them.\n\r", luckiestname);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      string += buf.data();
    ...
    news->message = str_dup(string.data());
    NewsVect.push_back(news);
    static std::string scarystring;
    scarystring.clear();
    ...
    if ((day >= 1 && day < 7) || (day >= 14 && day < 21)) {
      ...
      static std::string logstring2;
      logstring2.clear();
      logstring2 += "VILLAINS ";
      for (int i = 0; i < 50; i++) {
        buf = haven::format_text("%s:[%d] %d %d %d. ", villains[i], i, villain_scores[i], villain_praise[i], villain_tier[i]);
        logstring2 += buf.data();
      }
      log_string(logstring2.data());
      ...
      buf = haven::format_text("In the supernatural underground people are claiming %s is particularly intimidating, while others note that for what they are, %s is particularly unintimidating.", villains[villainpraisepointer], villains[villaindisspointer]);
      else if (villainpraisepointer > -1 && villain_scores[villainpraisepointer] >= 100)
      buf = haven::format_text("In the supernatural underground people are claiming %s is particularly intimidating.", villains[villainpraisepointer]);
      else if (villaindisspointer > -1 && villain_scores[villaindisspointer] < 20)
      buf = haven::format_text("In the supernatural underground people are claiming %s is particularly unintimidating.", villains[villaindisspointer]);
      else
      buf = haven::format_text("There are few rumors in the supernatural underworld recently.");

      char *scartemp = str_dup(buf.data());
      scartemp = format_string(scartemp);
      buf = haven::format_text("%s\n", scartemp);
      scarystring += buf.data();

      buf = haven::format_text("Others being discussed as having a particularly monsterous nature are");
      static std::string secondstring;
      secondstring.clear();
      secondstring += buf.data();
      for (int i = 0; i < 50; i++) {
        if (villain_tier[i] >= 3 && i != villainpraisepointer && i != villaindisspointer && i != notscarypoint) {
          buf = haven::format_text(", %s", villains[i]);
          secondstring += buf.data();
        }
      }
      buf = haven::format_text(" and %s.", villains[notscarypoint]);
      secondstring += buf.data();
      char *scartwotemp = str_dup(secondstring.data());
      scartwotemp = format_string(scartwotemp);
      buf = haven::format_text("%s\n", scartwotemp);
      scarystring += buf.data();
    }
    static std::string supersocialstring;
    supersocialstring.clear();
    if (pstatlevel > 0) {
      buf = haven::format_text("Rumor has it %s is the best %s in the town.\n\r", pstatname, focus_name(pstat));
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      supersocialstring += buf.data();
    }

    if (ftypelevel > 0 && number_percent() % 2 == 0) {
      if (ftype == STYPE_PROFFOCUS)
      buf = haven::format_text("People are claiming %s is the best connected professional in town.", ftypename);
      else if (ftype == STYPE_ARCANEFOCUS)
      buf = haven::format_text("People are claiming that %s is the most powerful arcanist in town.", ftypename);
      else if (ftype == STYPE_COMBATFOCUS)
      buf = haven::format_text("People are claiming that %s is the most powerful warrior in town.", ftypename);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      supersocialstring += buf.data();
    ...
    if (safe_strlen(reckless) > 3) {
      buf = haven::format_text("Supernatural predators have noticed several particularly vulnerable people this week%s.\n\r", reckless);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      supersocialstring += buf.data();
    ...
    if (safe_strlen(intelvalue) > 3) {
      buf = haven::format_text("Societies note several individuals with valuable intelligence%s.\n\r", intelvalue);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      supersocialstring += buf.data();
    ...
    supersocialstring += "\n\r";
    if (herolevel >= 20) {
      buf = haven::format_text("Rumor has it %s is the bravest hero in town.\n\r", heroname);
      char *buftemp = str_dup(buf.data());
      buftemp = format_string(buftemp);
      buf = haven::format_text("%s", buftemp);
      supersocialstring += buf.data();
    ...
    char *buftemp2 = str_dup(buf.data());
    buftemp2 = format_string(buftemp2);
    buf = haven::format_text("%s\r", buftemp2);
    newstring += buf.data();
    newstring += "\n\r";
    if ((day >= 1 && day < 7) || (day >= 14 && day < 21)) {
      newstring += scarystring.data();
      newstring += "\n\r";
    }

    newstring += supersocialstring.data();
    FILE *weather;
    ...
    news->timer = 5000;
    free_string(news->message);
    news->message = str_dup(newstring.data());
    news->stats[0] = -2;
    NewsVect.push_back(news);
  ...
  char *const event_names[] = {
    "None",        "MindControl",     "Discredit",  "Steal", "Outcast",     "Brainwash",       "Heist",      "Operation", "Recruitment", "Teaching",        "Occurance",  "Condition", "Dominance",   "Aegis",           "Cleanse",    "Catastrophe", "nothing",     "RevokeSanctuary", "Rob",        "disruption", "storm",       "flood",           "blackout",   "hurricane", "malady",      "haunt",           "dreambelief", "uninvite", "mute", "sabotage", "ambush", "compromised"};
...
*/
  ...
  bool only_faction(CHAR_DATA *ch) {
    ...
    it != room->people->end();) {
      ...
      if (fch == NULL || IS_NPC(fch))
      continue;
      if ((fch->fcore != ch->fcore || fch->fcore == 0) && (fch->legacy_society != ch->legacy_society || fch->legacy_society == 0) && (fch->fsociety != ch->fsociety || fch->fsociety == 0))
      return FALSE;
    }
    return TRUE;
  }
