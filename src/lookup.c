...
#define FNAME_FILE "../data/fname.txt"
  ...
  int race_lookup(const char *name) {
    // Keep the persisted faculty key while accepting the public archetype name.
    if (!str_cmp(name, "civic educator") || !str_cmp(name, "civic educators")
        || !str_cmp(name, "institute staff")) return RACE_FACULTY;
    if (!str_cmp(name, "deputy")) return RACE_CIVIL_SERVANT; // Old saves.
  ...
  */
  ...
  int get_disc(CHAR_DATA *ch, int disc, bool usingit) {
    ...
    if (is_super(ch) && (disc == DIS_PISTOLS || disc == DIS_CARBINES || disc == DIS_RIFLES || disc == DIS_SHOTGUNS)) {
      val = UMAX(0, val * (100 - (get_tier(ch) * (5 - get_focus_power(ch, SKILL_GUNDISCFOCUS)))) / 100);
    }
    ...
    if ((IS_AFFECTED(ch, AFF_PROTECT) || in_society_domain(ch)) && disc == DIS_FATE)
    val += 20;
    ...
    if (range > 0 && val > 0 && has_territory_bonus(ch, TERRITORY_AMMO))
      val += 5;
    return val;
  }
  ...
  // Four-rank trees grant their final benefits at three purchased points.
  // Generic skill reads and focus spending still use the actual rank.
  int focus_power_level(int skill, int rank) {
    switch (skill) {
    ...
    case SKILL_SOCIALFOCUS:
      return rank == 3 ? 4 : rank;
    default:
      return rank;
    }
  }

  int get_focus_power(CHAR_DATA *ch, int skill) {
    return focus_power_level(skill, get_skill(ch, skill));
  }

  int focus_power_bonus(CHAR_DATA *ch, int type) {
    int bonus = 0;
    if (IS_NPC(ch)) return 0;
    for (int i = 0; i < skill_table_count; ++i) {
      int skill = skill_table[i].vnum;
      if (skilltype(skill) != type || ch->skills[skill] == 0) continue;
      int rank = ch->skills[skill] + regress_mod(ch, skill);
      bonus += focus_power_level(skill, rank) - rank;
    }
    return bonus;
  }
  ...
  int get_skill(CHAR_DATA *ch, int skill) {
    ...
    for (int i = 0; i < 10; ++i)
      if (ch->pcdata->deactivated_stats[i] == skill) return value;
    int type = skilltype(skill);
    if ((type == STYPE_SUPERNATURAL || type == STYPE_DEFENSE || type == STYPE_SABILITIES)
        && (event_cleanse == 1 || is_neutralized(ch))) return value;
    // These are single-purchase rank-2 skills, not incremental bonuses.
    if (skill == SKILL_FORENSICS || skill == SKILL_PROTECTIONDETAIL)
      return UMAX(value, 2);
    return value + bonus;
  }
  ...
  */
  ...
  bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim) {
    if ((dissent_crowd(victim) && !dissent_participant(ch))
        || (dissent_crowd(ch) && !dissent_participant(victim)))
      return TRUE;
    ...
    if (institute_room(victim->in_room) && (clinic_patient(victim) || college_student(victim, TRUE))) {
      ...
      it != EventVect.end(); ++it) {
        ...
        if ((*it)->type == EVENT_CLEANSE && !full_moon()) {
          return TRUE;
        }
      }
    }
  ...
  // Personal balances are stored in cents; payday amounts are in dollars.
  bool personal_sanctuary(CHAR_DATA *ch) {
    return ch != NULL && !IS_NPC(ch) && ch->pcdata != NULL
      && !IS_FLAG(ch->act, PLR_GM) && !IS_FLAG(ch->act, PLR_GUEST)
      && !IS_FLAG(ch->act, PLR_DEAD) && !is_super(ch)
      && ch->fcore == 0 && ch->fsociety == 0 && ch->legacy_society == 0;
  }

  bool indentured_servant(CHAR_DATA *ch) {
    return personal_sanctuary(ch) && ch->pcdata->total_money < 0;
  }

  bool debt_blocks_sanctuary(CHAR_DATA *ch) {
    FACTION_TYPE *patron = ch && ch->vassal > 0 ? clan_lookup(ch->vassal) : NULL;
    return personal_sanctuary(ch) && !(patron && patron->valid)
  ...
  bool seems_under_understanding(CHAR_DATA *ch, CHAR_DATA *pers) {
    ...
    if (ch->legacy_society != 0 && clan_lookup(ch->legacy_society) != NULL && clan_lookup(ch->legacy_society)->attributes[FACTION_UNDERSTANDING] > 0)
    return TRUE;

    if (ch->fsociety != 0 && clan_lookup(ch->fsociety) != NULL && clan_lookup(ch->fsociety)->attributes[FACTION_UNDERSTANDING] > 0)
    return TRUE;
  ...
  bool under_understanding(CHAR_DATA *ch, CHAR_DATA *pers) {
    ...
    if(!IS_NPC(pers) && !str_cmp(pers->pcdata->fixation_name, ch->name) && pers->pcdata->fixation_level >= 10)
    return FALSE;
    ...
    for (vector<EVENT_TYPE *>::iterator it = EventVect.begin(); it != EventVect.end(); ++it) {
      ...
      if ((!in_haven(ch->in_room) || deepforest(ch->in_room)) && ch->in_room->area->vnum != 12) {
        if (((*it)->type == EVENT_CLEANSE || (*it)->typetwo == EVENT_CLEANSE) && !full_moon()) {
          return FALSE;
        }
      }
    }
...
#define PROPERTY_TRAILER 9
  ...
  void gain_personal_karma(CHAR_DATA *ch, int amount) {
    if (ch == NULL || IS_NPC(ch) || ch->pcdata == NULL ||
        ch->pcdata->account == NULL || IS_FLAG(ch->act, PLR_GUEST) || amount <= 0)
      return;

    ACCOUNT_TYPE *account = ch->pcdata->account;
    account->pkarmaspent = UMAX(account->pkarmaspent, ch->spentpkarma);
    long long total = (long long)UMAX(0, account->pkarma) + UMAX(0, account->pkarmaspent);
    if (total >= PERSONAL_KARMA_CAP) {
      account->pkarma_remainder = 0;
      return;
    }

    // Each successive 10,000 costs 1, 2, 3, 4, then 5 raw reward points.
    // 150,000 raw points to cap: about 100 days at 1,500 raw points/day.
    // Carry small rewards across saves, and price boundary crossings separately.
    long long remaining = (long long)amount + URANGE(0, account->pkarma_remainder, 4);
    account->pkarma_remainder = 0;
    while (remaining > 0 && total < PERSONAL_KARMA_CAP) {
      int divisor = total / PERSONAL_KARMA_BAND + 1;
      int room = UMIN(PERSONAL_KARMA_CAP, divisor * PERSONAL_KARMA_BAND) - total;
      int gain = UMIN((long long)room, remaining / divisor);
      account->pkarma += gain;
      total += gain;
      remaining -= (long long)gain * divisor;
      if (gain < room) {
        account->pkarma_remainder = remaining;
        break;
      }
    }
  }
  ...
  int available_pkarma(CHAR_DATA *ch) {
    ...
    amount = UMIN(amount, PERSONAL_KARMA_CAP - ch->spentpkarma);
    amount = UMAX(amount, 0);

    return amount;
  }
  
  // Existing accounts start in the rollout month; new accounts start when created.
  void refresh_recolors(ACCOUNT_TYPE *account) {
    if (account == NULL || current_time <= 0)
      return;
    time_t now = current_time;
    struct tm *utc = gmtime(&now);
    if (utc == NULL)
      return;
    int month = (utc->tm_year + 1900) * 12 + utc->tm_mon;
    if (account->recolor_month == 0)
      account->recolor_month = 2026 * 12 + 8 - 1; // September 2026 rollout.
    if (month <= account->recolor_month)
      return;
    long long balance = (long long)account->colours + 3LL * (month - account->recolor_month);
    account->colours = (int)UMIN(balance, 2147483647LL);
    account->recolor_month = month;
  }
  ...
  bool elligible_modifier(CHAR_DATA *ch, int mod) {
    ...
    if (mod == MODIFIER_ADDICTION) {
      ...
      if (ch->race == RACE_IMPORTANT)    {return TRUE;}
      if (ch->race == RACE_CIVIL_SERVANT)       {return TRUE;}
      if (ch->race == RACE_NEWVAMPIRE)   {return TRUE;}
      if (ch->race == RACE_NEWWEREWOLF)  {return TRUE;}
      if (ch->race == RACE_NEWFAEBORN)   {return TRUE;}
      if (ch->race == RACE_NEWDEMONBORN) {return TRUE;}
      ...
      if (ch->fcore != FACTION_CORTEX) return FALSE;
      if (ch->race == RACE_CIVILIAN) return TRUE;
      if (ch->race == RACE_LOCAL) return TRUE;
      if (ch->race == RACE_VISITOR) return TRUE;
      if (ch->race == RACE_STUDENT) return TRUE;
      if (ch->race == RACE_SOLDIER) return TRUE;
  ...
  int sunlight_debuff(CHAR_DATA *ch) {
    if (!IS_NPC(ch) && is_vampire(ch) && ch->pcdata->habit[HABIT_FEEDING] == 2)
      return 0;
  ...
  // Keep the report beside the calculation so it uses the actual active rules.
  static void lifeforce_effect(CHAR_DATA *view, const char *name, int amount,
                              const char *unit) {
    if (view == NULL || amount == 0)
      return;
    printf_to_char(view, "  %s%s`x - %s by %d %s.\n\r",
                   amount > 0 ? "`G" : "`Y", name,
                   amount > 0 ? "Increases" : "Reduces",
                   amount > 0 ? amount : -amount, unit);
  }
  ...
  int monster_lf_mod(CHAR_DATA *ch, CHAR_DATA *view) {
    ...
    if (is_vampire(ch) && ch->in_room != NULL) {
      ...
      if (view != NULL)
      lifeforce_effect(view, "Vampire sunlight", -(sunlight_debuff(ch)), "monster modifier points");

      if (ch->modifier == MODIFIER_DRINKING
      && ch->pcdata->last_drained_person + (3600 * 24 * 60) < current_time && ch->played / 3600 > 100) {
        if (view != NULL)
        lifeforce_effect(view, "Exsanguination", -(val - val * 8 / 10), "monster modifier points");
        val = val * 8 / 10;
      }
    }
    if (is_werewolf(ch)) {
      if (new_moon()) {
        if (view != NULL)
        lifeforce_effect(view, "New moon", -(val - val * 4 / 5), "monster modifier points");
        val = val * 4 / 5;
      }
      ...
      if (full_moon()) {
        if (ch->pcdata->habit[HABIT_LUNACY] == 1) {
          if (view != NULL)
          lifeforce_effect(view, "Chained wolf", -(val - val * 4 / 5), "monster modifier points");
          val = val * 4 / 5;
        }
        if (sunphase(NULL) < 2 || sunphase(NULL) == 7) {
          if (ch->pcdata->habit[HABIT_LUNACY] == 2) {
            if (view != NULL)
            lifeforce_effect(view, "Lunacy", val * 1 / 10, "monster modifier points");
            val = val * 11 / 10;
          }
          if (ch->pcdata->habit[HABIT_LUNACY] == 3) {
            if (view != NULL)
            lifeforce_effect(view, "Lunacy", val * 3 / 10, "monster modifier points");
            val = val * 13 / 10;
          }
        }
      }
      ...
      && ch->pcdata->last_outside_fullmoon < current_time - (3600 * 24 * 60) && ch->played / 3600 > 100) {
        if (view != NULL)
        lifeforce_effect(view, "Missed full moon", -(val - val * 8 / 10), "monster modifier points");
        val = val * 8 / 10;
      }
    }
    if (val < 20)
      lifeforce_effect(view, "Minimum monster modifier points", 20 - val, "monster modifier points");
    val = UMAX(val, 20);
    return val;
  }
  ...
  int clan_lifeforce_mod(CHAR_DATA *ch, CHAR_DATA *view, FACTION_TYPE *fac)
  {
    ...
    if (!str_cmp(ch->name, fac->leader)) {
      ...
      val = -5;
      if(view != NULL)
      lifeforce_effect(view, "Leader effect", val, "base modifier points");
    }

    if (fac->axes[AXES_CORRUPT] == AXES_FARLEFT || fac->axes[AXES_CORRUPT] == AXES_DEMONIC) {
      if (get_tier(ch) >= 3)
      bonus += 6;
      ...
      bonus += 3;
      else if (get_tier(ch) == 2 && fac->patron_relation != SOCIETY_PATRON_WORSHIP)
      bonus -= 3;
      else if(fac->patron_relation != SOCIETY_PATRON_WORSHIP)
      bonus -= 4;
    }
  ...
  int lifeforce_mod(CHAR_DATA *ch, CHAR_DATA *view) {
    ...
    if (pact_holder(ch)) {
      val += 5;
      lifeforce_effect(view, "Demonic pact", 5, "base modifier points");
    }
    ...
    it != EventVect.end(); ++it) {
      ...
      if ((*it)->type == EVENT_DREAMBELIEF) {
        if (safe_strlen(dream_detail(ch, ch->pcdata->identity_world, DREAM_DETAIL_SHORT)) < 1) {
          val -= 15;
          if (view != NULL)
          lifeforce_effect(view, "Dream belief", -15, "base modifier points");
        }
      }
    }
    ...
    if (weather != 0) {
      val += weather;
      if (view != NULL)
      lifeforce_effect(view, "Weather specialization", weather, "base modifier points");
    }
    ...
    if (!IS_FLAG(ch->act, PLR_GUEST)) //exempting institute PCs and guests -- Disco
    {
      ...
      && str_cmp(ch->name, "Ritualist")) {
        ...
        && (!clinic_staff(ch, FALSE) && !college_staff(ch, FALSE))) {
          val -= 15;
          if (view != NULL) {
            lifeforce_effect(view, "No sanctuary", -15, "base modifier points");
          }
        }
      }
      ...
      if (get_tier(ch) == 2 && ch->faction == 0 && ch->vassal == 0 && !has_weakness(NULL, ch)) {
        val -= 2;
        if (view != NULL)
        lifeforce_effect(view, "No vassal bond", -2, "base modifier points");
      }
      else if (get_tier(ch) >= 3 && ch->faction == 0 && ch->vassal == 0 && !has_weakness(NULL, ch)) {
        val -= 4;
        if (view != NULL)
        lifeforce_effect(view, "No vassal bond", -4, "base modifier points");
      }
      else if (ch->faction == 0 && ch->vassal != 0 && !has_weakness(NULL, ch)) {
        val += 5;
        if (view != NULL)
        lifeforce_effect(view, "Vassal bond", 5, "base modifier points");
      }
    }
    ...
    if (under_opression(ch)) {
      val -= 15;
      if (view != NULL)
      lifeforce_effect(view, "Oppressed", -15, "base modifier points");
    }

    int tbonus = soc_lf_mod(ch, NULL);
    
    if(tbonus != 0 && view != NULL)
    lifeforce_effect(view, "Society (faction)", tbonus, "base modifier points");
    ...
    if (IS_AFFECTED(ch, AFF_GRIEF)) {
      if (get_true_age(ch) >= 100) {
        val -= 25;
        if (view != NULL)
        lifeforce_effect(view, "Grief", -25, "base modifier points");
      }
      else {
        val -= 50;
        if (view != NULL)
        lifeforce_effect(view, "Grief", -50, "base modifier points");
      }
    }
      ...
      for (int i = 0; i < 20; i++) {
        if (!str_cmp((*it)->kidnap_name[i], ch->name)) {
          if (get_true_age(ch) >= 100) {
            val -= 5;
            if (view != NULL)
            lifeforce_effect(view, "Kidnapping", -5, "base modifier points");
          }
          else {
            val -= 10;
            if (view != NULL)
            lifeforce_effect(view, "Kidnapping", -10, "base modifier points");
          }
        }
      }
    }
    ...
    if (ch->played / 3600 > 50 && safe_strlen(ch->pcdata->home_territory) < 2 && ch->race != RACE_LOCAL && ch->race != RACE_IMPORTANT && ch->race != RACE_CELEBRITY && ch->race != RACE_PILLAR && ch->race != RACE_FACULTY) {
      val -= 10;
      if (view != NULL)
      lifeforce_effect(view, "No home territory", -10, "base modifier points");
    }

    if (!IS_NPC(ch) && ch->pcdata->overworked > 0) {
      val -= 25;
      if (view != NULL)
      lifeforce_effect(view, "Overworked", -25, "base modifier points");
    }
    ...
    if (!IS_NPC(ch)) {
      ...
      val += weak;
      if (view != NULL && weak != 0)
      lifeforce_effect(view, "Weakness", weak, "base modifier points");
    }

    if (get_age(ch) > 60) {
      if (view != NULL)
      lifeforce_effect(view, "Old age", -(UMIN(20, (get_age(ch) - 60))), "base modifier points");

      val -= UMIN(20, (get_age(ch) - 60));
    }
    if (val < 30)
      lifeforce_effect(view, "Minimum base modifier points", 30 - val, "base modifier points");
    val = UMAX(30, val);
    ...
    if (!is_undead(ch)) {
      if (is_ill(ch)) {
        val -= 5;
        if (view != NULL)
        lifeforce_effect(view, "Illness", -5, "base modifier points");
      }
    }
    
    if (ch->wounds == 3) {
      if (view != NULL)
      lifeforce_effect(view, "Critical wound", -(val - val / 2), "base modifier points");
      val /= 2;
    }
    else if (ch->wounds == 2) {
      if (view != NULL)
      lifeforce_effect(view, "Severe wound", -(val - val * 8 / 10), "base modifier points");
      val = val * 8 / 10;
    }
    else if (ch->wounds == 1) {
      if (view != NULL)
      lifeforce_effect(view, "Mild wound", -(val - val * 95 / 100), "base modifier points");
      val = val * 95 / 100;
    }
  ...
  int get_lifeforce(CHAR_DATA *ch, bool used, CHAR_DATA *view) {
    // Cauterization is a nonlethal effectiveness penalty. Leave LF reserves
    // untouched and return before the low-reserve collapse/coma handling.
    if (!IS_NPC(ch) && ch->pcdata->cauterize_until > current_time) {
      if (view) send_to_char("  `RCauterized bite`x - Lifeforce capped at 10 for fourteen days; this penalty cannot kill you through LF loss.\n\r", view);
      return 10;
    }
    ...
    val += cortex_lifeforce_bonus(ch, view);

    if (ch->pcdata->deluded_duration > current_time) {
      val -= ch->pcdata->deluded_cost;
      lifeforce_effect(view, "Delusion", -ch->pcdata->deluded_cost, "LF");
    }

    if (IS_FLAG(ch->act, PLR_VILLAIN)) {
      val += 25;
      lifeforce_effect(view, "Villain", 25, "LF");
    }
    else if (cardinal(ch) && sincount(ch) > 0) {
      val += 25;
      lifeforce_effect(view, "Cardinal sin", 25, "LF");
    }
    else if (cardinal(ch)) {
      val -= 30;
      lifeforce_effect(view, "Cardinal without sin", -30, "LF");
    }

    if(ch->pcdata->fixation_mourning > current_time) {
      val -= 20;
      lifeforce_effect(view, "Fixation mourning", -20, "LF");
    }

    if (ch->pcdata->dream_sex > 2) {
      val -= ch->pcdata->dream_sex;
      if (view != NULL)
      lifeforce_effect(view, "Dream sex", -(ch->pcdata->dream_sex), "LF");
    }
    ...
    if (is_vampire(ch)) {
      ...
      val += bonus;
      if (view != NULL && bonus != 0)
      lifeforce_effect(view, "Vampire Nighttime", bonus, "LF");

      if (IS_AFFECTED(ch, AFF_NEUTRALIZED)) {
        val -= 25;
        if (view != NULL)
        lifeforce_effect(view, "Neutralized", -(25), "LF");
      }
    }

    val += ch->pcdata->lf_modifier;
    if (view != NULL)
    lifeforce_effect(view, "Wellness modifier", ch->pcdata->lf_modifier, "LF");
    ...
    if (pact_holder(ch) && ch->pcdata->emotes[EMOTE_PACTANTAG] < 10 && ch->played / 3600 > 100) {
      if (view != NULL)
      lifeforce_effect(view, "Unfulfilled pact", -(val - val * 8 / 10), "LF");
      val = val * 8 / 10;
    }
    if (ch->modifier == MODIFIER_RELIC && !has_relic(ch)) {
      if (view != NULL)
      lifeforce_effect(view, "Missing relic", -(val - val * 8 / 10), "LF");
      val = val * 8 / 10;
    }
    if (!IS_NPC(ch) && safe_strlen(ch->pcdata->maim) > 3) {
      if (view != NULL)
      lifeforce_effect(view, "Maiming", -(val - val * 90 / 100), "LF");
      val = val * 90 / 100;
    }
    ...
    if (IS_AFFECTED(ch, AFF_SUNBLESS) && sun >= 2 && sun <= 6) {
      if (view != NULL)
      lifeforce_effect(view, "Sun blessing", val * 15 / 100, "LF");
      val = val * 115 / 100;
    }
    if (IS_AFFECTED(ch, AFF_MOONBLESS) && (sun < 2 || sun > 6)) {
      if (view != NULL)
      lifeforce_effect(view, "Moon blessing", val * 15 / 100, "LF");
      val = val * 115 / 100;
    }
    if (IS_AFFECTED(ch, AFF_WATERBLESS) && nearby_water(ch)) {
      if (view != NULL)
      lifeforce_effect(view, "Water blessing", val * 15 / 100, "LF");
      val = val * 115 / 100;
    }
    if (safe_strlen(ch->pcdata->maintained_target) > 1 && str_cmp(ch->pcdata->maintained_target, "casting")) {
      if (view != NULL)
      lifeforce_effect(view, "Maintained ritual", -(ch->pcdata->maintain_cost), "LF");
      val -= ch->pcdata->maintain_cost;
    }
    if (ch->pcdata->ill_time > current_time) {
      if (view != NULL)
      send_to_char("  `YIllness`x - Caps lifeforce at 80 LF.\n\r", view);
      val = UMIN(val, 80);
    }
    ...
    if(in_society_domain(ch))
    {
      ...
      val = val*120/100;
      if (view != NULL)
      send_to_char("  `GSanctified domain`x - Raises lifeforce to at least 80 LF, then increases it by 20%.\n\r", view);
    }

    if (val < 10)
      lifeforce_effect(view, "Minimum LF", 10 - val, "LF");
    val = UMAX(val, 10);
    if (has_territory_bonus(ch, TERRITORY_WELL_FED)) {
      lifeforce_effect(view, "Well-fed territory", val * 110 / 100 - val, "LF");
      val = val * 110 / 100;
    }
    if (indentured_servant(ch)) {
      if (view != NULL)
        send_to_char("  `YIndentured servant`x - Sixteen-hour work days cap lifeforce at 30 LF until your bank debt is repaid.\n\r", view);
      val = UMIN(val, 30);
    }
    return val;
  }
  ...
  int get_display_lifeforce(CHAR_DATA *ch) {
    ...
    if (IS_AFFECTED(ch, AFF_BITTEN) && ch->pcdata->bittenloss > 0
        && ch->pcdata->cauterize_until <= current_time)
  ...
  int unused_lifeforce(CHAR_DATA *ch) {
    int amount = ch->lifeforce - feeding_lf_penalty(ch);
    amount -= ch->lf_taken;
    amount -= ch->lf_sused;
    return amount;
  }
  ...
  void give_lifeforce_nouse(CHAR_DATA *ch, int amount, char *reason) {
    record_feeding(ch, amount, reason);
    char buf[MSL];
  ...
  bool victim_faction(FACTION_TYPE *fac, FACTION_TYPE *opp) {
    ...
    return FALSE;
    if (same_alliance(fac, opp))
    return FALSE;

    if(safe_strlen(fac->eidilon) > 2 && !str_cmp(fac->eidilon, opp->eidilon))
    {
      if(fac->patron_relation == SOCIETY_PATRON_WORSHIP)
      {
        if(opp->patron_relation == SOCIETY_PATRON_OPPOSE)
        return TRUE;
      }
      if(fac->patron_relation == SOCIETY_PATRON_OPPOSE)
      {
        if(opp->patron_relation == SOCIETY_PATRON_WORSHIP)
        return TRUE;
      }
    }
